...
- `MAX_GAMESTATE_CHARS` is increased from 20,480 to 32,768 bytes, allowing larger custom-map and mod gamestates on existing protocol-22 servers. The full gamestate must still fit one 32,768-byte network message together with its framing, baselines, and pending commands, so the usable configstring payload tops out a few KiB lower; a server whose gamestate exceeds that budget now fails with a clear error instead of sending a truncated gamestate.
