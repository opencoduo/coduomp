...
- `MAX_GAMESTATE_CHARS` is increased from 20,480 to 32,768 bytes, allowing larger custom-map and mod gamestates on existing protocol-22 servers. The gamestate delivered at connect must fit its single 32,768-byte network message (configstrings, framing, and entity baselines together); a server that exceeds that budget logs a clear console warning while behaving exactly as before (only the oversized join fails, without affecting the running server). Configstring updates during play can then grow the client's retained pool to the full 32,768 bytes, past the retail client's 20,480 ceiling.
