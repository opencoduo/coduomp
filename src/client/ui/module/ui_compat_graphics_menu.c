...
static const char *ui_compat_prepend_menu_script(const char *prefix,
                                                  const char *script)
{
...
static void ui_compat_prepend_named_item_actions(menuDef_t *menu,
                                                  const char *itemName,
                                                  const char *prefix)
{
    if (menu == NULL)
    ...
    for (int32_t index = 0; index < menu->itemCount; ++index) {
        ...
        if (item != NULL && item->window.name != NULL &&
            Q_stricmp(item->window.name, itemName) == 0) {
            item->action =
                ui_compat_prepend_menu_script(prefix, item->action);
        }
    }
}
...
static itemDef_t *ui_compat_find_cvar_item(menuDef_t *menu,
                                           const char *cvar,
                                           int32_t *itemIndex)
{
    for (int32_t index = 0; index < menu->itemCount; ++index) {
...
static itemDef_t *ui_compat_find_action_item(menuDef_t *menu,
                                             const char *command,
                                             int32_t *itemIndex)
{
    for (int32_t index = 0; index < menu->itemCount; ++index) {
        ...
        if (item != NULL && item->action != NULL &&
            strstr(item->action, command) != NULL) {
            if (itemIndex != NULL)
                *itemIndex = index;
            return item;
        }
...
static itemDef_t *ui_compat_clone_menu_item(const itemDef_t *source,
                                            menuDef_t *parent)
{
    ...
    item->parent = parent;
    item->window.flags &= ~(WINDOW_MOUSEOVER | WINDOW_HASFOCUS |
                            WINDOW_MOUSEOVERTEXT);
    return item;
}

/* NOT_FROM_ORIGINAL_SOURCE: initializes a numeric multi selector with bounded
 * arrays owned by the parsed-menu memory pool. */
static void ui_compat_set_numeric_multi(
    itemDef_t *item, const ui_compat_multi_value_t *values, int32_t count)
{
...
void ui_compat_extend_graphics_menu(void)
{
    ...
    static const ui_compat_multi_value_t displayModes[] = {
        ...
        { "@CODUOMP_GRAPHICS_FULLSCREEN", 1.0f },
        { "@CODUOMP_GRAPHICS_BORDERLESS", 2.0f }
    };
    static const ui_compat_multi_value_t aspectModes[] = {
        { "@CODUOMP_GRAPHICS_FILL_SCREEN", 0.0f },
        { "@CODUOMP_GRAPHICS_LETTERBOX", 1.0f }
    };
    menuDef_t *const menu = Menus_FindByName("options_graphics");
    menuDef_t *const restartMenu = Menus_FindByName("vid_restart_popmenu");
    menuDef_t *const listenRestartMenu =
    ...
    resolutionItem =
        ui_compat_find_cvar_item(menu, "ui_r_mode", NULL);
    displayModeItem =
    ...
    displayModeItem->text =
        String_Alloc("@CODUOMP_GRAPHICS_DISPLAY_MODE");
    ...
    menu->onOpen = ui_compat_prepend_menu_script(
        stageCompatibilityCvars, menu->onOpen);
    ui_compat_prepend_named_item_actions(
        restartMenu, "yes", applyCompatibilityCvars);
    ui_compat_prepend_named_item_actions(
        listenRestartMenu, "ok", applyCompatibilityCvars);
    if (listenRestartMenu != NULL) {
        listenRestartMenu->onESC = ui_compat_prepend_menu_script(
...
{
    static const char closeAdvancedMenu[] =
        "close options_advanced; ";
    static const char openAdvancedMenu[] =
        "play \"mouse_click\"; "
    ...
    menuDef_t *const performanceMenu =
    ...
    if (optionsMenu == NULL || performanceMenu == NULL ||
        Menus_FindByName("options_advanced") != NULL ||
        menuCount >= MAX_MENUS ||
        optionsMenu->itemCount >= MAX_MENUITEMS ||
        performanceMenu->itemCount <
            UI_COMPAT_ADVANCED_TEMPLATE_ITEM_COUNT) {
        return;
    }

    performanceNavigation = ui_compat_find_action_item(
        optionsMenu, "open options_performance",
        &performanceNavigationIndex);
    controlTemplate = ui_compat_find_cvar_item(
        performanceMenu, "r_swapinterval", NULL);
    ...
    advancedMenu->window.flags &= ~(WINDOW_MOUSEOVER | WINDOW_HASFOCUS |
                                    WINDOW_VISIBLE | WINDOW_MOUSEOVERTEXT);
    ...
    for (int32_t index = 0;
         index < UI_COMPAT_ADVANCED_TEMPLATE_ITEM_COUNT; ++index) {
        advancedMenu->items[index] = ui_compat_clone_menu_item(
            performanceMenu->items[index], advancedMenu);
        ++advancedMenu->itemCount;
    }
    advancedMenu->items[UI_COMPAT_ADVANCED_TEMPLATE_ITEM_COUNT - 1]->text =
        String_Alloc("@CODUOMP_ADVANCED");
    ...
    for (int32_t index = 0; index < optionsMenu->itemCount; ++index) {
        ...
        if (item != NULL && item->action != NULL &&
            strstr(item->action, "open options_") != NULL) {
            item->action = ui_compat_prepend_menu_script(
                closeAdvancedMenu, item->action);
        }
    }
    optionsMenu->onClose = ui_compat_prepend_menu_script(
        closeAdvancedMenu, optionsMenu->onClose);
    ...
    for (int32_t index = 0; index < optionsMenu->itemCount; ++index) {
        ...
        if (item != NULL && item->action != NULL &&
            (strstr(item->action, "open options_graphics_defaults") != NULL ||
             strstr(item->action, "open options_driverinfo") != NULL)) {
            item->window.rectClient.y += 15.0f;
        }
    }

    advancedNavigation = ui_compat_clone_menu_item(
        performanceNavigation, optionsMenu);
    advancedNavigation->window.name =
        String_Alloc("coduomp_advanced_options");
    ...
    for (int32_t index = optionsMenu->itemCount;
         index > performanceNavigationIndex + 1; --index) {
        optionsMenu->items[index] = optionsMenu->items[index - 1];
    }
    optionsMenu->items[performanceNavigationIndex + 1] =
        advancedNavigation;
...
void ui_compat_brand_main_menu_version(void)
{
    ...
    for (int32_t index = 0; index < menu->itemCount; ++index) {
        ...
        if (item != NULL && item->window.name != NULL &&
            item->cvar != NULL &&
            Q_stricmp(item->window.name,
                      "background_version_display") == 0 &&
            Q_stricmp(item->cvar, "shortversion") == 0) {
...
void ui_compat_remove_single_player_menu_item(void)
{
    ...
    for (int32_t index = 0; index < menu->itemCount; ++index) {
        ...
        if (item == NULL || item->text == NULL ||
            Q_stricmp(item->text, "@MENU_SINGLE_PLAYER") != 0) {
            continue;
        }
    ...
    for (int32_t index = 0; index < menu->itemCount; ++index) {
        ...
        if (item != NULL && item->text != NULL &&
            Q_stricmp(item->text, "@MENU_QUIT") == 0 &&
