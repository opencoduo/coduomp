...
static const char *const ui_monthReferences[12] = {
...
void UI_StartServerRefresh(qboolean full)
{
    ...
    trap_RealTime(&time);
    refreshDate = va("%s %i, %i   %i:%02i",
                     UI_SafeTranslateString(
                         ui_monthReferences[time.tm_mon]),
                     time.tm_mday, time.tm_year + UI_CALENDAR_YEAR_BASE,
    ...
    if (!full) {
        ...
        ui_serverRefreshTime =
            ...
            UI_SERVER_REFRESH_SHORT_MILLISECONDS;
        return;
    }
    ...
    ui_serverRefreshTime =
        ui_displayContextStorage.context.realTime +
        UI_SERVER_REFRESH_LONG_MILLISECONDS;
