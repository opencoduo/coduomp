...
static const ui_compat_translation_t uiCompatTranslations[] = {
    ...
    { "CODUOMP_GRAPHICS_FIELD_OF_VIEW", "Field of View", "Campo de visi\363n" },
    { "CODUOMP_GRAPHICS_CONTRAST", "Contrast", "Contraste" },
...
static const char *ui_compat_translate_string(const char *reference)
{
    const int32_t language =
        (int32_t)trap_Cvar_VariableValue("cl_language");

    for (size_t index = 0;
         index < sizeof(uiCompatTranslations) /
                     sizeof(uiCompatTranslations[0]);
         ++index) {
        const ui_compat_translation_t *const translation =
            &uiCompatTranslations[index];

        if (strcmp(reference, translation->reference) == 0) {
            return language == UI_COMPAT_LANGUAGE_SPANISH
                       ? translation->spanish
                       : translation->english;
        }
    }
