...
// 0x40041804
static uiNativeWindow_t ui_hiddenWindows[UI_GAMMA_WINDOW_CAPACITY] UI_STORAGE_USED;
...
UI_WINDOW_CALLBACK UI_HideGameWindow(uiNativeWindow_t window, uiNativeWindowParam_t unused)
{
#if defined(_WIN32)
    static const char gameWindowTitle[UI_WINDOW_TITLE_BYTES] = "CoD:United Offensive Multiplayer";
    ...
    if (GetWindowTextA(window, title, sizeof(title)) != 0) {
        ...
        for (titleIndex = 0; titleIndex < UI_WINDOW_TITLE_BYTES; ++titleIndex) {
            if ((unsigned char)title[titleIndex] != (unsigned char)gameWindowTitle[titleIndex]) {
                return TRUE;
            }
        }
    }
    ...
    if ((style & WS_VISIBLE) != 0) {
        ...
        SetWindowLongA(window, GWL_EXSTYLE, extendedStyle & ~WS_EX_TOPMOST);
    }
    return TRUE;
#else
