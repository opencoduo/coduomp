...
void UI_RunMenuScript(char **arguments)
{
    ...
    if (Q_stricmpn("StartServer", command,
                   UI_LEADING_SCRIPT_COMPARE_LIMIT) == 0) {
        if (ui_dedicated.integer == 0 &&
            coduo_fp_to_i32_extended((long double)trap_Cvar_VariableValue(
                "sv_punkbuster")) != 0 &&
            coduo_fp_to_i32_extended((long double)trap_Cvar_VariableValue(
                "cl_punkbuster")) == 0) {
    ...
    if (Q_stricmpn("resetDefaults", command,
                   UI_LEADING_SCRIPT_COMPARE_LIMIT) == 0) {
    ...
    if (Q_stricmpn("getCDKey", command,
                   UI_LEADING_SCRIPT_COMPARE_LIMIT) == 0) {
        return;
    }
    if (Q_stricmpn("verifyCDKey", command,
                   UI_LEADING_SCRIPT_COMPARE_LIMIT) == 0) {
        ...
        Q_strcat(checksum, sizeof(checksum),
                 UI_Cvar_VariableString("cdkey5"));
        if (ui_compat_verify_cd_key(key, checksum)) {
            trap_Cvar_Set("ui_cdkeyvalid",
                          UI_SafeTranslateString("EXE_CDKEYVALID"));
            ui_compat_set_cd_key(key, checksum);
        } else {
            trap_Cvar_Set("ui_cdkeyvalid",
                          UI_SafeTranslateString("EXE_CDKEYINVALID"));
        }
        return;
    }
    if (Q_stricmpn("loadArenas", command,
                   UI_LEADING_SCRIPT_COMPARE_LIMIT) == 0) {
        UI_LoadArenas();
...
stricmp_chain:
    ...
    if (Q_stricmp(command, "RefreshServers") == 0 ||
        Q_stricmp(command, "RefreshFilter") == 0) {
    ...
    if (Q_stricmp(command, "RunSPDemo") == 0) {
        if (ui_displayContextStorage.demoAvailable) {
            ...
            if (ui_currentMap < 0 || ui_currentMap >= ui_mapCount ||
                ui_gameType < 0 || ui_gameType >= ui_gameTypeCount) {
                Com_Printf("WARNING: RunSPDemo has an invalid game or map selection\n");
                return;
            }
            trap_Cmd_ExecuteText(
                EXEC_APPEND,
                va("demo %s_%s\n", ui_maps[ui_currentMap].mapName,
                   ui_gameTypes[ui_gameType].gameType));
        }
        return;
    }
    ...
    if (Q_stricmp(command, "RunMod") == 0) {
        ...
        if (ui_mods[ui_modIndex].directory != NULL &&
            ui_mods[ui_modIndex].directory[0] != '\0') {
    ...
    if (Q_stricmp(command, "ServerStatus") == 0) {
        ...
        if (ui_currentServer >= 0 && ui_currentServer < ui_displayServerCount) {
            ui_compat_lan_get_server_address(
                ui_netSource, ui_displayServers[ui_currentServer],
                ui_serverStatusAddress, sizeof(ui_serverStatusAddress));
    ...
    if (Q_stricmp(command, "FoundPlayerServerStatus") == 0) {
        ...
        Q_strncpyz(
            ui_serverStatusAddress,
            ui_foundPlayerServerAddresses[
                ui_foundPlayerServerIndex + UI_FOUND_PLAYER_RESERVED_SLOTS],
    ...
    if (Q_stricmp(command, "JoinServer") == 0) {
        ...
        ui_compat_lan_get_server_address(ui_netSource, index, serverAddress,
    ...
    if (Q_stricmp(command, "FoundPlayerJoinServer") == 0) {
        if (ui_foundPlayerServerIndex >= 0 &&
            ui_foundPlayerServerIndex < ui_foundPlayerServerCount) {
            /* Reads the reserved-slot-0 scheme described above. */
            trap_Cmd_ExecuteText(
                EXEC_APPEND,
                va("connect %s\n",
                   ui_foundPlayerServerAddresses[ui_foundPlayerServerIndex +
                       UI_FOUND_PLAYER_RESERVED_SLOTS]));
        }
        return;
    }
    ...
    if (Q_stricmp(command, "ServerSort") == 0) {
        if (Int_Parse(arguments, &index)) {
            ...
            coduo_crt_qsort(
                ui_displayServers,
                (size_t)(uint32_t)ui_displayServerCount,
                sizeof(ui_displayServers[0]), UI_ServersQsortCompare);
        }
        return;
    }
    if (Q_stricmp(command, "nextSkirmish") == 0 ||
        Q_stricmp(command, "SkirmishStart") == 0) {
        return;
    }
    ...
    if (Q_stricmp(command, "voteKick") == 0 ||
        Q_stricmp(command, "voteTempBan") == 0) {
        if (ui_playerIndex >= 0 && ui_playerIndex < ui_playerCount) {
            trap_Cmd_ExecuteText(
                EXEC_APPEND,
                va(Q_stricmp(command, "voteKick") == 0
                       ? "callvote kick \"%s\"\n"
    ...
    if (Q_stricmp(command, "addFavorite") == 0) {
        ...
        if (ui_currentServer >= 0 && ui_currentServer < ui_displayServerCount) {
            ui_compat_lan_get_server_info(
                ui_netSource, ui_displayServers[ui_currentServer], serverInfo,
                sizeof(serverInfo));
            Q_strncpyz(favoriteName,
                       Info_ValueForKey(serverInfo, "hostname"),
                       sizeof(favoriteName));
            Q_strncpyz(favoriteAddress, Info_ValueForKey(serverInfo, "addr"),
                       sizeof(favoriteAddress));
        }
    ...
    if (Q_stricmp(command, "deleteFavorite") == 0) {
        if (ui_netSource == LAN_SERVER_SOURCE_FAVORITES &&
            ui_currentServer >= 0 && ui_currentServer < ui_displayServerCount) {
            UI_UpdateDisplayServers();
            /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
            ...
            ui_compat_lan_get_server_info(
                ui_netSource, ui_displayServers[ui_currentServer], serverInfo,
                sizeof(serverInfo));
            Q_strncpyz(favoriteAddress,
                       Info_ValueForKey(serverInfo, "addr"),
                       sizeof(favoriteAddress));
            if (favoriteAddress[0] != '\0') {
                ui_compat_lan_remove_server(LAN_SERVER_SOURCE_FAVORITES,
                                            favoriteAddress);
            }
        }
        return;
    ...
    if (Q_stricmp(command, "createFavorite") == 0) {
        ...
        Q_strncpyz(favoriteName, UI_Cvar_VariableString("ui_favoriteName"),
                   sizeof(favoriteName));
        Q_strncpyz(favoriteAddress,
                   UI_Cvar_VariableString("ui_favoriteAddress"),
                   sizeof(favoriteAddress));
        UI_AddServerToFavoritesList(favoriteName, favoriteAddress);
        return;
    }
    if (Q_stricmp(command, "orders") == 0) {
        ...
        index = coduo_fp_to_i32_extended(
            (long double)trap_Cvar_VariableValue("cg_selectedPlayer"));
        /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
        if (index < 0) {
        ...
        if (index < ui_teamPlayerCount) {
            ...
            Com_sprintf(serverAddress, sizeof(serverAddress), value,
                        ui_playerNumbers[index]);
    ...
    if (Q_stricmp(command, "showSpecScores") == 0) {
        if (coduo_crt_atoi(
                UI_Cvar_VariableString("ui_isSpectator")) != 0) {
    ...
    if (Q_stricmp(command, "getLanguage") == 0) {
        char language[8] = "";
        Q_strncpyz(language, UI_Cvar_VariableString("cl_language"),
                   sizeof(language));
    ...
    if (Q_stricmp(command, "updateLanguage") == 0) {
        char language[8] = "";
        Q_strncpyz(language, UI_Cvar_VariableString("ui_language"),
                   sizeof(language));
