...
{
    enum { UI_GAMETYPE_NAME_COMPARE_LIMIT = 99999 };
    char gameType[MAX_STRING_CHARS];
    int32_t gameTypeCount;
    int32_t index;

    trap_Cvar_VariableStringBuffer("g_gametype", gameType,
                                   (int32_t)sizeof(gameType));
    ...
    for (index = 0; index < gameTypeCount; ++index) {
        ...
        if (candidate != NULL &&
            Q_stricmpn(candidate, gameType,
                       UI_GAMETYPE_NAME_COMPARE_LIMIT) == 0) {
