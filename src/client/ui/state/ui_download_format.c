...
void UI_ReadableSize(char *buffer, int32_t bufferSize, int32_t value)
{
    if (value > UI_BYTES_PER_GIGABYTE) {
        ...
        Com_sprintf(buffer, bufferSize, "%d", value / UI_BYTES_PER_GIGABYTE);
        length = (int32_t)strlen(buffer);
        unitName = UI_SafeTranslateString("EXE_GIGABYTE");
        remainder = value % UI_BYTES_PER_GIGABYTE;
        scaledRemainder = (int32_t)((uint32_t)remainder * (uint32_t)UI_SIZE_FRACTION_SCALE);
        Com_sprintf(buffer + length, bufferSize - length, ".%02d %s", scaledRemainder / UI_BYTES_PER_GIGABYTE, unitName);
        return;
    }
    if (value > UI_BYTES_PER_MEGABYTE) {
        ...
        Com_sprintf(buffer, bufferSize, "%d", value / UI_BYTES_PER_MEGABYTE);
        length = (int32_t)strlen(buffer);
        unitName = UI_SafeTranslateString("EXE_MEGABYTE");
        remainder = value % UI_BYTES_PER_MEGABYTE;
        scaledRemainder = (int32_t)((uint32_t)remainder * (uint32_t)UI_SIZE_FRACTION_SCALE);
        Com_sprintf(buffer + length, bufferSize - length, ".%02d %s", scaledRemainder / UI_BYTES_PER_MEGABYTE, unitName);
        return;
    }
...
void UI_PrintTime(char *buffer, int32_t bufferSize, int32_t time)
{
    if (time > UI_SECONDS_PER_HOUR) {
        ...
        int32_t minutes = (time % UI_SECONDS_PER_HOUR) / UI_SECONDS_PER_MINUTE;
        const char *minutesName = UI_SafeTranslateString("EXE_MINUTES");
        const char *hoursName = UI_SafeTranslateString("EXE_HOURS");

        Com_sprintf(buffer, bufferSize, "%d %s %d %s", hours, hoursName, minutes, minutesName);
        return;
    }
    if (time > UI_SECONDS_PER_MINUTE) {
        ...
        Com_sprintf(buffer, bufferSize, "%d %s %d %s", minutes, minutesName, seconds, secondsName);
        return;
    }
    Com_sprintf(buffer, bufferSize, "%d %s", time, UI_SafeTranslateString("EXE_SECONDS"));
}
