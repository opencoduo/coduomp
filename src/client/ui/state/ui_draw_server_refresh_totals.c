...
void UI_DrawServerRefreshTotals(const rectDef_t *rect, int32_t font, float scale, const vec4_t color, int32_t textStyle)
{
    ...
    if (servers != 0) {
        const char *const format = UI_SafeTranslateString("GMI_EXE_REFRESHTOTALS");
    ...
    trap_R_Text_Paint(rect->x, rect->y, font, scale, color, text, UI_REFRESH_TOTALS_TEXT_STYLE, UI_REFRESH_TOTALS_TEXT_LIMIT, textStyle);
}
