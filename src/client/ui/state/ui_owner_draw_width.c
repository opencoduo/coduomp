...
static const char *const ui_netSourceWidthLabels[] = {
    "EXE_LOCAL", "EXE_INTERNET", "EXE_FAVORITES"
};
...
int32_t UI_OwnerDrawWidth(int32_t ownerDraw, int32_t font, float scale)
{
    ...
    case UI_OWNERDRAW_HANDICAP: {
        ...
        float handicap = Com_ClampFloat(
            (float)UI_HANDICAP_MINIMUM, (float)UI_HANDICAP_MAXIMUM,
            trap_Cvar_VariableValue("handicap"));
        text = ui_handicapLabels[
            UI_HANDICAP_LABEL_LAST_INDEX -
            coduo_fp_to_i32_extended((long double)handicap) /
                UI_HANDICAP_STEP];
        break;
    }
    ...
    case UI_OWNERDRAW_NETSOURCE:
        ...
        if (ui_netSource < LAN_SERVER_SOURCE_LOCAL ||
            ui_netSource >= LAN_SERVER_SOURCE_COUNT) {
            ui_netSource = LAN_SERVER_SOURCE_LOCAL;
        }
        text = trap_SE_LocalizeMessage(
            va("EXE_NETSOURCE\x14%s", ui_netSourceWidthLabels[ui_netSource]),
            "net source");
        break;
    case UI_OWNERDRAW_SERVERFILTER:
        ...
        text = trap_SE_LocalizeMessage(
            va("EXE_SERVERFILTER\x14%s",
               ui_serverFilters[ui_serverFilterType].label),
            "server filter");
        break;
    case UI_OWNERDRAW_REFRESH_DATE:
        text = UI_Cvar_VariableString(
            va("ui_lastServerRefresh_%i", ui_netSource));
        break;
    case UI_OWNERDRAW_KEY_STATUS:
        text = UI_SafeTranslateString(g_waitingForKey
                                      ? "EXE_KEYWAIT" : "EXE_KEYCHANGE");
        break;
    case UI_OWNERDRAW_REFRESH_TOTALS: {
        ...
        Com_sprintf(ui_ownerDrawWidthBuffer,
                    sizeof(ui_ownerDrawWidthBuffer),
