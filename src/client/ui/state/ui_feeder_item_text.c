...
const char *UI_FeederItemText(float feeder, int32_t index, int32_t column, int32_t *imageHandle)
{
    ...
    *imageHandle = UI_FEEDER_IMAGE_HANDLE_NONE;
    ...
    if (feeder == UI_FEEDER_SERVERS) {
        ...
        if (ui_cachedServerInfoColumn != column || ui_cachedServerInfoTime > (int32_t)((uint32_t)ui_displayContextStorage.context.realTime +
                                                                                       (uint32_t)UI_SERVER_INFO_REFRESH_WINDOW)) {
            ui_compat_lan_get_server_info((int32_t)ui_netSource, ui_displayServers[index], ui_cachedServerInfo, UI_SERVER_INFO_SIZE);
            ui_cachedServerInfoColumn = column;
            ui_cachedServerInfoTime = ui_displayContextStorage.context.realTime;
        }

        ping = coduo_crt_atoi(Info_ValueForKey(ui_cachedServerInfo, "ping"));
        if (column < 0 || column >= UI_SERVER_COLUMN_COUNT) {
            return empty;
        }
        switch (column) {
        case UI_SERVER_COLUMN_PASSWORD:
            return coduo_crt_atoi(Info_ValueForKey(ui_cachedServerInfo, "pswrd")) != 0 ? "X" : empty;

        case UI_SERVER_COLUMN_HARDWARE: {
            int32_t hardware = coduo_crt_atoi(Info_ValueForKey(ui_cachedServerInfo, "hw"));
            if (hardware >= 0 && hardware < UI_SERVER_HARDWARE_SHADER_COUNT) {
        ...
        case UI_SERVER_COLUMN_NAME:
            ...
            return Info_ValueForKey(ui_cachedServerInfo, ping > 0 ? "hostname" : "addr");

        case UI_SERVER_COLUMN_MAP:
            return Info_ValueForKey(ui_cachedServerInfo, "mapname");

        case UI_SERVER_COLUMN_CLIENTS: {
            const int32_t botCount = coduo_crt_atoi(Info_ValueForKey(ui_cachedServerInfo, "bots"));
            const char *maximum = Info_ValueForKey(ui_cachedServerInfo, "sv_maxclients");
            const char *clients = Info_ValueForKey(ui_cachedServerInfo, "clients");
            Com_sprintf(ui_serverClientText, sizeof(ui_serverClientText), "%s (%s)", clients, maximum);
            ...
            if (botCount > 0) {
                ...
                if (prefixLength + 1 < sizeof(ui_serverClientText)) {
                    Com_sprintf(ui_serverClientText + prefixLength + 1, sizeof(ui_serverClientText) - prefixLength - 1, " %i", botCount);
        ...
        case UI_SERVER_COLUMN_PUNKBUSTER:
            if (coduo_crt_atoi(Info_ValueForKey(ui_cachedServerInfo, "pb")) != 0) {
                *imageHandle = ui_punkbusterShader;
            }
            return empty;

        case UI_SERVER_COLUMN_MOD:
            return coduo_crt_atoi(Info_ValueForKey(ui_cachedServerInfo, "mod")) != 0 ? "X" : empty;

        case UI_SERVER_COLUMN_PING:
            return ping > 0 ? Info_ValueForKey(ui_cachedServerInfo, "ping") : "...";
        }
    }
    ...
    if (feeder == UI_FEEDER_SERVER_STATUS) {
        ...
        if (index < 0 || index >= ui_serverStatusInfo.numLines || column < 0 || column >= UI_STATUS_COLUMN_COUNT) {
            return empty;
        }
