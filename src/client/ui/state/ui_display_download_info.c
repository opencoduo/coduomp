                            ...
                            float centerPoint, float yStart, float scale)
{
    ...
    int32_t downloadCount =
        (int32_t)trap_Cvar_VariableValue("cl_downloadCount");
    ...
    if (downloadSize > 0) {
        ...
        progressWidth =
            (int32_t)(((long double)downloadCount / downloadSize) * 640.0f);
        fillColor[0] = 1.0f;
        fillColor[1] = 0.0f;
        fillColor[2] = 0.0f;
        fillColor[3] = 0.15f;
        UI_FillRect(0.0f, yStart + 184.0f, (float)(progressWidth + 2),
                    85.0f, fillColor);
        UI_FillRect(0.0f, yStart + 185.0f, (float)(progressWidth + 1),
                    83.0f, fillColor);
        UI_FillRect(0.0f, yStart + 186.0f, (float)progressWidth, 81.0f,
                    fillColor);
    }

    trap_R_SetColor(labelColor);
    trap_R_Text_Paint(24.0f, yStart + 210.0f, font, scale, labelColor,
                      UI_SafeTranslateString(ui_downloadLabelDownloading), 0,
                      UI_DOWNLOAD_TEXT_LIMIT, UI_DOWNLOAD_TEXT_STYLE);
    trap_R_Text_Paint(24.0f, yStart + 235.0f, font, scale, labelColor,
                      UI_SafeTranslateString(ui_downloadLabelEstimatedTime), 0,
                      UI_DOWNLOAD_TEXT_LIMIT, UI_DOWNLOAD_TEXT_STYLE);
    trap_R_Text_Paint(24.0f, yStart + 260.0f, font, scale, labelColor,
                      UI_SafeTranslateString(ui_downloadLabelTransferRate), 0,
                      UI_DOWNLOAD_TEXT_LIMIT, UI_DOWNLOAD_TEXT_STYLE);

    if (downloadSize > 0) {
        /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
        downloadPercentage = (int32_t)(
            (int64_t)downloadCount * UI_DOWNLOAD_PERCENT_SCALE / downloadSize);
    }

    trap_R_Text_Paint(
        124.0f, yStart + 210.0f, font, scale, labelColor,
        downloadSize > 0
            ? va("%s (%d%%)", downloadName, downloadPercentage)
            : downloadName,
        0, 0, UI_DOWNLOAD_TEXT_STYLE);
    ...
    if (downloadCount >= UI_DOWNLOAD_MINIMUM_RATE_COUNT && downloadTime != 0) {
        int32_t elapsedSeconds =
            (ui_displayContextStorage.context.realTime - downloadTime) /
            UI_MILLISECONDS_PER_SECOND;
        int32_t transferBytesPerSecond = 0;

        if (elapsedSeconds != 0) {
            transferBytesPerSecond = downloadCount / elapsedSeconds;
        }
        UI_ReadableSize(transferRate, sizeof(transferRate),
                        transferBytesPerSecond);

        /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
        if (downloadSize >= UI_BYTES_PER_KILOBYTE &&
            transferBytesPerSecond != 0) {
            int32_t totalSeconds = downloadSize / transferBytesPerSecond;
            int32_t downloadedKilobytes =
                downloadCount / UI_BYTES_PER_KILOBYTE;
            int32_t totalKilobytes = downloadSize / UI_BYTES_PER_KILOBYTE;
            int32_t remainingSeconds =
                totalSeconds -
                downloadedKilobytes * totalSeconds / totalKilobytes;
            ...
            UI_PrintTime(timeLeft, sizeof(timeLeft),
                         estimateTotal / UI_DOWNLOAD_ESTIMATE_SAMPLES);
            trap_R_Text_Paint(159.0f, yStart + 235.0f, font, scale,
                              labelColor, timeLeft, 0, 0,
                              UI_DOWNLOAD_TEXT_STYLE);

            copiedLabel = UI_SafeTranslateString("EXE_COPIED");
            ofLabel = UI_SafeTranslateString("EXE_OF");
            copiedText = va("(%s %s %s %s)", downloadedSize, ofLabel,
                            totalSize, copiedLabel);
            Text_PaintCenter(centerPoint, yStart + 320.0f, copiedText, scale,
                             labelColor, font);
        } else {
            const char *copiedLabel;

            Text_PaintCenter(centerPoint, yStart + 235.0f,
                             UI_SafeTranslateString("EXE_ESTIMATING"), scale,
                             labelColor, font);
            copiedLabel = UI_SafeTranslateString("EXE_COPIED");
            if (downloadSize != 0) {
                copiedText = va("(%s %s %s %s)", downloadedSize,
                                UI_SafeTranslateString("EXE_OF"), totalSize,
                                copiedLabel);
            } else {
        ...
        if (transferBytesPerSecond != 0) {
            trap_R_Text_Paint(124.0f, yStart + 260.0f, font, scale,
                              labelColor,
                              va("%s/%s", transferRate,
        ...
        copiedText = va("(%s %s %s %s)", downloadedSize,
                        UI_SafeTranslateString("EXE_OF"), totalSize,
                        copiedLabel);
        Text_PaintCenter(centerPoint, yStart + 340.0f, copiedText, scale,
                         labelColor, font);
    }
}
