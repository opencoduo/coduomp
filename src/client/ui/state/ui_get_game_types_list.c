...
{
    enum {
        UI_GAMETYPE_SUFFIX_COMPARE_LIMIT = 99999
    };
    ...
    fileCount = trap_FS_GetFileList("maps/mp/gametypes", "gsc", fileList, sizeof(fileList));
    for (fileIndex = 0; fileIndex < fileCount; ++fileIndex) {
        ...
        if (filename[0] != '_') {
            ...
            if (Q_stricmpn(".gsc", suffix, UI_GAMETYPE_SUFFIX_COMPARE_LIMIT) == 0) {
                suffix[0] = '\0';
            }
            if (ui_gameTypeCount == UI_MAX_GAMETYPES || ui_joinGameTypeCount == UI_MAX_GAMETYPES) {
            ...
            ui_joinGameTypes[ui_joinGameTypeCount].gameType = ui_gameTypes[ui_gameTypeCount].gameType;
            {
                char *fileText = UI_LoadMenuTextFile(va("maps/mp/gametypes/%s.txt", filename));
                const char *displayName;

                if (fileText == NULL) {
                    displayName = ui_gameTypes[ui_gameTypeCount].gameType;
                } else {
                    displayName = String_Alloc(Com_Parse(&fileText));
                }
                ui_gameTypes[ui_gameTypeCount].displayName = displayName;
                ui_joinGameTypes[ui_joinGameTypeCount].displayName = displayName;
            }
            ++ui_gameTypeCount;
