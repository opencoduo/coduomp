...
static const uiStatusField_t ui_statusFields[] = {
    {"sv_hostname", "@EXE_SV_INFO_SERVERNAME", UI_STATUS_VALUE_TEXT},
    ...
    {"sv_floodprotect", "@EXE_SV_INFO_FLOODPROTECT",
     UI_STATUS_VALUE_BOOLEAN},
    {"sv_allowanonymous", "@EXE_SV_INFO_ALLOWANON", UI_STATUS_VALUE_TEXT},
    {"sv_maxclients", "@EXE_SV_INFO_MAXCLIENTS", UI_STATUS_VALUE_TEXT},
    {"sv_privateclients", "@EXE_SV_INFO_PRIVATECLIENTS",
     UI_STATUS_VALUE_TEXT},
    {"scr_friendlyFire", "@EXE_SV_INFO_FRIENDLY_FIRE",
     UI_STATUS_VALUE_FRIENDLY_FIRE},
    ...
    {"scr_allow_jeeps", "@GMI_EXE_SV_INFO_ALLOW_JEEPS",
     UI_STATUS_VALUE_BOOLEAN},
    {"scr_allow_tanks", "@GMI_EXE_SV_INFO_ALLOW_TANKS",
     UI_STATUS_VALUE_BOOLEAN},
    {"g_timeoutsallowed", "@PATCH_1_5_SV_INFO_TIMEOUTSALLOWED",
     UI_STATUS_VALUE_TEXT}
};
...
{
    enum { UI_STATUS_KEY_COMPARE_LIMIT = 99999 };
    int32_t outputIndex = 0;
    size_t fieldIndex;

    for (fieldIndex = 0;
         fieldIndex < sizeof(ui_statusFields) / sizeof(ui_statusFields[0]);
         ++fieldIndex) {
        ...
        for (lineIndex = 0; lineIndex < statusInfo->numLines; ++lineIndex) {
            ...
            if (line->column[1] == NULL || line->column[1][0] != '\0' ||
                line->column[0] == NULL ||
                Q_stricmpn(field->key, line->column[0],
                           UI_STATUS_KEY_COMPARE_LIMIT) != 0) {
                continue;
            }
            ...
            if (field->valueType == UI_STATUS_VALUE_BOOLEAN) {
                output->column[3] =
                    coduo_crt_atoi(output->column[3]) != 0
                                        ? "@EXE_YES"
                                        : "@EXE_NO";
            } else if (field->valueType == UI_STATUS_VALUE_FRIENDLY_FIRE) {
                output->column[3] =
                    UI_StatusFriendlyFireName(
                        coduo_crt_atoi(output->column[3]));
            }
            ++outputIndex;
        }
    }
