                      ...
                      const vec4_t color, int32_t textStyle)
{
    ...
    message = trap_SE_LocalizeMessage(
        va("EXE_SERVERFILTER\x14%s",
           ui_serverFilters[ui_serverFilterType].label),
        "server filter");
    trap_R_Text_Paint(rect->x, rect->y, font, scale, color, message,
                      UI_SERVER_FILTER_TEXT_STYLE,
                      UI_SERVER_FILTER_TEXT_LIMIT, textStyle);
}
