...
void Item_TextColor(itemDef_t *item, vec4_t color)
{
    ...
    if ((item->window.flags & (WINDOW_FADINGOUT | WINDOW_FADINGIN)) != 0 && display->realTime > item->window.nextTime) {
        item->window.nextTime = coduo_int32_from_bits((uint32_t)display->realTime + (uint32_t)menu->fadeCycle);
        if ((item->window.flags & WINDOW_FADINGOUT) != 0) {
             ...
             * 80-bit difference. */
            long double fadedAlpha = (long double)item->window.foreColor[3] - fadeAmount;
            ...
            long double fadedAlpha = (long double)fadeInAmount + item->window.foreColor[3];
            item->window.foreColor[3] = (float)fadedAlpha;
    ...
    if (item->cvarTest != NULL && item->cvarTest[0] != '\0' && item->enableCvar != NULL && item->enableCvar[0] != '\0' &&
        (item->cvarFlags & UI_ITEM_ENABLE_DISABLE_MASK) != 0 && !Item_EnableShowViaCvar(item, qtrue)) {
