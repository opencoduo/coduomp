...
void CG_FireFlameChunks(void)
{
    ...
    if (cg_fireFlameChunksCvarsRegistered == 0) {
        cgame_syscall(CG_CVAR_REGISTER, (intptr_t)&cg_rFullscreenCvar, (intptr_t)r_fullscreenCvarName, (intptr_t)cvarEnabledValue,
                      CG_FIRE_CVAR_FLAGS);
        cgame_syscall(CG_CVAR_REGISTER, (intptr_t)&cg_rOverbrightBitsCvar, (intptr_t)r_overbrightBitsCvarName, (intptr_t)cvarEnabledValue,
                      CG_FIRE_CVAR_FLAGS);
    } else {
        ...
        while (cur != (flameChunk_t *)0) {
...

            {
                ...
                if (cur->kind == 0) {           /* 0x30027eb1 CMP [+0x2c],0; JNE 0x30027f22 */
                     ...
                     * twice. */
                    long double curveBase = (long double)1.0f - (long double)cur->driftSpeed / (long double)900.0f;
                ...
                if (cur->lifeStartTime > 0.0) {
                    ...
                    for (;;) {
                        ...
                        if (child != (flameChunk_t *)0) {
                            ...
                            cur->lifeFraction = (float)((cur->lifeStartTime - cur->spawnTime) / (cur->endTime - cur->spawnTime));
                            ...
                            *child = *cur;              /* 0x30027fc9 REP MOVSD parent->child */
                             ...
                             * child->spawnTime). */
                            child->lifeFraction = (float)((cur->lifeStartTime - child->spawnTime) / (child->endTime - child->spawnTime));
                            ...
                            {
                                int32_t rt =
                                    coduo_fp_to_i32_extended((long double)cur->lifeStartTime); /* 0x30027fe9: FLD m64; no m32 spill */
                                CG_AdvanceFlameChunkSize(child, rt);        /* 0x30027ff7, EAX=child */
                                rt = coduo_fp_to_i32_extended((long double)cur->lifeStartTime);       /* 0x30027ffc: second FLD m64 */
...
#define FLAME_JITTER_SAMPLE(dst) \
    do { \
        ...
        long double a_ = (2.0f * (rf_ / FLAME_RAND_DIVISOR) - 1.0f) * 0.15f; \
        long double p_ = powl(1.0L - cur->lifeFraction, 3.0L); \
        (dst) = (a_ + p_ * 1.5L - ((long double)cur->driftSpeed / 900.0f) * 0.0f) * (long double)(double)0.3f; /* c3d8 */ \
    } while (0)
...
#undef FLAME_JITTER_SAMPLE
                                ...
                                if (cur->kind == FLAME_CHUNK_KIND_FIRE) {
                                    ...
                                    if (cur->driftSpeed < 1.0f) { /* 0x300281af FCOMP 1.0f(bce0) */
                                        ...
                                        if (cur->boneHandle == 0 /* 0x300281ca TEST [+0x40] */
                                            ...
                                            CG_FlamethrowerTrace(0x2810011, cg_flameTraceMaxs, cg_flameTraceMins, &traceOut, computeOut,
                                                                 hit, cur->ownerInfoIndex);
                                    ...
                                    child->spawnTime = cur->lifeStartTime; /* 0x30028325 */
                                    {
                                        ...
                                        if (cur->boneHandle == 0) { /* 0x30028345 */
                                            ...
                                            double j = ((double)(float)coduo_crt_rand() / FLAME_RAND_DIVISOR) * 600.0; /* c3a8 */
                                            ...
                                            {
                                                float powF = (float)powl(1.0L - cur->lifeFraction, 0.75L);
                                            ...
                                            if (cur->boneHandle != 0) { /* 0x300284b6 (true here) */
                                                ...
                                                float pick = (sel < 60.0f) ? 60.0f : (sel > 120.0f) ? 120.0f : sel;
                                                /* All-float chain 0x3002851f..0x30028537, ONE
                                                 * rounding (FSTP float @0x3002853b). */
                                                float fv = (float)coduo_crt_rand() / FLAME_RAND_DIVISOR * 25.0f + pick;
                                            ...
                                            if (cg_entities[cur->ownerInfoIndex].currentState.eType == ET_VEHICLE) {
                                     ...
                                     * parent's offset direction, jitter it, copy the world position. */
                                    {
                                        ...
                                        child->driftDir[0] = (float)((2.0f * ((float)coduo_crt_rand() / FLAME_RAND_DIVISOR) - 1.0f) *
                                                                     right[0]); /* 0x30028636 [esp+0x6c] */
                                        child->driftDir[1] = (float)((2.0f * ((float)coduo_crt_rand() / FLAME_RAND_DIVISOR) - 1.0f) *
                                                                     right[1]); /* 0x3002865f [esp+0x70] */
                                        child->driftDir[2] = (float)((2.0f * ((float)coduo_crt_rand() / FLAME_RAND_DIVISOR) - 1.0f) *
                                                                     right[2]); /* 0x3002868c [esp+0x74] */
                                        ...
                                        {
                                            long double s = (float)coduo_crt_rand() / FLAME_RAND_DIVISOR * 0.45f + 0.25f; /* 0x30028696 */
                                            child->driftDir[0] =
                                                child->driftDir[0] + s * (1.0f - fabsf(cur->centerOffset[2])) * forward[0]; /* [esp+0x60] */
                                        }
                                        {
                                            long double s = (float)coduo_crt_rand() / FLAME_RAND_DIVISOR * 0.45f + 0.25f; /* 0x300286e1 */
                                            child->driftDir[1] =
                                                child->driftDir[1] + s * (1.0f - fabsf(cur->centerOffset[2])) * forward[1]; /* [esp+0x64] */
                                        }
                                        {
                                            long double s = (float)coduo_crt_rand() / FLAME_RAND_DIVISOR * 0.45f + 0.25f; /* 0x30028730 */
                                            child->driftDir[2] =
                                                child->driftDir[2] + s * (1.0f - fabsf(cur->centerOffset[2])) * forward[2]; /* [esp+0x68] */
                                        }
                                        ...
                                        child->driftSpeed = (float)coduo_crt_rand() / FLAME_RAND_DIVISOR * fabsf(cur->expansionRate) *
                                                                (1.0f - cur->lifeFraction) +
                                                            2.0f;
                                    ...
                                    if (cur->ownerInfoIndex != cg_snap->ps.psClientNum) { /* 0x30028847..0x30028855 */
                                        ...
                                        child->spawnTime =
                                            child->spawnTime -
                                            (powl((long double)cur->lifeFraction, 0.5L) + 1.0) *
                                                sizeDelta; /* 0x30028884; base FLD m32 @0x30028869, result raw (FADD @0x3002887a) */
                                        child->endTime =
                                            child->endTime -
                                            (powl((long double)cur->lifeFraction, 0.5L) + 1.0) *
                                                sizeDelta; /* 0x300288a5; base FLD m32 @0x3002888a, result raw (FADD @0x3002889b) */
                                    }
                                }
                                ...
                                if (cur->kind == 0) {
                                     ...
                                     *   field_a8 = acc * (parent->expansionRate/60.0f) * -2.25f;       (bff0/c380) */
                                    {
                                        ...
                                        float pInv = (1.0f - (float)powl((long double)cur->driftSpeed / 900.0f, 2.0L) + 1.0f) *
                                                     0.0f; /* [esp+0x14] */
                                        float rf = (float)coduo_crt_rand();
                                        float radiusPow = (float)powl((long double)child->radius / 290.0f, 2.0L);
                                        float lifePow = (float)powl((long double)cur->lifeFraction, 1.0L);
                                        long double acc = (long double)rf / FLAME_RAND_DIVISOR * 3.0f + pInv;
                                        acc += (long double)radiusPow * 16.0f;
                                        acc += (long double)lifePow * 10.0f;
                                        child->expansionRate = (float)(acc * ((long double)cur->expansionRate / 60.0f) * -2.25f); /* c380 */
                                    }
                                     ...
                                     *    + 0.2(c370)) * (parent->smokeDensityRate*0.25f(be58) + 105.0f(c368)) * 0.75(be38). */
                                    {
                                        ...
                                        float rf = (float)coduo_crt_rand();
                                        float powF = (float)powl(1.0L - cur->lifeFraction, 5.0L);
                                        long double left = ((long double)rf / FLAME_RAND_DIVISOR) * 0.0 + (long double)powF * 0.5f +
                                                           (long double)(double)0.2f;
                                        long double right = (long double)child->smokeDensityRate * 0.25f + 105.0f;
                                        child->smokeDensityRate = (float)(left * right * 0.75f); /* be38 */
                                    }
                                    ...
                                    {
                                        float powF = (float)powl(1.0L - cur->lifeFraction, 2.0L);
                                        child->spawnScale = (float)((long double)powF * 0.65f + 0.35f);
                                    }
                                    ...
                                    if (cur->ownerInfoIndex == cg_snap->ps.psClientNum) {
                                        ...
                                        child->driftSpeed = (child->lifeFraction * 0.2f + 0.1f) * child->driftSpeed;
                                    } else {
                                ...
                                if (cur->boneHandle == 0 /* 0x30028bed */
                                    ...
                                    long double gateThreshold = (long double)gateRand / FLAME_RAND_DIVISOR;
                                    float gate = (float)powl((long double)cur->driftSpeed / 900.0f, 2.0L);
                                    if ((long double)gate * w3c * w4c > gateThreshold) {
                                        ...
                                        *smoke = *child; /* REP MOVSD child(ebp)->smoke */
                                         ...
                                         *   to [-15.0f(c34c), -2.0f(c150)]). EDI still parent via [esp+0x24]. */
                                        {
                                             ...
                                             * are `float ptr`. */
                                            float v = (cur->lifeFraction * 1.85f + 0.55f) * smoke->expansionRate;
                                        ...
                                        {
                                            float rf = (float)coduo_crt_rand();
                                            long double roll = (2.0f * ((long double)rf / FLAME_RAND_DIVISOR) - 1.0f) * 25.0f + 180.0f;
                                         ...
                                         *   (rand/32768)*2600.0(c340) + parent->lifeStartTime + 1600.0(c398). */
                                        {
                                            float rf = (float)coduo_crt_rand();
                                            long double endTime =
                                                ((long double)rf / FLAME_RAND_DIVISOR) * 2600.0 + (long double)cur->lifeStartTime + 1600.0;
                                        ...
                                        smoke->lifeFraction =
                                            (float)((cur->lifeStartTime - smoke->spawnTime) / (smoke->endTime - smoke->spawnTime));
                                        ...
                                        {
                                            int32_t st = coduo_fp_to_i32_extended((long double)cur->lifeStartTime); /* 0x30028ec3 */
                                            CG_AdvanceFlameChunkSize(smoke, st); /* 0x30028ed1 */
                                            st = coduo_fp_to_i32_extended((long double)cur->lifeStartTime); /* 0x30028ed6 */
                        ...
                        finalize_child:
                             ...
                             * the current flame time. (ebx is restored to the parent here.) */
                            {
                                ...
                                child->lifeFraction = (float)(((double)ft - child->spawnTime) / (child->endTime - child->spawnTime));
                            ...
                            long double distSquared = (long double)ddz * (long double)ddz + (long double)ddy * (long double)ddy +
                            ...
                            if (stepKind == 0 /* 0x30028fd8 TEST ESI */
                                ...
                                stepAccum = (double)cur->radius * 4.0 / dist * stepAccum; /* 0x30029003..0x30029013 */
                            }
                        }
                         ...
                         * subsequent /32768.0f and the two products stay in x87 width. */
                        {
                            ...
                            cur->lifeStartTime += (double)rf / FLAME_RAND_DIVISOR * stepAccum + stepAccum / 2.0; /* fdiv 2.0(double bde8) */
                        }
            ...
            inner_final_tail:
                 ...
                 *   re-read is done here; `next` already holds it. EBP=0; EDI=1. */
                {
                    ...
                    cur->lifeFraction = (float)(((double)ft - cur->spawnTime) / (cur->endTime - cur->spawnTime));
    ...
    if (cg_flameInfo[cg_snap->ps.psClientNum].activeUntil < coduo_int32_from_bits(cg_flameTime)) {
