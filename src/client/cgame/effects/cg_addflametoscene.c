                        ...
                        float alpha, int32_t finalFrame)
{
    ...
    if (!(chunk->kind == FLAME_KIND_DAMAGE &&
          chunk->ownerSentinel != UINT32_MAX) &&
        coduo_crt_rand() % 5 == 0 && chunk->lifeFraction < 0.89999998f) {
        CG_FlameDamage(chunk->worldPos, chunk->ownerClientNum,
                                chunk->radius, chunk);
    }

    const float selectedRadius =
        finalFrame == 1 ? chunk->radiusBaseA : chunk->radiusBaseB;
    const int32_t selectedRadiusInt = coduo_fp_to_i32_extended(selectedRadius);
    float halfExtent = chunk->radius * 0.5f;
    vec3_t center = {
        chunk->worldPos[0], chunk->worldPos[1], chunk->worldPos[2]
    };

    if ((chunk->centerOffset[0] != 0.0f ||
         chunk->centerOffset[1] != 0.0f ||
         chunk->centerOffset[2] != 0.0f) &&
        chunk->centerOffset[2] < 0.0f) {
        center[2] = (float)(
            (long double)center[2] -
            (long double)fabsf(chunk->centerOffset[2]) *
                (long double)20.0f);
    }
    ...
    vec3_t facing = {
        center[0] - cg_refdef.vieworg[0],
        center[1] - cg_refdef.vieworg[1],
        center[2] - cg_refdef.vieworg[2]
    };
    ...
    const long double placement = halfExtent < facingDistance
                                ? -halfExtent
                                : -(long double)facingDistance - 20.0f;
    ...
    const long double lateLifeDelta =
        (long double)chunk->lifeFraction - (long double)0.5f;
    ...
    const int32_t colorLevel = coduo_fp_to_i32_extended(
        powl(1.0f - (long double)lateLife * 2.0f, 1.0L) * 80.0f + 20.0f);
    ...
    if (chunk->kind == FLAME_KIND_SMOKE) {
        ...
        rgba[0] = (uint8_t)coduo_fp_to_i32_extended(
            (long double)smokeColorBase * (long double)0.97647059f);
        rgba[1] = (uint8_t)coduo_fp_to_i32_extended(
            (long double)smokeColorBase * (long double)0.93725491f);
        rgba[2] = (uint8_t)coduo_fp_to_i32_extended(
            (long double)smokeColorBase * (long double)0.85882354f);
        rgba[3] = (uint8_t)coduo_fp_to_i32_extended(
            (long double)alpha * (long double)76.5f);
    } else if (chunk->kind == FLAME_KIND_DAMAGE &&
               chunk->ownerSentinel == UINT32_MAX) {
        ...
        const long double doubledLife =
            (long double)chunk->lifeFraction +
            (long double)chunk->lifeFraction;
        ...
        rgba[0] = (uint8_t)coduo_fp_to_i32_extended(
            (!(doubledLife < 1.0f) ? 1.0L : doubledLife) * 255.0f);
        rgba[1] = (uint8_t)coduo_fp_to_i32_extended(
            (!(doubledLife < 1.0f) ? 1.0L : doubledLife) * 255.0f);
        rgba[2] = (uint8_t)coduo_fp_to_i32_extended(
            (!(doubledLife < 1.0f) ? 1.0L : doubledLife) * 255.0f);
    ...
    if (chunk->kind != FLAME_KIND_SMOKE &&
        cg_rFullscreenCvar.integer != 0 &&
        cg_rOverbrightBitsCvar.integer != 0) {
        rgba[0] >>= 1;
        rgba[1] >>= 1;
        rgba[2] >>= 1;
    }
    ...
    vec3_t forwardEnd = {
        (float)((long double)cg_refdef.vieworg[0] +
                (long double)cg_refdef.viewaxis[0][0] * 1024.0L),
        (float)((long double)cg_refdef.vieworg[1] +
                (long double)cg_refdef.viewaxis[0][1] * 1024.0L),
        (float)((long double)cg_refdef.vieworg[2] +
                (long double)cg_refdef.viewaxis[0][2] * 1024.0L)
    };
    vec3_t projected;
    ProjectPointOnLine(projected, cg_refdef.vieworg, center, forwardEnd);
    vec3_t projectedDirection = {
        projected[0] - cg_refdef.vieworg[0],
        projected[1] - cg_refdef.vieworg[1],
    ...
    const long double forwardDot =
        (long double)cg_refdef.viewaxis[0][2] *
            (long double)projectedDirection[2] +
        (long double)cg_refdef.viewaxis[0][1] *
            (long double)projectedDirection[1] +
        (long double)cg_refdef.viewaxis[0][0] *
            (long double)projectedDirection[0];
    if (forwardDot < 0.0f) {
        return;
    }
     ...
     * extents can live in the float scratch the DLL spills them to. */
    {
        ...
        const long double upCornerX =
            negExtent * cg_flameSpriteUp[0] + center[0];         /* raw */
        const long double upCornerY =
            negExtent * cg_flameSpriteUp[1] + center[1];         /* raw */
        const float upCornerZ =
            (float)(negExtent * cg_flameSpriteUp[2] + center[2]); /* FSTP [ESP+0x20] */

        verts[0].xyz[0] = (float)(negExtent * cg_flameSpriteRight[0] + upCornerX);
        const long double corner0Y =
            negExtent * cg_flameSpriteRight[1] + upCornerY;      /* raw */
        verts[0].xyz[1] = (float)corner0Y;
        verts[0].xyz[2] = (float)(negExtent * cg_flameSpriteRight[2] + upCornerZ);

        const long double doubledExtentRaw =
            (long double)halfExtent + (long double)halfExtent;  /* retained ST0 */
        const float doubledExtent =
            (float)doubledExtentRaw;                            /* non-popping FST scratch */
        verts[1].xyz[0] = (float)(
            doubledExtentRaw *
                (long double)cg_flameSpriteUp[0] +
            (long double)verts[0].xyz[0]);
        const long double corner1Y =
            corner0Y + doubledExtent * cg_flameSpriteUp[1];      /* raw Y carry */
        verts[1].xyz[1] = (float)corner1Y;
        const long double corner1Z =
            doubledExtent * cg_flameSpriteUp[2] + verts[0].xyz[2];
        verts[1].xyz[2] = (float)corner1Z;

        verts[2].xyz[0] = (float)(
            (long double)doubledExtent *
                (long double)cg_flameSpriteRight[0] +
            (long double)verts[1].xyz[0]);
        verts[2].xyz[1] =
            (float)(doubledExtent * cg_flameSpriteRight[1] + corner1Y); /* raw Y carry */
        const long double corner2Z =
            doubledExtent * cg_flameSpriteRight[2] + corner1Z;   /* raw Z carry */
        verts[2].xyz[2] = (float)corner2Z;

        const long double negDoubledExtentRaw =
            (long double)halfExtent * (long double)-2.0f;       /* retained ST0 */
        const float negDoubledExtent =
            (float)negDoubledExtentRaw;                         /* non-popping FST scratch */
        verts[3].xyz[0] = (float)(
            negDoubledExtentRaw *
                (long double)cg_flameSpriteUp[0] +
            (long double)verts[2].xyz[0]);
        verts[3].xyz[1] = (float)(
            (long double)negDoubledExtent *
                (long double)cg_flameSpriteUp[1] +
            (long double)verts[2].xyz[1]);
        verts[3].xyz[2] =
            (float)(negDoubledExtent * cg_flameSpriteUp[2] + corner2Z); /* raw Z carry */
    }
    ...
    int32_t frame = coduo_fp_to_i32_extended(
        floor((double)((long double)animationFraction *
                       (long double)43.0f)));
    if (frame < 0) {
        frame = 0;
    ...
    for (int32_t i = 0; i < FLAME_POLY_VERTEX_COUNT; ++i) {
        verts[i].xyz[2] = (float)(
            (long double)clampedExtent *
                (long double)0.55000001f +
            (long double)verts[i].xyz[2] -
            (long double)8.8000002f);
    }

    if (finalFrame == 1) {
        qhandle_t material = chunk->overrideMaterial != 0
                                 ? (qhandle_t)chunk->overrideMaterial
                                 : cg_flameFireMaterials[frame];
        cgame_syscall(CG_R_ADDPOLYTOSCENE, material,
                      FLAME_POLY_VERTEX_COUNT, verts);
        return;
    }

    /* 0x3002714a..0x3002715a: FILD frame; FMUL double 0.7 (0x3007c208); FADD
     * double 20.0 (0x3007c128); CALL _ftol2 — all raw, no store. */
    const int32_t smokeFrame = coduo_fp_to_i32_extended(
        (long double)frame * (long double)0.7 + (long double)20.0);
    if (smokeFrame < FLAME_FIRE_FRAME_COUNT) {
        ...
        const float earlyLife = chunk->lifeFraction < 0.15000001f
                                    ? chunk->lifeFraction
                                    : 0.15000001f;
        ...
        const long double smokeDensity =
            powl((long double)chunk->smokeDensityRate * 0.0071428572f, 1.0L) + 0.5;
        ...
        for (int32_t i = 0; i < FLAME_POLY_VERTEX_COUNT; ++i) {
            verts[i].xyz[2] = (float)(
                (long double)clampedSmokeLife *
                    (long double)2.5f *
                ((long double)earlyLife *
                    (long double)6.6666665f) *
                (long double)halfExtent *
                (long double)0.64999998f *
                smokeDensity +
                (long double)verts[i].xyz[2]);
        }
        cgame_syscall(CG_R_ADDPOLYTOSCENE, cg_flameFireMaterials[smokeFrame],
                      FLAME_POLY_VERTEX_COUNT, verts);
    }
