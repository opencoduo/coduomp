...
enum {
    SHOCK_SCREEN_BLEND_TIME, SHOCK_SCREEN_BLEND_FADE_TIME,
    SHOCK_VIEW_KICK_PERIOD, SHOCK_VIEW_KICK_RADIUS, SHOCK_SOUND,
    SHOCK_SOUND_FADE_IN_TIME, SHOCK_SOUND_FADE_OUT_TIME,
    SHOCK_SOUND_LOOP_FADE_TIME, SHOCK_SOUND_LOOP_END_DELAY,
    SHOCK_SOUND_ROOM_TYPE, SHOCK_SOUND_WET_LEVEL, SHOCK_SOUND_MOD_END_DELAY,
    SHOCK_VOLUME_AUTO, SHOCK_VOLUME_MENU, SHOCK_VOLUME_WEAPON,
    SHOCK_VOLUME_VOICE, SHOCK_VOLUME_ITEM, SHOCK_VOLUME_BODY,
    SHOCK_VOLUME_LOCAL, SHOCK_VOLUME_MUSIC, SHOCK_VOLUME_ANNOUNCER,
    SHOCK_VOLUME_SHELLSHOCK, SHOCK_MOUSE, SHOCK_MOUSE_MAX_PITCH_SPEED,
    SHOCK_MOUSE_MAX_YAW_SPEED, SHOCK_MOUSE_SENSITIVITY_SCALE,
    SHOCK_MOUSE_FADE_TIME
};

#define SET_SHOCK_MILLISECONDS(field_, index_, minimumOne_) do {                 \
    int32_t shockMilliseconds_ =                                                \
        Script_RoundToNearestInt(cg_shockParamTargets[(index_)]->value * 1000.0f); \
    (field_) = (minimumOne_) && shockMilliseconds_ < 1                          \
             ? 1 : shockMilliseconds_;                                          \
} while (0)

#define SET_SHOCK_UNIT(field_, index_) do {                                     \
    float shockUnitValue_ = cg_shockParamTargets[(index_)]->value;              \
    if (shockUnitValue_ < 0.0f) shockUnitValue_ = 0.0f;                         \
    if (shockUnitValue_ > 1.0f) shockUnitValue_ = 1.0f;                         \
...
void CG_SetShellShockParams(shellshock_t *out)
{
    ...
    SET_SHOCK_MILLISECONDS(out->screenBlendFadeTime,
                           SHOCK_SCREEN_BLEND_FADE_TIME, qtrue);
    ...
    SET_SHOCK_MILLISECONDS(out->soundLoopFadeTime,
                           SHOCK_SOUND_LOOP_FADE_TIME, qtrue);
    SET_SHOCK_MILLISECONDS(out->soundLoopEndDelay,
                           SHOCK_SOUND_LOOP_END_DELAY, qfalse);
    Q_strncpyz(out->soundRoomType, cg_shockParamTargets[SHOCK_SOUND_ROOM_TYPE]->string,
               (int)sizeof(out->soundRoomType));
    SET_SHOCK_UNIT(out->soundWetLevel, SHOCK_SOUND_WET_LEVEL);
    SET_SHOCK_MILLISECONDS(out->soundModEndDelay,
                           SHOCK_SOUND_MOD_END_DELAY, qfalse);
