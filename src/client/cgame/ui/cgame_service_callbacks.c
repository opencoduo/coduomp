...
{
    char buffer[128] = {0};
    cgame_syscall(CG_CVAR_VARIABLE_STRING_BUFFER, (intptr_t)name, (intptr_t)buffer, sizeof(buffer));
...
int32_t CG_OwnerDrawWidth(int32_t ownerDraw, int32_t font, float scale) /* 0x3002d590 */
{
    enum {
    ...
    if (ownerDraw == OWNERDRAW_WIDTH_GAMETYPE)
        return coduo_int32_from_bits((uint32_t)cgame_syscall(CG_R_TEXT_WIDTH, (intptr_t)cgs_gametype, font, CG_FloatBits(scale), 0));
    if (ownerDraw != OWNERDRAW_WIDTH_FRAGGED_BY)
        return 0;
    const char *text = cg_fraggedByName[0] != '\0' ? va("Fragged by %s", cg_fraggedByName) : g_str_empty;
    return coduo_int32_from_bits((uint32_t)cgame_syscall(CG_R_TEXT_WIDTH, (intptr_t)text, font, CG_FloatBits(scale), 0));
}

int32_t CG_PlayCinematic(const char *name, float x, float y, float w, float h) /* 0x3002d610 */
{
    enum {
        CG_CINEMATIC_PLAY_FLAGS = 1 << 1
    }; /* CIN_LOOP */
    ...
    return (int32_t)cgame_syscall(CG_CIN_PLAY_CINEMATIC, (intptr_t)name, xInt, yInt, wInt, hInt, CG_CINEMATIC_PLAY_FLAGS);
}
...
void CG_DrawCinematic(int32_t handle, float x, float y, float w, float h) /* 0x3002d670 */
{
    ...
    cgame_syscall(CG_CIN_SET_EXTENTS, handle, xInt, yInt, wInt, hInt);
