...
static qboolean cgame_compat_hide_player_health_menu(const menuDef_t *menu, qboolean passiveHudPass)
{
    if (passiveHudPass == qfalse || menu == NULL || (cg_predictedPlayerState.entityStateFlags & EF_IN_VEHICLE) == 0 ||
        menu->window.name == NULL) {
        return qfalse;
    }

    return Q_stricmpn(menu->window.name, "Health", CGAME_COMPAT_UNBOUNDED_NAME_LENGTH) == 0 ? qtrue : qfalse;
}
