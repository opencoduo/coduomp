...
void CG_UIDisplayContextInit(void)
{
    ...
    /* ORIGINAL_SOURCE_DIFFERENCE: retail stores the four trap-wrapper
     * addresses 0x3003e0f0/0x3003de30/0x3003dde0/0x3003de10 directly. Their
     * recovered signatures carry float arguments as opaque syscall dwords,
     * while displayContextDef_t carries semantic floats. Calling through a
     * cast function pointer is not defined C even where i386 happens to give
     * both signatures identical stack slots, and register-based 64-bit ABIs
     * are not compatible at all. These typed adapters preserve the argument
     * bits while keeping the callback boundary defined on every host. */
