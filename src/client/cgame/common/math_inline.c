...
{
    out[0] = scale * in[0]; out[1] = scale * in[1];
    out[2] = scale * in[2]; out[3] = scale * in[3];
}
...
void MatrixTransformVector3(const vec3_t in, const float matrix[3][3], vec3_t out) /* 0x30049a40 */
{
    ...
    out[0] = (float)(
        ((double)matrix[0][1] * (double)in[1] +
         (double)matrix[0][2] * (double)in[2]) +
        (double)matrix[0][0] * (double)in[0]);
    out[1] = (float)(
        ((double)matrix[1][0] * (double)in[0] +
         (double)matrix[1][1] * (double)in[1]) +
        (double)matrix[1][2] * (double)in[2]);
    out[2] = (float)(
        ((double)matrix[2][0] * (double)in[0] +
         (double)matrix[2][1] * (double)in[1]) +
        (double)matrix[2][2] * (double)in[2]);
}
