...
long double CG_CubicInterpolate(float t, float p0, float p1,
                                float p2, float p3)
{
    ...
    return p1Wide + tWide * ((p2Wide - p0Wide) +
           tWide * ((p0Wide - p1Wide) - a + tWide * a));
}

// Source RVA: 0x3001ffd0
void TransformPoint(const float matrix[16], const vec3_t point,
                                 vec3_t out)
{
    ...
    const long double outX =
        (((long double)matrix[8] * point[2] +
          (long double)matrix[4] * point[1]) +
         (long double)matrix[0] * point[0]) + matrix[12];
    out[0] = (float)outX;

    const long double outY =
        (((long double)matrix[1] * point[0] +
          (long double)matrix[9] * point[2]) +
         (long double)matrix[5] * point[1]) + matrix[13];
    out[1] = (float)outY;

    const long double outZ =
        (((long double)matrix[2] * point[0] +
          (long double)matrix[10] * point[2]) +
         (long double)matrix[6] * point[1]) + matrix[14];
    out[2] = (float)outZ;
}
...
void SetShellshockState(shellshock_t *params,
                                     int32_t startTime, int32_t duration)
{
