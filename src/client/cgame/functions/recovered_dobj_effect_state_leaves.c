...
static CG_EFFECT_ALWAYS_INLINE void cgame_compat_effect_seconds(int32_t frameTime, float *secondsOut)
{
    ...
    __asm__ __volatile__("fildl %1\n\t"
                         ...
                         "fstps %0"
                         : "=m"(*secondsOut)
                         : "m"(frameTime), "m"(CG_EFFECT_SECONDS_PER_MILLISECOND)
                         : "st");
#else
    *secondsOut = (float)((long double)frameTime * (long double)CG_EFFECT_SECONDS_PER_MILLISECOND);
#endif
}
...
{
    (void)cgame_syscall(CG_Z_FREE_INTERNAL, (intptr_t)cg_flameChunks);
    cg_flameChunks = 0;
}
...
void SetEffectFrameTime(int32_t effectContext)
{
    ...
    (void)cgame_syscall(CG_DOBJ_ADVANCE_SERVER_TIME, effectContext, CG_FloatBits(seconds));
}

// Source RVA: 0x30016400
void CG_SetDObjInfo(int32_t entityNum, uint32_t key, XModel *model)
{
...
qboolean CG_CheckDObjInfoMatches(int32_t entityNum, uint32_t key, const XModel *model)
{
    return cg_dObjInfoKeys[entityNum] == key && cg_dObjInfoHandles[entityNum] == model ? qtrue : qfalse;
}
...
void CG_ScrollScoreboardUp(void)
{
    if (cg_scoreboardScrollPos > 0) {
        cg_scoreboardScrollPos = coduo_int32_from_bits((uint32_t)cg_scoreboardScrollPos - (uint32_t)cg_scoreboardScrollStep_vmCvar.integer);
        if (cg_scoreboardScrollPos < 0)
