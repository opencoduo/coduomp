...
enum {
    OBJPTR_ANGLE2SHORT_MASK = 0xffff
};
...
#define OBJPTR_ICON_SIZE_SCALE \
    (43.75)                /* 0x3007c1e8 = 0x4045e00000000000
...
void CG_DrawObjectivePointers(const rectDef_t *rect, const vec3_t color)
{
    ...
    const float centerX = (float)((long double)cg_hudCompassSize_vmCvar.value * (long double)rect->w * 0.5L + (long double)rect->x);
    const long double centerYAnchor = (long double)cg_hudCompassSize_vmCvar.value * (long double)rect->h * 0.5L + (long double)rect->y;
    const long double centerYSlide = ((long double)cg_hudCompassSize_vmCvar.value - 1.0L) * (long double)OBJPTR_CENTER_BIAS;
    ...
    if ((uint32_t)clientNum >= (uint32_t)MAX_CLIENTS) {
        Com_Error(ERR_DROP,
                  "\x15"
                  "CG_DrawObjectivePointers: invalid client number %i",
                  clientNum);
        return;
    }
    ...
    for (int i = 0; i < PLAYERSTATE_OBJECTIVE_COUNT; ++i) {
        ...
        if (obj->entityNum == ENTITYNUM_NONE) {
            ...
            const centity_t *ent = cg_entities + obj->entityNum;
        ...
        vec3_t dir;
        if ((cg_snap->ps.entityStateFlags & EF_IN_VEHICLE) && cg_snap->ps.viewLockedEntityNum != ENTITYNUM_NONE) {
            const int32_t followedEntityNum = cg_snap->ps.viewLockedEntityNum;
            /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered client-module boundary input and state before use. */
            if ((uint32_t)followedEntityNum >= (uint32_t)MAX_GENTITIES) {
                Com_Error(ERR_DROP,
                          "\x15"
                          "CG_DrawObjectivePointers: invalid "
                          "view-lock entity %i",
                          followedEntityNum);
                return;
            }
            const centity_t *followed = &cg_entities[followedEntityNum];
        ...
        uint32_t shortAngle = (uint32_t)coduo_fp_to_i32_extended(((long double)vectoyaw(dir) - (long double)cg_compassRefYaw) *
        ...
        float distance = (float)__builtin_sqrtl((long double)dir[1] * (long double)dir[1] + (long double)dir[0] * (long double)dir[0]);
        ...
        if (distance <= nearDist) {
            ...
            if (denom != 0.0f) /* 0x30030083..0x30030094 FUCOMPP guard */
                alphaFrac = (float)(((long double)distance - (long double)nearDist) / (long double)denom); /* 0x30030096..0x300300a4 */
            /* 0x300300a8: 1.0 - (1.0 - minAlpha) * alphaFrac. */
            drawColor[3] = (float)(1.0L - (1.0L - (long double)minAlpha) * (long double)alphaFrac);
        }
        ...
        if (distance <= maxSizeDist) {
            ...
            if (denom2 != 0.0f) /* 0x3003010e..0x3003011f FUCOMPP guard */
                sizeFrac = (float)(((long double)nearDist - (long double)distance) / (long double)denom2); /* 0x30030121..0x3003012f */
            /* 0x30030133: 1.0 - (1.0 - maxSize) * sizeFrac. */
            size = (float)(1.0L - (1.0L - (long double)maxSize) * (long double)sizeFrac);
        }
        ...
        float iconSize = (float)((long double)cg_hudCompassSize_vmCvar.value * (long double)size * (long double)OBJPTR_ICON_SIZE_SCALE);
        ...
        float bearingRad = (float)((long double)bearingDeg * (long double)OBJPTR_DEG2RAD_PI / (long double)OBJPTR_DEG2RAD_180);
        float cosB, sinB;
        ...
        float ringRadius = cg_hudCompassSize_vmCvar.value * OBJPTR_RING_RADIUS_SCALE;
        float iconX = (float)(((long double)centerX - (long double)ringRadius * 0.5L) - (long double)sinB * (long double)iconSize);
        float iconY = (float)(((long double)centerY - (long double)ringRadius * 0.5L) - (long double)cosB * (long double)iconSize);
