...
enum {
    HINT_CFGSTRING_BASE = 1365
}; // gameState hint slot base (index + 1365)
...
static qboolean cgame_compat_hint_format_is_valid(const char *format, const char *signature)
{
...
static qboolean cgame_compat_splice_hint_key(char *dst, size_t dstSize, const char *tmpl, const char *marker, const char *keyName)
{
...
too_long:
    Com_Printf("WARNING: rejected oversized cursor-hint key substitution\n");
    return qfalse;
}
...
void CG_DrawCursorhint(rectDef_t *hintRect, int32_t fontContext, int32_t scaleBits, const vec4_t color, int32_t textStyle)
{
    ...
    } else if (kind >= CURSOR_HINT_OFF && kind < CURSOR_HINT_BUILTIN_ICON_COUNT) {
        hintShader = cgs_media_usableHintShaders[kind];
    } else if (kind >= CURSOR_HINT_WEAPON_BASE && kind < CURSOR_HINT_WEAPON_BASE + MAX_WEAPONS) {
        hintShader = cg_weaponHudIcons[kind - CURSOR_HINT_WEAPON_BASE];
    } else if (kind >= CURSOR_HINT_WEAPON_BASE + MAX_WEAPONS && kind < CURSOR_HINT_WEAPON_BASE + 2 * MAX_WEAPONS) {
        hintShader = cg_weaponAmmoIcons[kind - (CURSOR_HINT_WEAPON_BASE + MAX_WEAPONS)];
    }
    ...
    vec_t *fadeColor =
        CG_FadeColor(coduo_int32_from_bits((uint32_t)cg_overlayFadeStartTime), coduo_int32_from_bits((uint32_t)cg_overlayFadeDuration));
    ...
    if (style == HINT_STYLE_WOBBLE) {
        const long double phase = coduo_x87_sinl((long double)coduo_int32_from_bits((uint32_t)cg_time) * (long double)HINT_PHASE_RATE);
        fadeColor[3] = (float)((phase + 1.0L) * (long double)fadeColor[3] * 0.5L);
    ...
    if (style >= HINT_STYLE_WOBBLE) {
        ...
        if (style == HINT_STYLE_PULSE_SUBSEC) {
            ...
            int32_t rem = coduo_int32_from_bits((uint32_t)cg_overlayFadeStartTime) % 1000;
            pulseScale = (float)((long double)rem * (long double)HINT_MS_TO_FRAC);
        } else {
    ...
    if (kind >= HINT_KIND_PICKUP_LO && kind <= HINT_KIND_PICKUP_HI) {
        ...
        if (weap->wideListIcon != 0) {
            iconWideBias = (float)((long double)r->w * (long double)-0.5f);
        ...
        int32_t emptySlot = BG_GetEmptySlotForWeapon(&cg_predictedPlayerState, weaponIndex);
        if (emptySlot == 0) {
            ...
            if (curWeap->slot == weap->slot) {
                ...
                const char *forStr = CG_SafeTranslateString_Internal("cgame", "CGAME_FOR"); // 0x30079880
                const char *swapFmt = CG_SafeTranslateString_Internal("cgame", "CGAME_SWAPWEAPONS"); // 0x3007986c
                const char *pickName = cgWeap->displayName; // [+0x10]+0xb0
                const char *curName = cg_weaponInfos[curWeaponIndex].displayName; // +0xb0
                ...
                const char *swapMsg;
                /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered client-module boundary input and state before use. */
                if (cgame_compat_hint_format_is_valid(swapFmt, "s") == qfalse) {
                    swapMsg = swapFmt;
                } else {
                    swapMsg = va(swapFmt, activateKey, curName, forStr, pickName);
                }
                ...
                int32_t occupant = (int8_t)cg_predictedPlayerState.weaponSlots[weap->slot]; // MOVSX byte
                ...
                int32_t occSlotWeapon = (int8_t)cg_predictedPlayerState.weaponSlots[weap->slot]; // MOVSX [ECX+0x30483708]
                /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered client-module boundary input and state before use. */
                if ((uint32_t)occSlotWeapon >= MAX_WEAPONS) {
                    Com_Printf("WARNING: invalid cursor-hint weapon slot value %i\n", occSlotWeapon);
                    return;
                }
                const char *occName = cg_weaponInfos[occSlotWeapon].displayName;
                ...
                if (cgame_compat_hint_format_is_valid(swapFmt, "s") == qfalse) {
                    ...
                    swapMsg = va(swapFmt, activateKey, occName, forStr, pickName);
                }
        ...
        if (marker != 0 && *marker != '\0') {
            ...
            if (cgame_compat_splice_hint_key(pickupTextBuf, sizeof(pickupTextBuf), tmpl, marker, outKey) == qfalse) {
                drawText = tmpl;
                goto measure_and_paint;
            }
    ...
    if (kind == CURSOR_HINT_LMG) {
        ...
        const char *tmpl = CG_SafeTranslateString_Internal("cgame", "GMI_CGAME_LMGMOUNTPOINT"); // 0x3007980c
        if (cgame_compat_hint_format_is_valid(tmpl, "s") == qfalse) {
            drawText = tmpl;
        } else {
    ...
    if (cg_usableHintCommandIndex >= 0) {
        ...
        const int32_t cfgIndex = coduo_int32_from_bits((uint32_t)cg_usableHintCommandIndex + HINT_CFGSTRING_BASE);
        ...
        if (marker != 0) {
            ...
            /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered client-module boundary input and state before use. */
            if (cgame_compat_splice_hint_key(rawHintTextBuf, sizeof(rawHintTextBuf), hint, hint + markerIdx, outKey) == qfalse) {
                drawText = hint;
            } else {
    ...
    if (kind == CURSOR_HINT_HEALTH) {
        ...
        const char *tmpl = CG_SafeTranslateString_Internal("cgame", "CGAME_PICKUPHEALTH"); // 0x300797e4
        if (cgame_compat_hint_format_is_valid(tmpl, "s") == qfalse) {
            drawText = tmpl;
        } else {
...
measure_and_paint:
    ...
    {
        float placeY = (float)((long double)r->y - (long double)drawTextHeight);
        float placeX = (float)((long double)r->x + ((long double)r->w - (long double)drawTextWidth) * 0.5L);
        trap_R_Text_Paint((intptr_t)CG_FloatBits(placeX), (intptr_t)CG_FloatBits(placeY), fontContext, scaleBits, (intptr_t)fadeColor,
                          (intptr_t)drawText, 0, 0, textStyle);
    }
...
set_color_and_icon:
    ...
    {
        float iconX = (float)((long double)r->x - (long double)pulseHalf + (long double)iconWideBias);
        float iconY = (float)((long double)r->y - (long double)pulseHalf);
        float iconW = (float)((long double)r->w * (long double)iconWidthScale + (long double)pulseScale);
        float iconH = (float)((long double)r->h + (long double)pulseScale);
    ...
    if (cg_usableHintColorByte != 0) {
        ...
        float frac = (float)((long double)coduo_int32_from_bits((uint32_t)cg_usableHintColorByte) * (long double)HINT_BYTE_TO_FRAC);
        fadeColor[0] = 0.0f;
        fadeColor[1] = 0.0f;
        fadeColor[2] = 1.0f;
        fadeColor[3] = 0.5f;
        ...
        CG_FilledBar(0, NULL, NULL, r->x, (float)((long double)r->h + (long double)r->y + 4.0L), r->w, 8.0f, fadeColor, frac);
    }
