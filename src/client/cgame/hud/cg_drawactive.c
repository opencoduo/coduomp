...
void CG_DrawActive(int32_t stereoView)
{
    ...
    if (cg_snap == NULL) {                                /* 0x3001c12c TEST/JNZ */
        ...
        if (serverLoadMapSet != 0) {                         /* 0x3001c148 MOV AL,[..0030] */
            cgame_syscall(CG_CVAR_SET, (intptr_t)"cl_serverloadmap", (intptr_t)""); /* 0x3001c15b..0x3001c167 */
        }
        if (cl_serverloadgametype.string[0] != 0) { /* 0x3001c170 MOV AL,[..8970] */
            cgame_syscall(CG_CVAR_SET, (intptr_t)"cl_serverloadgametype", (intptr_t)""); /* 0x3001c179..0x3001c185 */
        }
        if (cl_serverloadwaiting.integer != 0) { /* 0x3001c18e MOV EAX,[..346c] */
            cgame_syscall(CG_CVAR_SET, (intptr_t)"cl_serverloadwaiting", (intptr_t)"0"); /* 0x3001c197..0x3001c1a3 */
        }
         ...
         */
        {
            const char *serverInfo = &cg_gameState.stringData[cg_gameState.stringOffsets[0]]; /* 0x3001c1ac..0x3001c1b8 */
            ...
            if (mapName != NULL && mapName[0] != '\0') { /* 0x3001c1c2..0x3001c1c9 */
                const char *shaderName = va("levelshots/%s.tga", mapName); /* 0x3001c1cc..0x3001c1da CALL va */
                CG_DrawInformation(0); /* 0x3001c1da CALL 0x3002a530 */
                /* trap(0x59, name, 2) -> shader handle (CG_R_RegisterShader). */
                levelShotShader = coduo_int32_from_bits(
                    (uint32_t)cgame_syscall(CG_R_REGISTERSHADER, (intptr_t)shaderName, 2)); /* 0x3001c1e2..0x3001c1e7 */
            }
            if (levelShotShader == 0) { /* 0x3001c1f2 TEST ESI,ESI / else path */
                CG_DrawInformation(0); /* 0x3001c1f6 CALL 0x3002a530 */
                levelShotShader = coduo_int32_from_bits(
                    (uint32_t)cgame_syscall(CG_R_REGISTERSHADER, (intptr_t)"menu/art/unknownmap", 2)); /* 0x3001c200..0x3001c209 */
            }
        }
         ...
         */
        {
            ...
            memcpy(&hb, &h, 4);
            {
                ...
                memcpy(&one, &f1, 4);
                memcpy(&zero, &f0, 4);
            } /* PUSH 0x3f800000 (1.0), PUSH 0 (0.0) */
        ...
        if (expectedHunkUsage > 0) { /* 0x3001c29b TEST + 0x3001c2c1 JLE */
            int32_t currentHunkUsage = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_HUNK_USED)); /* 0x3001c2c3 trap(0x3f) */
            ...
            long double loadQuotient = (long double)currentHunkUsage / expectedHunkUsage; /* 0x3001c2cf FILD; 0x3001c2d6 FIDIV */
        ...
        cg_updateScreenActive = coduo_int32_from_bits((uint32_t)cg_updateScreenActive - 1u); /* 0x3001c31c..0x3001c325 DEC */
    ...
    /* ---- Normal 3D view: a snapshot is installed ---- */
    {
        ...
        if (stereoSep != 0.0f) { /* 0x3001c38b..0x3001c3b9 FUCOMPP 0.0 / JNP */
            ...
            cg_refdef.vieworg[0] =
                (float)((long double)cg_refdef.viewaxis[1][0] * negSep + (long double)cg_refdef.vieworg[0]); /* 0x3001c3c1..0x3001c3cf */
            cg_refdef.vieworg[1] =
                (float)((long double)cg_refdef.viewaxis[1][1] * negSep + (long double)cg_refdef.vieworg[1]); /* 0x3001c3d5..0x3001c3e3 */
            cg_refdef.vieworg[2] =
                (float)((long double)cg_refdef.viewaxis[1][2] * negSep + (long double)cg_refdef.vieworg[2]); /* 0x3001c3e9..0x3001c3f5 */
        }
        ...
        refFlags = cg_refdef.rdflags; /* 0x3001c3fb MOV ECX */
        {
            ...
            if (skyboxEnabled == 0) { /* 0x3001c409 TEST/JNZ-skip */
                cg_refdef.rdflags = refFlags & ~(uint32_t)RDF_SKYBOX_PORTAL_ACTIVE; /* 0x3001c413..0x3001c418 */
            }
        }

        /* Submit the scene to the engine renderer. */
        cgame_syscall(CG_R_RENDER_SCENE, (intptr_t)&cg_refdef.x); /* 0x3001c41d..0x3001c424 trap(0x45,&refdef) */
