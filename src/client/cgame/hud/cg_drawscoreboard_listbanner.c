...
enum {
    BANNER_STRICMP_UNBOUNDED = 99999
};
...
static CG_BANNER_COMPAT_ALWAYS_INLINE int cgame_compat_banner_casecmpn(const char *a, const char *b, int32_t n)
{
    while (n-- > 0) {
...
void CG_DrawScoreboard_ListBanner(float fade)
{
    ...
    blackShader = (qhandle_t)cgame_syscall(CG_R_REGISTERSHADER, (intptr_t)"black", 5);
    trap_R_DrawStretchPic(CG_FloatBits(120.0f * cgs_screenXScale), CG_FloatBits(22.0f * cgs_screenYScale),
                          CG_FloatBits(400.0f * cgs_screenXScale), CG_FloatBits(430.0f * cgs_screenYScale), CG_FloatBits(0.0f),
                          CG_FloatBits(0.0f), CG_FloatBits(1.0f), CG_FloatBits(1.0f), blackShader);

    /* --- divider bars: SetColor({1,1,1, fade*0.1}); draw the "white" bar table. */
    color[3] = fade * 0.1f;
    cgame_syscall(CG_R_SETCOLOR, (intptr_t)color);
    CG_DrawInformation(0);
    whiteShader = (qhandle_t)cgame_syscall(CG_R_REGISTERSHADER, (intptr_t)"white", 5);
    for (i = 0; i < 6; i++) {
        trap_R_DrawStretchPic(
            CG_FloatBits(cg_scoreboardBannerBars[i][0] * cgs_screenXScale), CG_FloatBits(cg_scoreboardBannerBars[i][1] * cgs_screenYScale),
            CG_FloatBits(cg_scoreboardBannerBars[i][2] * cgs_screenXScale), CG_FloatBits(cg_scoreboardBannerBars[i][3] * cgs_screenYScale),
            CG_FloatBits(0.0f), CG_FloatBits(0.0f), CG_FloatBits(1.0f), CG_FloatBits(1.0f), whiteShader);
    }
    ...
    gametypeToken = (const char *)(intptr_t)cgame_syscall(CG_UI_GET_GAMETYPE_DISPLAY_NAME, (intptr_t)cgs_gametype);
    gametypeText =
        (const char *)(intptr_t)cgame_syscall(CG_SE_LOCALIZE_MESSAGE, (intptr_t)gametypeToken, (intptr_t)"scoreboard gametype display");
    metric = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_R_TEXT_HEIGHT, 0, CG_FloatBits(0.41f)));
    int32_t gametypeCenterDelta = coduo_int32_from_bits(24u - (uint32_t)metric);
    float gametypeY = (float)((long double)51.0f - (long double)gametypeCenterDelta * (long double)0.5f);
    cgame_syscall(CG_R_TEXT_PAINT, CG_FloatBits(129.0f),                              /* x */
                  CG_FloatBits(gametypeY),                           /* y */
                  0, CG_FloatBits(0.41f),                               /* scale */
                  (intptr_t)color, (intptr_t)gametypeText, 0, 0, 3);
    ...
    /* strip a trailing ".bsp" (case-insensitive) */
    {
        ...
        char *suffix = (char *)((uintptr_t)(void *)&mapName[0] + (uintptr_t)(intptr_t)suffixOffset);
    ...
    mapBase = mapName;
    if (cgame_compat_banner_casecmpn(mapName, "maps", 4) == 0 && (mapName[4] == '/' || mapName[4] == '\\'))
        mapBase = mapName + 5;
    /* strip a further leading "mp/" or "mp\" (same gate: mismatch at 0x30036566
     * skips the [EBP+2] separator check). */
    if (cgame_compat_banner_casecmpn(mapBase, "mp", 2) == 0 && (mapBase[2] == '/' || mapBase[2] == '\\'))
        mapBase += 3;
    ...
    labelWidth = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_R_TEXT_WIDTH, (intptr_t)mapBase, 0, CG_FloatBits(0.41f), 0));
    textWidth = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_R_TEXT_WIDTH, (intptr_t)gametypeText, 0, CG_FloatBits(0.41f), 0));
    int32_t combinedWidth = coduo_int32_from_bits((uint32_t)textWidth + (uint32_t)labelWidth + (uint32_t)BANNER_TEXT_PAD);
    if (combinedWidth > BANNER_FIT_WIDTH_LIMIT) {
        for (;;) {
            if (!(scale > 0.075f))
                break;
            scale = (float)((long double)scale - (long double)0.025f);
            labelWidth = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_R_TEXT_WIDTH, (intptr_t)mapBase, 0, CG_FloatBits(scale), 0));
            ...
            textWidth = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_R_TEXT_WIDTH, (intptr_t)gametypeText, 0, CG_FloatBits(0.41f), 0));
            combinedWidth = coduo_int32_from_bits((uint32_t)textWidth + (uint32_t)labelWidth + (uint32_t)BANNER_TEXT_PAD);
            if (combinedWidth <= BANNER_FIT_WIDTH_LIMIT)
                break;
        }
    }
    /* draw mapBase right-aligned: measure at the final scale, x = 511 - (width+4). */
    textWidth = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_R_TEXT_WIDTH, (intptr_t)mapBase, 0, CG_FloatBits(scale), 0));
    metric = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_R_TEXT_HEIGHT, 0, CG_FloatBits(scale)));
    int32_t paddedMapWidth = coduo_int32_from_bits((uint32_t)textWidth + (uint32_t)BANNER_TEXT_PAD);
    float mapX = (float)((long double)511.0f - (long double)paddedMapWidth);
    int32_t mapCenterDelta = coduo_int32_from_bits(24u - (uint32_t)metric);
    float mapY = (float)((long double)51.0f - (long double)mapCenterDelta * (long double)0.5f);
    cgame_syscall(CG_R_TEXT_PAINT, CG_FloatBits(mapX),                          /* x */
                  CG_FloatBits(mapY),                          /* y */
                  0, CG_FloatBits(scale), (intptr_t)color, (intptr_t)mapBase, 0, 0, 3);
    ...
    if (serverAddr != 0 && Q_stricmpn("0.0.0.0:0", serverAddr, BANNER_STRICMP_UNBOUNDED) == 0) {
    ...
    labelWidth = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_R_TEXT_WIDTH, (intptr_t)serverAddr, 0, CG_FloatBits(0.2f), 0));
    textWidth = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_R_TEXT_WIDTH, (intptr_t)cgs_hostname, 0, CG_FloatBits(0.2f), 0));
    combinedWidth = coduo_int32_from_bits((uint32_t)textWidth + (uint32_t)labelWidth + (uint32_t)BANNER_TEXT_PAD);
    if (combinedWidth > BANNER_FIT_WIDTH_LIMIT) {
        for (;;) {
            if (!(scale > 0.075f))
            ...
            labelWidth = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_R_TEXT_WIDTH, (intptr_t)serverAddr, 0, CG_FloatBits(scale), 0));
            textWidth = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_R_TEXT_WIDTH, (intptr_t)cgs_hostname, 0, CG_FloatBits(scale), 0));
            combinedWidth = coduo_int32_from_bits((uint32_t)textWidth + (uint32_t)labelWidth + (uint32_t)BANNER_TEXT_PAD);
            if (combinedWidth <= BANNER_FIT_WIDTH_LIMIT)
                break;
        }
    ...
    float hostY = (float)((long double)447.0f - (long double)hostCenterDelta * (long double)0.5f);
    cgame_syscall(CG_R_TEXT_PAINT, CG_FloatBits(129.0f),                                /* x */
                  CG_FloatBits(hostY),                          /* y */
                  0, CG_FloatBits(scale), (intptr_t)color, (intptr_t)cgs_hostname, 0, 0, 3);
    textWidth = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_R_TEXT_WIDTH, (intptr_t)serverAddr, 0, CG_FloatBits(scale), 0));
    int32_t paddedAddressWidth = coduo_int32_from_bits((uint32_t)textWidth + (uint32_t)BANNER_TEXT_PAD);
    float addressX = (float)((long double)511.0f - (long double)paddedAddressWidth);
    cgame_syscall(CG_R_TEXT_PAINT, CG_FloatBits(addressX),                       /* x */
                  CG_FloatBits(hostY),                          /* y */
                  0, CG_FloatBits(scale), (intptr_t)color, (intptr_t)serverAddr, 0, 0, 3);
}
