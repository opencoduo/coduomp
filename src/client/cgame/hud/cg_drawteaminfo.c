...
void CG_DrawTeamInfo(void)
{
    ...
    if ((int32_t)((uint32_t)cg_time - (uint32_t)teamChatMsgTimes[lastPos % chatHeight]) > cg_chatTime_vmCvar.integer) {
        lastPos = coduo_int32_from_bits((uint32_t)lastPos + 1u);          /* 0x300187bf INC ECX */
    ...
    while (i >= lastPos) {
        ...
        int32_t age = (int32_t)((uint32_t)cg_time - (uint32_t)teamChatMsgTimes[ringIndex]);
        ...
        long double left = (long double)cg_chatTime_vmCvar.value - (long double)age;  /* 0x300187e5..0x300187e9 */
        float alpha;
        ...
        if (line != NULL) {
            ...
            if ((int8_t)line[0] == '^' && colorCode != '\0' && colorCode != '^' && colorCode >= '0' &&
                colorCode <= '9') { /* 0x3001883d..0x30018853 */
                cgame_syscall(CG_CL_LOOKUP_COLOR, (int32_t)(uint8_t)colorCode, (intptr_t)&lineColor[0]);   /* 0x30018855..0x30018866 */
            }
        }
        ...
        int32_t fromTop = coduo_int32_from_bits((uint32_t)i - (uint32_t)teamChatPos); /* 0x30018895..0x30018897 */
        int32_t yBar = coduo_int32_from_bits((uint32_t)fromTop * 10u + 84u); /* 0x3001889f/0x300188b5 LEA */
        ...
        barColor[3] = (float)((long double)alpha * (long double)0.6f); /* 0x300188d4/0x300188d8 */

        int32_t textWidth = coduo_int32_from_bits(
            (uint32_t)cgame_syscall(CG_R_TEXT_WIDTH, (intptr_t)line, 0, 0x3e555555, 0)); /* font 0, scale 0.20833, max 0 */
        int32_t barChars = coduo_int32_from_bits((uint32_t)textWidth + 24u); /* 0x300188f0 +24 */
        ...
        float barW = (float)((long double)cgs_screenXScale * (long double)barWidth); /* 0x30018937..3d */
        float barY = (float)((long double)cgs_screenYScale * (long double)(float)yBar); /* 0x30018945..4b */
        float barX = (float)((long double)cgs_screenXScale * (long double)compatLeftX);
        trap_R_DrawStretchPic(CG_FloatBits(barX), CG_FloatBits(barY), CG_FloatBits(barW), CG_FloatBits(barH), 0, 0, /* s1=0, t1=0 */
        ...
        // and mode 3.
        int32_t fromTopText = coduo_int32_from_bits((uint32_t)i - (uint32_t)teamChatPos); /* 0x3001896d..0x30018973 */
        int32_t yText = coduo_int32_from_bits((uint32_t)fromTopText * 10u + 93u); /* 0x30018977..0x30018984 */
        vec4_t whiteColor = {1.0f, 1.0f, 1.0f, alpha};
        cgame_syscall(CG_R_TEXT_PAINT, CG_FloatBits(8.0f + compatLeftX), CG_FloatBits((float)yText), /* baseline y */
                      0, /* style */
