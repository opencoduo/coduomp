...
void CG_ServerCommand(void)
{
    ...
    case 'g':   /* 0x3003ada8 */
        ...
        return;
    case 'h':   /* 0x3003adc1 */
        if (cg_teamChatsOnly_vmCvar.integer) return;
        /* 0x3003ade0: the raw localize syscall — NOT CG_TranslateMessage
         * (0x3002d850): chat lines get no "[{command}]" binding-marker
         * replacement. */
        text = (const char *)(intptr_t)cgame_syscall(
            CG_SE_LOCALIZE_MESSAGE, (intptr_t)CG_Argv(1),
            (intptr_t)"chat message");
    ...
    case 'i':   /* 0x3003ae4a */
        ...
        Com_PrintMessage("%s\n", name);
        return;
    case 'j': CG_ParseVoiceChat(0); return;   /* 0x3003aec6 */
    case 'k': CG_ParseVoiceChat(1); return;   /* 0x3003aee5 */
    case 'l': CG_ParseVoiceChat(2); return;   /* 0x3003af04 */
    case 'm':   /* 0x3003af23 */
        value = coduo_crt_atoi(CG_Argv(1));
        /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered client-module boundary input and state before use. */
        if (value < -CG_COMPLAINT_STATUS_COUNT || value >= MAX_CLIENTS) {
            Com_Error(ERR_DROP,
                      "\x15" "CG_ServerCommand: invalid complaint client "
                      "number %i",
                      value);
            return;
        }
        ...
        return;
    case 'n': CG_MapRestart(qtrue); return;   /* 0x3003af79 */
    case 'o': {  /* 0x3003af98 */
    ...
    case 't': CG_OpenScriptMenu(); return;    /* 0x3003b0a6 */
    case 'u': CG_CloseScriptMenu(); return;   /* 0x3003b0c0 */
    case 'v':   /* 0x3003b0da */
        ...
        return;
    case 'B': CG_MapRestart(qfalse); return;  /* 0x3003b12c */
    case 'C':   /* 0x3003b14b */
        trap_Cvar_VariableStringBuffer("cg_autodemo", name, 2);
        if (coduo_crt_atoi(name) != 1) return;
        ...
        cgame_syscall(CG_EXECUTE_COMMAND,
                      (intptr_t)va("record %s-%s", text, name));
        return;
    case 'D':   /* 0x3003b1cf */
        trap_Cvar_VariableStringBuffer("cg_autodemo", name, 2);
    ...
    case 'E':   /* 0x3003b221 */
        ...
        cgame_syscall(CG_EXECUTE_COMMAND,
                      (intptr_t)va("screenshotJPEG %s-%s", text, name));
        return;
    case 'F':   /* 0x3003b2a5 */
        cgame_syscall(CG_FX_REWIND_TIME, coduo_crt_atoi(CG_Argv(1)));
        return;
    case 'S': CG_LocalSound_f(); return;      /* 0x3003b08c */
    default:    /* 0x3003b2dc */
        trap_Argv(0, g_textScratchBuffer, sizeof(g_textScratchBuffer));
        ...
        if (value > 1) {
            ...
            for (int32_t i = 1; i < value; ++i) {
                trap_Argv(i, g_textScratchBuffer,
                          sizeof(g_textScratchBuffer));
