...
{
    int32_t sequence = coduo_int32_from_bits((uint32_t)ps->eventIndex - (uint32_t)MAX_PS_EVENTS);
    while (sequence < ps->eventIndex) {
        int32_t predictedSequence = cg_predictedEventSequence;
        int32_t oldestPredicted = coduo_int32_from_bits((uint32_t)predictedSequence - (uint32_t)MAX_PREDICTED_EVENTS);
        if (sequence >= predictedSequence || sequence <= oldestPredicted) {
            goto next_sequence;
        }
        ...
        CG_EntityEvent(&cg_predictedEventEntity, event, qtrue);
