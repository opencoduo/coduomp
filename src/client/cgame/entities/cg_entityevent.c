...
enum {
    ...
    EV_TABLE_LO = 0x8b,
    EV_TABLE_HI = 0xd3,  /* jump-table window [0x8b, 0xd3) */
    ...
    CG_SURFACE_SOUND_COUNT = sizeof(cg_grenadeExplodeSurfaceSounds) / sizeof(cg_grenadeExplodeSurfaceSounds[0]),
    CG_SHELL_FLASH_SOUND_COUNT = sizeof(cg_shellFlashSounds) / sizeof(cg_shellFlashSounds[0]),
    CG_BARRAGE_SOUND_COUNT = sizeof(cg_barrageIncomingSounds) / sizeof(cg_barrageIncomingSounds[0]),
};

_Static_assert(CG_SURFACE_SOUND_COUNT == sizeof(cg_grenadeBounceSurfaceSounds) / sizeof(cg_grenadeBounceSurfaceSounds[0]),
               "grenade surface sound rows differ in size");
_Static_assert(CG_SURFACE_SOUND_COUNT <= CG_IMPACT_SURFACE_TYPES, "surface sound domain exceeds impact effect domain");
_Static_assert(CG_SHELL_FLASH_SOUND_COUNT == CG_BARRAGE_SOUND_COUNT, "event parameter sound rows differ in size");
_Static_assert(CG_EVENT_SOUND_NONE == CG_SHELL_FLASH_SOUND_COUNT, "event sound sentinel no longer follows the sound rows");
...
static const char *(*const cg_projectileExplosionSurfaceSoundBanks[PROJECTILE_EXPLOSION_SOUND_BANK_COUNT])[CG_SURFACE_SOUND_COUNT] = {
    [PROJECTILE_EXPLOSION_GRENADE] = &cg_grenadeExplodeSurfaceSounds,     [PROJECTILE_EXPLOSION_SMOKE] = &cg_unusedSurfaceSoundSet0,
    [PROJECTILE_EXPLOSION_ROCKET] = &cg_rocketExplodeSurfaceSounds,       [PROJECTILE_EXPLOSION_MOLOTOV] = &cg_unusedSurfaceSoundSet1,
    [PROJECTILE_EXPLOSION_ARTILLERY] = &cg_artilleryExplodeSurfaceSounds, [PROJECTILE_EXPLOSION_MORTAR] = &cg_mortarExplodeSurfaceSounds,
    [PROJECTILE_EXPLOSION_TANK] = &cg_tankExplodeSurfaceSounds,           [PROJECTILE_EXPLOSION_B17] = &cg_eventSurfaceSounds,
};
...
void CG_EntityEvent(centity_t *self, int event, int predicted)
{
    ...
    if ((uint32_t)event >= (uint32_t)EV_MAX_EVENTS) {
        Com_Error(ERR_DROP,
                  "\x15"
                  "CG_EntityEvent: invalid event %i",
                  event);
        return;
    }
    ...
    if (event >= EV_STEP_A_LO && event < EV_STEP_A_HI) {          /* [1, 0x18) */
        if (cg_footsteps) {
            CG_PlayEntitySoundAliasByName(entNum, (const char *)(intptr_t)cg_stepRunSurfaceSounds[event - EV_STEP_A_LO]);
        }
    ...
    if (event >= EV_STEP_D_LO && event < EV_STEP_D_HI) {          /* [0x46, 0x5d) */
        if (cg_footsteps) {
            CG_PlayEntitySoundAliasByName(entNum, (const char *)(intptr_t)cg_stepSprintSurfaceSounds[event - EV_STEP_D_LO]);
        }
    ...
    if (event >= EV_STEP_B_LO && event < EV_STEP_B_HI) {          /* [0x18, 0x2f) */
        if (cg_footsteps) {
            CG_PlayEntitySoundAliasByName(entNum, (const char *)(intptr_t)cg_stepWalkSurfaceSounds[event - EV_STEP_B_LO]);
        }
    ...
    if (event >= EV_STEP_C_LO && event < EV_STEP_C_HI) {          /* [0x2f, 0x46) */
        if (cg_footsteps) {
            CG_PlayEntitySoundAliasByName(entNum, (const char *)(intptr_t)cg_stepProneSurfaceSounds[event - EV_STEP_C_LO]);
        }
        CG_PlayEntitySoundAliasByName(entNum, cg_soundGearRattleWalk);
        return;
    }
    ...
    if (event >= EV_FLESH_LO && event < EV_FLESH_HI) {
        CG_PlayEntitySoundAliasByName(entNum, (const char *)(intptr_t)cg_shellFlashSurfaceSounds[event - EV_FLESH_LO]);
    ...
    if (event >= EV_BODY_LO && event < EV_BODY_HI) {
        CG_PlayEntitySoundAliasByName(entNum, (const char *)(intptr_t)cg_shellFlashSurfaceSounds[event - EV_BODY_LO]);
        ...
        if (clientNum == (int)cg_clientNum) {
            ...
            long double envelope =
                ((long double)cg_envHi - (long double)cg_envLo) * ((long double)eventParm * (long double)0.01f) + (long double)cg_envLo;
            ...
            if (!(envelope <= (long double)12.0f)) {
                ...
                envelope = ((envelope - (long double)12.0f) * (long double)(1.0f / 26.0f) + (long double)1.0f) * (long double)4.0f;
    ...
    case EV_STANCE_FORCE_PRONE: {
        const char *voice = (event == EV_STANCE_FORCE_STAND) ? "0" : (event == EV_STANCE_FORCE_CROUCH) ? "1" : "2";
        if (clientNum != (int)cg_clientNum) {
    ...
    case EV_STEP_VIEW:
        ...
        }
        {
            ...
            int32_t sinceLast = coduo_int32_from_bits((uint32_t)now - (uint32_t)cg_weaponChangeViewOffsetTime);
            ...
            if (sinceLast < 100) {
                int32_t remaining = coduo_int32_from_bits(100u - (uint32_t)sinceLast);
                base = ((long double)remaining * (long double)cg_weaponChangeViewOffset) * (long double)0.01f * (long double)0.9f;
            } else {
                base = (long double)0.0f;
                /* 0x30022c8a: FLD [0x3007bcec] = 0.0f */
            }

            int32_t eventOffset = coduo_int32_from_bits((uint32_t)eventParm - 128u);
    ...
    case EV_DROPWEAPON: {
        if (event == EV_NOAMMO) {
            ...
            if (weaponIndex <= 0 || weaponIndex > bg_numWeapons || (uint32_t)weaponIndex >= (uint32_t)MAX_WEAPONS ||
                ...
                Com_Printf("WARNING: CG_EntityEvent: invalid weapon index %i "
                           "for event %i\n",
                           weaponIndex, event);
                return;
            }
    ...
    case EV_FIRE_WEAPON_MG42:
        ...
        CG_FireWeapon((uint32_t)self->currentState.eventParm, self, (entityState_t *)self, event, 0); /* EAX=eventParm */
        return;
    ...
    case EV_MELEE_SWIPE: {
        ...
        weaponInfo_t *weaponInfo = bg_weaponInfos[weaponIndex];
        if (weaponInfo->ricochet == 0 && weaponInfo->weaponType != WEAPTYPE_PROJECTILE) {
     ...
     * entNum. */
    case EV_SOUND_ALIAS: {
        int32_t configStringIndex = coduo_int32_from_bits(self->currentState.eventParm + (uint32_t)CS_WEAPON_SOUND_BASE);
    ...
    case EV_GRENADE_BOUNCE: {
        ...
        if ((uint32_t)surfaceType >= (uint32_t)CG_IMPACT_SURFACE_TYPES) {
            Com_Printf("WARNING: CG_EntityEvent: invalid surface type %i "
                       "for event %i\n",
                       surfaceType, event);
            return;
        }
        ...
        if (weaponInfo->cloth != 0) {
            ...
            if ((uint32_t)surfaceType >= (uint32_t)CG_SURFACE_SOUND_COUNT) {
                Com_Printf("WARNING: CG_EntityEvent: invalid sound surface type %i "
                           "for event %i\n",
                           surfaceType, event);
                return;
            }
        ...
        flashHandle = (uint32_t)cgame_compat_read_target_i32_index(
            &cg_impactEffects[0][0], coduo_int32_from_bits(12u * CG_IMPACT_SURFACE_TYPES + (uint32_t)surfaceType));
        if (flashHandle == 0) {
            return;
        }
        cgame_syscall(CG_PLAY_EFFECT_ORIENTED, coduo_int32_from_bits(flashHandle), (intptr_t)&self->lerpOrigin, (intptr_t)evOrigin);
        return;
    }

    case EV_GRENADE_SPOON: /* 0xb6, 0x30023298 */
        CG_PlaySoundAliasByName(LOCAL_SOUND_ENTITY, &self->lerpOrigin, cg_soundUsGrenadeLever);
        return;

    /* 0xd0 (0x300232b4): flag-selected sound at &self->lerpOrigin. */
    case EV_OVERHEATING:
        if ((self->currentState.eType & EFLAGS_STEP_ALT) != 0) {
            CG_PlaySoundAliasByName(LOCAL_SOUND_ENTITY, &self->lerpOrigin, cg_soundMgOverheatVehicle);
            return;
        }
        CG_PlaySoundAliasByName(LOCAL_SOUND_ENTITY, &self->lerpOrigin, cg_soundMgOverheat);
        return;
    ...
    case EV_PROJECTILE_LAUNCH: {
        ...
        if ((uint32_t)p >= (uint32_t)CG_SHELL_FLASH_SOUND_COUNT) {
            Com_Printf("WARNING: CG_EntityEvent: invalid sound selector %i "
                       "for event %i\n",
                       p, event);
            return;
        }
    ...
    case EV_PROJECTILE_EXPLODE_NOMARKS: {
        ...
        /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered client-module boundary input and state before use. */
        if ((uint32_t)explosionType > (uint32_t)PROJECTILE_EXPLOSION_NONE) {
            Com_Printf("WARNING: CG_EntityEvent: invalid explosion type %i "
                       "for event %i\n",
                       explosionType, event);
            return;
        }
        if (explosionType != PROJECTILE_EXPLOSION_NONE && (uint32_t)surfaceType >= (uint32_t)CG_SURFACE_SOUND_COUNT) {
            Com_Printf("WARNING: CG_EntityEvent: invalid surface type %i "
                       "for event %i\n",
                       surfaceType, event);
            return;
        ...
        if (explosionType == PROJECTILE_EXPLOSION_SMOKE) {
            ...
            boltInfo.boneIndex =
                coduo_int32_from_bits((uint32_t)cgame_syscall(CG_RESOLVE_TAG, self->currentState.number, (intptr_t)"g_body"));
            if (boltInfo.boneIndex >= 0) {
                ...
                uint32_t handle = (uint32_t)cgame_compat_read_target_i32_index(
                    &cg_impactEffects[0][0], coduo_int32_from_bits(14u * CG_IMPACT_SURFACE_TYPES + (uint32_t)surfaceType));
            ...
            if ((self->currentState.eFlags & EFLAGS_PROJECTILE_HIT_VEHICLE) != 0) {
                ...
                uint32_t flatIndex = 13u * CG_IMPACT_SURFACE_TYPES + 23u + (uint32_t)explosionType * CG_IMPACT_SURFACE_TYPES;
                uint32_t handle = (uint32_t)cgame_compat_read_target_i32_index(&cg_impactEffects[0][0], coduo_int32_from_bits(flatIndex));
                if (handle != 0) {
                    cgame_syscall(CG_PLAY_EFFECT_ORIENTED, coduo_int32_from_bits(handle), (intptr_t)&self->lerpOrigin, (intptr_t)&evOrigin);
                    handled = 1;
                }
            }
            if (!handled) {
                ...
                uint32_t flatIndex =
                    13u * CG_IMPACT_SURFACE_TYPES + (uint32_t)explosionType * CG_IMPACT_SURFACE_TYPES + (uint32_t)surfaceType;
                uint32_t handle = (uint32_t)cgame_compat_read_target_i32_index(&cg_impactEffects[0][0], coduo_int32_from_bits(flatIndex));
                if (handle != 0) {
                    cgame_syscall(CG_PLAY_EFFECT_ORIENTED, coduo_int32_from_bits(handle), (intptr_t)&self->lerpOrigin, (intptr_t)&evOrigin);
                }
        ...
        if (explosionType != PROJECTILE_EXPLOSION_NONE) {
            const char *const *soundBank = *cg_projectileExplosionSurfaceSoundBanks[explosionType];
        ...
        if (w->projectileExplosionEffect != 0) {
            cgame_syscall(CG_PLAY_EFFECT_ORIENTED, coduo_int32_from_bits(w->projectileExplosionEffect), (intptr_t)&self->lerpOrigin,
                          (intptr_t)&evOrigin);
        }
        {
            cgWeaponInfo_t *w2 = &cg_weaponInfos[self->currentState.weapon];
            const char *nm = w2->projectileExplosionSound;
            if (nm == NULL || nm[0] == '\0') {
                return;
            }
