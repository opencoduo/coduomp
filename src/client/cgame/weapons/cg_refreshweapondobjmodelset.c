...
enum { CG_WEAPON_NAME_COPY_SIZE = 0x3f };
 ...
 * "tag_weapon". */
enum { CG_WEAPON_DOBJ_MODEL_SET_COUNT = 2 };
...
void CG_RefreshWeaponDObjModelSet(int weaponIndex, const char *configName)
{
    ...
    intptr_t runtimeTree =
        cgame_syscall(CG_DOBJ_GET_TREE, (intptr_t)info->viewDObjSelf);
    ...
    int32_t dobjHandle = coduo_int32_from_bits(
        (uint32_t)weaponIndex + (uint32_t)CG_VIEW_WEAPON_DOBJ_HANDLE_BASE);
    ...
    (void)cgame_syscall(CG_SAFE_CLIENT_DOBJ_FREE,
                        dobjHandle, 0);
    ...
    if (strlen(weapon->handModel) >
        ...
        strlen(weapon->gunModel) > MAX_QPATH - sizeof("xmodel/")) {
        Com_Error(ERR_DROP,
                  "\x15" "CG_RefreshWeaponDObjModelSet: model path is too long");
        return;
    }
    ...
    modelSet[0].modelIndex = (int16_t)CG_RegisterModel(
        configName[0] != '\0' ? configName : handModelPath, 6);
    ...
    modelSet[0].model = (XModel *)(intptr_t)cgame_syscall(
        CG_DOBJ_WRAP_MODEL, handModelIndex);
    ...
    modelSet[1].model = (XModel *)(intptr_t)cgame_syscall(
        CG_DOBJ_WRAP_MODEL, modelSet[1].modelIndex);
     ...
     * is discarded.) */
    (void)cgame_syscall(CG_CLIENT_DOBJ_CREATE, (intptr_t)modelSet,
                        CG_WEAPON_DOBJ_MODEL_SET_COUNT, runtimeTree,
                        dobjHandle);
    ...
    info->viewDObjSelf = (struct DObj_s *)(intptr_t)cgame_syscall(
        CG_DOBJ_GET_HANDLE, dobjHandle);
