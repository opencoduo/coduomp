...
void CG_CalcWeaponMovementAngles(vec3_t angles)
{
    ...
    if (currentSpeed > threshold &&
        ...
        long double factor =
            ((long double)currentSpeed - threshold) /
            ((long double)ps->speed - threshold);
        vec3_t bias = {
            cg_gun_move_f_vmCvar.value,
            cg_gun_move_r_vmCvar.value,
            cg_gun_move_u_vmCvar.value
        };
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            target[i] = (float)(factor * (long double)(*move)[i] +
                                factor * (long double)bias[i]);
        }
    }
    ...
    if (ps->viewHeightTarget == ps->crouchViewHeight ||
        ...
        vec3_t crouchBias = {
            cg_gun_ofs_f_vmCvar.value,
            cg_gun_ofs_r_vmCvar.value,
            cg_gun_ofs_u_vmCvar.value
        };
        const vec3_t *stanceOffset =
            ps->viewHeightTarget == ps->crouchViewHeight
                ? &weapon->duckedOffset
                : &weapon->proneOffset;
        long double stance0 =
            (long double)target[0] + (long double)(*stanceOffset)[0];
        long double stance1 =
            (long double)target[1] + (long double)(*stanceOffset)[1];
...

    {
        ...
        const float minimumStep = (float)(
            (long double)frameTime *
            (long double)CG_WEAPON_MOVE_MINIMUM_STEP_SCALE);
        ...
        for (int i = 0; i < 3; ++i) {
            if (target[i] != state[i]) {
                ...
                const long double smooth = (long double)smoothBias +
                    (long double)((long double)ps->viewHeightCurrent ==
                                  (long double)ps->proneViewHeight
                        ? weapon->moveSmoothProne : weapon->moveSmooth);
                long double step =
                    ((long double)target[i] - (long double)state[i]) * smooth *
        ...
        if (ps->adsFraction == 0.0f) {
            ...
            const long double scale =
                1.0L - (long double)ps->adsFraction * 2.0L;
            for (int i = 0; i < 3; ++i) {
                angles[i] = (float)((long double)angles[i] +
                                    (long double)state[i] * scale);
            }
        }
    }
}
