...
{
    if (cg_fakeTrajectoryTime == (int32_t)cg_time && cg_fakeTrajectoryEntity == entityNum) {
        return;
    }
    ...
    if (weapon != NULL && (weapon->ammoType == WEAPON_AMMO_TYPE_LMG || weapon->ammoType == WEAPON_AMMO_TYPE_HMG ||
    ...
    if (!CG_DObjGetWorldTagMatrix((void *)dobjHandle, tagName, cent, &tagMatrix)) {
        return;
    }
    ...
    if (tagName != NULL && Q_stricmpn(cg_muzzleTagNames[4], tagName, 99999) == 0) {
        axis_t viewAxis;
        trace_t aimTrace;
        ...
        aimEnd[0] = (float)((long double)viewAxis[0][0] * FAKE_TRAJECTORY_AIM_DISTANCE + (long double)cg_refdef.vieworg[0]);
        aimEnd[1] = (float)((long double)viewAxis[0][1] * FAKE_TRAJECTORY_AIM_DISTANCE + (long double)cg_refdef.vieworg[1]);
        aimEnd[2] = (float)((long double)viewAxis[0][2] * FAKE_TRAJECTORY_AIM_DISTANCE + (long double)cg_refdef.vieworg[2]);
        CG_Trace(FAKE_TRAJECTORY_TRACE_MODEL, aimEnd, vec3_origin, &aimTrace, cg_refdef.vieworg, vec3_origin, entityNum);
        if (aimTrace.allsolid != 0 || aimTrace.startsolid != 0) {
            Com_PrintMessage(cg_fakeTrajectoryStartedInSolidMessage, (int32_t)cg_time);
        }
        ...
        vec3_t aimedAngles = {AngleSubtract(currentAngles[0], pitch), AngleSubtract(currentAngles[1], yaw),
    ...
    matrix43_t muzzleMatrix = {.axis = {{tagMatrix.axis[0][0], tagMatrix.axis[0][1], tagMatrix.axis[0][2]},
                                        {tagMatrix.axis[1][0], tagMatrix.axis[1][1], tagMatrix.axis[1][2]},
                                        {tagMatrix.axis[2][0], tagMatrix.axis[2][1], tagMatrix.axis[2][2]}},
                               .origin = {tagMatrix.origin[0], tagMatrix.origin[1], tagMatrix.origin[2]}};
    vec3_t muzzleOrigin = {muzzleMatrix.origin[0], muzzleMatrix.origin[1], muzzleMatrix.origin[2]};

    for (int32_t i = 0; i < FAKE_TRAJECTORY_COUNT; i++) {
        float spread = (float)coduo_crt_rand() * FAKE_TRAJECTORY_RAND_SCALE * weapon->maxSpread + weapon->hipSpreadStandMin;
        vec3_t end;
        trace_t trace;
        trace_t refined;

        BG_Bullet_Endpos(spread, end, muzzleMatrix.axis[0]);
        CG_Trace(FAKE_TRAJECTORY_TRACE_MODEL, end, vec3_origin, &trace, muzzleOrigin, vec3_origin, entityNum);

        if (tagName != NULL && Q_stricmpn(cg_muzzleTagNames[4], tagName, 99999) == 0) {
            CG_Trace(FAKE_TRAJECTORY_REFINE_MODEL, end, vec3_origin, &refined, muzzleOrigin, vec3_origin, entityNum);
        } else {
            CG_Trace(FAKE_TRAJECTORY_REFINE_MODEL, end, vec3_origin, &refined, muzzleOrigin, vec3_origin, FAKE_TRAJECTORY_WORLD_ENTITY);
        }

        if (refined.fraction < trace.fraction && refined.entityNum != FAKE_TRAJECTORY_NO_ENTITY) {
        ...
        if (trace.fraction < 1.0f && (trace.surfaceFlags & FAKE_TRAJECTORY_SURFACE_NO_IMPACT) == 0) {
            vec3_t effectDir2 = {trace.endpos[0] - muzzleOrigin[0], trace.endpos[1] - muzzleOrigin[1], trace.endpos[2] - muzzleOrigin[2]};
            ...
            float reflectScale = (float)(((((long double)trace.normal[2] * (long double)effectDir2[2]) +
                                           ((long double)trace.normal[1] * (long double)effectDir2[1])) +
                                          ((long double)trace.normal[0] * (long double)effectDir2[0])) *
                                         (long double)-2.0f);
            effectDir2[0] = (float)((long double)trace.normal[0] * (long double)reflectScale + (long double)effectDir2[0]);
            effectDir2[1] = (float)((long double)trace.normal[1] * (long double)reflectScale + (long double)effectDir2[1]);
            effectDir2[2] = (float)((long double)trace.normal[2] * (long double)reflectScale + (long double)effectDir2[2]);
            int32_t surfaceType = (int32_t)((trace.surfaceFlags >> 20) & 31u);
            int32_t mountPos = 1 + (tagName == cg_muzzleTagNames[5]);
            CG_BulletHitEvent(entityNum, trace.endpos, trace.normal, effectDir2, weaponIndex, surfaceType,
                              coduo_int32_from_bits((uint32_t)entityNum + 1u), mountPos);
            return;
        }
