...
static qboolean CG_WeaponCommandReady(void)
{
    ...
    return cg_snap != NULL &&
           (cg_predictedPlayerState.playerStateFlags & PMF_FOLLOW) == 0 &&
...
{
    if (!CG_WeaponCommandReady()) return;
    if ((cg_predictedPlayerState.entityStateFlags & 0x106000) != 0 &&
        !(cg_predictedPlayerState.vehicleType == 1 &&
          cg_predictedPlayerState.vehiclePosition == 3)) return;

    int32_t current = cg_weaponSelect_vmCvar.integer;
    int32_t alternate = bg_weaponInfos[current]->altWeapon;
    if (alternate == 0) {
        const char *message = (const char *)(intptr_t)cgame_syscall(
            CG_SE_LOCALIZE_MESSAGE,
            (intptr_t)"CGAME_THIS_WEAPON_HAS_NO_ALTERNATE",
...
void CG_NextWeapon_f(void) /* 0x300474c0 */
{
    if (cg_scoreboardShowing) {
        if (cg_scoreboardScrollPos > 0) {
            ...
            if (cg_scoreboardScrollPos < 0) cg_scoreboardScrollPos = 0;
        }
        return;
    }
    ...
    if ((cg_snap->ps.playerStateFlags & PSF_ACTIVE_PLAYER) == 0) return;
    if ((int32_t)(cg_time - (uint32_t)cg_weaponSelectTime) <
        cg_weaponCycleDelay.integer) return;
    ...
    if (cg_scoreboardShowing) {
        if (cg_scoreboardOverflowed) {
            cg_scoreboardScrollPos = coduo_int32_from_bits(
                (uint32_t)cg_scoreboardScrollPos +
                (uint32_t)cg_scoreboardScrollStep_vmCvar.integer);
            const int32_t limit = coduo_int32_from_bits(
                (uint32_t)cg_scoreboardNumClients - 1u);
...
void CG_WeaponSlot_f(void) /* 0x30047750 */
{
    if (!CG_WeaponCommandReady()) return;
    if ((cg_predictedPlayerState.entityStateFlags & EF_IN_VEHICLE) &&
        !(cg_snap->ps.vehicleType == 1 && cg_snap->ps.vehiclePosition == 3)) return;

    cg_weaponSelectTime = (int32_t)cg_time;
    const char *argument = CG_Argv(1);
    int32_t slot = BG_GetWeaponSlotForName(argument);
    if (slot == 0) slot = coduo_crt_atoi(CG_Argv(1));
    if (slot <= 0 || slot >= WEAPSLOT_COUNT) return;
