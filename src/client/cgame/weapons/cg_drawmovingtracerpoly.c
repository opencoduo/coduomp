...
void CG_DrawMovingTracerPoly(const vec3_t tailPoint, const vec3_t headPoint, float width)
{
    ...
    dotRight = (long double)cg_refdef.viewaxis[1][2] * (long double)direction[2] +
               (long double)cg_refdef.viewaxis[1][1] * (long double)direction[1] +
               (long double)cg_refdef.viewaxis[1][0] * (long double)direction[0];
    dotUp = (long double)cg_refdef.viewaxis[2][2] * (long double)direction[2] +
    ...
    {
        float rightPartial0 = (float)((long double)cg_refdef.viewaxis[1][0] * dotUp);
        float rightPartial1 = (float)((long double)cg_refdef.viewaxis[1][1] * dotUp);
        side[0] = (float)((long double)rightPartial0 + (long double)cg_refdef.viewaxis[2][0] * -dotRight);
        side[1] = (float)((long double)rightPartial1 + (long double)cg_refdef.viewaxis[2][1] * -dotRight);
        side[2] = (float)((long double)cg_refdef.viewaxis[1][2] * dotUp + (long double)cg_refdef.viewaxis[2][2] * -dotRight);
    }
    VectorNormalize(side);

    if (sv_night_vmCvar.integer) {
        width = (float)((long double)width * (long double)cg_tracernightscale_vmCvar.value);
    }
    ...
    verts[0].st[0] = 1.0f;
    verts[0].st[1] = 1.0f;
    verts[0].lightmapCoords[0] = 1.0f;
    verts[0].lightmapCoords[1] = 1.0f;
    verts[1].st[0] = 1.0f;
    verts[1].st[1] = 0.0f;
    verts[1].lightmapCoords[0] = 1.0f;
    verts[1].lightmapCoords[1] = 0.0f;
    verts[2].st[0] = 0.0f;
    verts[2].st[1] = 0.0f;
    verts[2].lightmapCoords[0] = 0.0f;
    verts[2].lightmapCoords[1] = 0.0f;
    verts[3].st[0] = 0.0f;
    verts[3].st[1] = 1.0f;
    verts[3].lightmapCoords[0] = 0.0f;
    verts[3].lightmapCoords[1] = 1.0f;
    for (i = 0; i < 4; ++i) {
    ...
    for (i = 0; i < 3; ++i) {
        center[i] = (float)((long double)direction[i] * 0.5L + (long double)tailPoint[i]);
    }
    {
        ...
        for (i = 0; i < 3; ++i) {
            plusRight[i] = (long double)cg_refdef.viewaxis[1][i] * (long double)width + (long double)center[i];
            minusRight[i] = (long double)cg_refdef.viewaxis[1][i] * -(long double)width + (long double)center[i];
            plusUp[i] = (long double)cg_refdef.viewaxis[2][i] * (long double)width;
            minusUp[i] = (long double)cg_refdef.viewaxis[2][i] * -(long double)width;
        ...
        verts[0].xyz[0] = (float)(plusUp[0] + (long double)plusRightRounded[0]);
        verts[0].xyz[1] = (float)(plusUp[1] + plusRight[1]);
        verts[0].xyz[2] = (float)(plusUp[2] + plusRight[2]);

        verts[1].xyz[0] = (float)((long double)plusUpRounded[0] + (long double)minusRightRounded[0]);
        verts[1].xyz[1] = (float)((long double)plusUpRounded[1] + minusRight[1]);
        verts[1].xyz[2] = (float)((long double)plusUpRounded[2] + minusRight[2]);

        verts[2].xyz[0] = (float)(minusUp[0] + (long double)minusRightRounded[0]);
        verts[2].xyz[1] = (float)(minusUp[1] + minusRight[1]);
        verts[2].xyz[2] = (float)(minusUp[2] + (long double)minusRightRounded[2]);

        verts[3].xyz[0] = (float)((long double)minusUpRounded[0] + (long double)plusRightRounded[0]);
        verts[3].xyz[1] = (float)((long double)minusUpRounded[1] + plusRight[1]);
        verts[3].xyz[2] = (float)((long double)minusUpRounded[2] + (long double)plusRightRounded[2]);
    }
