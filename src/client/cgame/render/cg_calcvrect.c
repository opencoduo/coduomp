...
void CG_CalcVrect(void)
{
...

    {
        ...
        if (cgame_compat_uses_classic_aspect() != qfalse) {
            ...
            } else if ((int64_t)vidWidth * 3 < (int64_t)vidHeight * 4) {
        ...
        int32_t width = (presentationWidth * widthSize / 100) & ~1;
        int32_t height = (presentationHeight * heightSize / 100) & ~1;

        /* Center the view rect on screen (signed halving rounds toward zero). */
        int32_t viewX = presentationX + (presentationWidth - width) / 2;
        int32_t viewY = presentationY + (presentationHeight - height) / 2;
