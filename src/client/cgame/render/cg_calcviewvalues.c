...
qboolean CG_CalcViewValues(void)
{
    ...
    if (cg_predictedPlayerState.pmType == PM_TYPE_INTERMISSION) {
        ...
        memcpy(cg_refdef.vieworg, cg_predictedPlayerState.psOrigin, sizeof(cg_refdef.vieworg));
        memcpy(cg_refdefViewAngles, cg_predictedPlayerState.viewAngles, sizeof(cg_refdefViewAngles));

        AngleVectors(cg_refdefViewAngles, cg_refdef.viewaxis[0], right, cg_refdef.viewaxis[2]);
     ...
     * then uses vertical speed; the ordinary path uses horizontal magnitude. */
    {
        ...
        cg_bobCyclePhase = (float)(((long double)bobByte / (long double)CG_BOB_CYCLE_DIVISOR) * (long double)CG_BOB_PHASE_TWO_PI +
                                   (long double)CG_BOB_PHASE_TWO_PI);

        if ((cg_predictedPlayerState.playerStateFlags & CG_VIEW_BOB_VERTICAL_SPEED_FLAG) != 0) {
            int32_t elapsed = coduo_int32_from_bits(cg_time - (uint32_t)cg_predictedPlayerState.lastJumpCommandTime);
            cg_weaponMoveSpeed = (elapsed < CG_VIEW_BOB_VERTICAL_DELAY_MS) ? 0.0f : cg_predictedPlayerState.velocity[2];
        } else {
            ...
            cg_weaponMoveSpeed = (float)coduo_x87_sqrtl((long double)vy * (long double)vy + (long double)vx * (long double)vx);
        }
    }
    ...
    if (cg_errordecay_vmCvar.value > 0.0f) {
        int32_t elapsed = coduo_int32_from_bits(cg_time - (uint32_t)cg_predictedErrorTime);
        float elapsedFloat = (float)elapsed;
        float decay =
            (float)(((long double)cg_errordecay_vmCvar.value - (long double)elapsedFloat) / (long double)cg_errordecay_vmCvar.value);
    ...
    } else if ((cg_predictedPlayerState.entityStateFlags & EF_IN_VEHICLE) != 0 &&
    ...
    if (cg_predictedPlayerState.pmType != PM_TYPE_NOCLIP) {
        ...
        if (CG_PointContents(lowerPoint, CG_VIEW_TRACE_EXCLUDE_NONE, CG_VIEW_COLLISION_MASK) != 0) {
            vec3_t upperPoint;
            trace_t trace;
            ...
            CG_Trace(CG_VIEW_COLLISION_HANDLE, upperPoint, cg_viewCollisionMaxs, &trace, upperPoint, cg_viewCollisionMins,
