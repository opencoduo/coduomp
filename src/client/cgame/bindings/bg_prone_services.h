    ...
    const vec3_t start, const vec3_t end, bg_prone_debug_color_t color)
{
    if (bg_compat_prone_debug_enabled() != qfalse) {
        cgame_syscall(CG_ADD_DEBUG_LINE, (intptr_t)start, (intptr_t)end,
                      (intptr_t)bg_compat_prone_debug_color(color),
...
{
    CG_DebugBox(mins, maxs, bg_compat_prone_debug_color(color),
                cg_debugProneCheckDepthCheck.integer);
}
...
static inline void bg_compat_prone_debug_circle(
    ...
    bg_prone_debug_color_t color)
{
    CG_DebugCircle(normal, center, radius,
