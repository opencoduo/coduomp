...
static inline void bg_compat_controller_set_control_tag_angles(void *dobjOwner, uint32_t *partBits, const char *tagName,
                                                               const vec3_t angles)
{
    const int32_t boneIndex = (int32_t)cgame_syscall(CG_DOBJ_GET_BONE_INDEX, (intptr_t)dobjOwner, tagName);

    if (boneIndex < 0) {
        return;
    }
    if (cgame_syscall(CG_DOBJ_SET_CONTROL_ROT_TRANS_INDEX, (intptr_t)dobjOwner, (intptr_t)partBits, boneIndex) == 0) {
        return;
    }
                                                      ...
                                                      const vec3_t angles)
{
    ...
    if (cgame_syscall(CG_DOBJ_SET_ROT_TRANS_INDEX, (intptr_t)dobjOwner, (intptr_t)partBits, boneIndex) == 0) {
        return;
    }
...
{
    return (qboolean)cgame_syscall(CG_XANIM_IS_PRIMITIVE, (uint16_t)(anim >> 16), (uint16_t)anim);
}
...
{
    return (int32_t)cgame_syscall(CG_XANIM_GET_LENGTH, (intptr_t)anims, animIndex);
}
...
{
    (void)cgame_syscall(CG_XANIM_CLEAR_GOAL_WEIGHT, (intptr_t)tree, (uint16_t)anim, CG_FloatBits(blendTime));
}
...
{
    (void)cgame_syscall(CG_XANIM_SET_TIME, (intptr_t)tree, (uint16_t)anim, CG_FloatBits(time));
}
...
{
    (void)cgame_syscall(CG_XANIM_SET_ANIM_RATE, (intptr_t)tree, (uint16_t)anim, CG_FloatBits(rate));
}
...
{
    return (qboolean)cgame_syscall(CG_XANIM_IS_PRIMITIVE, (int32_t)animTree, (uint32_t)animIndex);
}
...
{
    return (const char *)(intptr_t)cgame_syscall(CG_XANIM_GET_ANIM_NAME, (int32_t)animTree, (uint32_t)animIndex);
}
...
{
    (void)cgame_syscall(CG_XANIM_GET_REL_DELTA, (int32_t)animTree, (uint32_t)animIndex, (intptr_t)rotationDelta, (intptr_t)moveDelta, 0,
                        CG_FloatBits(1.0f));
}
...
{
    return (qboolean)cgame_syscall(CG_XANIM_IS_LOOPED_BY_TREE_INDEX, (int32_t)animTree, (uint32_t)animIndex);
}
