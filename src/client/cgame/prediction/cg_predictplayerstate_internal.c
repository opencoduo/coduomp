...
{
    if (cg_demoPlayback != 0 || (cg_snap->ps.playerStateFlags & PSF_FOLLOWING) != 0) {
    ...
    cg_pmove.adsInputBlocked = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_IS_IN_MATCH_TIMEOUT));

    cg_pmove.traceMask = cg_predictedPlayerState.pmType < PM_TYPE_DEAD ? MASK_PLAYERSOLID : MASK_DEADSOLID;
    if (cg_snap->ps.pmType == PM_TYPE_SPECTATOR) {
        cg_pmove.traceMask &= ~PMOVE_TRACE_MASK_SPECTATOR_CLEAR;
    }

    const int32_t currentCmdNumber = coduo_int32_from_bits((uint32_t)cgame_syscall(CG_GET_CURRENT_CMD_NUMBER));
    const int32_t oldestCmdNumber = coduo_int32_from_bits((uint32_t)currentCmdNumber - (uint32_t)(CMD_BACKUP - 1));
    usercmd_t oldestCmd;
    ...
    memcpy(&cg_predictedPlayerState, &cg_nextSnap->ps, sizeof(cg_predictedPlayerState));
    ...
    if ((cg_predictedPlayerState.entityStateFlags & EF_IN_VEHICLE) != 0) {
        int32_t followedEntityNum = cg_predictedPlayerState.viewLockedEntityNum;
        /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered client-module boundary input and state before use. */
        if ((uint32_t)followedEntityNum >= (uint32_t)MAX_GENTITIES) {
            Com_Error(ERR_DROP,
                      "\x15"
                      "CG_PredictPlayerState_Internal: invalid "
                      "view-lock entity %i",
                      followedEntityNum);
            return;
        }
        ...
        if (followed->currentValid != 0) {
            ...
            if (cg_predictedPlayerState.vehicleType == 1 && cg_predictedPlayerState.vehiclePosition == 3) {
    ...
    if (requestedPmoveMsec < PMOVE_MSEC_MIN) {
        cgame_syscall(CG_CVAR_SET_VALUE, (intptr_t)&pmove_msec, "8");
    } else if (requestedPmoveMsec > PMOVE_MSEC_MAX) {
        cgame_syscall(CG_CVAR_SET_VALUE, (intptr_t)&pmove_msec, "33");
    }
    ...
    while (cmdNumber <= currentCmdNumber) {
        ...
        if (cg_pmove.pmove_msec_min != 0) {
            PM_UpdateViewAngles(&cg_predictedPlayerState, cmd, CG_TraceCapsule);
        }

        if (cmd->commandTime <= cg_predictedPlayerState.commandTime || cmd->commandTime > latestCmd.commandTime) {
            goto next_command;
        }
        int32_t previousCmdNumber = coduo_int32_from_bits((uint32_t)cmdNumber - 1u);
        ...
        if (cg_predictedPlayerState.commandTime == oldPlayerState.commandTime) {
            ...
            CG_AdjustPositionForMover(cg_predictedPlayerState.psOrigin, cg_predictedPlayerState.groundEntityNum, cg_latestSnapshotTime,
                                      cg_physicsTime, adjustedOrigin, moverAngleDelta);
            ...
            int32_t yawDeltaShort = coduo_fp_to_i32_extended((long double)moverAngleDelta[1] * (long double)ANGLE2SHORT_SCALE) & 0xffff;
            cg_predictedPlayerState.deltaAngles[1] =
                coduo_int32_from_bits((uint32_t)cg_predictedPlayerState.deltaAngles[1] + (uint32_t)yawDeltaShort);
            ...
            if (cg_showmiss_vmCvar.integer != 0 &&
                (oldPlayerState.psOrigin[0] != adjustedOrigin[0] || oldPlayerState.psOrigin[1] != adjustedOrigin[1] ||
                 oldPlayerState.psOrigin[2] != adjustedOrigin[2])) {
                Com_PrintMessage("prediction error\n");
            }

            vec3_t miss;
            miss[0] = (float)((long double)oldPlayerState.psOrigin[0] - (long double)adjustedOrigin[0]);
            miss[1] = (float)((long double)oldPlayerState.psOrigin[1] - (long double)adjustedOrigin[1]);
            ...
            const long double miss2raw = (long double)oldPlayerState.psOrigin[2] - (long double)adjustedOrigin[2];
            miss[2] = (float)miss2raw;
            const long double missDistanceRaw = sqrtl((miss2raw * (long double)miss[2] + (long double)miss[1] * (long double)miss[1]) +
            ...
            if (missDistanceRaw > 0.1f) {
                ...
                if (cg_errordecay_vmCvar.integer != 0) {
                    const int32_t elapsed = coduo_int32_from_bits(cg_time - (uint32_t)cg_predictedErrorTime);
                    ...
                    long double decayRaw =
                        ((long double)cg_errordecay_vmCvar.value - (long double)elapsed) / (long double)cg_errordecay_vmCvar.value;
                    ...
                    cg_predictedError[0] = (float)((long double)cg_predictedError[0] * (long double)decay);
                    cg_predictedError[1] = (float)((long double)cg_predictedError[1] * (long double)decay);
                    /* The Z product at 0x30035cce remains in ST0 across the
                     * common X/Y tail and is not rounded before miss[2] is
                     * added. */
                    long double scaledZRaw = (long double)cg_predictedError[2] * (long double)decay;

                    float correctedX = (float)((long double)cg_predictedError[0] + (long double)miss[0]);
                    int32_t errorTime = cg_physicsTime;
                    cg_predictedErrorTime = errorTime;
                    cg_predictedError[0] = correctedX;
                    cg_predictedError[1] = (float)((long double)cg_predictedError[1] + (long double)miss[1]);
                    cg_predictedError[2] = (float)(scaledZRaw + (long double)miss[2]);
                } else {
        ...
        if (cg_pmove.pmove_msec_min != 0) {
            ...
            int32_t roundedNumerator = coduo_int32_from_bits((uint32_t)cmd->commandTime + (uint32_t)pmoveMsec - 1u);
            int32_t quotient = roundedNumerator / pmoveMsec;
            cmd->commandTime = coduo_int32_from_bits((uint32_t)quotient * (uint32_t)pmoveMsec);
        }
        ...
        if (cg_norender_vmCvar.integer != 0) {
            ...
            cmd->rightmove = 0;
            cmd->upmove = 0;
            int32_t commandAdvance = coduo_int32_from_bits((uint32_t)cmd->commandTime - (uint32_t)cg_predictedPlayerState.commandTime);
            if (commandAdvance > 1) {
                cmd->commandTime = coduo_int32_from_bits((uint32_t)cg_predictedPlayerState.commandTime + 1u);
            }
        }
        ...
        } else if ((cg_predictedPlayerState.playerStateFlags & PS_FLAG_DUCKED) != 0) {
            traceMaxs[2] = 50.0f; /* 0x30035e12: 0x42480000 */
        }

        trace_t overlap;
        CG_TraceCapsule(&overlap, cg_predictedPlayerState.psOrigin, traceMins, traceMaxs, cg_predictedPlayerState.psOrigin,
                        cg_predictedPlayerState.psClientNum, cg_pmove.traceMask);

        if (overlap.allsolid != 0 || overlap.startsolid != 0) {
            centity_t *hitEntity = cgame_compat_unchecked_cgentity((int32_t)overlap.entityNum);
            if (hitEntity->currentValid != 0 && hitEntity->currentState.eType == PMOVE_VEHICLE_TYPE) {
                vec3_t away = {cg_predictedPlayerState.psOrigin[0] - hitEntity->lerpOrigin[0],
                               cg_predictedPlayerState.psOrigin[1] - hitEntity->lerpOrigin[1],
                               cg_predictedPlayerState.psOrigin[2] - hitEntity->lerpOrigin[2]};
                (void)VectorNormalize(away);

                vec3_t velocityDirection;
                const float velocitySpeed = VectorNormalize2(cg_predictedPlayerState.velocity, velocityDirection);
                ...
                const long double horizontalDot =
                    (long double)velocityDirection[0] * (long double)away[0] + (long double)velocityDirection[1] * (long double)away[1];
                if (velocitySpeed < 1.0f || horizontalDot > 0.80000001f) {
                    ...
                    if ((cg_predictedPlayerState.playerStateFlags & PMF_PRONE) == 0 && velocitySpeed < 1.0f) {
                        ...
                        cg_predictedPlayerState.playerStateFlags |= PS_FLAG_PREDICTION_PUSH;
                    }
                }
            }
        }
    ...
    if (verbosePrediction > 1) {
        Com_PrintMessage("[%i : %i] ", cmd->commandTime, coduo_int32_from_bits(cg_time));
    }
    ...
    CG_AdjustPositionForMover(cg_predictedPlayerState.psOrigin, cg_predictedPlayerState.groundEntityNum, cg_latestSnapshotTime,
                              coduo_int32_from_bits(cg_time), cg_predictedPlayerState.psOrigin, moverAngleDelta);
