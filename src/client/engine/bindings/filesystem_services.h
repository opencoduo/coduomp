...
static inline qboolean filesystem_compat_accept_server_pak_name(const char *path)
{
    if (path == NULL || path[0] == '\0')
        return qfalse;
    if (coduo_compat_path_is_safe_relative(path) == qfalse || strchr(path, '@') != NULL) {
        Com_Printf("WARNING: refusing unsafe server pak name '%s'\n", path);
        return qfalse;
    }
    return qtrue;
}
...
{
    return coduomp_resolve_case_path(trustedRoot, path, resolvedPath, resolvedPathSize);
}
...
{
    return coduomp_server_namespace_append_cached_mods(listBuffer, bufferSize);
}
...
static inline void filesystem_compat_report_unsupported_pak_language(const char *base, const char *game, const char *pakName)
{
    ...
    if (fs_printedSupportedLanguages == qfalse) {
        ...
        for (int32_t language = 0; language < filesystem_compat_language_count(); ++language) {
