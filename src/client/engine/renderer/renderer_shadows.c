...
_Static_assert(_Alignof(renderer_shadow_edge_t) == 4, "i386 shadow-edge alignment changed");
_Static_assert(offsetof(renderer_shadow_edge_t, secondVertex) == 0x00, "i386 shadow-edge second vertex moved");
_Static_assert(offsetof(renderer_shadow_edge_t, facing) == 0x04, "i386 shadow-edge facing flag moved");
_Static_assert(sizeof(renderer_shadow_edge_t) == 0x08, "i386 shadow-edge size changed");
#endif
...
static qboolean rendererShadowTriangleFacing[R_MAX_SHADOW_TRIANGLES]; /* 0x027937d8 */
static renderer_shadow_edge_t rendererShadowEdges[R_MAX_TESS_VERTICES][R_MAX_SHADOW_EDGES_PER_VERTEX]; /* 0x028137d8 */
static int32_t rendererShadowEdgeCounts[R_MAX_TESS_VERTICES]; /* 0x038137d8 */
...
{
    const float groundDistance = backEnd.orientation.origin[2] - backEnd.currentEntity->e.shadowPlane;
    const vec3_t groundNormal = {backEnd.orientation.axis[0][2], backEnd.orientation.axis[1][2], backEnd.orientation.axis[2][2]};
    long double lightDirectionRaw[3] = {(long double)backEnd.currentEntity->lightDir[0], (long double)backEnd.currentEntity->lightDir[1],
                                        (long double)backEnd.currentEntity->lightDir[2]};
    vec3_t projectedDirection;
    long double lightDotRaw = (lightDirectionRaw[2] * (long double)groundNormal[2] + lightDirectionRaw[1] * (long double)groundNormal[1]) +
                              lightDirectionRaw[0] * (long double)groundNormal[0];
    float roundedLightDirectionZ = backEnd.currentEntity->lightDir[2];
    ...
    if (lightDotRaw < 0.5L) {
        ...
        lightDirectionRaw[0] += adjustment * (long double)groundNormal[0];
        lightDirectionRaw[1] += adjustment * (long double)groundNormal[1];
        lightDirectionRaw[2] += adjustment * (long double)groundNormal[2];
        roundedLightDirectionZ = (float)lightDirectionRaw[2];
        lightDotRaw = (lightDirectionRaw[2] * (long double)groundNormal[2] + lightDirectionRaw[1] * (long double)groundNormal[1]) +
                      lightDirectionRaw[0] * (long double)groundNormal[0];
    }
    ...
    projectedDirection[0] = (float)(lightDirectionRaw[0] * (long double)inverseLightDot);
    projectedDirection[1] = (float)(lightDirectionRaw[1] * (long double)inverseLightDot);
    projectedDirection[2] = roundedLightDirectionZ * inverseLightDot;

    for (int32_t vertexIndex = 0; vertexIndex < tess.vertexCount; ++vertexIndex) {
        float *vertex = &tess.xyz[(size_t)vertexIndex * (size_t)tess.vertexComponentCount];
        const float height = vertex[2] * groundNormal[2] + vertex[1] * groundNormal[1] + vertex[0] * groundNormal[0] + groundDistance;
...
void R_RenderShadowEdges(void)
{
    ...
    for (int32_t firstVertex = 0; firstVertex < tess.vertexCount; ++firstVertex) {
        const int32_t edgeCount = rendererShadowEdgeCounts[firstVertex];

        for (int32_t edgeIndex = 0; edgeIndex < edgeCount; ++edgeIndex) {
            const renderer_shadow_edge_t *edge = &rendererShadowEdges[firstVertex][edgeIndex];
            int32_t reverseFacingCounts[2] = {0, 0};

            if (edge->facing == qfalse)
                continue;

            for (int32_t reverseIndex = 0; reverseIndex < rendererShadowEdgeCounts[edge->secondVertex]; ++reverseIndex) {
                const renderer_shadow_edge_t *reverseEdge = &rendererShadowEdges[edge->secondVertex][reverseIndex];
            ...
            const float *firstFront = &tess.xyz[(size_t)firstVertex * (size_t)tess.vertexComponentCount];
            const float *firstBack = &tess.xyz[(size_t)(firstVertex + tess.vertexCount) * (size_t)tess.vertexComponentCount];
            const float *secondFront = &tess.xyz[(size_t)edge->secondVertex * (size_t)tess.vertexComponentCount];
            const float *secondBack = &tess.xyz[(size_t)(edge->secondVertex + tess.vertexCount) * (size_t)tess.vertexComponentCount];
...
void RB_ShadowTessEnd(void)
{
    ...
    if (tess.vertexCount >= R_MAX_SHADOW_VERTICES || glConfig.stencilBits < 4) {
        return;
    }
    ...
    for (int32_t vertexIndex = 0; vertexIndex < tess.vertexCount; ++vertexIndex) {
        const float *front = &tess.xyz[(size_t)vertexIndex * (size_t)tess.vertexComponentCount];
        float *back = &tess.xyz[(size_t)(vertexIndex + tess.vertexCount) * (size_t)tess.vertexComponentCount];
    ...
    memset(rendererShadowEdgeCounts, 0, (size_t)tess.vertexCount * sizeof(rendererShadowEdgeCounts[0]));

    for (int32_t triangleIndex = 0; triangleIndex < tess.indexCount / 3; ++triangleIndex) {
        ...
        const float *first = &tess.xyz[(size_t)firstIndex * (size_t)tess.vertexComponentCount];
        const float *second = &tess.xyz[(size_t)secondIndex * (size_t)tess.vertexComponentCount];
        const float *third = &tess.xyz[(size_t)thirdIndex * (size_t)tess.vertexComponentCount];
        ...
        normal[0] = firstToSecond[1] * firstToThird[2] - firstToSecond[2] * firstToThird[1];
        normal[1] = firstToSecond[2] * firstToThird[0] - firstToSecond[0] * firstToThird[2];
        normal[2] = firstToSecond[0] * firstToThird[1] - firstToSecond[1] * firstToThird[0];

        facingDot = normal[2] * lightDirection[2] + normal[1] * lightDirection[1] + normal[0] * lightDirection[0];
        facing = facingDot > 0.0f ? qtrue : qfalse;
        rendererShadowTriangleFacing[triangleIndex] = facing;

        R_AddEdgeDef(firstIndex, secondIndex, rendererShadowTriangleFacing[triangleIndex]);
        R_AddEdgeDef(secondIndex, thirdIndex, rendererShadowTriangleFacing[triangleIndex]);
        R_AddEdgeDef(thirdIndex, firstIndex, rendererShadowTriangleFacing[triangleIndex]);
    }
    ...
    qglColor3f(0.20000000298023224f, 0.20000000298023224f, 0.20000000298023224f);
...
void RB_ShadowFinish(void)
{
    ...
    RB_BeginImmediateMode();
    RB_glColor3f(0.60000002384185791f, 0.60000002384185791f, 0.60000002384185791f);
