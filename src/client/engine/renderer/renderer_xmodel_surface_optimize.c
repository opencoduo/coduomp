...
void R_OptimizeRigidXSurfaceARB(XSurface *surface, void *(*alloc)(size_t size))
{
    ...
    for (int32_t vertexIndex = 0; vertexIndex < surface->vertexCount; ++vertexIndex) {
        ...
        memcpy(&vertices[vertexIndex].rigidVertex, &surface->vertexData.rigidVertices[vertexIndex],
               sizeof(vertices[vertexIndex].rigidVertex));
    }

    const uint32_t indexBytes =
        ((uint32_t)(int32_t)surface->triangleCount * (uint32_t)sizeof(surface->triangles[0]) + (XMODEL_GPU_BUFFER_ALIGNMENT - 1u)) &
        ~(uint32_t)(XMODEL_GPU_BUFFER_ALIGNMENT - 1u);
    uint32_t indexBuffer = R_CreateBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, indexBytes, surface->triangles, GL_STATIC_DRAW_ARB);
    if (indexBuffer == 0) {
        return;
    }

    uint32_t vertexBuffer =
        R_CreateBufferARB(GL_ARRAY_BUFFER_ARB, (size_t)((uint32_t)(int32_t)surface->vertexCount * (uint32_t)sizeof(vertices[0])), vertices,
                          GL_STATIC_DRAW_ARB);
    if (vertexBuffer == 0) {
...
void R_OptimizeRigidXSurfaceNV(XSurface *surface, void *(*alloc)(size_t size))
{
    ...
    const uint32_t vertexBytes =
        ((uint32_t)(int32_t)surface->vertexCount * (uint32_t)sizeof(XSurfaceARBVert) + (XMODEL_GPU_BUFFER_ALIGNMENT - 1u)) &
        ~(uint32_t)(XMODEL_GPU_BUFFER_ALIGNMENT - 1u);

    optimized->memorySource = R_AllocMemoryNV(R_STATIC_VERTEX_MEMORY_PRIMARY, vertexBytes, &optimized->interleavedVertices);

    XSurfaceARBVert *vertices = (XSurfaceARBVert *)optimized->interleavedVertices;
    for (int32_t vertexIndex = 0; vertexIndex < surface->vertexCount; ++vertexIndex) {
        memcpy(vertices[vertexIndex].texCoord, surface->texCoords[vertexIndex], sizeof(vertices[vertexIndex].texCoord));
        memcpy(&vertices[vertexIndex].rigidVertex, &surface->vertexData.rigidVertices[vertexIndex],
               sizeof(vertices[vertexIndex].rigidVertex));
...
void R_OptimizeRigidXSurfaceATI(XSurface *surface, void *(*alloc)(size_t size))
{
    ...
    size_t indexBytes = glConfig.elementArrayATIAvailable != qfalse
                            ? (((size_t)surface->triangleCount * sizeof(surface->triangles[0]) + (XMODEL_GPU_BUFFER_ALIGNMENT - 1U)) &
                               ~(size_t)(XMODEL_GPU_BUFFER_ALIGNMENT - 1U))
                            : 0U;
    size_t vertexBytes = (size_t)surface->vertexCount * sizeof(vertices[0]);
    size_t totalBytes = (vertexBytes + indexBytes + (XMODEL_GPU_BUFFER_ALIGNMENT - 1U)) & ~(size_t)(XMODEL_GPU_BUFFER_ALIGNMENT - 1U);

    size_t indexOffset;
    const renderer_static_vertex_memory_source_t memorySource = R_AllocMemoryATI(R_STATIC_VERTEX_MEMORY_PRIMARY, totalBytes, &indexOffset);
    ...
    if (indexBytes != 0U) {
        qglUpdateObjectBufferATI(optimized->objectBuffer, optimized->indexOffset, (uint32_t)indexBytes, surface->triangles,
                                 GL_PRESERVE_ATI);
    }
    qglUpdateObjectBufferATI(optimized->objectBuffer, optimized->vertexOffset, (uint32_t)vertexBytes, vertices, GL_PRESERVE_ATI);
...
void XSurfaceRefresh_ARB(XSurface *surface, uint32_t refreshFlags)
{
    ...
    const uint32_t indexBytes =
        ((uint32_t)(int32_t)surface->triangleCount * (uint32_t)sizeof(surface->triangles[0]) + (XMODEL_GPU_BUFFER_ALIGNMENT - 1u)) &
        ~(uint32_t)(XMODEL_GPU_BUFFER_ALIGNMENT - 1u);
    const uint32_t vertexBytes = (uint32_t)(int32_t)surface->vertexCount * (uint32_t)sizeof(vertices[0]);
    ...
    if ((refreshFlags & XSURFACE_REFRESH_INDICES) != 0U) {
        ...
        qglBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, indexBytes, surface->triangles, GL_STATIC_DRAW_ARB);
    ...
    if ((refreshFlags & XSURFACE_REFRESH_VERTICES) != 0U) {
        ...
        qglBufferDataARB(GL_ARRAY_BUFFER_ARB, vertexBytes, vertices, GL_STATIC_DRAW_ARB);
