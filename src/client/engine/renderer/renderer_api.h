...
typedef struct refexport_s {
    ...
    int32_t (*GetShaderFromModel)(int32_t modelHandle,
                                  int32_t surfaceIndex);               /* 10 */
    ...
    void (*AddRefEntityToScene)(const refEntity_t *entity,
                                renderer_static_model_t
                                    *staticLighting);                  /* 18 */
    void (*AddPolyToScene)(int32_t shaderHandle, int32_t vertexCount,
                           const polyVert_t *vertices);                 /* 19 */
    void (*AddPolysToScene)(int32_t shaderHandle, int32_t vertexCount,
                            const polyVert_t *vertices,
                            int32_t polyCount);                         /* 20 */
    void (*AddLightToScene)(const vec3_t origin, float radius,
                            float red, float green, float blue);        /* 21 */
    void (*SetCullDist)(float distance);                                /* 22 */
    void (*AddCoronaToScene)(const vec3_t origin, float red, float green,
                             float blue, float scale, int32_t id,
                             int32_t flags);                            /* 23 */
    void (*SetFog)(int32_t fogIndex, int32_t fogStart, int32_t fogEnd,
                   float red, float green, float blue, float density); /* 24 */
    ...
    void (*StretchPic)(float x, float y, float width, float height,
                       float s1, float t1, float s2, float t2,
                       int32_t shaderHandle);                           /* 30 */
    void (*StretchPicGradient)(float x, float y, float width, float height,
                               float s1, float t1, float s2, float t2,
                               int32_t shaderHandle,
                               const float *gradientColor,
                               int32_t gradientType);                   /* 31 */
    void (*StretchPicRotate)(float x, float y, float width, float height,
                             float s1, float t1, float s2, float t2,
                             float angleDegrees,
                             int32_t shaderHandle);                     /* 32 */
    void (*DrawQuadPic)(const vec2_t positions[4],
                        const vec2_t texCoords[4],
                        int32_t shaderHandle);                           /* 33 */
    void (*StretchRaw)(int32_t x, int32_t y, int32_t width, int32_t height,
                       int32_t columns, int32_t rows, const uint8_t *data,
                       int32_t client, qboolean dirty);                 /* 34 */
    void (*UploadCinematic)(int32_t width, int32_t height,
    ...
    qboolean (*PickShader)(const vec3_t start, const vec3_t direction,
                           char *materialName, char *surfaceFlags,
                           char *contents, int32_t bufferSize);         /* 43 */
    void (*RegisterFont)(const char *name, int32_t pointSize,
                         fontInfo_t *font, int32_t loadMode);      /* 44 */
    ...
    void (*CubemapShot)(const char *fileName, int32_t faceSize,
                        cubemap_face_t face, float fresnelN0,
                        float fresnelN1);                              /* 48 */
    void (*CubemapWaterShot)(const char *fileName, int32_t faceSize,
                             cubemap_face_t face,
                             const vec3_t horizonColor,
                             const vec3_t zenithColor);                /* 49 */
    void (*LocateDebugStrings)(const struct client_debug_string_s *strings,
                               int32_t stringCount);                   /* 50 */
    void (*LocateDebugLines)(const struct client_debug_line_s *lines,
                             int32_t lineCount);                       /* 51 */
    void (*AddPlume)(const vec3_t origin, int32_t labelValue,
                     const vec3_t color, int32_t duration);            /* 52 */
    int32_t (*TextWidth)(const char *text, int32_t fontHandle,
                         float scale, float fixedAdvance,
                         int32_t limit);                               /* 53 */
    int32_t (*TextHeight)(int32_t fontHandle, float scale);             /* 54 */
    void (*TextPaint)(float x, float y, int32_t fontHandle, float scale,
                      const float color[4], const char *text,
                      float fixedAdvance, int32_t limit,
                      int32_t textStyle);                              /* 55 */
    int32_t (*TextConsoleWidth)(const uint16_t *encoded,
                                int32_t fontHandle, float scale,
                                float fixedAdvance,
                                int32_t encodedCount);                 /* 56 */
    void (*TextConsolePaint)(float x, float y, int32_t fontHandle,
                             float scale, const float color[4],
                             const uint16_t *encoded, float fixedAdvance,
                             int32_t encodedCount,
                             int32_t textStyle);                       /* 57 */
    void (*TextPaintWithCursor)(float x, float y, int32_t fontHandle,
                                float scale, const float color[4],
...
int32_t RE_MarkFragments(int32_t pointCount, const vec3_t *points,
                         ...
                         markFragment_t *fragmentBuffer,
                         int32_t shaderHandle);
qboolean RE_PickShader(const vec3_t start, const vec3_t direction,
                       char *materialName, char *surfaceFlags,
                       char *contents, int32_t bufferSize);
...
_Static_assert(sizeof(refimport_t) == 0x98,
               "i386 renderer import-table size changed");
#define RENDERER_ASSERT_I386_EXPORT_SLOT(member, slot)                         \
    _Static_assert(offsetof(refexport_t, member) == (slot) * 4,                \
                   "i386 renderer export slot " #member " moved");           \
    _Static_assert(sizeof(((refexport_t *)0)->member) == 4,                    \
                   "i386 renderer export slot " #member " extent changed")
_Static_assert(_Alignof(refexport_t) == 0x04,
               "i386 renderer export-table alignment changed");
...
_Static_assert(offsetof(refexport_t, ClearScene) == 0x44,
               "i386 renderer ClearScene export offset changed");
_Static_assert(offsetof(refexport_t, SaveFogState) == 0x64,
               "i386 renderer fog-save export offset changed");
_Static_assert(offsetof(refexport_t, RenderScene) == 0x6c,
               "i386 renderer RenderScene export offset changed");
_Static_assert(offsetof(refexport_t, BeginFrame) == 0x90,
               "i386 renderer BeginFrame export offset changed");
_Static_assert(offsetof(refexport_t, CubemapShot) == 0xc0,
               "i386 renderer CubemapShot export offset changed");
_Static_assert(offsetof(refexport_t, TextPaintWithCursor) == 0xe8,
               "i386 renderer final export offset changed");
_Static_assert(sizeof(refexport_t) == 0xec,
               "i386 renderer export-table size changed");
#endif
