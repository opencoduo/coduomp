...
static const vec4_t staticModelCountColor = {0.0f, 1.0f, 1.0f, 1.0f};
...
{
    renderer_registered_static_model_t *registration = entity->e.staticModelRegistration;
    renderer_static_model_t *lighting = entity->staticModelLighting;
    const int32_t lodIndex = XModelGetLodForDist(registration->model, R_GetLodDist(entity));
    ...
    for (surfaceIndex = 0; surfaceIndex < lod->surfaceCount; ++surfaceIndex) {
        renderer_static_model_surface_t *surface = &lod->surfaces[surfaceIndex];
        const int32_t globalSurfaceIndex = firstSurfaceIndex + surfaceIndex;
        renderer_cached_static_model_surface_t **cacheSlot = &lighting->surfaceLightingCache[globalSurfaceIndex];
        ...
        if (cached == NULL && surface->cachedShader != NULL) {
            ...
            if (entity->hasDynamicLights == 0) {
                cached = R_CacheStaticModelSurface(surface, lighting, globalSurfaceIndex, entity, inverseAxis);
                *cacheSlot = cached;
            }
        }

        if (cached == NULL) {
            R_AddDrawSurf((renderer_surface_t *)surface, surface->storageSource, surface->shader, 0, 0, 0);
            if ((surface->shader->lightingFlags & SHADER_LIGHTING_ENTITY_MASK) != 0) {
                needsLighting = qtrue;
            }
        } else {
            ...
            if ((surface->shader->lightingFlags & SHADER_LIGHTING_ENTITY_MASK) == 0) {
                R_AddDrawSurf((renderer_surface_t *)cached, tr.cachedStaticModelStorageSource, surface->cachedShader, 0, 0, 0);
            } else {
                R_SetupStaticModelLighting(&tr.refdef, entity);
                if (entity->hasDynamicLights == 0) {
                    R_AddDrawSurf((renderer_surface_t *)cached, tr.cachedStaticModelStorageSource, surface->cachedShader, 0, 0, 0);
    ...
    if ((entity->e.renderfx & RF_DEPTHHACK) == 0) {
        if (r_showtricounts->integer != 0) {
            R_AddScaledDebugString(entity->e.origin, staticModelCountColor, va("%i", totalIndexCount / 3));
        } else if (r_showsurfcounts->integer != 0) {
            R_AddScaledDebugString(entity->e.origin, staticModelCountColor, va("%i", lod->surfaceCount));
        }
    }
