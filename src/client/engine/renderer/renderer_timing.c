...
float R_UpdateOverTime(float currentValue, float targetValue, int32_t riseTime, int32_t fallTime, int32_t elapsedTime)
{
    if (currentValue < targetValue) {
        ...
        const long double candidate = (long double)elapsedTime / riseTime + currentValue;
        ...
        const long double candidate = (long double)currentValue - (long double)elapsedTime / fallTime;
        currentValue = (float)candidate;
        if (candidate < targetValue)
            return targetValue;
    }
