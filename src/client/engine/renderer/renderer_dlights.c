...
void R_TransformDlights(int32_t lightCount, renderer_light_t *lights, const orientationr_t *orientation)
{
    for (int32_t lightIndex = 0; lightIndex < lightCount; ++lightIndex) {
        ...
        if (light->position[3] == 0.0f) {
            ...
            light->transformedPosition[0] = (float)(((long double)light->position[0] * (long double)orientation->axis[0][0] +
                                                     (long double)light->position[2] * (long double)orientation->axis[0][2]) +
                                                    (long double)light->position[1] * (long double)orientation->axis[0][1]);
            light->transformedPosition[1] = (float)(((long double)light->position[0] * (long double)orientation->axis[1][0] +
                                                     (long double)light->position[2] * (long double)orientation->axis[1][2]) +
                                                    (long double)light->position[1] * (long double)orientation->axis[1][1]);
            light->transformedPosition[2] = (float)(((long double)light->position[2] * (long double)orientation->axis[2][2] +
                                                     (long double)light->position[1] * (long double)orientation->axis[2][1]) +
                                                    (long double)light->position[0] * (long double)orientation->axis[2][0]);
        } else {
            /* 0x004c5f42..0x004c5fda retains all three subtractions in x87
             * registers through the three stores. */
            const long double localPosition[3] = {(long double)light->position[0] - (long double)orientation->origin[0],
                                                  (long double)light->position[1] - (long double)orientation->origin[1],
                                                  (long double)light->position[2] - (long double)orientation->origin[2]};

            light->transformedPosition[0] = (float)((localPosition[2] * (long double)orientation->axis[0][2] +
                                                     localPosition[0] * (long double)orientation->axis[0][0]) +
                                                    localPosition[1] * (long double)orientation->axis[0][1]);
            light->transformedPosition[1] = (float)((localPosition[2] * (long double)orientation->axis[1][2] +
                                                     localPosition[0] * (long double)orientation->axis[1][0]) +
                                                    localPosition[1] * (long double)orientation->axis[1][1]);
            light->transformedPosition[2] = (float)((localPosition[2] * (long double)orientation->axis[2][2] +
                                                     localPosition[1] * (long double)orientation->axis[2][1]) +
                                                    localPosition[0] * (long double)orientation->axis[2][0]);
        }
    }
}
    ...
    while (dlightBits != 0) {
        if ((dlightBits & lightBit) != 0) {
            ...
            for (int32_t axis = 0; axis < 3; ++axis) {
                const long double high = (long double)light->transformedPosition[axis] + light->radius;
                const long double low = (long double)light->transformedPosition[axis] - light->radius;
...
uint32_t R_DlightTris(renderer_lit_surface_t *surface, uint32_t dlightBits)
{
    ...
    for (int32_t lightIndex = 0; lightIndex < tr.refdef.num_dlights; ++lightIndex) {
        const uint32_t lightBit = 1U << ((uint32_t)lightIndex & R_DLIGHT_BIT_INDEX_MASK);
        ...
        for (int32_t axis = 0; axis < 3; ++axis) {
            const long double high = (long double)light->transformedPosition[axis] + light->radius;
            const long double low = (long double)light->transformedPosition[axis] - light->radius;
            if (high <= surface->boundsMin[axis] || low >= surface->boundsMax[axis]) {
                overlaps = qfalse;
                break;
            }
        }
...
void R_DlightBmodel(bmodel_t *bmodel)
{
    ...
    for (int32_t lightIndex = 0; lightIndex < tr.refdef.num_dlights; ++lightIndex) {
        ...
        for (int32_t axis = 0; axis < 3; ++axis) {
            if (light->transformedPosition[axis] - bmodel->bounds[1][axis] > light->radius ||
                bmodel->bounds[0][axis] - light->transformedPosition[axis] > light->radius) {
                separated = qtrue;
                break;
            }
        }
        ...
        if (separated == qfalse) {
            dlightBits |= 1U << ((uint32_t)lightIndex & R_DLIGHT_BIT_INDEX_MASK);
        }
    }

    tr.currentEntity->dlightBits = dlightBits;
    for (int32_t surfaceIndex = 0; surfaceIndex < bmodel->numSurfaces; ++surfaceIndex) {
        renderer_surface_t *surface = bmodel->firstSurface[surfaceIndex].data;
