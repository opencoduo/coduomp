...
#define CODUOMP_LOAD_ARB_PROGRAM_ENTRY(type_, name_) \
    ...
    CODUOMP_LOAD_ARB_PROGRAM_ENTRY(
        renderer_gl_bind_program_arb_func_t, BindProgramARB);
    CODUOMP_LOAD_ARB_PROGRAM_ENTRY(
        renderer_gl_delete_programs_arb_func_t, DeleteProgramsARB);
    CODUOMP_LOAD_ARB_PROGRAM_ENTRY(
        renderer_gl_gen_programs_arb_func_t, GenProgramsARB);
    CODUOMP_LOAD_ARB_PROGRAM_ENTRY(
        renderer_gl_program_string_arb_func_t, ProgramStringARB);

#undef CODUOMP_LOAD_ARB_PROGRAM_ENTRY

    return qglBindProgramARB != NULL &&
...
void coduomp_output_gamma_initialize_compat(void)
{
    ...
    if (qglActiveTextureARB == NULL || glConfig.maxActiveTextures < 2 ||
        GLW_HasExtension(glConfig.extensionsString,
                         "GL_ARB_fragment_program") == qfalse ||
        coduomp_output_gamma_load_program_functions_compat() == qfalse) {
        ri.Printf(R_PRINT_WARNING,
                  "WARNING: exact OpenGL output gamma LUT is unavailable.\n");
        return;
    }

    qglGenProgramsARB(1, &coduompOutputGammaProgram);
    qglBindProgramARB(GL_FRAGMENT_PROGRAM_ARB,
                      coduompOutputGammaProgram);
    qglProgramStringARB(GL_FRAGMENT_PROGRAM_ARB,
                        GL_PROGRAM_FORMAT_ASCII_ARB,
                        (int32_t)strlen(programSource), programSource);
    ...
    ri.Printf(R_PRINT_ALL,
              "Exact OpenGL output gamma LUT is available.\n");
}
...
qboolean coduomp_output_gamma_activate_software_compat(const char *reason)
{
    if (coduompOutputGammaSoftwareAvailable == qfalse) {
        ri.Printf(R_PRINT_WARNING,
                  "WARNING: %s; no exact output gamma fallback is available.\n",
                  reason);
        return qfalse;
    }
    ...
    if (coduompOutputGammaSoftwareActive == qfalse) {
        ...
        ri.Printf(R_PRINT_ALL,
                  "%s; using the exact OpenGL output gamma LUT.\n",
                  reason);
    }
    return qtrue;
}
...
void coduomp_output_gamma_set_lut_compat(
    const uint8_t red[CODUOMP_OUTPUT_GAMMA_ENTRY_COUNT],
    const uint8_t green[CODUOMP_OUTPUT_GAMMA_ENTRY_COUNT],
    const uint8_t blue[CODUOMP_OUTPUT_GAMMA_ENTRY_COUNT])
{
    uint8_t lut[CODUOMP_OUTPUT_GAMMA_ENTRY_COUNT]
               [CODUOMP_OUTPUT_GAMMA_COMPONENT_COUNT];
    ...
    coduompOutputGammaLutIdentity = qtrue;
    for (int32_t entry = 0;
         entry < CODUOMP_OUTPUT_GAMMA_ENTRY_COUNT; ++entry) {
        lut[entry][0] = red[entry];
        lut[entry][1] = green[entry];
        lut[entry][2] = blue[entry];
        lut[entry][3] = 255;
        if (red[entry] != entry || green[entry] != entry ||
            blue[entry] != entry) {
    ...
    qglTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8,
                  CODUOMP_OUTPUT_GAMMA_ENTRY_COUNT, 1, 0,
                  GL_RGBA, GL_UNSIGNED_BYTE, lut);
    qglPopAttrib();

    if (previousTextureUnit >= 0 &&
        previousTextureUnit < glConfig.maxActiveTextures) {
        qglActiveTextureARB(GL_TEXTURE0_ARB +
                            (uint32_t)previousTextureUnit);
    }
}
...
{
    if (coduompXRandrOriginalGamma != NULL &&
        coduompXRandrGammaApi.freeGamma != NULL) {
...
#define CODUOMP_LOAD_XRANDR_SYMBOL(field_, name_) \
    coduomp_library_symbol(coduompXRandrGammaApi.xrandrLibrary, name_, \
                           &coduompXRandrGammaApi.field_, \
                           sizeof(coduompXRandrGammaApi.field_))

    if (coduompXRandrGammaApi.xrandrLibrary != NULL)
        return qtrue;

    coduompXRandrGammaApi.xrandrLibrary =
        coduomp_library_open("libXrandr.so.2");
    coduompXRandrGammaApi.x11Library =
        coduomp_library_open("libX11.so.6");
    if (coduompXRandrGammaApi.xrandrLibrary == NULL ||
        coduompXRandrGammaApi.x11Library == NULL) {
        coduomp_output_gamma_unload_xrandr_compat();
        return qfalse;
    }

    CODUOMP_LOAD_XRANDR_SYMBOL(getScreenResourcesCurrent,
                               "XRRGetScreenResourcesCurrent");
    CODUOMP_LOAD_XRANDR_SYMBOL(freeScreenResources,
    ...
    coduomp_library_symbol(coduompXRandrGammaApi.x11Library, "XFlush",
                           &coduompXRandrGammaApi.flush,
                           sizeof(coduompXRandrGammaApi.flush));

#undef CODUOMP_LOAD_XRANDR_SYMBOL

    if (coduompXRandrGammaApi.getScreenResourcesCurrent == NULL ||
        coduompXRandrGammaApi.freeScreenResources == NULL ||
        coduompXRandrGammaApi.getCrtcInfo == NULL ||
        coduompXRandrGammaApi.freeCrtcInfo == NULL ||
        coduompXRandrGammaApi.getCrtcGammaSize == NULL ||
        coduompXRandrGammaApi.getCrtcGamma == NULL ||
        coduompXRandrGammaApi.allocGamma == NULL ||
        coduompXRandrGammaApi.setCrtcGamma == NULL ||
        coduompXRandrGammaApi.freeGamma == NULL ||
...
static uint16_t coduomp_output_gamma_resample_entry_compat(
    const uint16_t *source, int32_t sourceCount,
    int32_t destinationIndex, int32_t destinationCount)
{
    if (sourceCount <= 1 || destinationCount <= 1)
        return source[0];

    const uint64_t numerator =
        (uint64_t)(uint32_t)destinationIndex *
        (uint64_t)(uint32_t)(sourceCount - 1);
    ...
    const uint32_t upper =
        lower + 1u < (uint32_t)sourceCount ? lower + 1u : lower;
    const uint64_t value =
        (uint64_t)source[lower] * (denominator - remainder) +
        (uint64_t)source[upper] * remainder + denominator / 2u;
    return (uint16_t)(value / denominator);
}

static int64_t coduomp_output_gamma_intersection_area_compat(
    int32_t firstX, int32_t firstY, int32_t firstWidth, int32_t firstHeight,
    int32_t secondX, int32_t secondY,
    int32_t secondWidth, int32_t secondHeight)
{
    ...
    const int64_t right = firstRight < secondRight
        ? firstRight : secondRight;
    const int64_t bottom = firstBottom < secondBottom
...
qboolean coduomp_output_gamma_try_xrandr_compat(
    uint16_t originalRamp[3][CODUOMP_OUTPUT_GAMMA_ENTRY_COUNT])
{
    void *nativeDisplay;
    ...
    if ((sessionType != NULL && strcmp(sessionType, "wayland") == 0) ||
        (sessionType == NULL && getenv("WAYLAND_DISPLAY") != NULL)) {
        return qfalse;
    }
    if (coduomp_sdl_get_x11_window_compat(
            &nativeDisplay, &nativeWindow,
            &windowX, &windowY, &windowWidth, &windowHeight) == qfalse ||
        nativeDisplay == NULL || nativeWindow == 0 ||
        coduomp_output_gamma_load_xrandr_compat() == qfalse) {
        return qfalse;
    }

    resources = coduompXRandrGammaApi.getScreenResourcesCurrent(
        (Display *)nativeDisplay, (Window)nativeWindow);
    if (resources == NULL) {
        coduomp_output_gamma_unload_xrandr_compat();
        return qfalse;
    }
    ...
    for (int32_t index = 0; index < resources->ncrtc; ++index) {
        XRRCrtcInfo *const info = coduompXRandrGammaApi.getCrtcInfo(
            (Display *)nativeDisplay, resources, resources->crtcs[index]);
        int64_t area = 0;

        if (info != NULL && info->mode != None &&
            info->width != 0 && info->height != 0) {
            area = coduomp_output_gamma_intersection_area_compat(
                windowX, windowY, windowWidth, windowHeight,
                info->x, info->y, (int32_t)info->width,
                (int32_t)info->height);
        }
    ...
    const int32_t gammaSize =
        coduompXRandrGammaApi.getCrtcGammaSize(
            (Display *)nativeDisplay, selectedCrtc);
    XRRCrtcGamma *const original = gammaSize > 0
        ? coduompXRandrGammaApi.getCrtcGamma(
              (Display *)nativeDisplay, selectedCrtc)
        : NULL;
    ...
    for (int32_t entry = 0;
        ...
        originalRamp[0][entry] =
            coduomp_output_gamma_resample_entry_compat(
                original->red, gammaSize, entry,
                CODUOMP_OUTPUT_GAMMA_ENTRY_COUNT);
        originalRamp[1][entry] =
            coduomp_output_gamma_resample_entry_compat(
                original->green, gammaSize, entry,
                CODUOMP_OUTPUT_GAMMA_ENTRY_COUNT);
        originalRamp[2][entry] =
            coduomp_output_gamma_resample_entry_compat(
                original->blue, gammaSize, entry,
                CODUOMP_OUTPUT_GAMMA_ENTRY_COUNT);
    }
    ...
    coduompXRandrGammaApplied = qfalse;
    ri.Printf(R_PRINT_ALL,
              "Using XRandR CRTC 0x%lx native gamma (%d entries).\n",
...
qboolean coduomp_output_gamma_set_xrandr_compat(
    const uint16_t ramp[3][CODUOMP_OUTPUT_GAMMA_ENTRY_COUNT])
{
    if (coduompXRandrGammaActive == qfalse ||
        coduompXRandrDisplay == NULL || coduompXRandrGammaSize <= 0)
        return qfalse;

    XRRCrtcGamma *const gamma =
        coduompXRandrGammaApi.allocGamma(coduompXRandrGammaSize);
    if (gamma == NULL)
        return qfalse;

    for (int32_t entry = 0;
         entry < coduompXRandrGammaSize; ++entry) {
        gamma->red[entry] = coduomp_output_gamma_resample_entry_compat(
            ramp[0], CODUOMP_OUTPUT_GAMMA_ENTRY_COUNT,
            entry, coduompXRandrGammaSize);
        gamma->green[entry] = coduomp_output_gamma_resample_entry_compat(
            ramp[1], CODUOMP_OUTPUT_GAMMA_ENTRY_COUNT,
            entry, coduompXRandrGammaSize);
        gamma->blue[entry] = coduomp_output_gamma_resample_entry_compat(
            ramp[2], CODUOMP_OUTPUT_GAMMA_ENTRY_COUNT,
            entry, coduompXRandrGammaSize);
    }

    coduompXRandrGammaApi.setCrtcGamma(
        coduompXRandrDisplay, coduompXRandrCrtc, gamma);
...
void coduomp_output_gamma_restore_xrandr_compat(void)
{
    ...
    coduompXRandrGammaApi.setCrtcGamma(
        coduompXRandrDisplay, coduompXRandrCrtc,
        coduompXRandrOriginalGamma);
    coduompXRandrGammaApi.flush(coduompXRandrDisplay);
    coduompXRandrGammaApplied = qfalse;
}
...
void coduomp_output_gamma_present_compat(void)
{
    ...
    const qboolean presentationActive =
        coduomp_get_output_presentation_compat(
    ...
    const float textureS =
        (float)glConfig.vidWidth / (float)textureWidth;
    const float textureT =
        (float)glConfig.vidHeight / (float)textureHeight;
    ...
    if (textureWidth != coduompOutputPresentationTextureWidth ||
        textureHeight != coduompOutputPresentationTextureHeight) {
        qglTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8,
                      textureWidth, textureHeight, 0,
                      GL_RGBA, GL_UNSIGNED_BYTE, NULL);
        qglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        qglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        qglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                         GL_CLAMP_TO_EDGE);
...
qboolean coduomp_capture_presented_frame_compat(
    int32_t x, int32_t y, int32_t width, int32_t height,
    uint32_t pixelFormat, uint8_t *pixels)
{
    int32_t outputWidth;
    int32_t outputHeight;
    int32_t viewportX;
    ...
    if (coduompOutputPresentationTexture == 0 ||
        glConfig.vidWidth > coduompOutputPresentationTextureWidth ||
        glConfig.vidHeight > coduompOutputPresentationTextureHeight ||
        x < 0 || y < 0 || width <= 0 || height <= 0 ||
        width > glConfig.vidWidth - x ||
        height > glConfig.vidHeight - y) {
        return qfalse;
    }

    const int32_t bytesPerPixel = pixelFormat == GL_RGB ? 3 : 4;
    const size_t textureRowBytes =
        (size_t)coduompOutputPresentationTextureWidth *
        (size_t)bytesPerPixel;
    const size_t textureByteCount =
        textureRowBytes * (size_t)coduompOutputPresentationTextureHeight;
    ...
    qglGetTexImage(GL_TEXTURE_2D, 0, pixelFormat, GL_UNSIGNED_BYTE,
                   texturePixels);
    ...
    for (int32_t row = 0; row < height; ++row) {
        memcpy(pixels +
                   (size_t)row * (size_t)width * (size_t)bytesPerPixel,
               texturePixels + (size_t)(y + row) * textureRowBytes +
