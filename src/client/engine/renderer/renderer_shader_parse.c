...
enum {
    ...
    R_SHADER_TYPE_PATH_CHARACTER_OVERHEAD =
        (sizeof("shadertypes/") - 1u) + (sizeof(".stype") - 1u)
};
...
uint8_t
    rendererShaderRequirements[SHADER_REQUIREMENT_COUNT]; /* 0x03880468 */
shaderStage_t
    rendererParsedShaderStages[R_MAX_SHADER_STAGES]; /* 0x03880478 */
static texModInfo_t rendererParsedShaderTexMods
    [R_MAX_SHADER_STAGES]
    [R_MAX_TEXTURE_UNITS]
    [R_MAX_TEXTURE_MODIFIERS]; /* 0x0387bed0 */
static shader_t
    *rendererShaderHashTable[R_SHADER_HASH_SIZE]; /* 0x038838b8 */
...
_Static_assert(_Alignof(shader_text_hash_node_t) == 4,
               "i386 shader-text hash node alignment changed");
_Static_assert(offsetof(shader_text_hash_node_t, definitionText) == 0x00,
               "i386 shader-text definition pointer moved");
_Static_assert(offsetof(shader_text_hash_node_t, next) == 0x04,
               "i386 shader-text collision link moved");
_Static_assert(sizeof(shader_text_hash_node_t) == 0x08,
               "i386 shader-text hash node size changed");
#endif
...
static shader_text_hash_node_t
    rendererShaderTextHashNodes[R_MAX_SHADER_STRING_POINTERS];
static shader_text_hash_node_t
    rendererShaderTextHashTable[R_SHADER_HASH_SIZE]; /* 0x0397a6c0 */
...
#if UINTPTR_MAX == UINT32_MAX
_Static_assert(_Alignof(shader_multitexture_collapse_rule_t) == 4,
               "i386 multitexture-collapse rule alignment changed");
_Static_assert(offsetof(shader_multitexture_collapse_rule_t,
                        firstBlendBits) == 0x00,
               "i386 first multitexture blend bits moved");
_Static_assert(offsetof(shader_multitexture_collapse_rule_t,
                        secondBlendBits) == 0x04,
               "i386 second multitexture blend bits moved");
_Static_assert(offsetof(shader_multitexture_collapse_rule_t,
                        textureEnvMode) == 0x08,
               "i386 multitexture environment mode moved");
_Static_assert(offsetof(shader_multitexture_collapse_rule_t,
                        collapsedBlendBits) == 0x0c,
               "i386 collapsed multitexture blend bits moved");
_Static_assert(sizeof(shader_multitexture_collapse_rule_t) == 0x10,
               "i386 multitexture-collapse rule size changed");
#endif

/* Original CoDUOMP.exe 0x005912f0..0x0059137f. */
static const shader_multitexture_collapse_rule_t
    shaderMultitextureCollapseRules[] = {
        {0, GLS_SRCBLEND_ZERO | GLS_DSTBLEND_SRC_COLOR,
         GL_MODULATE, 0},
        ...
        {GLS_SRCBLEND_ZERO | GLS_DSTBLEND_SRC_COLOR,
         ...
         GL_MODULATE,
         GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ZERO},
        {0, GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE,
         GL_ADD, 0},
        {GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE,
         GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE,
         GL_ADD, GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE},
        {0,
...
uint32_t R_GenerateShaderHashValue(const char *name)
{
    ...
    for (uint32_t index = 0; name[index] != '\0'; ++index) {
        ...
        int32_t character =
            coduo_crt_tolower((int8_t)(uint8_t)name[index]);
        if (character == '.')
            break;
        if (character == '\\')
            character = '/';
        hash += (uint32_t)character *
                (R_SHADER_HASH_CHARACTER_WEIGHT + index);
    }
...
void BuildShaderChecksumLookup(void)
{
    ...
    memset(rendererShaderTextHashTable, 0,
           sizeof(rendererShaderTextHashTable));
    ...
    for (;;) {
        ...
        definitionText = tokenMark.ungetToken != qfalse
            ? tokenMark.savedParse
            : tokenMark.parse;
        ...
        if (Q_stricmpn(token, "{",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
            Com_UngetToken();
            if (Com_SkipBracedSection(
                    &parseCursor, R_MAX_SHADER_BRACE_DEPTH) != qfalse) {
                ri.Printf(
                    R_PRINT_WARNING,
                    "Shader brace nesting depth exceeded... missing '}'?\n");
        ...
        if (excessiveBraceDepth != qfalse) {
            ri.Printf(R_PRINT_WARNING,
                      "Look before shader '%s'\n", token);
            excessiveBraceDepth = qfalse;
        }

        bucket = &rendererShaderTextHashTable[
            R_GenerateShaderHashValue(token)];
        if (bucket->definitionText == NULL) {
            bucket->definitionText = definitionText;
        } else {
            shader_text_hash_node_t *node;

            if (collisionNodeCount >=
                R_MAX_SHADER_STRING_POINTERS) {
                ri.Error(
                    ERR_DROP,
                    "\x15" "MAX_SHADER_STRING_POINTERS exceeded, "
                    "too many shaders");
            }

            node = &rendererShaderTextHashNodes[
                collisionNodeCount++];
...
void ScanAndLoadShaderFiles(void)
{
    ...
    shaderFiles = ri.FS_ListFiles(
        "scripts", ".shader", &shaderFileCount);
    fxShaderFiles = ri.FS_ListFiles(
        "fxshaders", ".shader", &fxShaderFileCount);
    ...
    for (int32_t fileIndex = 0;
        ...
        if (fileIndex < shaderFileCount) {
            Com_sprintf(fileName, sizeof(fileName),
                        "scripts/%s", shaderFiles[fileIndex]);
        } else {
            Com_sprintf(
                fileName, sizeof(fileName), "fxshaders/%s",
                fxShaderFiles[fileIndex - shaderFileCount]);
        }

        if (r_debugOptTex->integer != 0)
            ri.Printf(R_PRINT_ALL, "...loading '%s'\n", fileName);

        shaderTextLength += ri.FS_ReadFile(
            fileName, (void **)&fileBuffers[fileIndex]);
        if (fileBuffers[fileIndex] == NULL) {
            ri.Error(
                ERR_DROP,
                va("EXE_ERR_COULDNT_LOAD\x15%s", fileName));
        }
    }

    rendererShaderText = ri.Hunk_Alloc(
        (size_t)(shaderTextLength + fileCount * 2));
    for (int32_t fileIndex = 0;
         fileIndex < fileCount;
         ++fileIndex) {
    ...
    for (int32_t fileIndex = fileCount - 1;
         fileIndex >= 0;
         --fileIndex) {
...
void CreateDefaultShader(void)
{
    ...
    memset(&rendererParsedShader, 0,
           sizeof(rendererParsedShader));
    memset(rendererParsedShaderStages, 0,
           sizeof(rendererParsedShaderStages));
    strncpy(rendererParsedShader.name, "<default>",
            sizeof(rendererParsedShader.name) - 1);
    rendererParsedShader.name[
        sizeof(rendererParsedShader.name) - 1] = '\0';
    ...
    bundle->texCoordComponentCount =
        R_INTERNAL_SHADER_TEXCOORD_COMPONENTS;
...
void CreateShadowShader(void)
{
    ...
    memset(rendererParsedShaderStages, 0,
           sizeof(rendererParsedShaderStages));
    strncpy(rendererParsedShader.name, "<stencil shadow>",
            sizeof(rendererParsedShader.name) - 1);
    rendererParsedShader.name[
        sizeof(rendererParsedShader.name) - 1] = '\0';
...
void CreateShowTrisShader(void)
{
    ...
    memset(rendererParsedShaderStages, 0,
           sizeof(rendererParsedShaderStages));
    strncpy(rendererParsedShader.name, "<showtris>",
            sizeof(rendererParsedShader.name) - 1);
    rendererParsedShader.name[
        sizeof(rendererParsedShader.name) - 1] = '\0';
...
void CreateShowImagesShader(void)
{
    ...
    memset(rendererParsedShaderStages, 0,
           sizeof(rendererParsedShaderStages));
    strncpy(rendererParsedShader.name, "<showimages>",
            sizeof(rendererParsedShader.name) - 1);
    rendererParsedShader.name[
        sizeof(rendererParsedShader.name) - 1] = '\0';
...
void CreateScreenShader(void)
{
    ...
    memset(rendererParsedShaderStages, 0,
           sizeof(rendererParsedShaderStages));
    strncpy(rendererParsedShader.name, "<screen>",
            sizeof(rendererParsedShader.name) - 1);
    rendererParsedShader.name[
        sizeof(rendererParsedShader.name) - 1] = '\0';
    ...
    rendererParsedShader.surfaceFlags =
        SHADER_SURFACE_BASE_TEXCOORDS |
        SHADER_SURFACE_VERTEX_COLORS;
    rendererParsedShader.cullType = CT_TWO_SIDED;
...
void CreateExternalShaders(void)
{
    tr.flareShader = R_FindShader(
        "flareShader", LIGHTMAP_NONE, qtrue,
        R_IMAGE_TRACK_EFFECT);
    tr.spotLightShader = R_FindShader(
        "spotLight", LIGHTMAP_NONE, qtrue,
        R_IMAGE_TRACK_EFFECT);
    tr.dlightShader = R_FindShader(
        "dlightshader", LIGHTMAP_NONE, qtrue,
        R_IMAGE_TRACK_EFFECT);
}
...
void R_InitShaders(void)
{
    ...
    memset(rendererShaderHashTable, 0,
           sizeof(rendererShaderHashTable));
...
char *FindShaderInShaderText(const char *shaderName)
{
    ...
    node = &rendererShaderTextHashTable[
        R_GenerateShaderHashValue(shaderName)];
    while (node != NULL && node->definitionText != NULL) {
        char *parseCursor = node->definitionText;
        const char *const token = Com_Parse(&parseCursor);

        if (token[0] != '\0' && shaderName != NULL &&
            Q_stricmpn(token, shaderName,
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
            return parseCursor;
        }
        node = node->next;
...
shader_t *R_FindShaderByName(const char *name)
{
    ...
    shader = rendererShaderHashTable[
        R_GenerateShaderHashValue(strippedName)];
    while (shader != NULL) {
        if (Q_stricmpn(shader->name, strippedName,
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
            return shader;
        }
        shader = shader->next;
                           ...
                           renderer_image_track_t imageTrack)
{
    ...
    /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
    if (shaderTypeLength >=
        sizeof(fileName) - R_SHADER_TYPE_PATH_CHARACTER_OVERHEAD) {
        ri.Printf(R_PRINT_WARNING,
                  "WARNING: shader type '%s' is too long\n",
                  shaderTypeName);
        return NULL;
    }
    ...
    if (ri.FS_ReadFile(fileName, &fileBuffer) < 0) {
        ri.Printf(
            R_PRINT_WARNING,
            "WARNING: could not read shader type file '%s'\n",
            fileName);
        return NULL;
    }
    ...
    if (ParseShader(&parseCursor, qtrue, imageTrack) == qfalse) {
        ...
        ri.Error(
            ERR_DROP,
            "\x15" "ERROR: invalid shader type in file '%s'\n",
            fileName);
        return NULL;
    }
                               ...
                               renderer_image_track_t imageTrack)
{
    ...
    if (typeMarker == NULL || lastSeparator > typeMarker) {
        ri.Printf(
            R_PRINT_WARNING,
            "WARNING: found '/' or '\\' after '@' in shader '%s'\n",
            name);
        return NULL;
    }
    ...
    if (lightmapIndex == LIGHTMAP_NONE) {
        ...
        ri.Error(
            ERR_DROP,
            "\x15" "Bad lightmap index %i loading shader %s\n",
            lightmapIndex, name);
    }
    ...
    if (prefixLength + baseNameLength >= sizeof(shaderTypeName)) {
        ri.Printf(
            R_PRINT_WARNING,
            "WARNING: shader type in '%s' is too long\n",
            name);
        return NULL;
    }
    ...
    for (int32_t stageIndex = 0;
         stageIndex < shaderType->numUnfoggedPasses;
         ++stageIndex) {
        shaderStage_t *const stage =
            &rendererParsedShaderStages[stageIndex];
        *stage = *shaderType->stages[stageIndex];

        for (int32_t bundleIndex = 0;
             bundleIndex < R_MAX_TEXTURE_UNITS;
             ++bundleIndex) {
            textureBundle_t *const bundle =
            ...
            for (;;) {
                ...
                if (image != NULL &&
                    image->imageTrack ==
                        R_IMAGE_TRACK_GENERATED_TEXTURE) {
                    image = R_FindImageInstance(
                        name, imageTrack, image->imgName, NULL,
                ...
                } else if (shaderType->lightmapIndex >= 0 &&
                           image ==
                               tr.lightmaps[
                                   shaderType->lightmapIndex]) {
                    if (lightmapIndex < 0) {
                        const char *usage;

                        if (lightmapIndex == LIGHTMAP_NONE) {
                            usage = "used as a model skin";
                        } else if (lightmapIndex ==
                                   LIGHTMAP_WHITEIMAGE) {
                            usage =
                                "not used as a world shader";
                        } else if (lightmapIndex ==
                                   LIGHTMAP_BY_VERTEX) {
                            usage =
                                "used as a vertex-lit surface";
                        } else if (lightmapIndex == LIGHTMAP_2D) {
...
void R_BuildShaderFromImage(int32_t lightmapIndex, image_t *image)
{
    ...
    shaderStage_t *const firstStage =
        &rendererParsedShaderStages[0];
    shaderStage_t *const secondStage =
        &rendererParsedShaderStages[1];
    textureBundle_t *const firstBundle =
        &firstStage->bundle[0];
    textureBundle_t *const secondBundle =
        &secondStage->bundle[0];

    if (lightmapIndex == LIGHTMAP_NONE) {
        rendererParsedShader.surfaceFlags =
            R_FALLBACK_SURFACE_MODEL;
        firstBundle->image[0] = image;
        firstBundle->texCoordComponentCount =
            R_FALLBACK_TEXCOORD_COMPONENTS;
        firstStage->flags = SHADER_STAGE_ACTIVE;
        firstStage->rgbGen = CGEN_LIGHTING_DIFFUSE;
        firstStage->alphaGen = AGEN_IDENTITY;
        firstStage->stateBits =
            GLS_LIGHTING | GLS_DEPTHMASK_TRUE;
    } else if (lightmapIndex == LIGHTMAP_BY_VERTEX) {
        rendererParsedShader.surfaceFlags =
            R_FALLBACK_SURFACE_VERTEX_OR_2D;
        firstBundle->image[0] = image;
        firstBundle->texCoordComponentCount =
            R_FALLBACK_TEXCOORD_COMPONENTS;
        firstStage->flags = SHADER_STAGE_ACTIVE;
        ...
        firstBundle->texCoordComponentCount =
            R_FALLBACK_TEXCOORD_COMPONENTS;
        secondBundle->texCoordComponentCount =
            R_FALLBACK_TEXCOORD_COMPONENTS;
        secondStage->rgbGen = CGEN_IDENTITY;
        secondStage->stateBits |=
            GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ZERO;
        firstStage->stateBits = GLS_DEPTHMASK_TRUE;
    }
                       ...
                       qboolean mipRawImage, int32_t shaderUsage)
{
    ...
    while (shader != NULL) {
        if ((shader->lightmapIndex == lightmapIndex ||
             (shader->lightmapIndex == LIGHTMAP_NONE &&
              lightmapIndex >= 0 &&
              shaderUsage == R_IMAGE_TRACK_FX)) &&
            Q_stricmpn(shader->name, strippedName,
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
    ...
    memset(rendererParsedShaderStages, 0,
           sizeof(rendererParsedShaderStages));
    Q_strncpyz(rendererParsedShader.name, strippedName,
               sizeof(rendererParsedShader.name));
    rendererParsedShader.lightmapIndex = lightmapIndex;
    if (lightmapIndex == LIGHTMAP_NONE)
    ...
    for (int32_t stageIndex = 0;
        ...
        for (int32_t bundleIndex = 0;
             ...
             ++bundleIndex) {
            rendererParsedShaderStages[stageIndex]
                .bundle[bundleIndex].texMods =
                rendererParsedShaderTexMods[stageIndex]
                                            [bundleIndex];
        }
    }

    if (strrchr(strippedName, '@') != NULL) {
        shader = ShaderFromShaderType(
            strippedName, lightmapIndex,
            (renderer_image_track_t)shaderUsage);
    ...
    if (definitionText != NULL) {
        ...
        if (ParseShader(&definitionText, qfalse,
                        (renderer_image_track_t)shaderUsage) ==
            qfalse) {
            rendererParsedShader.flags =
                SHADER_FLAG_DEFAULTED |
                SHADER_FLAG_EXPLICITLY_DEFINED;
            rendererParsedShader.surfaceFlags = SURF_NOIMPACT;
        }
        return FinishShader();
    }
    ...
    uint32_t imageFlags =
        mipRawImage != qfalse
            ? IMAGE_FLAG_MIPMAP | IMAGE_FLAG_ALLOW_PICMIP
            : IMAGE_FLAG_CLAMP_S | IMAGE_FLAG_CLAMP_T;
    ...
    image_t *const image =
        R_FindImageFile(imageName, GL_TEXTURE_2D, imageFlags,
                        (renderer_image_track_t)shaderUsage,
                        NULL, 1.0f);
    if (image != NULL) {
...
int32_t R_RegisterShaderFromImage(const char *name,
                                  ...
                                  image_t *image)
{
    shader_t *shader = rendererShaderHashTable[
        R_GenerateShaderHashValue(name)];

    while (shader != NULL) {
        if ((shader->lightmapIndex == lightmapIndex ||
             (shader->flags & SHADER_FLAG_DEFAULTED) != 0) &&
            Q_stricmpn(shader->name, name,
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
            UpdateDelayLoadImagesForShader(shader, qfalse);
            return shader->index;
        }
    ...
    memset(rendererParsedShaderStages, 0,
           sizeof(rendererParsedShaderStages));
    Q_strncpyz(rendererParsedShader.name, name,
               sizeof(rendererParsedShader.name));
    rendererParsedShader.lightmapIndex = lightmapIndex;
                                  ...
                                  int32_t shaderUsage)
{
    ...
    shader = R_FindShader(
        name, lightmapIndex, qtrue, shaderUsage);
...
int32_t RE_RegisterShader(const char *name, int32_t shaderUsage)
{
    ...
    shader = R_FindShader(
        name, LIGHTMAP_2D, qtrue, shaderUsage);
    if ((shader->flags & SHADER_FLAG_DEFAULTED) != 0)
        return 0;
    return shader->index;
}
                               ...
                               int32_t shaderUsage)
{
    ...
    shader = R_FindShader(
        name, LIGHTMAP_2D, qfalse, shaderUsage);
    if ((shader->flags & SHADER_FLAG_DEFAULTED) != 0)
        return 0;
    return shader->index;
}
                       ...
                       int32_t mergeIndex)
{
    ...
    if (shaderCount == 1) {
        ...
        Com_sprintf(rendererParsedShader.name,
                    sizeof(rendererParsedShader.name),
                    "*sheet%03i", mergeIndex);
        targetShader = GeneratePermanentShader();
        if (targetShader == tr.defaultShader) {
            ri.Printf(
                R_PRINT_WARNING,
                "WARNING: too many shaders after building "
                "texture sheets\n");
            return;
        }
    }
    ...
    for (int32_t stageIndex = 0;
         stageIndex < targetShader->numUnfoggedPasses;
         ++stageIndex) {
        shaderStage_t *const stage =
            targetShader->stages[stageIndex];
...
int32_t compare_mergable_shaders(const void *left,
                                 const void *right)
{
    shader_t *const leftShader =
        *(shader_t *const *)left;
    shader_t *const rightShader =
        *(shader_t *const *)right;
    ...
    const ptrdiff_t sheetOrder =
        ((const uint8_t *)leftImage->link.textureSheet -
         (const uint8_t *)rightImage->link.textureSheet) /
        (ptrdiff_t)sizeof(image_t);

    if (sheetOrder != 0)
        return (int32_t)sheetOrder;
    return CompareMergableShaders(
        leftShader, rightShader, leftImage, rightImage);
}
...
void R_MergeShadersForImageSheets(void)
{
    ...
    coduo_crt_qsort(mergeCandidates, (size_t)candidateCount,
                        sizeof(mergeCandidates[0]),
                        compare_mergable_shaders);
    ...
    for (int32_t first = 0; first < candidateCount;) {
        ...
        while (end < candidateCount) {
            ...
            image_t *const candidateImage =
                candidate->primaryImage;
            const ptrdiff_t sheetOrder =
                ((const uint8_t *)firstImage->link.textureSheet -
                 (const uint8_t *)candidateImage->link.textureSheet) /
                (ptrdiff_t)sizeof(image_t);

            if (sheetOrder != 0 ||
                CompareMergableShaders(
                    firstShader, candidate,
                    firstImage, candidateImage) != 0) {
                break;
            }
            ++end;
        }

        R_MergeShaderList(&mergeCandidates[first],
                          end - first, mergeIndex);
        ++mergeIndex;
        first = end;
    }
}
...
{
    image_t *nonTextureImages[
        R_MAX_SHADER_STAGES * R_MAX_TEXTURE_UNITS];
    ...
    for (int32_t imageIndex = 0;
         imageIndex < nonTextureImageCount;
         ++imageIndex) {
    ...
    const shader_texture_combine_args_t *second)
{
    ...
    for (int32_t argumentIndex = 0; argumentIndex < 3; ++argumentIndex) {
        if (second->sources[argumentIndex] !=
            first->sources[argumentIndex]) {
            return (int32_t)(second->sources[argumentIndex] -
                             first->sources[argumentIndex]);
        }
    }
    for (int32_t argumentIndex = 0; argumentIndex < 3; ++argumentIndex) {
        if (second->operands[argumentIndex] !=
            first->operands[argumentIndex]) {
            return (int32_t)(second->operands[argumentIndex] -
                             first->operands[argumentIndex]);
        }
    }
                             ...
                             const shader_texture_combine_t *second)
{
    ...
    int32_t comparison =
        CompareTexEnvCombineArgs(&first->rgb, &second->rgb);
    if (comparison != 0)
        return comparison;
    comparison =
        CompareTexEnvCombineArgs(&first->alpha, &second->alpha);
    if (comparison != 0)
        return comparison;

    for (int32_t component = 0; component < 4; ++component) {
        if (second->environmentColor[component] ==
            first->environmentColor[component]) {
            continue;
        }
        return second->environmentColor[component] <
                       first->environmentColor[component]
                   ? -1
                   : 1;
    }
    ...
    const image_t *secondEquivalentImage)
{
    if (first->image[0] != second->image[0]) {
        if (first->image[0] != firstEquivalentImage ||
            second->image[0] != secondEquivalentImage) {
            return 1;
        }
    }
    ...
    if (first->numImageAnimations > 1 ||
        second->numImageAnimations > 1) {
        if (first->numImageAnimations !=
            second->numImageAnimations) {
            return first->numImageAnimations -
                   second->numImageAnimations;
        }
        if (first->imageAnimationSpeed !=
            second->imageAnimationSpeed) {
            return first->imageAnimationSpeed <
                           second->imageAnimationSpeed
                       ? -1
                       : 1;
        }
        for (int32_t imageIndex = 1;
             imageIndex < first->numImageAnimations;
             ++imageIndex) {
            if (first->image[imageIndex] !=
                second->image[imageIndex]) {
                return 1;
            }
        }
    }
    ...
    if (first->waterMap != second->waterMap)
        return 1;
    if (first->texCoordComponentCount !=
        second->texCoordComponentCount) {
        return first->texCoordComponentCount -
               second->texCoordComponentCount;
    }
    if (first->textureEnvMode != second->textureEnvMode) {
        return (int32_t)(first->textureEnvMode -
                         second->textureEnvMode);
    }
    if (first->tcGen != second->tcGen)
        return (int32_t)first->tcGen -
               (int32_t)second->tcGen;

    if (first->tcGen != TCGEN_TEXTURE) {
        if (first->tcGen == TCGEN_VECTOR &&
            memcmp(first->tcGenVectors,
                   second->tcGenVectors,
                   sizeof(first->tcGenVectors)) != 0) {
            return 1;
        }
        ...
        for (int32_t texModIndex = 0;
             texModIndex < first->numTexMods;
             ++texModIndex) {
            if (memcmp(&first->texMods[texModIndex],
                       &second->texMods[texModIndex],
                       sizeof(first->texMods[texModIndex])) != 0) {
                return 1;
            }
        }

        const int32_t textureShaderComparison =
            CompareNvTexShaders(second->textureShader,
                                first->textureShader);
    ...
    if (first->isLightmap != second->isLightmap)
        return (int32_t)first->isLightmap -
               (int32_t)second->isLightmap;
    if (first->isVideoMap != second->isVideoMap)
        return (int32_t)first->isVideoMap -
               (int32_t)second->isVideoMap;
    if (first->clampAnimation != second->clampAnimation)
        return (int32_t)first->clampAnimation -
               (int32_t)second->clampAnimation;

    return CompareTexEnvCombine(second->textureCombine,
                                first->textureCombine);
}
                               ...
                               const image_t *rightImage)
{
    ...
    for (int32_t stageIndex = 0;
         stageIndex < left->numUnfoggedPasses;
         ++stageIndex) {
        ...
        for (int32_t bundleIndex = 0;
             bundleIndex < R_MAX_TEXTURE_UNITS;
             ++bundleIndex) {
            const int32_t bundleComparison = CompareTextureBundles(
                &leftStage->bundle[bundleIndex],
                &rightStage->bundle[bundleIndex],
                leftImage, rightImage);
            if (bundleComparison != 0)
                return bundleComparison;
        }

        if (leftStage->fragmentShaderATI !=
            rightStage->fragmentShaderATI) {
            return (int32_t)(leftStage->fragmentShaderATI -
                             rightStage->fragmentShaderATI);
        }

        const int32_t registerCombinerComparison =
            CompareNvRegisterCombiners(leftStage->registerCombiners,
                                       rightStage->registerCombiners);
        if (registerCombinerComparison != 0)
            return registerCombinerComparison;

        if (leftStage->rgbGen != rightStage->rgbGen)
            return (int32_t)leftStage->rgbGen -
                   (int32_t)rightStage->rgbGen;

        if (leftStage->rgbGen == CGEN_WAVEFORM) {
            const int32_t waveformComparison =
                CompareWaveForms(&rightStage->rgbWave,
                                 &leftStage->rgbWave);
        ...
        if (leftStage->rgbGen == CGEN_WAVEFORM ||
            leftStage->rgbGen == CGEN_CONSTANT) {
            for (int32_t component = 0; component < 4; ++component) {
                if (leftStage->constantColor[component] !=
                    rightStage->constantColor[component]) {
                    return (int32_t)leftStage->constantColor[component] -
                           (int32_t)rightStage->constantColor[component];
                }
            }
        }

        if (leftStage->alphaGen != rightStage->alphaGen)
            return (int32_t)leftStage->alphaGen -
                   (int32_t)rightStage->alphaGen;

        if (leftStage->alphaGen == AGEN_WAVEFORM) {
            const int32_t waveformComparison =
                CompareWaveForms(&rightStage->alphaWave,
                                 &leftStage->alphaWave);
            if (waveformComparison != 0)
                return waveformComparison;
        }
        ...
        if ((leftStage->alphaGen == AGEN_WAVEFORM ||
             leftStage->alphaGen == AGEN_CONSTANT) &&
            leftStage->constantColor[3] !=
                rightStage->constantColor[3]) {
            return (int32_t)leftStage->constantColor[3] -
                   (int32_t)rightStage->constantColor[3];
        }

        if (leftStage->stateBits != rightStage->stateBits)
            return (int32_t)(leftStage->stateBits -
                             rightStage->stateBits);
    }
                          ...
                          const char *functionName)
{
    ...
    ri.Printf(
        ...
        functionName, expected, token, rendererParsedShader.name);
    return qfalse;
}
...
{
    const float negativePeak =
        fabsf(waveform->base - waveform->amplitude);
    const float positivePeak =
        fabsf(waveform->base + waveform->amplitude);
...
void ParseDeform(char **text)
{
    ...
    deform =
        &rendererParsedShader.deforms[rendererParsedShader.numDeforms];

    if (Q_stricmpn(token, "projectionShadow",
                   R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
        deform->deformation = DEFORM_PROJECTION_SHADOW;
    } else if (Q_stricmpn(token, "autosprite",
                          R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
        deform->deformation = DEFORM_AUTOSPRITE;
    } else if (Q_stricmpn(token, "autosprite2",
                          R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
        deform->deformation = DEFORM_AUTOSPRITE2;
    } else if (Q_stricmpn(token, "text", 4) == 0) {
        int32_t textIndex =
            (int32_t)(int8_t)(uint8_t)token[4] - '0';
        if (textIndex < 0 || textIndex > 7)
            textIndex = 0;
        deform->deformation =
            (shader_deform_type_t)(DEFORM_TEXT0 + textIndex);
    } else if (Q_stricmp(token, "bulge") == 0) {
        rendererParsedShader.surfaceFlags |=
            SHADER_SURFACE_TIME_DEPENDENT |
            SHADER_SURFACE_REQUIRES_VERTEX_BASIS |
            SHADER_SURFACE_BASE_TEXCOORDS;
        ...
        rendererParsedShader.boundsExpansion +=
            MaxWaveFormDeformation(&deform->deformationWave);
        ...
        if (token[0] == '\0') {
            ri.Printf(R_PRINT_WARNING,
                      "WARNING: missing flap axis in shader '%s'\n",
                      rendererParsedShader.name);
            return;
        }

        if (Q_stricmp(token, "s") == 0 ||
            Q_stricmp(token, "x") == 0) {
            deform->deformation = DEFORM_FLAP_S;
        } else if (Q_stricmp(token, "t") == 0 ||
                   Q_stricmp(token, "y") == 0) {
        ...
        if (rendererParsedShader.numDeforms == 0 ||
            rendererParsedShader
                    .deforms[rendererParsedShader.numDeforms - 1]
                    .deformation != DEFORM_WAVE) {
            ri.Printf(
                R_PRINT_WARNING,
        ...
        *deform =
        ...
        if (token[0] == '\0') {
            ri.Printf(R_PRINT_WARNING,
                      "WARNING: missing scale to syncNormal in shader '%s'\n",
                      rendererParsedShader.name);
            return;
        }
        deform->deformationWave.amplitude *= (float)atof(token);
        ...
        ParseWaveForm(text, &deform->deformationWave);
        const float maximumDeformation =
            MaxWaveFormDeformation(&deform->deformationWave);
        const long double moveLengthRaw = sqrtl(
            ((long double)deform->moveVector[0] *
                 (long double)deform->moveVector[0] +
             (long double)deform->moveVector[1] *
                 (long double)deform->moveVector[1]) +
            (long double)deform->moveVector[2] *
                (long double)deform->moveVector[2]);
        ...
        (void)moveLength;
        rendererParsedShader.boundsExpansion = (float)(
            (long double)maximumDeformation * moveLengthRaw +
            (long double)rendererParsedShader.boundsExpansion);
...
static const char *const rendererSkyImageSuffixes[R_SKYBOX_FACE_COUNT] = {
    "rt", "bk", "lf", "ft", "up", "dn"
};
...
void ParseSkyParms(char **text, renderer_image_track_t imageTrack)
{
    ...
    if (strcmp(token, "-") != 0) {
        for (int32_t side = 0; side < R_SKYBOX_FACE_COUNT; ++side) {
            Com_sprintf(
                imageName, sizeof(imageName), "%s_%s.tga",
                token, rendererSkyImageSuffixes[side]);
            rendererParsedShader.skyOuterBox[side] =
                R_FindImageFile(
    ...
    if (strcmp(token, "-") != 0) {
        for (int32_t side = 0; side < R_SKYBOX_FACE_COUNT; ++side) {
            Com_sprintf(
                imageName, sizeof(imageName), "%s_%s.tga",
                token, rendererSkyImageSuffixes[side]);
            rendererParsedShader.skyInnerBox[side] =
                R_FindImageFile(
                    imageName, GL_TEXTURE_2D, imageFlags, imageTrack,
                    NULL, 1.0f);
...
void ParseSurfaceParm(char **text)
{
    ...
    for (const surfaceParm_t *parm = surfaceParms;
         parm->name != NULL; ++parm) {
        if (Q_stricmpn(token, parm->name,
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
...
void ParseTexMod(shaderStage_t *stage, int32_t bundleIndex, char **text)
{
    ...
    if (bundle->numTexMods == R_MAX_TEXTURE_MODIFIERS) {
        ri.Error(ERR_DROP,
                 "\x15" "ERROR: too many tcMod stages in shader '%s'\n",
                 rendererParsedShader.name);
    }
    ...
    if (bundle->image[0] == NULL) {
        ri.Error(ERR_DROP,
                 "\x15" "ERROR: tcMod stage before texture in shader '%s'\n",
                 rendererParsedShader.name);
    }

    if (bundle->image[0]->target == GL_TEXTURE_CUBE_MAP_ARB) {
        if (Q_stricmpn(token, "negate",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0 ||
            Q_stricmpn(token, "reverse",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
    ...
    ri.Printf(R_PRINT_WARNING,
              "WARNING: unknown or invalid tcMod '%s' in shader '%s'\n",
              token, rendererParsedShader.name);
    ++bundle->numTexMods;
    return;
    ...
    qboolean alphaChannel, char **text)
{
    ...
    if (MatchShaderTokenOnLine(
            text, "=", "texEnvCombine funxtion") == qfalse) {
        return qfalse;
    }
    ...
    if (Q_stricmpn(token, "REPLACE",
                   R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
        arguments->operation = GL_REPLACE;
        argumentCount = 1;
    } else if (Q_stricmpn(token, "MODULATE",
                          R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
        arguments->operation = GL_MODULATE;
        argumentCount = 2;
    } else if (Q_stricmpn(token, "ADD",
                          R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
        ...
        if (rendererShaderRequirements[
                SHADER_REQUIREMENT_TEXTURE_ENV_DOT3_ARB] == 0) {
            ri.Printf(
                R_PRINT_WARNING,
        ...
        if (alphaChannel != qfalse) {
            ri.Printf(R_PRINT_WARNING,
                      "WARNING: DOT3_RGB_ARB only valid for rgb channel\n");
            return qfalse;
        }
        ...
        if (alphaChannel != qfalse) {
            ri.Printf(R_PRINT_WARNING,
                      "WARNING: DOT3_RGBA_ARB only valid for rgb channel\n");
            return qfalse;
        }
    ...
    if (MatchShaderTokenOnLine(
            text, "(", "texEnvCombine function") == qfalse) {
        return qfalse;
    }

    for (int32_t argumentIndex = 0;
        ...
        if (token[0] == 'C') {
            ...
            arguments->operands[argumentIndex] =
                oneMinusSource != qfalse
    ...
    textureBundle_t *bundle, char **text)
{
    ...
    if (Q_stricmpn(token, "const",
                   R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
        if (MatchShaderTokenOnLine(
                text, "=", "texEnvCombine const") == qfalse) {
            return qfalse;
        }
        if (MatchShaderTokenOnLine(
                text, "(", "texEnvCombine const") == qfalse) {
    ...
    if (token[0] == '\0')
        goto missing_rgb_equation;
    if (Q_stricmpn(token, "rgb",
                   R_SHADER_TOKEN_COMPARE_LIMIT) != 0) {
        goto missing_rgb_equation;
    }

    if (ParseTexEnvCombineFunction(
            &combine->rgb, qfalse, text) == qfalse) {
        return qfalse;
    }
    ...
    if (token[0] != '\0' && Q_stricmp(token, "alpha") == 0) {
        ...
        if (ParseTexEnvCombineFunction(
                &combine->alpha, qtrue, text) == qfalse) {
            return qfalse;
        }
        token = Com_Parse(text);
    } else {
    ...
    if (token[0] != '}') {
        ri.Printf(
            R_PRINT_WARNING,
            "WARNING: missing '}' in texEnvCombine in shader '%s'\n",
            rendererParsedShader.name);
        return qfalse;
    }
    return qtrue;
    ...
    image_t **outImage)
{
    ...
    *outImage = NULL;
    ...
    if ((rendererParsedShader.flags &
         SHADER_FLAG_USE_PICMIP2) != 0) {
        imageFlags |= IMAGE_FLAG_USE_PICMIP2;
    }
    if ((rendererParsedShader.flags &
         SHADER_FLAG_NO_PICMIP) == 0) {
        imageFlags |= IMAGE_FLAG_ALLOW_PICMIP;
    }
    if ((rendererParsedShader.flags &
         SHADER_FLAG_NO_MIPMAPS) == 0) {
        imageFlags |= IMAGE_FLAG_MIPMAP;
    }
    if ((rendererParsedShader.flags &
         SHADER_FLAG_NO_IMAGE_OVERBRIGHT) != 0) {
    ...
    for (;;) {
        ...
        if (imageName[0] == '\0') {
            ri.Printf(
                R_PRINT_WARNING,
                "WARNING: missing image name in shader %s\n",
                rendererParsedShader.name);
            return qfalse;
        }

        if (Q_stricmpn(imageName, "nopicmip",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
            imageFlags &= ~IMAGE_FLAG_ALLOW_PICMIP;
            continue;
        }
        if (Q_stricmpn(imageName, "picmip",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
            imageFlags |= IMAGE_FLAG_ALLOW_PICMIP;
            continue;
        }
        if (Q_stricmpn(imageName, "nomipmaps",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
            imageFlags &= ~IMAGE_FLAG_MIPMAP;
            continue;
        }
        if (Q_stricmpn(imageName, "mipmaps",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
            imageFlags |= IMAGE_FLAG_MIPMAP;
            continue;
        }
        if (Q_stricmpn(imageName, "clamp",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
            imageFlags |= IMAGE_FLAG_CLAMP_S | IMAGE_FLAG_CLAMP_T;
            continue;
        }
        if (Q_stricmpn(imageName, "clampx",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0 ||
            Q_stricmpn(imageName, "clamps",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
            imageFlags |= IMAGE_FLAG_CLAMP_S;
            continue;
        }
        if (Q_stricmpn(imageName, "clampy",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0 ||
            Q_stricmpn(imageName, "clampt",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
    ...
    if (Q_stricmpn(imageName, "$lightmap",
                   R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
        if (colorScale != NULL) {
            ri.Printf(
                R_PRINT_WARNING,
        ...
        rendererParsedShader.surfaceFlags |=
            R_PARSE_IMAGE_LIGHTMAP_SOURCE_FLAG;
        if (rendererParsedShader.lightmapIndex < 0) {
            *outImage = tr.identityLightImage;
        } else {
            *outImage =
                tr.lightmaps[rendererParsedShader.lightmapIndex];
        }
        return qtrue;
    }

    if (Q_stricmpn(imageName, "$whiteimage",
                   R_SHADER_TOKEN_COMPARE_LIMIT) == 0 ||
        Q_stricmpn(imageName, "*white",
                   R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
        if (colorScale != NULL) {
            ri.Printf(
                R_PRINT_WARNING,
        ...
        rendererParsedShader.surfaceFlags |=
            R_PARSE_IMAGE_COLOR_SOURCE_FLAG;
        *outImage = tr.whiteImage;
        return qtrue;
    }

    if (Q_stricmpn(imageName, "$dlight",
                   R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
        if (colorScale != NULL) {
            ri.Printf(
                R_PRINT_WARNING,
    ...
    if ((imageFlags & IMAGE_FLAG_HEIGHT_TO_NORMAL) == 0 &&
        r_graymap->integer != 0 &&
        rendererParsedShader.lightmapIndex !=
            R_PARSE_IMAGE_GRAYMAP_EXCLUDED_MODE_A &&
        rendererParsedShader.lightmapIndex !=
            R_PARSE_IMAGE_GRAYMAP_EXCLUDED_MODE_B) {
        rendererParsedShader.surfaceFlags |=
            R_PARSE_IMAGE_COLOR_SOURCE_FLAG;
        ...
        *outImage = R_FindImageFile(
            imageName, GL_TEXTURE_2D, imageFlags,
            R_IMAGE_TRACK_GENERATED_TEXTURE,
    ...
    if (only16Bit != qfalse) {
        ...
        if (glConfig.colorBits >
            R_PARSE_IMAGE_16_BIT_COLOR_DEPTH) {
            return qtrue;
        }
    } else if (only32Bit != qfalse &&
               glConfig.colorBits <=
                   R_PARSE_IMAGE_16_BIT_COLOR_DEPTH) {
        return qtrue;
    }
    ...
    if (loadImage != qfalse && *outImage == NULL) {
        if (tr.delayedImageGroup != 0 &&
            rendererParsedShader.lightmapIndex ==
                R_PARSE_IMAGE_NO_LIGHTMAP_MODE) {
            imageFlags &= ~IMAGE_FLAG_DELAYED_UPLOAD;
        }
        ...
        *outImage = R_FindImageFile(
        ...
        if (*outImage == NULL) {
            ...
            ri.Printf(
                R_PRINT_WARNING,
                "WARNING: Couldn't load image '%s' in shader %s\n",
    ...
    const char *parameterName, char **text, float *value)
{
    ...
    if (token[0] == '\0') {
        ri.Printf(
            R_PRINT_WARNING,
            "WARNING: waterMap missing %s in shader %s\n",
    ...
    *value = (float)atof(token);
    if (*value <= 0.0f) {
        ri.Printf(
            R_PRINT_WARNING,
            "WARNING: %s must be > 0 in waterMap in shader %s\n",
            parameterName, rendererParsedShader.name);
        return 0.0f;
    }
    ...
    shaderStage_t *stage, char **text, int32_t bundleIndex)
{
    ...
    if (ParseWaterMapInt(
            "texture width", text,
            R_WATER_MAP_MIN_TEXTURE_SIZE,
            R_WATER_MAP_MAX_TEXTURE_SIZE,
            &configuration.textureWidth) == qfalse ||
        /* Both binaries reuse "texture width" here for the height error. */
        ParseWaterMapInt(
            "texture width", text,
            R_WATER_MAP_MIN_TEXTURE_SIZE,
            R_WATER_MAP_MAX_TEXTURE_SIZE,
            &configuration.textureHeight) == qfalse ||
        ParseWaterMapPositiveFloat(
            "horizontal world length", text,
            &configuration.horizontalWorldLength) == 0.0f ||
        ParseWaterMapPositiveFloat(
            "vertical world length", text,
            &configuration.verticalWorldLength) == 0.0f ||
        ParseWaterMapPositiveFloat(
            "wind velocity", text,
            &configuration.windVelocity) == 0.0f ||
        ParseWaterMapFloat(
            "wind x direction", text,
            &configuration.windDirection[0]) == 0.0f ||
        ParseWaterMapFloat(
            "wind y direction", text,
            &configuration.windDirection[1]) == 0.0f) {
        return qfalse;
    }
    ...
    if (VectorNormalize2D(configuration.windDirection) == 0.0f) {
        ri.Printf(
            R_PRINT_WARNING,
            "WARNING: wind direction is 0 0 in waterMap in shader %s\n",
            rendererParsedShader.name);
        return qfalse;
    }

    if (ParseWaterMapPositiveFloat(
            "amplitude", text, &configuration.amplitude) == 0.0f) {
        return qfalse;
    }
    ...
    int32_t parameterIndex)
{
    ...
    shader_texture_shader_t *textureShader =
        stage->bundle[bundleIndex].textureShader;
    ...
    int32_t parameterIndex)
{
    ...
    if (token[0] == '\0') {
        ri.Printf(
            R_PRINT_WARNING,
            "WARNING: shader '%s' - missing value in 'nvTexShader'\n",
            rendererParsedShader.name);
        return qfalse;
    }
    ...
    qboolean allowExpandMapping)
{
    ...
    previousTextureShader =
        stage->bundle[bundleIndex - 1].textureShader;

    if (previousTextureShader == NULL ||
        previousTextureShader->operation != GL_DOT_PRODUCT_NV) {
        ...
        case GL_DOT_PRODUCT_CONST_EYE_REFLECT_CUBE_MAP_NV:
            if (previousTextureShader != NULL &&
                previousTextureShader->operation ==
                    GL_DOT_PRODUCT_DIFFUSE_CUBE_MAP_NV) {
                break;
            }
            if (bundleIndex >= 2) {
                shader_texture_shader_t *twoBundlesBack =
                    stage->bundle[bundleIndex - 2].textureShader;
                if (twoBundlesBack != NULL &&
                    twoBundlesBack->operation == GL_DOT_PRODUCT_NV) {
                    break;
                }
            }
    ...
    if (strncmp(token, "tex", 3) == 0 &&
        isdigit((unsigned char)token[3]) != 0 &&
        token[3] - '0' < bundleIndex &&
        token[4] == '\0') {
        textureShader->previousTextureInput =
            GL_TEXTURE0_ARB + (uint32_t)(token[3] - '0');
        return qtrue;
    }
    ...
    ri.Printf(
        ...
        rendererParsedShader.name,
        bundleIndex != 0 ? va(" to tex%i", bundleIndex) : "");
    return qfalse;
}
...
{
    if (MatchShaderTokenOnLine(
            text, "(", "nvTexShader function") == qfalse) {
        return qfalse;
    }
    ...
    for (int32_t argumentIndex = 0;
        ...
        if (argumentIndex != 0 &&
            MatchShaderTokenOnLine(
                text, ",", "nvTexShader argument list") == qfalse) {
            return qfalse;
        }

        switch (argumentTypes[argumentIndex]) {
        case NV_TEXTURE_SHADER_ARG_TEXTURE_INPUT:
            if (ParseNVTextureInput(
                    stage, text, bundleIndex, qfalse) == qfalse) {
                return qfalse;
            }
            break;
        ...
        case NV_TEXTURE_SHADER_ARG_EXPANDABLE_TEXTURE_INPUT:
            if (ParseNVTextureInput(
                    stage, text, bundleIndex, qtrue) == qfalse) {
                return qfalse;
            }
            break;
        ...
        case NV_TEXTURE_SHADER_ARG_CULL_MODE_3:
            if (ParseNVCullParm(
                    stage, text, bundleIndex,
                    argumentTypes[argumentIndex] -
                        NV_TEXTURE_SHADER_ARG_CULL_MODE_0) == qfalse) {
                return qfalse;
            }
            break;
        ...
        case NV_TEXTURE_SHADER_ARG_FLOAT_3:
            if (ParseNVFloatParm(
                    stage, text, bundleIndex,
                    argumentTypes[argumentIndex] -
                        NV_TEXTURE_SHADER_ARG_FLOAT_0) == qfalse) {
                return qfalse;
            }
            break;
        }
    ...
    return MatchShaderTokenOnLine(
        text, ")", "nvTexShader function");
}
    ...
    shaderStage_t *stage, char **text, int32_t bundleIndex)
{
    ...
    if (rendererShaderRequirements[
            SHADER_REQUIREMENT_TEXTURE_SHADER_NV] == 0) {
        ri.Printf(
            R_PRINT_WARNING,
    ...
    if (operationName[0] == '\0') {
        ri.Printf(
            R_PRINT_WARNING,
            "WARNING: missing arguments to nvTexShader in shader '%s'\n",
            rendererParsedShader.name);
        return qfalse;
    }
...
#define NV_MATCH_4(name_, operation_, a_, b_, c_, d_) \
    ...
    NV_MATCH_4("cull_fragment", GL_CULL_FRAGMENT_NV,
               NV_TEXTURE_SHADER_ARG_CULL_MODE_0,
               NV_TEXTURE_SHADER_ARG_CULL_MODE_1,
               NV_TEXTURE_SHADER_ARG_CULL_MODE_2,
               NV_TEXTURE_SHADER_ARG_CULL_MODE_3);
    NV_MATCH_0("pass_through", GL_PASS_THROUGH_NV);
    NV_MATCH_1("dependent_ar", GL_DEPENDENT_AR_TEXTURE_2D_NV,
               NV_TEXTURE_SHADER_ARG_TEXTURE_INPUT);
    NV_MATCH_1("dependent_gb", GL_DEPENDENT_GB_TEXTURE_2D_NV,
               NV_TEXTURE_SHADER_ARG_TEXTURE_INPUT);

#define NV_MATCH_DOT(name_, operation_) \
    NV_MATCH_1(name_, operation_, NV_TEXTURE_SHADER_ARG_EXPANDABLE_TEXTURE_INPUT)
...
#undef NV_MATCH_DOT
    ...
    NV_MATCH_4("dot_product_reflect_cube_map_const_eye_3of3",
               GL_DOT_PRODUCT_CONST_EYE_REFLECT_CUBE_MAP_NV,
               NV_TEXTURE_SHADER_ARG_EXPANDABLE_TEXTURE_INPUT,
               NV_TEXTURE_SHADER_ARG_FLOAT_0,
               NV_TEXTURE_SHADER_ARG_FLOAT_1,
               NV_TEXTURE_SHADER_ARG_FLOAT_2);
    NV_MATCH_4("dot_product_cube_map_and_reflect_cube_map_const_eye_3of3",
               GL_DOT_PRODUCT_CONST_EYE_REFLECT_CUBE_MAP_NV,
               NV_TEXTURE_SHADER_ARG_EXPANDABLE_TEXTURE_INPUT,
               NV_TEXTURE_SHADER_ARG_FLOAT_0,
               NV_TEXTURE_SHADER_ARG_FLOAT_1,
...
#undef NV_MATCH_0
    ...
    ri.Printf(
        R_PRINT_WARNING,
        "WARNING: unknown nvTexShader function '%s' in shader '%s'\n",
    ...
    char **text, vec4_t constantColors[2], qboolean *finished)
{
    ...
    *finished = qfalse;
    ...
    for (int32_t component = 0; component < 4; ++component) {
        ...
        if (componentRaw == 0.0L &&
            coduo_crt_isalpha(
                (int32_t)(int8_t)(uint8_t)token[0]) == 0) {
            ri.Printf(
                R_PRINT_WARNING,
        ...
        if (component == 3) {
            return MatchShaderToken(
                text, ")", "nvRegCombiners const");
        }
        if (MatchShaderToken(
                text, ",", "nvRegCombiners const") == qfalse) {
            return qfalse;
        }
    }
    ...
    qboolean perStageConstants)
{
    ...
    while (ParseNVRC_ConstColor(
               text, constantColors, &finished) != qfalse) {
        if (perStageConstants != qfalse &&
            rendererShaderRequirements[
                SHADER_REQUIREMENT_REGISTER_COMBINERS2_NV] == 0) {
            ri.Printf(
                R_PRINT_WARNING,
...
{
    for (int32_t definitionIndex = 0;
         definitionIndex < definitionCount; ++definitionIndex) {
        const renderer_nv_register_definition_t *definition =
...
static const renderer_nv_register_definition_t
    nvReadWriteRegisterDefinitions[] = {
        {"col0", GL_PRIMARY_COLOR_NV, 0},
    ...
    uint32_t defaultComponentUsage, qboolean *valid)
{
    ...
    if (ParseNVRC_RegisterFromTable(
            token, nvReadWriteRegisterDefinitions,
            (int32_t)(sizeof(nvReadWriteRegisterDefinitions) /
                      sizeof(nvReadWriteRegisterDefinitions[0])),
            registerName, componentUsage,
            defaultComponentUsage) == qfalse) {
        Com_UngetToken();
        *valid = qtrue;
        return qfalse;
    }

    if (*registerName == GL_TEXTURE0_ARB + 2 &&
        rendererShaderRequirements[
            SHADER_REQUIREMENT_MAX_TEXTURE_UNITS_3] == 0 &&
        rendererShaderRequirements[
            SHADER_REQUIREMENT_MAX_TEXTURE_UNITS_4] == 0) {
        ri.Printf(
            R_PRINT_WARNING,
    ...
    if (*registerName == GL_TEXTURE0_ARB + 3 &&
        rendererShaderRequirements[
            SHADER_REQUIREMENT_MAX_TEXTURE_UNITS_4] == 0) {
        ri.Printf(
            R_PRINT_WARNING,
    ...
    uint32_t requiredComponentUsage, qboolean *valid)
{
    ...
    *valid = qtrue;
    ...
    Com_UngetToken();
    if (ParseNVRC_ReadWriteRegister(
            text, registerName, &parsedComponentUsage,
            requiredComponentUsage, valid) == qfalse) {
        return qfalse;
    }
    ...
    *valid = qfalse;
    ...
    ri.Printf(
        ...
        "in shader '%s'\n",
        requiredComponentUsage == GL_RGB ? "rgb" : "alpha",
        rendererParsedShader.name);
    return qfalse;
}
    ...
    uint32_t defaultComponentUsage)
{
    ...
    if (ParseNVRC_ReadWriteRegister(
            text, registerName, componentUsage,
            defaultComponentUsage, &valid) != qfalse) {
        return qtrue;
    }
    if (valid == qfalse)
        return qfalse;
    ...
    const char *token = Com_Parse(text);
    if (ParseNVRC_RegisterFromTable(
            token, nvReadOnlyRegisterDefinitions,
            (int32_t)(sizeof(nvReadOnlyRegisterDefinitions) /
                      sizeof(nvReadOnlyRegisterDefinitions[0])),
            registerName, componentUsage,
            defaultComponentUsage) != qfalse) {
        return qtrue;
    }
    ...
    ri.Printf(
        R_PRINT_WARNING,
        "WARNING: missing register in nvRegCombiners in shader '%s'\n",
        rendererParsedShader.name);
    Com_UngetToken();
    return qfalse;
}
    ...
    uint32_t defaultComponentUsage)
{
    ...
    return ParseNVRC_ReadRegister(
        text, registerName, componentUsage,
        defaultComponentUsage);
}
    ...
    uint32_t defaultComponentUsage)
{
    ...
    if (coduo_crt_stricmp(token, "expand") == 0) {
        input->mapping =
            negate != qfalse ? GL_EXPAND_NEGATE_NV
                             : GL_EXPAND_NORMAL_NV;
    } else if (coduo_crt_stricmp(token, "half_bias") == 0) {
        input->mapping =
            negate != qfalse ? GL_HALF_BIAS_NEGATE_NV
        ...
        input->mapping =
            negate != qfalse ? GL_SIGNED_NEGATE_NV
                             : GL_SIGNED_IDENTITY_NV;
        Com_UngetToken();
        parenthesizedMapping = qfalse;
    }

    if (parenthesizedMapping != qfalse &&
        MatchShaderToken(
            text, "(",
            "nvRegCombiners register mapping") == qfalse) {
        return qfalse;
    }

    if (ParseNVRC_FinalRegister(
            text, &input->input, &input->componentUsage,
            defaultComponentUsage) == qfalse) {
        return qfalse;
    }

    if (input->input == GL_FOG &&
        input->componentUsage == GL_ALPHA) {
        ri.Printf(
            R_PRINT_WARNING,
    ...
    uint32_t defaultComponentUsage)
{
    ...
    if (parenthesizedMapping != qfalse &&
        MatchShaderToken(
            text, "(",
            "nvRegCombiners final combiner register mapping") == qfalse) {
        return qfalse;
    }
    ...
    if (defaultComponentUsage == GL_ALPHA &&
        input->componentUsage == GL_RGB) {
        ri.Printf(
            R_PRINT_WARNING,
    ...
    qboolean *valid)
{
    ...
    uint32_t *outputRegister =
        functionIndex == 0
            ? &portion->output.abOutput
            : &portion->output.cdOutput;

    if (ParseNVRC_GeneralOutputRegister(
            text, outputRegister, componentUsage, valid) == qfalse) {
        return qfalse;
    }
    ...
    *valid = qfalse;
    if (MatchShaderToken(
            text, "=",
            "nvRegCombiners general combiner function") == qfalse) {
        return qfalse;
    }

    if (ParseNVRC_GeneralMappedRegister(
            text, &portion->inputs[firstInputIndex],
            componentUsage) == qfalse) {
        return qfalse;
    }
    ...
    if (ParseNVRC_GeneralMappedRegister(
            text, &portion->inputs[firstInputIndex + 1],
            componentUsage) == qfalse) {
        return qfalse;
    }
    ...
    uint32_t componentUsage)
{
    ...
    if (ParseNVRC_SingleGeneralFunction(
            text, portion, componentUsage, 0, &valid) == qfalse) {
        if (valid != qfalse) {
            ri.Printf(
                R_PRINT_WARNING,
    ...
    if (ParseNVRC_SingleGeneralFunction(
            text, portion, componentUsage, 1, &valid) == qfalse) {
        return valid;
    }

    if (portion->output.abDotProduct != 0 ||
        portion->output.cdDotProduct != 0) {
        return qtrue;
    }

    if (ParseNVRC_GeneralOutputRegister(
            text, &portion->output.sumOutput,
            componentUsage, &valid) == qfalse) {
        return valid;
    }

    if (MatchShaderToken(
            text, "=",
            "sum/mux function in nvRegCombiners") == qfalse) {
        return qfalse;
    }
    ...
    return MatchShaderToken(
        text, ";",
        "sum/mux function in nvRegCombiners");
}
    ...
    qboolean *valid)
{
    ...
    *valid = qfalse;
    ...
    if (MatchShaderToken(
            text, "{",
            "nvRegCombiners general combiner") == qfalse) {
        return qfalse;
    }
    if (ParseNVRC_GeneralFunction(
            text, portion, componentUsage) == qfalse) {
        return qfalse;
    }
    ...
    if (coduo_crt_stricmp(
            token,
            "bias_by_negative_one_half_scale_by_two") == 0) {
        portion->output.scale = GL_SCALE_BY_TWO_NV;
        portion->output.bias = GL_BIAS_BY_NEGATIVE_ONE_HALF_NV;
    } else if (coduo_crt_stricmp(
                   token, "bias_by_negative_one_half") == 0) {
        portion->output.scale = GL_NONE;
        portion->output.bias = GL_BIAS_BY_NEGATIVE_ONE_HALF_NV;
    } else if (coduo_crt_stricmp(
                   token, "scale_by_one_half") == 0) {
        portion->output.scale = GL_SCALE_BY_ONE_HALF_NV;
        portion->output.bias = GL_NONE;
    } else if (coduo_crt_stricmp(
                   token, "scale_by_two") == 0) {
        portion->output.scale = GL_SCALE_BY_TWO_NV;
        portion->output.bias = GL_NONE;
    } else if (coduo_crt_stricmp(
                   token, "scale_by_four") == 0) {
        portion->output.scale = GL_SCALE_BY_FOUR_NV;
        portion->output.bias = GL_NONE;
    } else {
        Com_UngetToken();
        *valid = MatchShaderToken(
            text, "}",
            "nvRegCombiners general combiner");
        return *valid;
    }

    if (MatchShaderToken(
            text, "(",
            "nvRegCombiners general combiner scale-and-bias") ==
        qfalse) {
        return qfalse;
    }
    if (MatchShaderToken(
    ...
    char **text, renderer_general_combiner_t *combiner)
{
    ...
    if (ParseNVRC_ConstColors(
            text, combiner->constantColors, qtrue) == qfalse) {
        return qfalse;
    }

    if (ParseNVRC_GeneralPortion(
            text, combiner, &valid) == qfalse) {
        if (valid != qfalse) {
            ri.Printf(
                R_PRINT_WARNING,
    ...
    char **text, renderer_final_combiner_t *combiner)
{
    ...
    for (;;) {
        ...
        if (coduo_crt_stricmp(token, "final_product") == 0) {
            ...
            if (ParseNVRC_FinalMappedRegister(
                    text, &combiner->inputs[4],
                    GL_RGB) == qfalse) {
                return qfalse;
            }
            if (MatchShaderToken(
                    text, "*",
                    "nvRegCombiners final combiner final_product") ==
                qfalse) {
                return qfalse;
            }
            if (ParseNVRC_FinalMappedRegister(
                    text, &combiner->inputs[5],
                    GL_RGB) == qfalse) {
                return qfalse;
            }

            if (combiner->inputs[4].componentUsage != GL_RGB ||
                combiner->inputs[5].componentUsage != GL_RGB) {
                ri.Printf(
                    R_PRINT_WARNING,
        ...
        if (coduo_crt_stricmp(token, "clamp_color_sum") == 0) {
            ...
            if (MatchShaderToken(
                    text, "(",
                    "nvRegCombiners final combiner clamp_color_sum") ==
                qfalse) {
                return qfalse;
            }
            if (MatchShaderToken(
    ...
    char **text, renderer_final_combiner_t *combiner)
{
    static const char context[] =
        "final rgb lerp function in nvRegCombiners";

    if (MatchShaderToken(text, "(", context) == qfalse)
        return qfalse;
    if (ParseNVRC_FinalMappedRegister(
            text, &combiner->inputs[0], GL_RGB) == qfalse) {
        return qfalse;
    }
    ...
    if (MatchShaderToken(text, ",", context) == qfalse)
        return qfalse;
    if (ParseNVRC_FinalMappedRegister(
            text, &combiner->inputs[1], GL_RGB) == qfalse) {
        return qfalse;
    }
    if (MatchShaderToken(text, ",", context) == qfalse)
        return qfalse;
    if (ParseNVRC_FinalMappedRegister(
            text, &combiner->inputs[2], GL_RGB) == qfalse) {
        return qfalse;
    }
...
{
    static const char context[] =
        "final rgb function in nvRegCombiners";
    const renderer_combiner_input_t zero = {
    ...
    char **text, renderer_final_combiner_t *combiner)
{
    ...
    for (;;) {
        ...
        if (coduo_crt_stricmp(token, "out.rgb") == 0) {
            ...
            if (MatchShaderToken(
                    text, "=",
                    "out.rgb in nvRegCombiners") == qfalse) {
                return qfalse;
            }
        ...
        if (coduo_crt_stricmp(token, "out.a") == 0) {
            ...
            if (MatchShaderToken(
                    text, "=",
                    "out.a in nvRegCombiners") == qfalse) {
                return qfalse;
            }
            if (ParseNVRC_FinalMappedRegister(
                    text, &combiner->inputs[6],
                    GL_ALPHA) == qfalse) {
                return qfalse;
            }
            if (MatchShaderToken(
    ...
    renderer_combiner_portion_t *portion, uint32_t componentUsage)
{
    for (int32_t inputIndex = 0;
         inputIndex < R_NV_COMBINER_INPUT_COUNT;
         ++inputIndex) {
...
qboolean ParseNVRegCombiners(shaderStage_t *stage, char **text)
{
    ...
    if (rendererShaderRequirements[
            SHADER_REQUIREMENT_REGISTER_COMBINERS_NV] == 0 &&
        rendererShaderRequirements[
            SHADER_REQUIREMENT_REGISTER_COMBINERS2_NV] == 0) {
        ri.Printf(
            R_PRINT_WARNING,
    ...
    if (MatchShaderToken(
            text, "{", "nvRegCombiners") == qfalse) {
        return qfalse;
    }

    for (int32_t stageIndex = 0;
         stageIndex < R_NV_GENERAL_COMBINER_LIMIT;
         ++stageIndex) {
        ParseNVRC_ClearGeneralCombinerPortion(
            &combiners.general[stageIndex].rgb, GL_RGB);
        ParseNVRC_ClearGeneralCombinerPortion(
            &combiners.general[stageIndex].alpha, GL_ALPHA);
    }

    /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
    memset(&combiners.final, 0, sizeof(combiners.final));
    for (int32_t inputIndex = 0;
         inputIndex < R_NV_FINAL_COMBINER_INPUT_COUNT;
         ++inputIndex) {
        combiners.final.inputs[inputIndex].input = GL_ZERO;
        combiners.final.inputs[inputIndex].mapping =
            GL_UNSIGNED_IDENTITY_NV;
        combiners.final.inputs[inputIndex].componentUsage =
            inputIndex == R_NV_FINAL_ALPHA_INPUT_INDEX
                ? GL_ALPHA
                : GL_RGB;
    }
    combiners.final.clampColorSum = qfalse;

    memset(combiners.constantColors, 0,
           sizeof(combiners.constantColors));
    if (ParseNVRC_ConstColors(
            text, combiners.constantColors, qfalse) == qfalse) {
        return qfalse;
    }
    ...
    for (int32_t stageIndex = 0;
         stageIndex < R_NV_GENERAL_COMBINER_LIMIT;
         ++stageIndex) {
        memcpy(combiners.general[stageIndex].constantColors,
               combiners.constantColors,
               sizeof(combiners.constantColors));
    }
    ...
    for (;;) {
        ...
        if (combiners.generalCombinerCount ==
                R_NV_REGISTER_COMBINERS1_GENERAL_LIMIT &&
            rendererShaderRequirements[
                SHADER_REQUIREMENT_REGISTER_COMBINERS2_NV] == 0) {
            ri.Printf(
                R_PRINT_WARNING,
        ...
        if (ParseNVRC_General(
                text,
                &combiners.general[
                    combiners.generalCombinerCount]) == qfalse) {
            return qfalse;
        }
    ...
    for (int32_t stageIndex = 0;
         stageIndex < combiners.generalCombinerCount;
         ++stageIndex) {
        if (memcmp(combiners.constantColors,
                   combiners.general[stageIndex].constantColors,
                   sizeof(combiners.constantColors)) != 0) {
            combiners.perStageConstants = qtrue;
            break;
        }
    }

    stage->registerCombiners =
        (renderer_register_combiners_t *)ri.Hunk_Alloc(
            sizeof(*stage->registerCombiners));
...
uint32_t ParseATIFS_ConstReg(char **text)
{
    ...
    /* Preserve this recovered boundary's validated input, state, and compatibility invariants. */
    if (coduo_crt_strnicmp(token, "const", 5) == 0 &&
        token[5] >= '0' && token[5] <= '8' &&
        token[6] == '\0') {
...
uint32_t ParseATIFS_TexCoord(char **text)
{
    ...
    if (coduo_crt_strnicmp(token, "tc", 2) == 0 &&
        token[2] >= '0' && token[2] <= '7' &&
        token[3] == '\0') {
    ...
    renderer_atifs_constant_definition_t *constantDefinitions)
{
    ...
    if (MatchShaderToken(
            text, "=",
            "atiFragmentShader constant definitions") == qfalse ||
        MatchShaderToken(
            text, "(",
            "atiFragmentShader constant definitions") == qfalse) {
        return qfalse;
    ...
    renderer_atifs_constant_definition_t *definition =
        &constantDefinitions[constantRegister - GL_CON_0_ATI];
    ...
    qboolean allowTemporaryRegisterSource)
{
    for (;;) {
        ...
        const int32_t destinationIndex =
            (int32_t)(destination - GL_REG_0_ATI);
        renderer_atifs_texture_read_t *textureRead =
            &textureReads[destinationIndex];
        ...
        if (MatchShaderToken(
                text, "(",
                "routing instruction of atiFragmentShader") == qfalse) {
            return qfalse;
        }
        ...
        if (source == GL_NONE) {
            ...
            if (textureRead->swizzle == GL_SWIZZLE_STQ_ATI ||
                textureRead->swizzle == GL_SWIZZLE_STQ_DQ_ATI) {
                /* Preserve this recovered boundary's validated input, state, and compatibility invariants. */
                ri.Printf(
                    R_PRINT_WARNING,
    ...
    for (int32_t readIndex = 0;
         readIndex < R_ATIFS_TEMP_REGISTER_COUNT;
         ++readIndex) {
        const renderer_atifs_texture_read_t *read =
            &textureReads[readIndex];
        if (read->source < GL_TEXTURE0_ARB ||
            read->source > GL_TEXTURE8_ARB) {
            continue;
        }

        for (int32_t previousIndex = 0;
             previousIndex < readIndex;
             ++previousIndex) {
            const renderer_atifs_texture_read_t *previous =
                &textureReads[previousIndex];
            if (previous->source != GL_NONE &&
                previous->source == read->source &&
                previous->swizzle != read->swizzle) {
                ri.Printf(
                    R_PRINT_WARNING,
    ...
    qboolean *writesRgb, qboolean *writesAlpha)
{
    ...
    *destination =
        GL_REG_0_ATI + (uint32_t)(token[1] - '0');
    if (token[2] == '\0') {
        *writesRgb = qtrue;
        return;
    }
    ...
    char **text, renderer_atifs_argument_t *argument)
{
    ...
    if ((token[0] == 'r' || token[0] == 'R') &&
        ...
        argument->source =
            GL_REG_0_ATI + (uint32_t)(token[1] - '0');
        prefixLength = 2;
    } else if (coduo_crt_strnicmp(token, "const", 5) == 0 &&
               token[5] >= '0' && token[5] <= '8') {
        argument->source =
            GL_CON_0_ATI + (uint32_t)(token[5] - '0');
        prefixLength = 6;
    ...
    qboolean *usesPrimaryOrSecondaryColor)
{
    for (int32_t argumentIndex = 0;
         argumentIndex < argumentCount;
         ++argumentIndex) {
        renderer_atifs_argument_t *argument =
        ...
        if (strcmp(token, "-") == 0) {
            ...
            if (strcmp(token, "2") == 0) {
                ...
                if (strcmp(token, "*") == 0) {
                    if (ParseATIFS_ArgReg(text, argument) == qfalse)
                        return qfalse;
                    argument->modifier =
                        GL_2X_BIT_ATI | GL_NEGATE_BIT_ATI;
                } else {
                    Com_UngetToken();
            ...
            if (strcmp(token, "-") == 0) {
                if (ParseATIFS_ArgReg(text, argument) == qfalse)
                    return qfalse;
                argument->modifier =
                    GL_BIAS_BIT_ATI | GL_NEGATE_BIT_ATI;
            } else {
                Com_UngetToken();
                argument->source = GL_ONE;
                argument->modifier = GL_BIAS_BIT_ATI;
            ...
            if (strcmp(token, "-") == 0) {
                token = Com_Parse(text);
                if (strcmp(token, "0.5") == 0 ||
                    strcmp(token, ".5") == 0) {
                    argument->modifier = GL_BIAS_BIT_ATI;
                } else if (strcmp(token, "1") == 0) {
                    argument->modifier =
                        GL_COMP_BIT_ATI | GL_NEGATE_BIT_ATI;
                } else {
                    ri.Printf(
                        R_PRINT_WARNING,
        ...
        if (MatchShaderToken(
                ...
                "instruction argument list in atiFragmentShader") ==
            qfalse) {
            return qfalse;
        }

        if (argument->source == GL_PRIMARY_COLOR_ARB ||
            argument->source == GL_SECONDARY_INTERPOLATOR_ATI) {
    ...
    qboolean *usesPrimaryOrSecondaryColor)
{
    ...
    for (;;) {
        ...
        Com_ParseSetMark(text, &mark);
        ParseATIFS_DestReg(
            text, &destination, &rgbWriteMask,
            &writesRgb, &writesAlpha);
        if (destination == GL_NONE)
            return qtrue;

        if (MatchShaderToken(
                text, "=",
                "fragment instruction of atiFragmentShader") == qfalse) {
            return qfalse;
        }
        ...
        if ((writesRgb != qfalse && pairHasColor != qfalse) ||
            forceNextPair != qfalse) {
            ++pairIndex;
            ++operationPairs;
        ...
        renderer_atifs_instruction_t *instruction =
            writesAlpha != qfalse
                ? &operationPairs->alpha
                : &operationPairs->color;
        if (ParseATIFS_FunctionArgs(
                text, instruction->arguments, argumentCount,
                usesPrimaryOrSecondaryColor) == qfalse) {
            return qfalse;
        }
        ...
        if (writesRgb != qfalse) {
            operationPairs->color.destinationMask =
                rgbWriteMask == GL_RGB
                    ? GL_NONE
                    : rgbWriteMask;
        }
        ...
        forceNextPair =
            writesAlpha != qfalse || operation == GL_DOT4_ATI;
    }
}
...
qboolean ParseATIFS_Phase(
    ...
    qboolean *usesPrimaryOrSecondaryColor)
{
    if (ParseATIFS_TexReads(
            text, phase->textureReads,
            allowTemporaryRegisterSource) == qfalse) {
        return qfalse;
    }

    return ParseATIFS_FragOps(
        text, phase->operationPairs,
        usesPrimaryOrSecondaryColor);
}
...
{
    const renderer_atifs_argument_t *arguments =
        instruction->arguments;

    if (instruction->operation == GL_MOV_ATI) {
        qglColorFragmentOp1ATI(
            instruction->operation, instruction->destination,
            instruction->destinationMask,
            instruction->destinationModifier,
            arguments[0].source, arguments[0].componentUsage,
            arguments[0].modifier);
    } else if (instruction->operation >= GL_ADD_ATI &&
               instruction->operation <= GL_DOT4_ATI) {
        qglColorFragmentOp2ATI(
            ...
            arguments[0].source, arguments[0].componentUsage,
            arguments[0].modifier,
            arguments[1].source, arguments[1].componentUsage,
            arguments[1].modifier);
    } else if (instruction->operation >= GL_MAD_ATI &&
               instruction->operation <= GL_DOT2_ADD_ATI) {
        qglColorFragmentOp3ATI(
            ...
            arguments[1].source, arguments[1].componentUsage,
            arguments[1].modifier,
            arguments[2].source, arguments[2].componentUsage,
            arguments[2].modifier);
    }
}
    ...
    qboolean compileShader)
{
    ...
    if (rendererShaderRequirements[
            SHADER_REQUIREMENT_FRAGMENT_SHADER_ATI] == 0) {
        ri.Printf(
            R_PRINT_WARNING,
    ...
    if (MatchShaderToken(
            text, "{",
            "start of atiFragmentShader") == qfalse) {
        return qfalse;
    }

    memset(&program, 0, sizeof(program));
    if (ParseATIFS_ConstDefs(
            text, program.constantDefinitions) == qfalse ||
        ParseATIFS_Phase(
            text, &program.parsed.phases[0], qfalse,
            &usesPrimaryOrSecondaryColor) == qfalse) {
        return qfalse;
    }
    ...
    if (usesPrimaryOrSecondaryColor != qfalse) {
        ...
        if (ParseATIFS_Phase(
                text, &program.parsed.phases[1], qtrue,
                &usesPrimaryOrSecondaryColor) == qfalse ||
            MatchShaderToken(
                text, "}",
                "end of atiFragmentShader") == qfalse) {
            return qfalse;
        }
        phaseCount = 2;
    }
    ...
    const uint32_t fragmentShader =
        (uint32_t)tr.fragmentShaderCount + 1u;
    (void)qglGetError();
    qglBindFragmentShaderATI(fragmentShader);
    qglBeginFragmentShaderATI();

    for (int32_t constantIndex = 0;
         constantIndex < 8;
         ++constantIndex) {
        const renderer_atifs_constant_definition_t *constant =
            &program.parsed.uploadConstants[constantIndex];
        if (constant->defined != qfalse) {
            qglSetFragmentShaderConstantATI(
                GL_CON_0_ATI + (uint32_t)constantIndex,
                constant->value);
        }
    }

    for (int32_t phaseIndex = 0;
         phaseIndex < phaseCount;
         ++phaseIndex) {
        const renderer_atifs_phase_t *phase =
            &program.parsed.phases[phaseIndex];

        for (int32_t registerIndex = 0;
             registerIndex < R_ATIFS_TEMP_REGISTER_COUNT;
             ++registerIndex) {
            const renderer_atifs_texture_read_t *textureRead =
                &phase->textureReads[registerIndex];
            if (textureRead->source == GL_NONE)
                continue;

            const uint32_t destination =
                GL_REG_0_ATI + (uint32_t)registerIndex;
            if (textureRead->sampleMap != qfalse) {
                qglSampleMapATI(
                    destination, textureRead->source,
                    textureRead->swizzle);
            } else {
                qglPassTexCoordATI(
                    destination, textureRead->source,
        ...
        for (int32_t pairIndex = 0;
             pairIndex < R_ATIFS_OPERATION_PAIR_COUNT;
             ++pairIndex) {
            const renderer_atifs_operation_pair_t *pair =
                &phase->operationPairs[pairIndex];
            if (pair->color.operation == GL_NONE &&
                pair->alpha.operation == GL_NONE) {
                break;
            }
    ...
    shaderStage_t *stage, char **text)
{
    ...
    if (stage->vertexProgram == NULL) {
        ri.Printf(
            R_PRINT_WARNING,
            "WARNING: couldn't load vertex program '%s' in shader '%s'\n",
            name, rendererParsedShader.name);
        return qfalse;
    }
    ...
    char **text, shader_requirement_operand_t *operand)
{
    ...
    if (operand->dependency >= 0) {
        ...
        token = Com_ParseOnLine(text);
        if (coduo_crt_isalnum((unsigned char)token[0]) == 0 &&
            token[0] != '_') {
            return qfalse;
        }
    ...
    operand->type = SHADER_REQUIREMENT_OPERAND_NUMBER;
    {
        ...
        for (; index < length; ++index) {
            if (isdigit((unsigned char)value[index]) == 0 &&
                value[index] != '.') {
                operand->type =
                    SHADER_REQUIREMENT_OPERAND_STRING;
                break;
            }
        }
        if (length == 0)
    ...
    if (value != operand->value) {
        strncpy(operand->value, value,
                sizeof(operand->value) - 1);
    ...
    const shader_requirement_operand_t *second)
{
    ...
    static const shader_requirement_operator_t reverseOperator[] = {
        SHADER_REQUIREMENT_EQUAL,
        SHADER_REQUIREMENT_NOT_EQUAL,
        SHADER_REQUIREMENT_GREATER,
        SHADER_REQUIREMENT_GREATER_OR_EQUAL,
        SHADER_REQUIREMENT_LESS,
        SHADER_REQUIREMENT_LESS_OR_EQUAL
    };
    float literalValue;
    ...
    if (literalOperand->type !=
        SHADER_REQUIREMENT_OPERAND_NUMBER) {
        return;
    }
    ...
    if (dependencyOperand->dependency !=
        SHADER_REQUIREMENT_MAX_TEXTURE_UNITS_ARB) {
        const qboolean literalIsOne = literalValue == 1.0f;
        const qboolean dependencyIsNegated =
            (dependencyOperand->leadingNotCount & 1) != 0;
        qboolean recordDependency = qfalse;

        if (operatorKind == SHADER_REQUIREMENT_NOT_EQUAL) {
            recordDependency = dependencyIsNegated
                                   ? !literalIsOne
                                   : literalIsOne;
        ...
        if (recordDependency != qfalse) {
            rendererShaderRequirements[
                dependencyOperand->dependency] = 1;
        }
        return;
    }
...
evaluate_single_operand:
    ...
    if (suppressDependencyUpdates == qfalse &&
        ...
        first.dependency !=
            SHADER_REQUIREMENT_MAX_TEXTURE_UNITS_ARB &&
        first.dependency !=
            SHADER_REQUIREMENT_MAX_TEXTURE_UNITS_4 &&
        (first.leadingNotCount & 1) == 0) {
    ...
    qboolean *outStageActive)
{
    ...
    memset(rendererShaderRequirements, 0,
           sizeof(rendererShaderRequirements));
    ...
    for (;;) {
        ...
        if (token[0] == '}') {
            if (stageActive != qfalse &&
                ...
                ri.Printf(
                    R_PRINT_WARNING,
                    "WARNING: shader '%s' has bundle with no map\n",
                    rendererParsedShader.name);
                return qfalse;
            }
            ...
            if (textureShaderBits != 0) {
                for (int32_t textureUnit = 0;
                     ...
                     ++textureUnit) {
                    textureBundle_t *textureBundle =
                        &stage->bundle[textureUnit];
                    if (textureBundle->textureShader != NULL)
                        continue;

                    textureBundle->textureShader =
                        ri.Hunk_Alloc(
                            sizeof(*textureBundle->textureShader));
                    if (textureBundle->image[0] == NULL) {
                        textureBundle->textureShader->operation = 0;
                    } else {
                        textureBundle->textureShader->operation =
                            textureBundle->image[0]->target;
                    }
                }
            }
            ...
            if (stage->rgbGen == CGEN_BAD) {
                if (srcBlendBits == 0 ||
                    srcBlendBits == GLS_SRCBLEND_ONE ||
                    srcBlendBits == GLS_SRCBLEND_SRC_ALPHA) {
                    stage->rgbGen = CGEN_IDENTITY_LIGHTING;
                } else {
            ...
            if (srcBlendBits == GLS_SRCBLEND_ONE &&
                dstBlendBits == GLS_DSTBLEND_ZERO) {
                srcBlendBits = 0;
                dstBlendBits = 0;
            ...
            stage->stateBits =
                depthMaskBits | srcBlendBits | dstBlendBits |
                alphaTestBits | depthFuncBits | lightingBits |
                textureShaderBits | registerCombinerBits |
                fragmentShaderBits | vertexProgramBits;
        ...
        if (registerCombinerBits != 0) {
            ri.Printf(
                R_PRINT_WARNING,
                "WARNING: nvRegCombiners must be the last thing "
                "in a stage\n");
            return qfalse;
        }
        ...
        if (Q_stricmp(token, "map") == 0) {
            image_t **outImage =
                &bundle->image[bundle->numImageAnimations];

            if (ParseImage(
                    text, allowTextureName, stageActive, imageTrack,
                    multiplyImage != qfalse ? imageColorScale : NULL,
                    outImage) == qfalse) {
                return qfalse;
            }
            ...
            if (*outImage != NULL) {
                ...
                if (bundle->numImageAnimations > 1 &&
                    bundle->imageAnimationSpeed == 0.0f) {
                    ri.Printf(
                        R_PRINT_WARNING,
        ...
        if (Q_stricmp(token, "animMap") == 0 ||
            Q_stricmp(token, "oneshotanimMap") == 0) {
            const qboolean oneShot =
                Q_stricmp(token, "oneshotanimMap") == 0
                    ? qtrue
                    : qfalse;
            token = Com_ParseOnLine(text);
        ...
        if (Q_stricmp(token, "videoMap") == 0) {
            ...
            bundle->videoMapHandle = ri.CIN_PlayCinematic(
                token, 0, 0, 256, 256,
                CIN_LOOP | CIN_SILENT | CIN_SHADER);
            if (bundle->videoMapHandle != -1) {
                bundle->isVideoMap = 1;
                bundle->image[0] =
                    tr.scratchImages[bundle->videoMapHandle];
            }
            continue;
        }
        ...
        if (Q_stricmp(token, "cubeMap") == 0) {
            ...
            if (rendererShaderRequirements[
                    SHADER_REQUIREMENT_TEXTURE_CUBE_MAP_ARB] == 0) {
                ri.Printf(
                    R_PRINT_WARNING,
            ...
            if (stageActive != qfalse) {
                image = R_FindImageFile(
                    imageName, GL_TEXTURE_CUBE_MAP_ARB, imageFlags,
                    cubeImageTrack,
        ...
        if (Q_stricmp(token, "atiFragmentShader") == 0) {
            ...
            parsed =
                ParseATIFragmentShader(stage, text, stageActive);
            Com_SetSpaceDelimited(qtrue);
        ...
        if (Q_stricmp(token, "blendfunc") == 0) {
            ...
            if (bundleIndex != 0) {
                if (parsedSrcBlend == GLS_SRCBLEND_ONE &&
                    parsedDstBlend == GLS_DSTBLEND_ONE) {
                    if (rendererShaderRequirements[
                            SHADER_REQUIREMENT_TEXTURE_ENV_ADD_ARB] ==
                        0) {
                    ...
                    (parsedSrcBlend == GLS_SRCBLEND_DST_COLOR &&
                     parsedDstBlend == GLS_DSTBLEND_ZERO) ||
                    (parsedSrcBlend == GLS_SRCBLEND_ZERO &&
                     parsedDstBlend == GLS_DSTBLEND_SRC_COLOR)) {
                    bundle->textureEnvMode = GL_MODULATE;
                } else if (
                    parsedSrcBlend == GLS_SRCBLEND_SRC_ALPHA &&
                    parsedDstBlend ==
        ...
        if (Q_stricmp(token, "rgbGen") == 0) {
            ...
            constantLighting =
                Q_stricmp(token, "constLighting") == 0
                    ? qtrue
                    : qfalse;
            if (Q_stricmp(token, "const") == 0 ||
                constantLighting != qfalse) {
                vec3_t color;
                ...
                for (int32_t component = 0;
                    ...
                    stage->constantColor[component] =
                        (uint8_t)(int32_t)(255.0f *
                                          color[component]);
                }
        ...
        if (Q_stricmp(token, "texgen") == 0 ||
            Q_stricmp(token, "tcGen") == 0) {
            if (stageActive == qfalse) {
                Com_SkipRestOfLine(text);
                continue;
            }
            if (bundle->image[0] == NULL) {
                ri.Printf(
                    R_PRINT_WARNING,
                    "WARNING: texgen before image in shader '%s'\n",
                    rendererParsedShader.name);
                return qfalse;
            }
            ...
            if (Q_stricmp(token, "normal") == 0) {
                ...
                bundle->tcGen =
                    TCGEN_CUBEMAP_EYE_TO_VERTEX;
                bundle->texCoordComponentCount = 3;
            } else if (Q_stricmp(token, "vertexToEye") == 0) {
                bundle->tcGen =
                    TCGEN_CUBEMAP_VERTEX_TO_EYE;
                bundle->texCoordComponentCount = 3;
            } else if (Q_stricmp(token, "reflection") == 0) {
                rendererParsedShader.surfaceFlags |=
                    SHADER_SURFACE_REQUIRES_VERTEX_BASIS;
                bundle->tcGen =
                    TCGEN_CUBEMAP_REFLECTION;
                bundle->texCoordComponentCount = 3;
            } else if (Q_stricmp(token, "lightvector") == 0) {
                bundle->tcGen =
                    TCGEN_CUBEMAP_LIGHT_VECTOR;
                bundle->texCoordComponentCount = 3;
            } else if (Q_stricmp(token, "lighthalfangle") == 0) {
                bundle->tcGen =
                    TCGEN_CUBEMAP_LIGHT_HALF_ANGLE;
                bundle->texCoordComponentCount = 3;
            } else if (Q_stricmp(token, "sunhalfangle") == 0) {
                bundle->tcGen =
                    TCGEN_CUBEMAP_SUN_HALF_ANGLE;
                bundle->texCoordComponentCount = 3;
            } else if (
                Q_stricmp(
                    token,
                    "nv_dot_product_reflect_cube_map_eye_from_qs_1of3") ==
                0) {
                rendererParsedShader.surfaceFlags |=
                    SHADER_SURFACE_REQUIRES_VERTEX_BASIS |
                    SHADER_SURFACE_TANGENT_SPACE_INPUT_MASK;
                bundle->tcGen =
                    TCGEN_CUBEMAP_DOT3_REFLECT_S;
                bundle->texCoordComponentCount = 4;
            } else if (
                Q_stricmp(
                    token,
                    "nv_dot_product_reflect_cube_map_eye_from_qs_2of3") ==
                0) {
                rendererParsedShader.surfaceFlags |=
                    SHADER_SURFACE_REQUIRES_VERTEX_BASIS |
                    SHADER_SURFACE_TANGENT_SPACE_INPUT_MASK;
                bundle->tcGen =
                    TCGEN_CUBEMAP_DOT3_REFLECT_T;
                bundle->texCoordComponentCount = 4;
            } else if (
                Q_stricmp(
                    token,
                    "nv_dot_product_reflect_cube_map_eye_from_qs_3of3") ==
                0) {
                rendererParsedShader.surfaceFlags |=
                    SHADER_SURFACE_REQUIRES_VERTEX_BASIS |
                    SHADER_SURFACE_TANGENT_SPACE_INPUT_MASK;
                bundle->tcGen =
                    TCGEN_CUBEMAP_DOT3_REFLECT_R;
                bundle->texCoordComponentCount = 4;
                ...
                rendererParsedShader.surfaceFlags |=
                    SHADER_SURFACE_LIGHTMAP_TEXCOORDS;
                bundle->tcGen = TCGEN_LIGHTMAP;
            } else if (Q_stricmp(token, "texture") == 0 ||
                       Q_stricmp(token, "base") == 0) {
                rendererParsedShader.surfaceFlags |=
                    SHADER_SURFACE_BASE_TEXCOORDS;
                bundle->tcGen = TCGEN_TEXTURE;
            } else if (Q_stricmp(token, "vector") == 0) {
                (void)ParseVector(
                    text, 3, bundle->tcGenVectors[0]);
                (void)ParseVector(
                    text, 3, bundle->tcGenVectors[1]);
        ...
        if (Q_stricmp(token, "tcMod") == 0) {
            ...
            for (;;) {
                ...
                if (remaining < 2 || tokenLength > remaining - 2) {
                    ri.Printf(
                        R_PRINT_WARNING,
                        "WARNING: tcMod line is too long in shader '%s'\n",
                        rendererParsedShader.name);
                    return qfalse;
                }
        ...
        if (Q_stricmp(token, "nextbundle") == 0) {
            ...
            ++bundleIndex;
            if (bundleIndex >= 2) {
                for (int32_t requiredUnits = bundleIndex + 1;
                     requiredUnits <= R_MAX_TEXTURE_UNITS;
                     ++requiredUnits) {
                    if (rendererShaderRequirements[
                            requiredUnits - 3] != 0) {
        ...
        if (Q_stricmp(token, "requires") == 0) {
            ...
            stageActive &=
                ParseStageRequirements(text, qfalse);
            continue;
        }
        ...
        if (Q_stricmp(token, "vertexProgram") == 0) {
            ...
            if (rendererShaderRequirements[
                    SHADER_REQUIREMENT_VERTEX_PROGRAM_ARB] == 0) {
                ri.Printf(
                    R_PRINT_WARNING,
            ...
            rendererParsedShader.surfaceFlags |=
                SHADER_SURFACE_REQUIRES_VERTEX_BASIS |
                SHADER_SURFACE_REQUIRES_NORMAL_ARRAY;
    ...
    for (;;) {
        ...
        if (token[0] == '}') {
            if (activeStageCount == 0 &&
                (rendererParsedShader.flags & SHADER_FLAG_SKY) == 0) {
                return qfalse;
            }
        ...
        if (token[0] == '{') {
            ...
            texModInfo_t temporaryTexMods
                [R_MAX_TEXTURE_UNITS][R_MAX_TEXTURE_MODIFIERS];
            qboolean stageActive;
            const uint32_t incomingShaderFlags =
                rendererParsedShader.flags;

            memset(&temporaryStage, 0, sizeof(temporaryStage));
            for (int32_t bundleIndex = 0;
                 bundleIndex < R_MAX_TEXTURE_UNITS; ++bundleIndex) {
                temporaryStage.bundle[bundleIndex].texMods =
                    temporaryTexMods[bundleIndex];
            }

            if (ParseStage(&temporaryStage, text, allowTextureName,
                           imageTrack, &stageActive) == qfalse) {
                return qfalse;
            }

            if (stageActive != qfalse) {
                shaderStage_t *const stage =
                    &rendererParsedShaderStages[activeStageCount];
                texModInfo_t (*const texMods)
                    [R_MAX_TEXTURE_MODIFIERS] =
                    rendererParsedShaderTexMods[activeStageCount];

                /* The original copies all four slots for all eight bundle;
                 * only entries below each bundle's numTexMods are live. */
                memcpy(texMods, temporaryTexMods,
                       sizeof(temporaryTexMods));
                memcpy(stage, &temporaryStage, sizeof(*stage));
                for (int32_t bundleIndex = 0;
                     bundleIndex < R_MAX_TEXTURE_UNITS; ++bundleIndex) {
                    stage->bundle[bundleIndex].texMods =
                        texMods[bundleIndex];
                }
                stage->flags |= SHADER_STAGE_ACTIVE;
                ++activeStageCount;
        ...
        if (Q_stricmpn(token, "qer", 3) == 0 ||
            Q_stricmpn(token, "radialNormals",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
            Com_SkipRestOfLine(text);
            continue;
        }

        if (Q_stricmpn(token, "q3map_sun",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
            float intensity;
            float azimuth;
            float elevation;
            ...
            tr.sunDirection[0] =
                cosf(azimuth) * elevationCosine;
            tr.sunDirection[1] =
                sinf(azimuth) * elevationCosine;
            tr.sunDirection[2] = sinf(elevation);
            continue;
        }

        if (Q_stricmpn(token, "deformVertexes",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
            ParseDeform(text);
            continue;
        }

        if (Q_stricmpn(token, "tesssize",
                       R_SHADER_TOKEN_COMPARE_LIMIT) == 0) {
            Com_SkipRestOfLine(text);
            continue;
        }
        ...
        if (Q_stricmp(token, "nolightscale") == 0) {
            rendererParsedShader.flags |=
                SHADER_FLAG_NO_IMAGE_OVERBRIGHT;
            continue;
        }
        ...
        if (Q_stricmp(token, "entityMergable") == 0) {
            rendererParsedShader.flags |=
                SHADER_FLAG_ENTITY_MERGABLE;
            continue;
        }

        if (Q_stricmp(token, "fogParms") == 0) {
            if (ParseVector(text, 3,
                            rendererParsedShader.fogColor) == qfalse) {
                return qfalse;
            }
        ...
        if (Q_stricmp(token, "sunfile") == 0) {
            ...
            if (token[0] == '\0') {
                ri.Printf(
                    R_PRINT_WARNING,
                    "WARNING: missing sun name for 'sunfile'\n");
                continue;
            }
        ...
        if (Q_stricmp(token, "cull") == 0) {
            ...
            if (Q_stricmp(token, "none") == 0 ||
                Q_stricmp(token, "twosided") == 0 ||
                Q_stricmp(token, "disable") == 0) {
                rendererParsedShader.cullType = CT_TWO_SIDED;
            } else if (Q_stricmp(token, "back") == 0 ||
                       Q_stricmp(token, "backside") == 0 ||
                       Q_stricmp(token, "backsided") == 0) {
...
void ComputeHardwareNeeds(void)
{
    ...
    for (int32_t stageIndex = 0;
         stageIndex < R_MAX_SHADER_STAGES; ++stageIndex) {
        shaderStage_t *const stage =
            &rendererParsedShaderStages[stageIndex];
        ...
        case CGEN_LIGHTING_AMBIENT:
            rendererParsedShader.surfaceFlags |=
                ...
                SHADER_SURFACE_REQUIRES_NORMAL_ARRAY;
            stage->flags |=
                SHADER_LIGHTING_AMBIENT |
                SHADER_STAGE_NORMAL_ARRAY;
            break;
        ...
        while (bundleCount < R_MAX_TEXTURE_UNITS &&
               stage->bundle[bundleCount].textureEnvMode != 0) {
            rendererParsedShader.surfaceFlags |=
                SHADER_SURFACE_TEXTURE_UNIT0 <<
        ...
        for (int32_t bundleIndex = 0;
             bundleIndex < bundleCount; ++bundleIndex) {
            textureBundle_t *const bundle =
                &stage->bundle[bundleIndex];
            const uint32_t generatedTexcoordFlag =
                SHADER_SURFACE_GENERATED_TEXCOORD0 <<
                (uint32_t)bundleIndex;

            if (bundle->numImageAnimations > 1) {
                rendererParsedShader.surfaceFlags |=
                    SHADER_SURFACE_TIME_DEPENDENT |
                    SHADER_SURFACE_ANIMATED_TEXTURES;
            } else {
            ...
            if (bundle->tcGen <= TCGEN_BAD ||
                bundle->tcGen > TCGEN_TEXTURE) {
                rendererParsedShader.surfaceFlags |=
                    generatedTexcoordFlag;
            }

            for (int32_t texModIndex = 0;
                 texModIndex < bundle->numTexMods; ++texModIndex) {
    ...
    const uint32_t lightingGeneratorFlags =
        rendererParsedShader.lightingFlags &
        SHADER_LIGHTING_ENTITY_MASK;
    if (lightingGeneratorFlags != 0 &&
        (lightingGeneratorFlags &
         (lightingGeneratorFlags - 1)) != 0) {
        ri.Printf(
            R_PRINT_ALL,
    ...
    for (int32_t deformIndex = 0;
         deformIndex < rendererParsedShader.numDeforms;
         ++deformIndex) {
        if (rendererParsedShader.deforms[deformIndex].deformation ==
            DEFORM_NORMALS) {
            rendererParsedShader.surfaceFlags |=
                SHADER_SURFACE_DEFORMED_NORMALS;
        } else {
            rendererParsedShader.surfaceFlags |=
                SHADER_SURFACE_DEFORMED_POSITIONS;
        }
    }
}
...
{
    shaderStage_t *const sourceStage =
        &rendererParsedShaderStages[0];
    shaderStage_t *const dlightStage =
        &rendererParsedShaderStages[1];
    const uint32_t blendBits =
        sourceStage->stateBits &
        (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS);

    if (rendererParsedShader.numUnfoggedPasses != 1)
        return;
    if (sourceStage->bundle[0].image[0] == NULL ||
        sourceStage->bundle[1].image[0] == NULL ||
        sourceStage->bundle[2].image[0] != NULL) {
        return;
    }
    if (sourceStage->bundle[0].isLightmap != 0 ||
        sourceStage->bundle[1].isLightmap == 0) {
        return;
    }
    if (sourceStage->bundle[0].textureEnvMode != GL_MODULATE ||
        sourceStage->bundle[1].textureEnvMode != GL_MODULATE) {
        return;
    }
    ...
    *dlightStage = *sourceStage;
    ...
    dlightStage->stateBits &=
        ~(GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS);
    if ((blendBits & GLS_SRCBLEND_BITS) ==
        GLS_SRCBLEND_SRC_ALPHA) {
        dlightStage->stateBits |=
...
qboolean CollapseMultitexture(void)
{
    ...
    const uint32_t ignoredStageFlags =
        (SHADER_STAGE_TEXCOORD_ARRAY0 << 1) - 1;

    if (qglActiveTextureARB == NULL ||
        (firstStage->flags & SHADER_STAGE_ACTIVE) == 0 ||
        (secondStage->flags & SHADER_STAGE_ACTIVE) == 0) {
        return qfalse;
    }
    if (((firstStage->flags ^ secondStage->flags) &
         ~ignoredStageFlags) != 0) {
        return qfalse;
    }
    ...
    if (bundleCounts[1] != 1 ||
        bundleCounts[0] + 1 > glConfig.maxActiveTextures) {
        return qfalse;
    }

    const uint32_t firstBlendBits =
        firstStage->stateBits &
        (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS);
    const uint32_t secondBlendBits =
        secondStage->stateBits &
        (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS);
    for (size_t ruleIndex = 0;
         ruleIndex <
             sizeof(shaderMultitextureCollapseRules) /
                 sizeof(shaderMultitextureCollapseRules[0]);
         ++ruleIndex) {
        const shader_multitexture_collapse_rule_t *const candidate =
            &shaderMultitextureCollapseRules[ruleIndex];
        if (candidate->firstBlendBits == firstBlendBits &&
            candidate->secondBlendBits == secondBlendBits) {
            rule = candidate;
            break;
        }
    }
    ...
    if (rule->textureEnvMode == GL_ADD) {
        if (glConfig.textureEnvAddAvailable == qfalse ||
            firstStage->rgbGen != CGEN_IDENTITY) {
            return qfalse;
        }
    }

    if (firstStage->rgbGen != secondStage->rgbGen ||
        firstStage->alphaGen != secondStage->alphaGen) {
        if (rule->textureEnvMode != GL_MODULATE ||
            secondStage->rgbGen != CGEN_IDENTITY ||
            (secondStage->alphaGen != AGEN_UNSPECIFIED &&
             secondStage->alphaGen != AGEN_IDENTITY)) {
            return qfalse;
        }
    }

    if (firstStage->rgbGen == CGEN_WAVEFORM &&
        memcmp(&firstStage->rgbWave,
               &secondStage->rgbWave,
               sizeof(firstStage->rgbWave)) != 0) {
        return qfalse;
    }
    if (firstStage->alphaGen == AGEN_WAVEFORM &&
        memcmp(&firstStage->alphaWave,
               &secondStage->alphaWave,
               sizeof(firstStage->alphaWave)) != 0) {
        return qfalse;
    }

    if (bundleCounts[0] == 1 &&
        firstStage->bundle[0].isLightmap != 0) {
        const textureBundle_t lightmapBundle =
            firstStage->bundle[0];
        firstStage->bundle[0] = secondStage->bundle[0];
        firstStage->bundle[1] = lightmapBundle;
    } else {
        firstStage->bundle[bundleCounts[0]] =
            secondStage->bundle[0];
    }
    firstStage->bundle[bundleCounts[0]].textureEnvMode =
        rule->textureEnvMode;
    firstStage->stateBits &=
        ~(GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS);
    firstStage->stateBits |= rule->collapsedBlendBits;

    memmove(&rendererParsedShaderStages[1],
            &rendererParsedShaderStages[2],
            sizeof(rendererParsedShaderStages[0]) *
                (R_MAX_SHADER_STAGES - 2));
    memset(&rendererParsedShaderStages[R_MAX_SHADER_STAGES - 1],
           0, sizeof(rendererParsedShaderStages[0]));
    return qtrue;
}
...
void SortNewShader(void)
{
    ...
    if (newShader->stages[0] != NULL) {
        ...
        if (firstStage->bundle[0].image[0] != NULL) {
            newFirstTexture =
                firstStage->bundle[0].image[0]->texnum;
        }
        if (firstStage->bundle[1].image[0] != NULL) {
            newSecondTexture =
                firstStage->bundle[1].image[0]->texnum;
        }
    }

    for (sortedIndex = tr.numShaders - 2;
         sortedIndex >= 0; --sortedIndex) {
        shader_t *const existingShader =
            tr.sortedShaders[sortedIndex];

        if (existingShader->sort < newSort)
            break;

        if (existingShader->sort == newSort &&
            existingShader->stages[0] != NULL) {
            shaderStage_t *const firstStage =
                existingShader->stages[0];
            image_t *const firstImage =
                firstStage->bundle[0].image[0];
            ...
            image_t *const secondImage =
                firstStage->bundle[1].image[0];
            if (secondImage != NULL &&
                secondImage->texnum <= newSecondTexture) {
                break;
            }
        }
...
shader_t *GeneratePermanentShader(void)
{
    if (tr.numShaders == R_MAX_SHADERS) {
        ...
        UpdateDelayLoadImagesForShader(
            &rendererParsedShader, qtrue);
        return tr.defaultShader;
    }
    ...
    *shader = rendererParsedShader;
    ...
    tr.numShaders =
        (int32_t)((uint32_t)tr.numShaders + 1u);

    for (int32_t stageIndex = 0;
         stageIndex < shader->numUnfoggedPasses;
         ++stageIndex) {
        const shaderStage_t *const scratchStage =
            &rendererParsedShaderStages[stageIndex];
        ...
        shaderStage_t *const permanentStage =
            ri.Hunk_Alloc(sizeof(*permanentStage));
        *permanentStage = *scratchStage;
        ...
        for (int32_t bundleIndex = 0;
             bundleIndex < R_MAX_TEXTURE_UNITS;
             ++bundleIndex) {
            const textureBundle_t *const scratchBundle =
                &scratchStage->bundle[bundleIndex];
            textureBundle_t *const permanentBundle =
                &permanentStage->bundle[bundleIndex];
            ...
            const uint32_t texModBytes =
                (uint32_t)permanentBundle->numTexMods *
                (uint32_t)sizeof(*permanentBundle->texMods);
            permanentBundle->texMods =
                ri.Hunk_Alloc(texModBytes);
            memcpy(permanentBundle->texMods,
                   scratchBundle->texMods, texModBytes);
        }
    }

    UpdateDelayLoadImagesForShader(
        shader, MergableShader(shader) == qfalse);
    SortNewShader();

    const uint32_t hash =
        R_GenerateShaderHashValue(shader->name);
...
shader_t *FinishShader(void)
{
    ...
    if ((rendererParsedShader.flags & SHADER_FLAG_SKY) != 0)
        rendererParsedShader.sort = SHADER_SORT_SKY;
    if ((rendererParsedShader.flags &
         SHADER_FLAG_POLYGON_OFFSET_MASK) != 0 &&
    ...
    while (stageIndex < R_MAX_SHADER_STAGES) {
        ...
        if ((stage->flags & SHADER_STAGE_DETAIL) != 0 &&
            r_detailtextures->integer == 0) {
            if (stageIndex < R_MAX_SHADER_STAGES - 1) {
                memmove(
                    &rendererParsedShaderStages[stageIndex],
                    &rendererParsedShaderStages[stageIndex + 1],
                    sizeof(rendererParsedShaderStages[0]) *
                        (size_t)(R_MAX_SHADER_STAGES -
                                 stageIndex - 1));

                for (int32_t lastStage =
                         R_MAX_SHADER_STAGES - 1;
                     lastStage > stageIndex; --lastStage) {
                    if ((rendererParsedShaderStages[lastStage].flags &
                         SHADER_STAGE_ACTIVE) != 0) {
                        memset(
                            &rendererParsedShaderStages[lastStage], 0,
                            sizeof(rendererParsedShaderStages[0]));
                        break;
                    }
                }
        ...
        for (int32_t bundleIndex = 0;
            ...
            for (int32_t imageIndex = 0;
                ...
                if (bundle->image[imageIndex] == NULL) {
                    ri.Printf(
                        R_PRINT_WARNING,
                        "Shader %s has a missing image\n",
                        rendererParsedShader.name);
                    stage->flags &= ~SHADER_STAGE_ACTIVE;
                    goto next_stage;
                }
                if (bundle->image[imageIndex]->target !=
                    bundle->image[0]->target) {
                    ri.Printf(
                        R_PRINT_WARNING,
        ...
        } else if ((rendererParsedShader.flags &
                    SHADER_FLAG_POLYGON_OFFSET) != 0) {
            stage->stateBits |= GLS_POLYGON_OFFSET;
        } else if ((rendererParsedShader.flags &
                    SHADER_FLAG_POLYGON_OFFSET_CONSTANT) != 0) {
            stage->stateBits |= GLS_POLYGON_OFFSET_ZERO_FACTOR;
        }

        if ((rendererParsedShader.flags & SHADER_FLAG_NO_FOG) == 0 ||
            (stage->flags & SHADER_STAGE_FOG) != 0) {
        ...
        if ((stage->stateBits &
            ...
            (rendererParsedShaderStages[0].stateBits &
             (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) != 0 &&
            rendererParsedShader.sort == SHADER_SORT_BAD) {
            if ((stage->stateBits & GLS_DEPTHMASK_TRUE) != 0) {
                rendererParsedShader.sort =
                    SHADER_SORT_SEE_THROUGH;
            } else if ((rendererParsedShaderStages[0].stateBits &
                        GLS_DSTBLEND_BITS) ==
                       GLS_DSTBLEND_ONE) {
                rendererParsedShader.sort =
...
next_stage:
    ...
    if ((rendererParsedShader.lightingFlags &
