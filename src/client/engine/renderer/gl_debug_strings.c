...
const char *GL_MemoryPointerToString(const void *pointerOrOffset)
{
    ...
    if (glConfig.vertexArrayRangeMode != R_VERTEX_ARRAY_RANGE_NONE) {
        const uintptr_t secondaryBegin = (uintptr_t)tr.staticVertexMemorySecondary.address;
        const uintptr_t secondaryEnd = secondaryBegin + tr.staticVertexMemorySecondaryLimit;
        const uintptr_t primaryBegin = (uintptr_t)tr.staticVertexMemoryPrimary.address;
        const uintptr_t primaryEnd = primaryBegin + tr.staticVertexMemoryPrimaryLimit;
