                             ...
                             int32_t shaderUsage)
{
    if (shaderNum < 0 || shaderNum >= rendererWorldData.numShaders) {
        ri.Error(ERR_DROP, "\x15ShaderForShaderNum: bad num %i",
                 shaderNum);
    }

    shader_t *shader = R_FindShader(
        rendererWorldData.shaders[shaderNum].shader,
        lightmapIndex, qtrue, shaderUsage);
    ...
    int32_t lightmapOrder[R_MAX_LIGHTMAPS])
{
    ...
    const int32_t surfaceCount =
        (int32_t)(lumpLength / sizeof(*diskSurfaces));
    int32_t originalLightmapCount = 0;
    for (int32_t surfaceIndex = 0;
         surfaceIndex < surfaceCount;
         ++surfaceIndex) {
        const int32_t lightmapIndex =
            diskSurfaces[surfaceIndex].lightmapNum;
        if (originalLightmapCount <= lightmapIndex) {
            originalLightmapCount = (int32_t)(
                (uint32_t)lightmapIndex + 1u);
        }
    }
    ...
    for (int32_t shaderIndex = 0;
        ...
        for (int32_t surfaceIndex = 0;
            ...
            if (diskSurface->shaderNum == shaderIndex &&
                diskSurface->lightmapNum >= 0) {
                const int32_t lightmapIndex =
                    diskSurface->lightmapNum;
                lightmapScratch[lightmapIndex] = (int32_t)(
                    (uint32_t)lightmapScratch[lightmapIndex] +
                    (uint32_t)diskSurface->numVerts);
            }
        }

        for (int32_t first = 0;
             first < originalLightmapCount;
             ++first) {
            if (lightmapScratch[first] != 0) {
                for (int32_t second = first + 1;
                     second < originalLightmapCount;
                     ++second) {
                    if (lightmapScratch[second] == 0)
                        continue;

                    uint32_t accumulated =
                        mergeCosts[first][second] +
                        (uint32_t)lightmapScratch[first] +
                        (uint32_t)lightmapScratch[second];
                    if ((int32_t)accumulated < 0)
                        accumulated = INT32_MAX;
                    mergeCosts[first][second] = accumulated;
                    mergeCosts[second][first] =
                        mergeCosts[first][second];
                }
    ...
    while (assignedCount < originalLightmapCount) {
        const int32_t remaining = (int32_t)(
            (uint32_t)originalLightmapCount -
            (uint32_t)assignedCount);
        int32_t atlasCapacity = (int32_t)(
        ...
        if (atlasCapacity >= 2) {
            ...
            for (int32_t first = 0;
                ...
                for (int32_t second = first + 1;
                    ...
                    if (lightmapScratch[second] != 0)
                        continue;
                    if (pairSecond < 0 ||
                        (int32_t)mergeCosts[first][second] >
                            (int32_t)mergeCosts[pairSecond][pairFirst]) {
                        pairFirst = second;
                        pairSecond = first;
                    }
                }
            ...
            for (int32_t remainingSlots = atlasCapacity - 2;
                 remainingSlots != 0;
                 --remainingSlots) {
                for (int32_t lightmapIndex = 0;
                     lightmapIndex < originalLightmapCount;
                     ++lightmapIndex) {
                    const uint32_t combined =
                        mergeCosts[pairSecond][lightmapIndex] +
                        mergeCosts[pairFirst][lightmapIndex];
                ...
                for (int32_t lightmapIndex = 0;
                    ...
                    if (lightmapScratch[lightmapIndex] != 0)
                        continue;
                    if (bestLightmap < 0 ||
                        (int32_t)mergeCosts[pairSecond][lightmapIndex] >
                            (int32_t)mergeCosts[pairSecond][bestLightmap]) {
...
{
    const uint8_t *lightmapPixels =
        rendererWorldFileBase + lightmapLump->fileofs;
    ...
    const int32_t originalLightmapCount =
        R_BuildLightmapMergability(surfaceLump, atlasDimensions,
                                   lightmapOrder);
    const int32_t sourceLightmapBytes =
        R_LIGHTMAP_SIZE * R_LIGHTMAP_SIZE *
        R_LIGHTMAP_SOURCE_COMPONENTS;

    const int32_t expectedLightmapDataLength = (int32_t)(
        (uint32_t)originalLightmapCount *
        (uint32_t)sourceLightmapBytes);
    if (lightmapDataLength != 0 &&
        lightmapDataLength != expectedLightmapDataLength) {
        ri.Error(ERR_DROP,
                 "\x15R_LoadLightmaps: incorrect lightmap lump size");
    }

    if (r_fullbright->integer != 0)
        lightmapDataLength = 0;

    const uint32_t atlasBufferBytes =
        (uint32_t)atlasDimensions[0][0] *
        (uint32_t)atlasDimensions[0][1] *
        (uint32_t)R_LIGHTMAP_SIZE *
        (uint32_t)R_LIGHTMAP_SIZE *
        (uint32_t)R_LIGHTMAP_DESTINATION_COMPONENTS;
    uint8_t *atlasPixels =
        (uint8_t *)ri.Hunk_AllocateTempMemory(
            (size_t)atlasBufferBytes);

    if (lightmapDataLength == 0 &&
        (int32_t)atlasBufferBytes > 0) {
        const uint8_t identity = (uint8_t)tr.identityLightByte;
        for (uint32_t offset = 0;
             offset < atlasBufferBytes;
             offset += R_LIGHTMAP_DESTINATION_COMPONENTS) {
    ...
    while (consumedLightmaps < originalLightmapCount) {
        ...
        const int32_t widthInLightmaps =
            atlasDimensions[atlasArrayIndex][0];
        const int32_t heightInLightmaps =
            atlasDimensions[atlasArrayIndex][1];
        const int32_t atlasTileCount = (int32_t)(
            (uint32_t)widthInLightmaps *
            (uint32_t)heightInLightmaps);
        const int32_t atlasWidth = (int32_t)(
            (uint32_t)widthInLightmaps *
            (uint32_t)R_LIGHTMAP_SIZE);
        const int32_t atlasHeight = (int32_t)(
            (uint32_t)heightInLightmaps *
            (uint32_t)R_LIGHTMAP_SIZE);
        const long double inverseWidthRaw =
            1.0L / (long double)widthInLightmaps;
        const long double inverseHeightRaw =
            1.0L / (long double)heightInLightmaps;
        ...
        while (tileIndex < atlasTileCount) {
            const int32_t originalIndex =
                lightmapOrder[(int32_t)(
                    (uint32_t)consumedLightmaps +
                    (uint32_t)tileIndex)];
            ...
            if (lightmapDataLength != 0) {
                ...
                R_CopyLightmap(
                    atlasPixels,
                    lightmapPixels + sourceOffset,
                    (int32_t)((uint32_t)tileX *
                              (uint32_t)R_LIGHTMAP_SIZE),
                    (int32_t)((uint32_t)tileY *
                              (uint32_t)R_LIGHTMAP_SIZE),
                    atlasWidth);
            }

            renderer_lightmap_placement_t *placement =
             ...
             * values. */
            placement->sOffset = (float)(
                (long double)tileX * inverseWidthRaw);
            placement->tOffset = (float)(
                (long double)tileY * inverseHeightRaw);

            tileIndexWord = (uint16_t)(tileIndexWord + 1u);
            tileIndex = (int16_t)tileIndexWord;
        }

        tr.lightmaps[atlasArrayIndex] = R_CreateImage(
            va("*lightmap%d", atlasArrayIndex), atlasPixels,
            atlasWidth, atlasHeight, GL_RGBA,
            IMAGE_FLAG_LIGHTMAP | IMAGE_FLAG_CLAMP_S | IMAGE_FLAG_CLAMP_T,
            R_IMAGE_TRACK_LIGHTMAP, NULL);

        consumedLightmaps = (int32_t)(
            (uint32_t)consumedLightmaps +
