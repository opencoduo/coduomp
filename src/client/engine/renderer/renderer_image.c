...
static const char rendererUsedImageWarningShader[] =
    "gfx/2d/warning@textures.jpg";
...
_Static_assert(_Alignof(renderer_image_allocation_t) == 4,
               "renderer image-allocation alignment changed");
_Static_assert(offsetof(renderer_image_allocation_t, kind) == 0,
               "renderer image-allocation kind moved");
_Static_assert(offsetof(renderer_image_allocation_t, memory) == 4,
               "renderer image-allocation pointer moved");
_Static_assert(sizeof(renderer_image_allocation_t) == 8,
               "renderer image-allocation layout changed");
#endif
                       ...
                       int32_t *textureMemory)
{
    if (rendererUsedImageFrame != tr.frameCount) {
        const int32_t previousFrame = (int32_t)(
            (uint32_t)tr.frameCount - 1u);
        ...
        for (int32_t imageIndex = 0;
             imageIndex < tr.imageCount;
             ++imageIndex) {
            const image_t *const image = tr.images[imageIndex];
            if (image->frameUsed != tr.frameCount &&
                image->frameUsed != previousFrame) {
                continue;
            }

            rendererUsedImageMemory = (int32_t)(
                (uint32_t)rendererUsedImageMemory +
                (uint32_t)image->cardMemory);
            if (image->imageTrack == R_IMAGE_TRACK_LIGHTMAP) {
                rendererUsedLightmapMemory = (int32_t)(
                    (uint32_t)rendererUsedLightmapMemory +
                    (uint32_t)image->cardMemory);
            }
            rendererUsedTextureMemory = (int32_t)(
                (uint32_t)rendererUsedTextureMemory +
                (uint32_t)image->textureMemory);
        }

        if (com_statmon->integer != 0 &&
            rendererUsedTextureMemory > R_USED_IMAGE_WARNING_BYTES) {
            StatMon_Warning(
                R_USED_IMAGE_WARNING_STATMON_ENTRY,
                R_USED_IMAGE_WARNING_DURATION_MSEC,
                rendererUsedImageWarningShader);
        }
    }
...
#if UINTPTR_MAX == UINT32_MAX
_Static_assert(_Alignof(renderer_texture_filter_mode_t) == 4,
               "renderer texture-filter mode alignment changed");
_Static_assert(offsetof(renderer_texture_filter_mode_t, name) == 0,
               "renderer texture-filter mode name moved");
_Static_assert(offsetof(renderer_texture_filter_mode_t, minFilter) == 4,
               "renderer texture-filter minimum moved");
_Static_assert(offsetof(renderer_texture_filter_mode_t, magFilter) == 8,
               "renderer texture-filter magnification mode moved");
_Static_assert(sizeof(renderer_texture_filter_mode_t) == 12,
               "renderer texture-filter mode layout changed");
#endif
...
static const char *const rendererImageTrackNames[R_IMAGE_TRACK_COUNT] = {
...
static int imagecompare(const void *firstIndexPointer,
                        const void *secondIndexPointer)
{
    const int32_t firstIndex =
        *(const int32_t *)firstIndexPointer;
    const int32_t secondIndex =
        *(const int32_t *)secondIndexPointer;
...
void R_ImageList_f(void)
{
    ...
    coduo_crt_qsort(sortedImageIndices,
                        (size_t)(uint32_t)tr.imageCount,
                        sizeof(sortedImageIndices[0]), imagecompare);

    ri.Printf(
        R_PRINT_ALL,
        "\n      -w-- -h-- -mm- -if--- wrap- -type- -KB--- --name-------\n");

    for (int32_t sortedIndex = 0;
         sortedIndex < tr.imageCount;
         ++sortedIndex) {
        const int32_t imageIndex =
            sortedImageIndices[sortedIndex];
        const image_t *const image = tr.images[imageIndex];

        totalTexels = (int32_t)(
            (uint32_t)totalTexels +
            (uint32_t)image->uploadWidth *
                (uint32_t)image->uploadHeight);
        ri.Printf(
            R_PRINT_ALL, "%4i: %4i %4i  %s  ",
            imageIndex, (int32_t)image->uploadWidth,
            (int32_t)image->uploadHeight,
        ...
        ri.Printf(R_PRINT_ALL, " %s %6.1f %s\n",
                  rendererImageTrackNames[image->imageTrack],
                  (double)image->cardMemory / 1024.0,
                  image->imgName);
        totalImageMemory = (int32_t)(
            (uint32_t)totalImageMemory +
            (uint32_t)image->cardMemory);
        imageMemoryByTrack[image->imageTrack] = (int32_t)(
            (uint32_t)imageMemoryByTrack[image->imageTrack] +
            (uint32_t)image->cardMemory);
    }
    ...
    ri.Printf(R_PRINT_ALL, " %i total images\n",
              tr.imageCount);
    ri.Printf(R_PRINT_ALL,
              " %.2f MB total image size\n\n",
              (double)totalImageMemory / (1024.0 * 1024.0));

    for (int32_t imageTrack = 0;
         imageTrack < R_IMAGE_TRACK_COUNT;
         ++imageTrack) {
        ri.Printf(
            R_PRINT_ALL, "%s: %.2f MB\n",
            rendererImageTrackNames[imageTrack],
            (double)imageMemoryByTrack[imageTrack] /
                (1024.0 * 1024.0));
    }
}
...
static void coduomp_jpeg_error_exit(j_common_ptr jpeg)
{
    coduomp_jpeg_error_manager_t *errorManager =
        (coduomp_jpeg_error_manager_t *)jpeg->err;
...
#if UINTPTR_MAX == UINT32_MAX
_Static_assert(_Alignof(renderer_dxt_color_block_t) == 4,
               "DXT color-block alignment changed");
_Static_assert(offsetof(renderer_dxt_color_block_t, color0) == 0,
               "DXT first endpoint moved");
_Static_assert(offsetof(renderer_dxt_color_block_t, color1) == 2,
               "DXT second endpoint moved");
_Static_assert(offsetof(renderer_dxt_color_block_t, colorIndices) == 4,
               "DXT color selectors moved");
_Static_assert(sizeof(renderer_dxt_color_block_t) == 8,
               "DXT color-block layout changed");

_Static_assert(_Alignof(renderer_dds_pixel_format_t) == 4,
               "DDS pixel-format alignment changed");
_Static_assert(offsetof(renderer_dds_pixel_format_t, size) == 0x00,
               "DDS pixel-format size moved");
_Static_assert(offsetof(renderer_dds_pixel_format_t, flags) == 0x04,
               "DDS pixel-format flags moved");
_Static_assert(offsetof(renderer_dds_pixel_format_t, fourCC) == 0x08,
               "DDS pixel-format FourCC moved");
_Static_assert(offsetof(renderer_dds_pixel_format_t, rgbBitCount) == 0x0c,
               "DDS pixel-format bit count moved");
_Static_assert(offsetof(renderer_dds_pixel_format_t, redMask) == 0x10,
               "DDS pixel-format red mask moved");
_Static_assert(offsetof(renderer_dds_pixel_format_t, greenMask) == 0x14,
               "DDS pixel-format green mask moved");
_Static_assert(offsetof(renderer_dds_pixel_format_t, blueMask) == 0x18,
               "DDS pixel-format blue mask moved");
_Static_assert(offsetof(renderer_dds_pixel_format_t, alphaMask) == 0x1c,
               "DDS pixel-format alpha mask moved");
_Static_assert(sizeof(renderer_dds_pixel_format_t) == 0x20,
               "DDS pixel-format layout changed");

_Static_assert(_Alignof(renderer_dds_header_t) == 4,
               "DDS header alignment changed");
_Static_assert(offsetof(renderer_dds_header_t, magic) == 0x00,
               "DDS magic moved");
_Static_assert(offsetof(renderer_dds_header_t, size) == 0x04,
               "DDS header size moved");
_Static_assert(offsetof(renderer_dds_header_t, flags) == 0x08,
               "DDS header flags moved");
_Static_assert(offsetof(renderer_dds_header_t, height) == 0x0c,
               "DDS height moved");
_Static_assert(offsetof(renderer_dds_header_t, width) == 0x10,
               "DDS width moved");
_Static_assert(offsetof(renderer_dds_header_t, pitchOrLinearSize) == 0x14,
               "DDS pitch or linear size moved");
_Static_assert(offsetof(renderer_dds_header_t, depth) == 0x18,
               "DDS depth moved");
_Static_assert(offsetof(renderer_dds_header_t, mipMapCount) == 0x1c,
               "DDS mip-map count moved");
_Static_assert(offsetof(renderer_dds_header_t, reserved1) == 0x20,
               "DDS reserved lanes moved");
_Static_assert(offsetof(renderer_dds_header_t, pixelFormat) == 0x4c,
               "DDS pixel format moved");
_Static_assert(offsetof(renderer_dds_header_t, caps) == 0x6c,
               "DDS capabilities moved");
_Static_assert(offsetof(renderer_dds_header_t, reserved2) == 0x7c,
               "DDS trailing reserved lane moved");
_Static_assert(sizeof(renderer_dds_header_t) == 128,
               "DDS header layout changed");
_Static_assert(offsetof(renderer_dds_header_t, pixelFormat.fourCC) == 0x54,
               "DDS FourCC offset changed");

_Static_assert(_Alignof(renderer_tga_header_t) == 1,
               "TGA header alignment changed");
_Static_assert(offsetof(renderer_tga_header_t, idLength) == 0x00,
               "TGA ID length moved");
_Static_assert(offsetof(renderer_tga_header_t, colorMapType) == 0x01,
               "TGA color-map type moved");
_Static_assert(offsetof(renderer_tga_header_t, imageType) == 0x02,
               "TGA image type moved");
_Static_assert(offsetof(renderer_tga_header_t, colorMapIndex) == 0x03,
               "TGA color-map index moved");
_Static_assert(offsetof(renderer_tga_header_t, colorMapLength) == 0x05,
               "TGA color-map length moved");
_Static_assert(offsetof(renderer_tga_header_t, colorMapSize) == 0x07,
               "TGA color-map size moved");
_Static_assert(offsetof(renderer_tga_header_t, xOrigin) == 0x08,
               "TGA x origin moved");
_Static_assert(offsetof(renderer_tga_header_t, yOrigin) == 0x0a,
               "TGA y origin moved");
_Static_assert(offsetof(renderer_tga_header_t, width) == 0x0c,
               "TGA width moved");
_Static_assert(offsetof(renderer_tga_header_t, height) == 0x0e,
               "TGA height moved");
_Static_assert(offsetof(renderer_tga_header_t, pixelSize) == 0x10,
               "TGA pixel size moved");
_Static_assert(offsetof(renderer_tga_header_t, imageDescriptor) == 0x11,
               "TGA image descriptor moved");
_Static_assert(sizeof(renderer_tga_header_t) == 18,
               "TGA header layout changed");
#endif
...
static void R_CreateDefaultImage(void)
{
    ...
    for (int32_t coordinate = 0;
         coordinate < R_DEFAULT_IMAGE_SIZE;
         ++coordinate) {
    ...
    tr.defaultImage = R_CreateImage(
        "*default", &pixels[0][0][0],
        R_DEFAULT_IMAGE_SIZE, R_DEFAULT_IMAGE_SIZE, GL_RGBA,
        IMAGE_FLAG_MIPMAP, R_IMAGE_TRACK_MISC, NULL);
}
...
static void R_CreateScreenImage(void)
{
    const int32_t textureWidth =
        SmallestTextureSizeFitting(glConfig.vidWidth);
    const int32_t textureHeight =
        SmallestTextureSizeFitting(glConfig.vidHeight);
    ...
    const uint32_t pixelBytes =
        (uint32_t)textureWidth * (uint32_t)textureHeight * 4u;
    uint8_t *pixels;
#if UINTPTR_MAX > UINT32_MAX
    ...
    tr.screenImage = R_CreateImage(
        "*screen", pixels, textureWidth, textureHeight, GL_RGBA,
        IMAGE_FLAG_COLOR_DEPTH | IMAGE_FLAG_CLAMP_S | IMAGE_FLAG_CLAMP_T,
...
static void R_CreateDlightImage(void)
{
    ...
    cvar_t *radius =
        ri.Cvar_Get("r_dlightRadius", "0.1124278", CVAR_NONE);
    cvar_t *shift =
        ri.Cvar_Get("r_dlightShift", "0.3162278", CVAR_NONE);

    const float shiftSquared = shift->value * shift->value;
    const float radiusAndShiftSquared =
        radius->value * radius->value + shiftSquared;
    const float shiftedUnitRadius = 1.0f + shiftSquared;
    const float intensityScale =
        255.0f /
        (1.0f / radiusAndShiftSquared - 1.0f / shiftedUnitRadius);
    ...
    for (int32_t x = 0; x < R_DLIGHT_IMAGE_SIZE; ++x) {
        ...
        for (int32_t y = 0; y < R_DLIGHT_IMAGE_SIZE; ++y) {
            ...
            const float normalizedDistanceSquared =
                ...
                normalizedDistanceScale;
            int32_t intensity = (int32_t)(
                intensityScale /
                    (normalizedDistanceSquared + shiftSquared) -
                intensityOffset);
    ...
    tr.dlightImage = R_CreateImage(
        "*dlight", &pixels[0][0][0],
        R_DLIGHT_IMAGE_SIZE, R_DLIGHT_IMAGE_SIZE, GL_RGBA,
        IMAGE_FLAG_CLAMP_S | IMAGE_FLAG_CLAMP_T,
        R_IMAGE_TRACK_MISC, NULL);
...
void R_CreateBuiltinImages(void)
{
    ...
    tr.identityLightImage = R_CreateImage(
        "*identityLight", &pixels[0][0][0],
        R_BUILTIN_UPLOAD_SIZE, R_BUILTIN_UPLOAD_SIZE, GL_RGBA,
        IMAGE_FLAG_NONE, R_IMAGE_TRACK_MISC, NULL);

    memset(pixels, 192, sizeof(pixels));
    tr.grayImage = R_CreateImage(
        "*gray", &pixels[0][0][0],
        R_BUILTIN_UPLOAD_SIZE, R_BUILTIN_UPLOAD_SIZE, GL_RGBA,
        IMAGE_FLAG_NONE, R_IMAGE_TRACK_MISC, NULL);

    for (int32_t imageIndex = 0;
         imageIndex < R_MAX_SCRATCH_IMAGES;
         ++imageIndex) {
        tr.scratchImages[imageIndex] = R_CreateImage(
            "*scratch", &pixels[0][0][0],
            R_BUILTIN_IMAGE_SIZE, R_BUILTIN_IMAGE_SIZE, GL_RGBA,
            IMAGE_FLAG_ALLOW_PICMIP | IMAGE_FLAG_CLAMP_S |
                IMAGE_FLAG_CLAMP_T,
            R_IMAGE_TRACK_MISC, NULL);
    }
...
void R_ColorShiftLightingBytes(const uint8_t input[4], uint8_t output[4])
{
    ...
    const uint32_t shift =
        (1u - (uint32_t)tr.overbrightBits) & 31u;
                    ...
                    int32_t destinationWidth)
{
    const uint32_t destinationPixelOffset =
        ((uint32_t)destinationY * (uint32_t)destinationWidth +
         (uint32_t)destinationX) *
        R_LIGHTMAP_DESTINATION_COMPONENTS;
    uint8_t *destinationRow =
        destination + (size_t)destinationPixelOffset;
    const uint32_t destinationRowBytes =
        (uint32_t)destinationWidth * R_LIGHTMAP_DESTINATION_COMPONENTS;
...
static const uint8_t mipBlendColors[16][4] = {
    {  0,   0,   0,   0},
    {255,   0,   0, 128},
    {  0, 255,   0, 128},
    {  0,   0, 255, 128},
...
static renderer_image_allocation_t
    rendererImageAllocations[R_MAX_IMAGE_ALLOCATIONS];
...
{
    /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered engine boundary input and state before use. */
    if ((uint32_t)rendererImageAllocationCount >=
        R_MAX_IMAGE_ALLOCATIONS) {
        ...
        ri.Error(ERR_DROP,
                 "\x15R_RememberImageAllocation: too many transient image "
                 "allocations\n");
        return;
    }
...
void R_FreeImageAllocations(void)
{
    while (rendererImageAllocationCount > 0) {
        renderer_image_allocation_t *allocation =
            &rendererImageAllocations[--rendererImageAllocationCount];
...
void *R_AllocTempMemory(size_t size)
{
    ...
    if (memory != NULL) {
        R_RememberImageAllocation(memory,
                                  R_IMAGE_ALLOCATION_TEMP_MEMORY);
    }
    return memory;
}
                            ...
                            int32_t width, int32_t height)
{
    ...
    case GL_RGB:
        return (int32_t)(
            (uint32_t)pixelCount *
            (uint32_t)(glConfig.colorBits > 16 ? 4 : 2));
    case GL_LUMINANCE:
        return pixelCount;
    case GL_RGB5:
    case GL_RGBA4:
    ...
    case GL_COMPRESSED_RGBA_S3TC_DXT1_EXT: {
        const int32_t blockWidth =
            (int32_t)((uint32_t)width + 3U) / 4;
        const int32_t blockHeight =
            (int32_t)((uint32_t)height + 3U) / 4;
        return (int32_t)(
            (uint32_t)blockWidth * (uint32_t)blockHeight * 8U);
    }
    case GL_COMPRESSED_RGBA_S3TC_DXT3_EXT:
    case GL_COMPRESSED_RGBA_S3TC_DXT5_EXT: {
        const int32_t blockWidth =
        ...
        const int32_t blockHeight =
            (int32_t)((uint32_t)height + 3U) / 4;
        return (int32_t)(
            (uint32_t)blockWidth * (uint32_t)blockHeight * 16U);
    }
    default:
        return 0;
    }
                  ...
                  uint32_t format, const uint8_t *pixels)
{
    ...
    if (internalFormat >= GL_COMPRESSED_RGB_S3TC_DXT1_EXT &&
        ...
        qglCompressedTexImage2DARB(
            ...
            GetCardMemoryAmount(internalFormat, width, height), pixels);
    } else {
        qglTexImage2D(target, level, (int32_t)internalFormat,
                             ...
                             int32_t blockStride)
{
    const int32_t blockCount =
        pixelCount > 0 ? ((pixelCount - 1) >> 4) + 1 : 0;
    const int32_t alpha = blendColor[3];
    const int32_t inverseAlpha = 255 - alpha;
    ...
    for (int32_t blockIndex = 0; blockIndex < blockCount; ++blockIndex) {
        renderer_dxt_color_block_t *block =
            (renderer_dxt_color_block_t *)blocks;
        const uint16_t oldColor0 = block->color0;
        const uint16_t oldColor1 = block->color1;
        const uint16_t newColor0 = (uint16_t)(
            (((oldColor0 & 0xf800) * inverseAlpha + red) >> 8 & 0xf800) |
            (((oldColor0 & 0x07e0) * inverseAlpha + green) >> 8 & 0x07e0) |
            (((oldColor0 & 0x001f) * inverseAlpha + blue) >> 8 & 0x001f));
        const uint16_t newColor1 = (uint16_t)(
            (((oldColor1 & 0xf800) * inverseAlpha + red) >> 8 & 0xf800) |
            (((oldColor1 & 0x07e0) * inverseAlpha + green) >> 8 & 0x07e0) |
            (((oldColor1 & 0x001f) * inverseAlpha + blue) >> 8 & 0x001f));

        if (blockStride == 8 ||
            (oldColor1 < oldColor0) == (newColor1 < newColor0)) {
            ...
            block->colorIndices ^=
                (~block->colorIndices >> 1) & UINT32_C(0x55555555);
        }
        blocks += blockStride;
    }
}
...
void R_MipMap8(uint8_t *pixels, int32_t width, int32_t height)
{
    ...
    if (outputWidth == 0 || outputHeight == 0) {
        ...
        for (int32_t outputIndex = 0;
            ...
            *destination++ =
                (uint8_t)(((int32_t)source[0] + source[1]) >> 1);
            source += 2;
        }
        return;
    }
    ...
    for (int32_t outputY = 0; outputY < outputHeight; ++outputY) {
        for (int32_t outputX = 0; outputX < outputWidth; ++outputX) {
            *destination++ = (uint8_t)(
                ((int32_t)source[0] + source[1] +
                 source[width] + source[width + 1]) >> 2);
            source += 2;
        }
        source += width;
    }
...
void R_MipMap2(uint8_t *pixels, int32_t width, int32_t height)
{
    ...
    const uint32_t outputSize =
        (uint32_t)outputWidth * (uint32_t)outputHeight * 4u;
    uint8_t *output =
        ri.Hunk_AllocateTempMemory((size_t)outputSize);

    for (int32_t outputY = 0; outputY < outputHeight; ++outputY) {
        for (int32_t outputX = 0; outputX < outputWidth; ++outputX) {
            const uint32_t destinationPixel =
                (uint32_t)outputY * (uint32_t)outputWidth +
                (uint32_t)outputX;
            uint8_t *destination =
                &output[destinationPixel * 4u];
                ...
                for (int32_t kernelY = 0; kernelY < 4; ++kernelY) {
                    ...
                    for (int32_t kernelX = 0; kernelX < 4; ++kernelX) {
                        ...
                        const uint32_t sourcePixel =
                            sourceY * (uint32_t)width + sourceX;
                        /* The optimized original emits the 1/2/4 weighted
                         * additions directly and has no kernel object. */
                        const int32_t weight =
                            (1 + (kernelX == 1 || kernelX == 2)) *
                            (1 + (kernelY == 1 || kernelY == 2));
                        sum += weight * pixels[sourcePixel * 4u +
                                               (uint32_t)channel];
                    }
                }
                  ...
                  renderer_image_load_mode_t loadMode)
{
    ...
    if (mipMarker != NULL) {
        ...
        /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered engine boundary input and state before use. */
        const int32_t nextMipNameLength = Com_sprintf(
            nextMipName, sizeof(nextMipName), "%.*s%i%s",
            (int32_t)(mipMarker + 1 - name), name,
            coduo_crt_atoi(mipMarker + 1) + 1, suffix);
        if (nextMipNameLength < 0 ||
            (size_t)nextMipNameLength >= sizeof(nextMipName)) {
            ri.Printf(R_PRINT_WARNING,
                      "WARNING: custom mipmap image name '%s' is too long\n",
                      name);
            return NULL;
        }
        strcpy(name, nextMipName);

        R_LoadImage(name, &loadedPixels, &loadedWidth, &loadedHeight,
                    &loadedFormat, &mipMapsAvailable, loadMode);
        if (loadedPixels != NULL) {
            const int32_t expectedWidth =
                (int32_t)((uint32_t)width + 1u) >> 1;
            const int32_t expectedHeight =
                (int32_t)((uint32_t)height + 1u) >> 1;

            if (loadedWidth == expectedWidth &&
                loadedHeight == expectedHeight) {
                const uint32_t loadedBytes =
                    (uint32_t)loadedWidth *
                    (uint32_t)loadedHeight * 4u;
                memcpy(pixels, loadedPixels,
                       (size_t)loadedBytes);
                return pixels;
            }
            ...
            ri.Printf(
                ...
                (int32_t)loadedWidth, (int32_t)loadedHeight);
        }
    }
    ...
    const int32_t sourceRowBytes =
        (int32_t)((uint32_t)width * 4u);
    const int32_t outputWidth = width >> 1;
    const int32_t outputHeight = height >> 1;
    uint8_t *source = pixels;
    uint8_t *destination = pixels;
    ...
    if (outputWidth == 0 || outputHeight == 0) {
        const int32_t outputCount = (int32_t)(
            (uint32_t)outputWidth + (uint32_t)outputHeight);

        for (int32_t outputIndex = 0;
             outputIndex < outputCount; ++outputIndex) {
            for (int32_t channel = 0; channel < 4; ++channel) {
                destination[channel] = (uint8_t)(
                    ((int32_t)source[channel] + source[channel + 4]) >> 1);
            }
            source += 8;
            destination += 4;
        }
    ...
    if (centerWeight <= 0.25f || centerWeight > 1.0f) {
        for (int32_t outputY = 0; outputY < outputHeight; ++outputY) {
            for (int32_t outputX = 0; outputX < outputWidth; ++outputX) {
                for (int32_t channel = 0; channel < 4; ++channel) {
                    destination[channel] = (uint8_t)(
                        ((int32_t)source[channel] + source[channel + 4] +
                         source[sourceRowBytes + channel] +
                         source[sourceRowBytes + channel + 4]) >> 2);
                }
                source += 8;
                destination += 4;
            }
    ...
    const float neighborWeight = (float)(
        0.25L * (1.0L - (long double)centerWeight));
    for (int32_t outputY = 0; outputY < outputHeight; ++outputY) {
        for (int32_t outputX = 0; outputX < outputWidth; ++outputX) {
            for (int32_t channel = 0; channel < 4; ++channel) {
                const int32_t sampleSum =
                    (int32_t)source[channel] + source[channel + 4] +
                    source[sourceRowBytes + channel] +
                    source[sourceRowBytes + channel + 4];
                destination[channel] =
                    coduo_fp_to_u8_extended(
                        (long double)centerWeight *
                            (long double)source[channel] +
                        (long double)neighborWeight *
                            (long double)sampleSum);
            }
            source += 8;
            destination += 4;
        }
                     ...
                     int32_t *textureMemory)
{
    ...
    *textureMemory = 0;
    ...
    const uint32_t selectedInternalFormat =
        PickInternalFormat(pixels, format, width, height,
                           flags, isLightmap);
    const qboolean generateMipMaps =
        (flags & IMAGE_FLAG_MIPMAP) != 0 ? qtrue : qfalse;

    if (scaledWidth == width && scaledHeight == height &&
        generateMipMaps == qfalse) {
        R_TexImage2D(uploadTarget, 0, selectedInternalFormat,
        ...
        *internalFormat = selectedInternalFormat;
        const int32_t levelMemory =
            GetCardMemoryAmount(selectedInternalFormat,
                                scaledWidth, scaledHeight);
        *cardMemory = (int32_t)(
            (uint32_t)*cardMemory + (uint32_t)levelMemory);
        *textureMemory = (int32_t)(
            (uint32_t)*textureMemory + (uint32_t)levelMemory);
    } else {
        while (sourceWidth > scaledWidth || sourceHeight > scaledHeight) {
            pixels = R_MipMap(mipName, pixels, sourceWidth, sourceHeight,
                              format, R_IMAGE_LOAD_PIXELS);
            if (pixels == NULL)
                return qfalse;
        ...
        R_LightScaleTexture(
            ...
            generateMipMaps == qfalse ? qtrue : qfalse,
            (flags & IMAGE_FLAG_NO_OVERBRIGHT) != 0 ? qtrue : qfalse,
            format);
        ...
        *cardMemory = (int32_t)(
        ...
        if (textureMipSkips == 0) {
            *textureMemory = (int32_t)(
            ...
            const int32_t skippedLevelMemory =
                levelMemory >> (textureMipSkips * 2);
            *textureMemory = (int32_t)(
                (uint32_t)*textureMemory +
                (uint32_t)skippedLevelMemory);
        } else {
            --textureMipSkips;
        }
        ...
        if (generateMipMaps != qfalse) {
            ...
            while (scaledWidth > 1 || scaledHeight > 1) {
                pixels = R_MipMap(mipName, pixels,
                                  scaledWidth, scaledHeight,
                                  format, R_IMAGE_LOAD_PIXELS);
                if (pixels == NULL)
                    return qfalse;
                ...
                *cardMemory = (int32_t)(
                ...
                if (r_colorMipLevels->integer != 0) {
                    const int32_t mipPixelCount = (int32_t)(
                        (uint32_t)scaledWidth *
                        (uint32_t)scaledHeight);
                    R_BlendOverTexture(pixels,
                                       mipPixelCount,
                                       mipBlendColors[mipLevel], format);
                }
                R_TexImage2D(uploadTarget, mipLevel,
                             selectedInternalFormat,
                             scaledWidth, scaledHeight, format, pixels);
            }
        }
    }
    ...
    if (generateMipMaps == qfalse) {
        ...
        qglTexParameteri(textureTarget, GL_TEXTURE_MIN_FILTER,
                         rendererTextureMinFilter);
        qglTexParameteri(textureTarget, GL_TEXTURE_MAG_FILTER,
                         rendererTextureMagFilter);
    }
                            ...
                            const float colorScale[4])
{
    ...
    const char *const rendererCubeFaceSuffixes[R_CUBE_FACE_COUNT] = {
        "rt", "lf", "bk", "ft", "up", "dn"
    };
    const int32_t
        rendererCubeDirectionAxes[R_CUBE_FACE_COUNT][3] = {
            {0, 2, 1}, {0, 2, 1}, {1, 0, 2},
            {1, 0, 2}, {2, 0, 1}, {2, 0, 1}
        };
    const float rendererCubeDirectionSigns[R_CUBE_FACE_COUNT][3] = {
        { 1.0f, -1.0f, -1.0f},
        {-1.0f,  1.0f, -1.0f},
        { 1.0f,  1.0f,  1.0f},
        {-1.0f,  1.0f, -1.0f},
    ...
    if (Q_stricmp(name, "$renormalize") == 0) {
        ...
        flags &= ~(IMAGE_FLAG_MIPMAP | IMAGE_FLAG_ALLOW_PICMIP |
                   IMAGE_FLAG_USE_PICMIP2);

        for (int32_t face = 0; face < R_CUBE_FACE_COUNT; ++face) {
            facePixels[face] = R_AllocTempMemory(
                (size_t)R_RENORMALIZE_CUBE_SIZE *
                R_RENORMALIZE_CUBE_SIZE * R_PIXEL_COMPONENT_COUNT);

            for (int32_t row = 0; row < R_RENORMALIZE_CUBE_SIZE; ++row) {
                const float rowCoordinate =
                    ((float)(row * 2 + 1) * coordinateScale) - 1.0f;

                for (int32_t column = 0;
                     column < R_RENORMALIZE_CUBE_SIZE; ++column) {
                    const float columnCoordinate =
                        ((float)(column * 2 + 1) * coordinateScale) - 1.0f;
                    vec3_t direction;
                    vec3_t normal;

                    direction[rendererCubeDirectionAxes[face][0]] =
                        rendererCubeDirectionSigns[face][0];
                    direction[rendererCubeDirectionAxes[face][2]] =
                        rendererCubeDirectionSigns[face][2] * rowCoordinate;
                    direction[rendererCubeDirectionAxes[face][1]] =
                        rendererCubeDirectionSigns[face][1] * columnCoordinate;
                    VectorNormalize2(direction, normal);

                    uint8_t *pixel =
                        &facePixels[face][
                            (row * R_RENORMALIZE_CUBE_SIZE + column) *
                            R_PIXEL_COMPONENT_COUNT];
        ...
        if (baseNameLength + sizeof("_**.tga") - 1 >=
            R_IMAGE_NAME_SIZE) {
        ...
        for (int32_t face = 0; face < R_CUBE_FACE_COUNT; ++face) {
            ...
            R_LoadImage(faceName, &facePixels[face], &faceWidth, &faceHeight,
                        &faceFormat, &mipMapsAvailable, R_IMAGE_LOAD_PIXELS);
            ...
            if (face == 0) {
                ...
                if (width != height) {
                    ri.Error(ERR_DROP,
                             "Cube map face images should be square ('%s')\n",
                             faceName);
                    return NULL;
                }
            } else {
    ...
    image_t *image = R_AllocImage(
        name, GL_TEXTURE_CUBE_MAP_ARB, width, height,
        flags | IMAGE_FLAG_CLAMP_S | IMAGE_FLAG_CLAMP_T,
        (int32_t)imageTrack);

    for (int32_t face = 0; face < R_CUBE_FACE_COUNT; ++face) {
        if (R_CreateImageInternal(
                image, facePixels[face],
                GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + (uint32_t)face,
                GL_RGBA, colorScale) == qfalse) {
            R_FreeImage(image);
            return NULL;
        }
    }
                             ...
                             renderer_image_track_t imageTrack)
{
    ...
    if (changedFlags != 0 && strcmp(name, "*white") != 0 &&
        strcmp(name, "*gray") != 0) {
        if ((changedFlags & IMAGE_FLAG_MIPMAP) != 0) {
            ri.Printf(R_PRINT_DEVELOPER,
                      "WARNING: reused image %s with mixed mipmap allowance\n",
                      name);
        }
        if ((changedFlags & IMAGE_FLAG_ALLOW_PICMIP) != 0) {
            ri.Printf(R_PRINT_DEVELOPER,
                      "WARNING: reused image %s with mixed picmip allowance\n",
                      name);
        }
        ...
        if ((changedFlags & (IMAGE_FLAG_CLAMP_S | IMAGE_FLAG_CLAMP_T)) != 0) {
            ri.Printf(R_PRINT_ALL,
                      "WARNING: reused image %s with mixed clamp modes\n",
                      name);
        }
    }
    ...
    if (image->imageTrack != imageTrack) {
        ri.Printf(R_PRINT_ALL,
                  "WARNING: image '%s' changed type from %s to %s.\n", name,
                  rendererImageTrackNames[image->imageTrack],
                  rendererImageTrackNames[imageTrack]);
    }
...
void R_LoadSingleDelayedImage(image_t *image)
{
    ...
    R_LoadImage(image->imgName, &pixels, &width, &height, &format,
                &mipMapsAvailable, R_IMAGE_LOAD_PIXELS);

    if (pixels == NULL ||
        width != image->width || height != image->height ||
        format != image->internalFormat ||
        (format != GL_RGBA && mipMapsAvailable == qfalse &&
         (image->flags & IMAGE_FLAG_MIPMAP) != 0)) {
        /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered engine boundary input and state before use. */
        R_FreeImageAllocations();
        ri.Printf(
            R_PRINT_DEVELOPER,
            "\x15" "delayed-load image '%s' changed from %ix%i type 0x%04x %s "
            "mipmaps to %ix%i type 0x%04x %s mipmaps\n",
            image->imgName, width, height, format,
            (image->flags & IMAGE_FLAG_MIPMAP) != 0 ? "with" : "without",
            image->width, image->height, image->internalFormat,
            mipMapsAvailable != qfalse ? "with" : "without");
        pixels = NULL;
    }

    if (pixels == NULL ||
        R_CreateImageInternal(image, pixels, GL_TEXTURE_2D,
                              format, NULL) == qfalse) {
                            ...
                            qboolean forceLoad)
{
    ...
    } else if (CompareMergableShaders(image->link.delayedShader, shader,
                                      image, image) != 0) {
        R_LoadSingleDelayedImage(image);
        return;
    }
    ...
    for (int32_t imageIndex = 0; imageIndex < tr.imageCount; ++imageIndex) {
        ...
        if (groupImage != NULL &&
            (groupImage->flags & IMAGE_FLAG_DELAYED_UPLOAD) != 0 &&
            groupImage->state.delay.group == previousGroup) {
                         ...
                         const float colorScale[4], float heightScale)
{
    ...
    if (imageTrack == R_IMAGE_TRACK_GENERATED_TEXTURE) {
        name = R_MangleTextureName(name, textureTarget, flags,
    ...
    image_t *image =
        R_FindExistingImage(name, textureTarget, flags, imageTrack);
    if (image != NULL) {
        if ((image->flags & IMAGE_FLAG_DELAYED_UPLOAD) != 0) {
            R_UpdateDelayLoadImage(
                image, NULL,
                (flags & IMAGE_FLAG_DELAYED_UPLOAD) == 0 ? qtrue : qfalse);
            if ((image->flags & IMAGE_FLAG_DELAYED_UPLOAD) != 0) {
                image->state.delay.groupTriCount +=
                    tr.delayedImageGroupTriCount;
            }
            return image;
        }
        ...
        if (image->link.textureSheet != NULL) {
            ...
            ri.Error(
                ERR_DROP,
                "\x15" "tried to load image '%s', which has been merged with "
    ...
    if (imageTrack == R_IMAGE_TRACK_GENERATED_TEXTURE) {
        image = R_AllocImage(name, textureTarget, 0, 0, flags,
                             (int32_t)imageTrack);
        R_FreeImageAllocations();
        return image;
    }
    ...
    if (textureTarget == GL_TEXTURE_2D) {
        if ((flags & IMAGE_FLAG_DELAYED_UPLOAD) != 0) {
            if ((flags & IMAGE_FLAG_HEIGHT_TO_NORMAL) != 0) {
                ...
                for (int32_t channel = 0; channel < 4; ++channel) {
                    ...
                    memcpy(&componentBits, &colorScale[channel],
                           sizeof(componentBits));
        ...
        uint32_t format;
        qboolean mipMapsAvailable;
        const renderer_image_load_mode_t loadMode =
            (flags & IMAGE_FLAG_DELAYED_UPLOAD) != 0
                ? R_IMAGE_LOAD_METADATA : R_IMAGE_LOAD_PIXELS;
        ...
        if (mipMapsAvailable == qfalse &&
            ...
            ri.Printf(
                R_PRINT_WARNING,
                "image '%s' requested mipmaps, but they aren't available\n",
                name);
            flags &= ~(IMAGE_FLAG_MIPMAP | IMAGE_FLAG_ALLOW_PICMIP |
                       IMAGE_FLAG_USE_PICMIP2);

            /* Preserve this recovered boundary's validated input, state, and compatibility invariants. */
            image = R_CreateImage(name, pixels, width, height, format,
                                  flags, (int32_t)imageTrack, colorScale);
            R_FreeImageAllocations();
            return image;
        }
        ...
        if (format != 0) {
            if ((flags & IMAGE_FLAG_HEIGHT_TO_NORMAL) == 0) {
                ...
                image = R_CreateImage(name, pixels, width, height, GL_RGBA,
                                      flags, (int32_t)imageTrack, colorScale);
                             ...
                             qboolean forceLoad)
{
    ...
    R_UnmangleTextureName(originalName, mangledName, imageName,
                          &textureTarget, &flags, colorScale, &heightScale);

    const float *effectiveColorScale = colorScale;
    if (colorScale[0] == 1.0f && colorScale[1] == 1.0f &&
        colorScale[2] == 1.0f && colorScale[3] == 1.0f) {
    ...
    image_t *image = R_FindImageFile(
        imageName, textureTarget, flags, imageTrack,
        effectiveColorScale, heightScale);
    if (image != NULL)
                        ...
                        uint16_t sourceWidth, uint16_t sourceHeight)
{
    const uint32_t sourceRowBytes =
        (uint32_t)sourceWidth * R_PIXEL_COMPONENT_COUNT;
    const uint32_t destinationRowBytes =
        (uint32_t)destinationImage->width * R_PIXEL_COMPONENT_COUNT;
    const uint32_t destinationPixelOffset =
        ((uint32_t)destinationY * (uint32_t)destinationImage->width +
         (uint32_t)destinationX) *
        R_PIXEL_COMPONENT_COUNT;
    uint8_t *destination =
        destinationPixels + (size_t)destinationPixelOffset;
...
void CopyImageTileLevel_DXT1(uint16_t destinationWidth,
                             ...
                             uint16_t sourceWidth, uint16_t sourceHeight)
{
    enum { DXT1_BLOCK_BYTES = 8, DXT_BLOCK_DIMENSION = 4 };
    uint8_t *destination = destinationPixels +
        ((size_t)destinationY * destinationWidth / DXT_BLOCK_DIMENSION +
         destinationX) * (DXT1_BLOCK_BYTES / DXT_BLOCK_DIMENSION);

    if (sourceWidth >= sourceHeight) {
        const size_t sourceRowBytes =
            (size_t)sourceWidth * DXT1_BLOCK_BYTES / DXT_BLOCK_DIMENSION;
        const size_t destinationRowBytes =
            (size_t)destinationWidth * DXT1_BLOCK_BYTES /
            DXT_BLOCK_DIMENSION;

        for (uint16_t sourceY = 0; sourceY < sourceHeight;
             sourceY += DXT_BLOCK_DIMENSION) {
            memcpy(destination, sourcePixels, sourceRowBytes);
            sourcePixels += sourceRowBytes;
            destination += destinationRowBytes;
        }
    ...
    const size_t sourceRowBytes =
        (size_t)sourceWidth * DXT1_BLOCK_BYTES / DXT_BLOCK_DIMENSION;
    const size_t destinationRowRemainder =
        (size_t)(destinationWidth - sourceHeight) * DXT1_BLOCK_BYTES /
        DXT_BLOCK_DIMENSION;

    for (uint16_t sourceX = 0; sourceX < sourceWidth;
         sourceX += DXT_BLOCK_DIMENSION) {
        const uint8_t *sourceBlock =
            sourcePixels +
            (size_t)sourceX * DXT1_BLOCK_BYTES / DXT_BLOCK_DIMENSION;
                        ...
                        uint16_t sourceWidth, uint16_t sourceHeight)
{
    ...
    CopyImageTileLevel_DXT1(destinationWidth, destinationPixels, sourcePixels,
    ...
    do {
        destinationPixels += GetCardMemoryAmount(destinationImage->internalFormat,
                                                  destinationWidth,
                                                  destinationHeight);
        sourcePixels += GetCardMemoryAmount(destinationImage->internalFormat,
                                             sourceWidth, sourceHeight);
        destinationX >>= 1;
        destinationY >>= 1;
        sourceWidth >>= 1;
        ...
        CopyImageTileLevel_DXT1(destinationWidth,
...
void CopyImageTileLevel_DXT3(uint16_t destinationWidth,
                             ...
                             uint16_t sourceWidth, uint16_t sourceHeight)
{
    enum { DXT3_BLOCK_BYTES = 16, DXT_BLOCK_DIMENSION = 4 };
    uint8_t *destination = destinationPixels +
        (size_t)destinationY * destinationWidth +
        (size_t)destinationX * DXT_BLOCK_DIMENSION;

    if (sourceWidth >= sourceHeight) {
        const size_t sourceRowBytes =
            (size_t)sourceWidth * DXT_BLOCK_DIMENSION;
        const size_t destinationRowBytes =
            (size_t)destinationWidth * DXT_BLOCK_DIMENSION;
    ...
    const size_t sourceRowBytes =
        (size_t)sourceWidth * DXT_BLOCK_DIMENSION;
    const size_t destinationRowRemainder =
        (size_t)(destinationWidth - sourceHeight) * DXT_BLOCK_DIMENSION;

    for (uint16_t sourceX = 0; sourceX < sourceWidth;
         sourceX += DXT_BLOCK_DIMENSION) {
        const uint8_t *sourceBlock =
            sourcePixels + (size_t)sourceX * DXT_BLOCK_DIMENSION;
                        ...
                        uint16_t sourceWidth, uint16_t sourceHeight)
{
    ...
    CopyImageTileLevel_DXT3(destinationWidth, destinationPixels, sourcePixels,
                            destinationX, destinationY,
                            sourceWidth, sourceHeight);
    ...
    do {
        ...
        CopyImageTileLevel_DXT3(destinationWidth,
                                destinationPixels, sourcePixels,
                                destinationX, destinationY,
                                sourceWidth, sourceHeight);
    } while (sourceWidth > 1 || sourceHeight > 1);
...
void CopyImageTileLevel_DXT5(uint16_t destinationWidth,
                             ...
                             uint16_t sourceWidth, uint16_t sourceHeight)
{
    enum { DXT5_BLOCK_BYTES = 16, DXT_BLOCK_DIMENSION = 4 };
    uint8_t *destination = destinationPixels +
        (size_t)destinationY * destinationWidth +
        (size_t)destinationX * DXT_BLOCK_DIMENSION;
                        ...
                        uint16_t sourceWidth, uint16_t sourceHeight)
{
    ...
    CopyImageTileLevel_DXT5(destinationWidth, destinationPixels, sourcePixels,
                            destinationX, destinationY,
                            sourceWidth, sourceHeight);
    ...
    do {
        ...
        CopyImageTileLevel_DXT5(destinationWidth,
                                destinationPixels, sourcePixels,
                                destinationX, destinationY,
                                sourceWidth, sourceHeight);
    } while (sourceWidth > 1 || sourceHeight > 1);
                   ...
                   uint16_t destinationX, uint16_t destinationY)
{
    ...
    --tr.delayedImageCount;
    R_LoadImage(image->imgName, &sourcePixels, &sourceWidth, &sourceHeight,
                &format, &mipMapsAvailable, R_IMAGE_LOAD_TILE);
    ...
    if ((uint32_t)sourceWidth + sourceHeight !=
            (uint32_t)image->width + image->height ||
        (sourceWidth != image->width && sourceWidth != image->height) ||
        format != image->internalFormat ||
        (format != GL_RGBA && mipMapsAvailable == qfalse &&
         (image->flags & IMAGE_FLAG_MIPMAP) != 0)) {
        ri.Error(ERR_DROP,
    ...
    while (picmip != 0 && sourceWidth > 4 && sourceHeight > 4) {
        sourcePixels = R_MipMap(image->imgName, sourcePixels,
                                sourceWidth, sourceHeight, format,
                                R_IMAGE_LOAD_TILE);
        if (sourcePixels == NULL) {
            ri.Error(ERR_DROP,
    ...
    /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered engine boundary input and state before use. */
    while (sourceWidth > glConfig.maxTextureSize ||
           sourceHeight > glConfig.maxTextureSize) {
        sourcePixels = R_MipMap(image->imgName, sourcePixels,
                                sourceWidth, sourceHeight, format,
                                R_IMAGE_LOAD_TILE);
        if (sourcePixels == NULL) {
    ...
    if (destinationX > destinationImage->width ||
        destinationY > destinationImage->height ||
        storedWidth > destinationImage->width - destinationX ||
        storedHeight > destinationImage->height - destinationY) {
        ri.Error(ERR_DROP,
                 "\x15image '%s' does not fit its texture sheet\n",
                 image->imgName);
    }

    switch (destinationImage->internalFormat) {
    case GL_RGBA:
        CopyImageTile_RGBA(destinationImage, destinationPixels, sourcePixels,
                           destinationX, destinationY,
                           sourceWidth, sourceHeight);
        break;
    case GL_COMPRESSED_RGB_S3TC_DXT1_EXT:
    case GL_COMPRESSED_RGBA_S3TC_DXT1_EXT:
        CopyImageTile_DXT1(destinationImage, destinationPixels, sourcePixels,
                           destinationX, destinationY,
                           sourceWidth, sourceHeight);
        break;
    case GL_COMPRESSED_RGBA_S3TC_DXT3_EXT:
        CopyImageTile_DXT3(destinationImage, destinationPixels, sourcePixels,
                           destinationX, destinationY,
                           sourceWidth, sourceHeight);
        break;
    case GL_COMPRESSED_RGBA_S3TC_DXT5_EXT:
        CopyImageTile_DXT5(destinationImage, destinationPixels, sourcePixels,
                           destinationX, destinationY,
                           sourceWidth, sourceHeight);
        break;
    default:
                        ...
                        uint16_t destinationX, uint16_t destinationY)
{
    while (group->next != NULL) {
        ...
        UploadImageGroup_r(child, destinationImage,
                           destinationPixels, destinationPixelsEnd,
                           destinationX, destinationY);
    ...
    if (r_showImages->integer != 0) {
        ri.Printf(R_PRINT_ALL,
                  ...
                  group->image->width, group->image->height);
    }

    LoadImageTile(group->image, destinationImage,
                  destinationPixels, destinationPixelsEnd,
                  destinationX, destinationY);
}
                      ...
                      int32_t groupIndex, uint32_t format, uint32_t flags)
{
    ...
    image_t *sheetImage =
        R_AllocImage(va("*sheet%03i", groupIndex), GL_TEXTURE_2D,
                     group->width, group->height, flags,
                     R_IMAGE_TRACK_MODEL);
    sheetImage->internalFormat = format;

    int32_t allocationBytes =
        GetCardMemoryAmount(format, group->width, group->height);
    if (allocationBytes <= 0) {
        ri.Error(ERR_DROP,
                 "\x15texture sheet %i has an invalid allocation size\n",
                 groupIndex);
        return;
    }
    if (format != GL_RGBA && (sheetImage->flags & IMAGE_FLAG_MIPMAP) != 0) {
        ...
        do {
            ...
            const int32_t levelBytes =
                GetCardMemoryAmount(format, mipWidth, mipHeight);
            if (levelBytes <= 0 || allocationBytes > INT32_MAX - levelBytes) {
                ri.Error(
                    ERR_DROP,
                    "\x15texture sheet %i mip allocation size overflow\n",
                    groupIndex);
                return;
            }
    ...
    (void)R_CreateImageInternal(sheetImage, pixels, GL_TEXTURE_2D,
                                format, NULL);
    ri.Z_Free(pixels);
}
...
{
    const int32_t leftShortSide =
        left->width < left->height ? left->width : left->height;
    const int32_t rightShortSide =
        right->width < right->height ? right->width : right->height;
    int32_t difference = leftShortSide - rightShortSide;

    if (difference == 0) {
        difference = (left->width + left->height) -
...
{
    const renderer_image_group_node_t *left =
        *(renderer_image_group_node_t *const *)leftElement;
    const renderer_image_group_node_t *right =
        *(renderer_image_group_node_t *const *)rightElement;
    const int32_t difference = left->group - right->group;

    return difference != 0
        ? difference : compare_mergable_common(left, right);
}
    ...
    int32_t groupCount, int32_t groupIndex)
{
    ...
    const int32_t doubledWidth =
        (int32_t)((uint32_t)width * 2u);
    const int32_t doubledHeight =
        (int32_t)((uint32_t)height * 2u);
    ...
    while (scanIndex < groupCount &&
           groups[scanIndex]->height == height) {
        ...
        if (scanWidth == doubledWidth) {
            ...
            while (scanIndex < groupCount &&
                   groups[scanIndex]->width < width &&
                   groups[scanIndex]->height == doubledHeight) {
                ++scanIndex;
            }
            if (scanIndex < groupCount &&
                groups[scanIndex]->width == width &&
                groups[scanIndex]->height == doubledHeight) {
                return R_IMAGE_MERGE_VERTICAL;
            }
                       ...
                       int32_t mergeNodeCount)
{
    ...
    while (groupIndex < groupCount) {
        ...
        const renderer_image_merge_direction_t direction =
            PickMergeDirection(groups, groupCount, groupIndex);
        ...
        while (insertionScan < groupCount) {
            ...
            if (candidate->height > merged->height ||
                (candidate->height == merged->height &&
                 candidate->width >= merged->width)) {
                break;
            }
        ...
        for (int32_t moveIndex = insertionScan;
             moveIndex < groupCount; ++moveIndex) {
...
int32_t CombineImageGroups(renderer_image_group_node_t **groups,
                           int32_t *groupCount)
{
    coduo_crt_qsort(groups, (size_t)*groupCount, sizeof(*groups),
                        compare_mergable_ungrouped);
    ...
    for (int32_t groupIndex = 1; groupIndex < *groupCount; ++groupIndex) {
        ...
        for (int32_t scanIndex = 0;
             scanIndex < *groupCount; ++scanIndex) {
            if (groups[scanIndex]->group == replacedGroup)
                groups[scanIndex]->group = survivingGroup;
        }

        coduo_crt_qsort(groups, (size_t)*groupCount, sizeof(*groups),
                            compare_mergable_grouped);
                               ...
                               uint32_t format, uint32_t flags)
{
    ...
    renderer_image_group_node_t *nodes = CODUOMP_ALLOCA(
        (size_t)nodeCapacity * sizeof(*nodes));
    renderer_image_group_node_t **groups = CODUOMP_ALLOCA(
        (size_t)nodeCapacity * sizeof(*groups));
    ...
    coduo_crt_qsort(groups, (size_t)imageCount, sizeof(*groups),
                        compare_mergable_grouped);

    int32_t mergeNodeCount = imageCount;
    for (int32_t groupStart = 0; groupStart < imageCount;) {
        int32_t groupEnd = groupStart + 1;
        while (groupEnd < imageCount &&
               groups[groupEnd]->group == groups[groupStart]->group) {
            ++groupEnd;
        }
        mergeNodeCount = MergeImageList(
            &groups[groupStart], groupEnd - groupStart,
    ...
    while ((combinedGroup = CombineImageGroups(groups, &imageCount)) != 0) {
        ...
        int32_t groupEnd = groupStart + 1;
        while (groupEnd < imageCount &&
               groups[groupEnd]->group == combinedGroup) {
            ++groupEnd;
        }
        mergeNodeCount = MergeImageList(
            &groups[groupStart], groupEnd - groupStart,
    ...
    for (int32_t groupIndex = 0;
         groupIndex < imageCount; ++groupIndex) {
...
int compare_image_types(const void *leftElement, const void *rightElement)
{
    ...
    const int32_t formatDifference =
        (int32_t)left->internalFormat - (int32_t)right->internalFormat;

    if (formatDifference != 0)
        return formatDifference;
    return (int32_t)(left->flags & IMAGE_FLAG_MIPMAP) -
           (int32_t)(right->flags & IMAGE_FLAG_MIPMAP);
}
...
void R_LoadDelayedImages(void)
{
    ...
    image_t **delayedImages = CODUOMP_ALLOCA(
        (size_t)tr.delayedImageCount * sizeof(*delayedImages));
    int32_t delayedImageCount = 0;

    for (int32_t imageIndex = 0;
         imageIndex < tr.imageCount; ++imageIndex) {
        image_t *image = tr.images[imageIndex];
        if (image != NULL &&
            (image->flags & IMAGE_FLAG_DELAYED_UPLOAD) != 0) {
            delayedImages[delayedImageCount++] = image;
        }
    }

    coduo_crt_qsort(delayedImages, (size_t)delayedImageCount,
                        sizeof(*delayedImages), compare_image_types);

    for (int32_t typeStart = 0; typeStart < delayedImageCount;) {
        int32_t typeEnd = typeStart + 1;
        while (typeEnd < delayedImageCount &&
               delayedImages[typeEnd]->internalFormat ==
                   delayedImages[typeStart]->internalFormat &&
               ((delayedImages[typeEnd]->flags ^
                 delayedImages[typeStart]->flags) & IMAGE_FLAG_MIPMAP) == 0) {
            ++typeEnd;
        }

        image_t *firstImage = delayedImages[typeStart];
        MergeAndLoadDelayedImages(
            &delayedImages[typeStart], typeEnd - typeStart,
                      ...
                      float heightScale)
{
    ...
    for (int32_t row = 0; row < height; ++row) {
        ...
        for (int32_t column = 0; column < width; ++column) {
            const int32_t previousColumn =
                column == 0 ? width - 1 : column - 1;
            const int32_t nextColumn =
                column + 1 == width ? 0 : column + 1;

            const size_t left =
                ((size_t)row * (size_t)width + (size_t)previousColumn) * 4U;
            const size_t right =
                ((size_t)row * (size_t)width + (size_t)nextColumn) * 4U;
            const size_t above =
                ((size_t)previousRow * (size_t)width + (size_t)column) * 4U;
            const size_t below =
                ((size_t)nextRow * (size_t)width + (size_t)column) * 4U;
            const size_t pixel =
                ((size_t)row * (size_t)width + (size_t)column) * 4U;
            ...
            const int32_t verticalDifference =
                pixels[below] + pixels[below + 1] + pixels[below + 2] -
                pixels[above] - pixels[above + 1] - pixels[above + 2];
            const long double normalXRaw =
                (long double)horizontalDifference *
                (long double)gradientScale;
            const long double normalYRaw =
                (long double)verticalDifference *
                (long double)gradientScale;
            ...
            const long double inverseLengthRaw = 1.0L / sqrtl(
                normalYRaw * (long double)normalY +
                normalXRaw * normalXRaw +
                (long double)normalZSquared);
            const float inverseLength = (float)inverseLengthRaw;

            normalPixels[pixel] =
                (uint8_t)((normalXRaw * inverseLengthRaw + 1.0L) *
                          (long double)encodeScale);
            normalPixels[pixel + 1] =
                (uint8_t)((normalY * inverseLength + 1.0f) * encodeScale);
            normalPixels[pixel + 2] =
                (uint8_t)((normalZ * inverseLength + 1.0f) * encodeScale);
                           ...
                           float colorScale[4], float *heightScale)
{
    ...
    if (separator == mangledTextureName) {
        ...
        *destination = '\0';
        ...
        memcpy(outputName + baseNameLength, mangledTextureName,
               textureNameLength);
        outputName[baseNameLength + textureNameLength] = '\0';
    }

    if (strncmp(mangledName, "$h2n", 4) == 0) {
        (void)sscanf(separator + 1, "%x/%x/%g", textureTarget, flags,
                     heightScale);
        colorScale[0] = 1.0f;
        colorScale[1] = 1.0f;
        colorScale[2] = 1.0f;
        ...
        const int32_t fieldCount =
            sscanf(separator + 1, "%x/%x/%g/%g/%g/%g", textureTarget,
                   flags, &colorScale[0], &colorScale[1], &colorScale[2],
                   &colorScale[3]);
        if (fieldCount < 3)
            colorScale[0] = 1.0f;
        if (fieldCount < 5) {
                                ...
                                float heightScale)
{
    ...
    if ((flags & IMAGE_FLAG_HEIGHT_TO_NORMAL) != 0) {
        mangledName = va("$h2n_%s:%x/%x/%g", name, textureTarget, flags,
                         heightScale);
    } else if (colorScale == NULL) {
        mangledName = va("$tex_%s:%x/%x", name, textureTarget, flags);
    } else if (colorScale[3] != 1.0f) {
        mangledName = va("$tex_%s:%x/%x/%g/%g/%g/%g", name, textureTarget,
                         flags, colorScale[0], colorScale[1], colorScale[2],
                         colorScale[3]);
    } else if (colorScale[0] == colorScale[1] &&
               colorScale[0] == colorScale[2]) {
        if (colorScale[0] == 1.0f) {
            mangledName = va("$tex_%s:%x/%x", name, textureTarget, flags);
        } else {
            mangledName = va("$tex_%s:%x/%x/%g", name, textureTarget, flags,
                             colorScale[0]);
        }
    } else {
        mangledName = va("$tex_%s:%x/%x/%g/%g/%g", name, textureTarget,
                         flags, colorScale[0], colorScale[1], colorScale[2]);
    }
    ...
    ri.Printf(R_PRINT_WARNING,
              "WARNING: mangled name for $texturename + %s is too long\n",
              name);
    return NULL;
}
                 ...
                 renderer_image_load_mode_t loadMode)
{
    ...
    *mipMapsAvailable = qfalse;
    ...
    if ((int32_t)baseNameLength >= MAX_QPATH) {
        ri.Error(ERR_DROP,
                 "\x15image name '%s' is longer than %i characters\n",
                 name, MAX_QPATH - 1);
    }
    ...
    LoadDDS(imageName, pixels, width, height, format,
            mipMapsAvailable, loadMode);
    ...
    for (int32_t row = 0; row < 4; ++row) {
        for (int32_t column = 0; column < 4; ++column) {
            const uint32_t selector =
                (selectors >> ((column * 4 + row) * 2)) & 3;
            transposedSelectors |= selector << ((row * 4 + column) * 2);
        }
    }
    for (int32_t byteIndex = 0; byteIndex < 4; ++byteIndex) {
        destination[4 + byteIndex] =
            (uint8_t)(transposedSelectors >> (byteIndex * 8));
    }
}
    ...
    for (int32_t row = 0; row < 4; ++row) {
        for (int32_t column = 0; column < 4; ++column) {
            const uint64_t selector =
                (alphaSelectors >> ((column * 4 + row) * 4)) & 15;
            transposedAlphaSelectors |=
                selector << ((row * 4 + column) * 4);
        }
    }
    for (int32_t byteIndex = 0; byteIndex < 8; ++byteIndex) {
        destination[byteIndex] =
            (uint8_t)(transposedAlphaSelectors >> (byteIndex * 8));
    }
}
                           ...
                           uint8_t destination[16])
{
    ...
    for (int32_t byteIndex = 0; byteIndex < 6; ++byteIndex) {
        alphaSelectors |=
            (uint64_t)source[2 + byteIndex] << (byteIndex * 8);
    }

    for (int32_t row = 0; row < 4; ++row) {
        for (int32_t column = 0; column < 4; ++column) {
            const uint64_t selector =
                (alphaSelectors >> ((column * 4 + row) * 3)) & 7;
            transposedAlphaSelectors |=
                selector << ((row * 4 + column) * 3);
        }
    }
    for (int32_t byteIndex = 0; byteIndex < 6; ++byteIndex) {
        destination[2 + byteIndex] =
            (uint8_t)(transposedAlphaSelectors >> (byteIndex * 8));
    }
}
             ...
             renderer_image_load_mode_t loadMode)
{
    ...
    *mipMapsAvailable = qfalse;
    if (header->mipMapCount != 0 && header->mipMapCount != 1) {
        const uint32_t largestDimension =
            header->width > header->height
                ? header->width : header->height;
        const uint32_t levelExtent =
            1u << (header->mipMapCount & 31);
    ...
    if (header->width > R_DDS_MAX_DIMENSION ||
        header->height > R_DDS_MAX_DIMENSION) {
        ri.Printf(R_PRINT_WARNING,
    ...
    if (loadMode != R_IMAGE_LOAD_METADATA) {
        ...
        do {
            requiredDataBytes +=
                (uint64_t)((levelWidth + 3u) / 4u) *
                (uint64_t)((levelHeight + 3u) / 4u) * blockBytes;
            if (*mipMapsAvailable == qfalse ||
        ...
        /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered engine boundary input and state before use. */
        if (requiredDataBytes >
            (uint64_t)(fileSize - R_DDS_DATA_OFFSET)) {
            ri.Printf(R_PRINT_WARNING,
                      "WARNING: image '%s' has truncated DDS data\n", name);
            *format = 0;
            return;
        }
    }
             ...
             renderer_image_load_mode_t loadMode)
{
    ...
    if (loadMode == R_IMAGE_LOAD_METADATA) {
        ...
        fileSize = R_ReadFile(name, &fileBuffer);
        if (fileBuffer == NULL ||
            fileSize < (int32_t)sizeof(renderer_tga_header_t)) {
            return;
        }
        header = fileBuffer;
    ...
    if (header->imageType != R_TGA_TYPE_UNCOMPRESSED_TRUECOLOR &&
        header->imageType != R_TGA_TYPE_RLE_TRUECOLOR &&
    ...
    if (header->colorMapType != 0) {
        /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
        ri.Error(ERR_DROP,
                 "\x15LoadTGA: %s colormaps not supported\n", name);
    }
    if ((header->pixelSize != R_TGA_PIXEL_SIZE_RGBA &&
         header->pixelSize != R_TGA_PIXEL_SIZE_RGB &&
    ...
    const uint16_t sourceWidth =
        (uint16_t)(header->width[0] | ((uint16_t)header->width[1] << 8));
    const uint16_t sourceHeight =
        (uint16_t)(header->height[0] | ((uint16_t)header->height[1] << 8));

    /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered engine boundary input and state before use. */
    const uint64_t pixelCountWide =
        (uint64_t)sourceWidth * (uint64_t)sourceHeight;
    const uint64_t outputByteCountWide = pixelCountWide * 4u;
    if (sourceWidth == 0 || sourceHeight == 0 ||
        outputByteCountWide > INT32_MAX) {
        ri.Printf(R_PRINT_WARNING,
                  "WARNING: image '%s' has invalid TGA dimensions %u x %u\n",
                  name, (unsigned)sourceWidth, (unsigned)sourceHeight);
        return;
    }
    ...
    *format = GL_RGBA;
    ...
    const uint32_t sourceBytesPerPixel =
        (uint32_t)header->pixelSize / 8u;

    /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered engine boundary input and state before use. */
    if ((size_t)(sourceEnd - source) < (size_t)header->idLength) {
        ri.Printf(R_PRINT_WARNING,
                  "WARNING: image '%s' has truncated TGA data\n", name);
        *format = 0;
        return;
    }
    ...
    if (header->imageType == R_TGA_TYPE_UNCOMPRESSED_TRUECOLOR ||
        ...
        const uint64_t requiredSourceBytes =
            pixelCountWide * sourceBytesPerPixel;
        ...
        while (decodedPixels < pixelCountWide) {
            ...
            const uint32_t packetLength =
                (packetHeader & R_TGA_PACKET_LENGTH_MASK) + 1u;
            ...
            const uint64_t packetBytes =
                (packetHeader & R_TGA_RLE_PACKET) != 0
                    ? sourceBytesPerPixel
                    : (uint64_t)packetLength * sourceBytesPerPixel;
    ...
    *pixels = output;
    ...
    if ((header->imageDescriptor & R_TGA_ORIGIN_TOP) != 0) {
        ...
        destination = output +
            (uint32_t)sourceWidth * (sourceHeight - 1u) * 4u;
    }
        ...
        for (int32_t row = 0; row < sourceHeight; ++row) {
            for (int32_t column = 0; column < sourceWidth; ++column) {
                if (header->pixelSize == R_TGA_PIXEL_SIZE_RGBA) {
                    ...
                    destination[2] = blue;
                    destination[3] = 255;
                } else if (header->pixelSize ==
                           R_TGA_PIXEL_SIZE_GRAYSCALE) {
    ...
    if (header->imageType == R_TGA_TYPE_RLE_TRUECOLOR) {
        ...
        while (row < sourceHeight) {
            ...
            const int32_t packetLength =
                (packetHeader & R_TGA_PACKET_LENGTH_MASK) + 1;

            if ((packetHeader & R_TGA_RLE_PACKET) == 0) {
                for (int32_t packetPixel = 0;
                     packetPixel < packetLength; ++packetPixel) {
                    uint8_t blue;
                    uint8_t green;
                    uint8_t red;
                    uint8_t alpha;
             ...
             renderer_image_load_mode_t loadMode)
{
    ...
    if (setjmp(errorManager.failure) != 0) {
        ...
        *format = 0;
        ri.Printf(R_PRINT_WARNING,
                  "WARNING: image '%s' has invalid JPEG data\n", name);
        return;
    }
    ...
    enum { R_JPEG_MAX_DIMENSION = 32768 };
    const uint64_t outputByteCountWide =
        (uint64_t)decoder.output_width *
        (uint64_t)decoder.output_height * 4u;
    if (decoder.output_width == 0 || decoder.output_height == 0 ||
        decoder.output_width > R_JPEG_MAX_DIMENSION ||
        decoder.output_height > R_JPEG_MAX_DIMENSION ||
        outputByteCountWide > INT32_MAX) {
        ri.Printf(R_PRINT_WARNING,
                  "WARNING: image '%s' has unsupported JPEG dimensions "
                  "%u x %u\n",
                  name, (unsigned)decoder.output_width,
                  (unsigned)decoder.output_height);
        jpeg_destroy_decompress(&decoder);
        return;
    }

    if (jpeg_start_decompress(&decoder) == FALSE ||
        decoder.output_components != 4) {
        jpeg_destroy_decompress(&decoder);
        ri.Printf(R_PRINT_WARNING,
                  "WARNING: image '%s' has invalid JPEG output\n", name);
        return;
    }
    ...
    *format = GL_RGBA;
    ...
    uint8_t *destination =
        R_AllocTempMemory((size_t)outputByteCountWide);
    *pixels = destination;
...
void GL_TextureMode(const char *textureMode)
{
    ...
    for (modeIndex = 0;
         modeIndex < sizeof(rendererTextureFilterModes) /
                         sizeof(rendererTextureFilterModes[0]);
         ++modeIndex) {
        if (Q_stricmp(rendererTextureFilterModes[modeIndex].name,
                      textureMode) == 0) {
            break;
        }
    }

    if (modeIndex == sizeof(rendererTextureFilterModes) /
                         sizeof(rendererTextureFilterModes[0])) {
    ...
    rendererTextureMinFilter =
        rendererTextureFilterModes[modeIndex].minFilter;
    rendererTextureMagFilter =
        rendererTextureFilterModes[modeIndex].magFilter;
    for (int32_t imageIndex = 0; imageIndex < tr.imageCount; ++imageIndex) {
        image_t *image = tr.images[imageIndex];
        ...
        if ((image->flags & IMAGE_FLAG_MIPMAP) == 0)
            continue;
        GL_Bind(image);
        qglTexParameteri(image->target, GL_TEXTURE_MIN_FILTER,
                         rendererTextureMinFilter);
        qglTexParameteri(image->target, GL_TEXTURE_MAG_FILTER,
                         rendererTextureMagFilter);
    }
}
...
int32_t generateHashValue(const char *name)
{
    ...
    for (uint32_t index = 0; name[index] != '\0'; ++index) {
        ...
        hash += (uint32_t)character *
                (R_IMAGE_HASH_CHARACTER_WEIGHT + index);
    }
                      ...
                      int32_t imageTrack)
{
    if (strlen(name) >= R_IMAGE_NAME_SIZE)
        ri.Error(ERR_DROP,
                 "\x15R_AllocImage: \"%s\" is too long\n", name);
    if (tr.imageCount == R_MAX_IMAGES)
        ri.Error(ERR_DROP,
                 "\x15R_AllocImage: MAX_DRAWIMAGES hit\n");

    image_t *image = ri.Hunk_Alloc(sizeof(*image));
    tr.images[tr.imageCount] = image;
    image->texnum =
        (uint32_t)tr.imageCount + (uint32_t)R_IMAGE_FIRST_TEXTURE_ID;
    ++tr.imageCount;
                               ...
                               const float *colorScale)
{
    const qboolean isLightmap =
        strncmp(image->imgName, "*lightmap", 9) == 0 ? qtrue : qfalse;
    ...
    if (colorScale != NULL) {
        ...
        for (int32_t pixelIndex = 0; pixelIndex < pixelCount; ++pixelIndex) {
            for (int32_t channel = 0; channel < 4; ++channel) {
                ...
                long double scaled =
                    (long double)colorScale[channel] *
                    (long double)pixels[componentIndex];

                if (scaled < 0.0L)
                    scaled = 0.0L;
                if (scaled > 255.0L)
                    scaled = 255.0L;
                pixels[componentIndex] =
                    coduo_fp_to_u8_extended(scaled);
            }
        }
    }
    ...
    if (UploadImage(image->imgName, pixels, image->target, uploadTarget,
                    format, image->width, image->height, image->flags,
                    isLightmap, &image->internalFormat,
                    &image->uploadWidth, &image->uploadHeight,
                    &cardMemory, &textureMemory) == qfalse) {
        return qfalse;
    }
    ...
    qglTexParameteri(image->target, GL_TEXTURE_WRAP_S,
                     (image->flags & IMAGE_FLAG_CLAMP_S) != 0
                         ? GL_CLAMP_TO_EDGE : GL_REPEAT);
    qglTexParameteri(image->target, GL_TEXTURE_WRAP_T,
                     (image->flags & IMAGE_FLAG_CLAMP_T) != 0
                         ? GL_CLAMP_TO_EDGE : GL_REPEAT);
    return qtrue;
}
...
image_t *R_CreateImage(const char *name, uint8_t *pixels,
                       ...
                       const float *colorScale)
{
    image_t *image = R_AllocImage(name, GL_TEXTURE_2D, width, height,
                                  flags, imageTrack);
    ...
    if ((flags & IMAGE_FLAG_DELAYED_UPLOAD) != 0) {
        ...
        image->flags &= ~IMAGE_FLAG_DELAYED_UPLOAD;
        tr.delayedImageCount = (int32_t)(
            (uint32_t)tr.delayedImageCount - 1u);
    }

    if (R_CreateImageInternal(image, pixels, GL_TEXTURE_2D,
                              format, colorScale) == qfalse) {
        R_FreeImage(image);
        return NULL;
    }
    return image;
