                           ...
                           renderer_mark_clip_vertex_t *outPoints, const vec3_t planeNormal, float planeDistance, float epsilon)
{
    ...
    for (pointIndex = 0; pointIndex < inPointCount; ++pointIndex) {
        ...
        const long double distanceRaw = ((long double)point->xyz[2] * planeNormal[2] + (long double)point->xyz[0] * planeNormal[0]) +
                                        (long double)point->xyz[1] * planeNormal[1] - (long double)planeDistance;
    ...
    *outPointCount = 0;
    ...
    if (sideCounts[R_MARK_SIDE_BACK] == 0) {
        *outPointCount = inPointCount;
        memcpy(outPoints, inPoints, (size_t)inPointCount * sizeof(*outPoints));
        return;
    }
    ...
    for (pointIndex = 0; pointIndex < inPointCount; ++pointIndex) {
        ...
        const renderer_mark_clip_vertex_t *point = &inPoints[pointIndex];
        renderer_mark_clip_vertex_t *outPoint = &outPoints[*outPointCount];
        ...
        if (nextSide != R_MARK_SIDE_ON && nextSide != side) {
            const renderer_mark_clip_vertex_t *nextPoint = &inPoints[(pointIndex + 1) % inPointCount];
            const float denominator = distances[pointIndex] - distances[pointIndex + 1];
            const float fraction = denominator == 0.0f ? 0.0f : distances[pointIndex] / denominator;
            int32_t component;

            for (component = 0; component < 3; ++component) {
                outPoint->xyz[component] = point->xyz[component] + fraction * (nextPoint->xyz[component] - point->xyz[component]);
            }
            for (component = 0; component < 2; ++component) {
                outPoint->lightmapCoords[component] =
                    point->lightmapCoords[component] + fraction * (nextPoint->lightmapCoords[component] - point->lightmapCoords[component]);
            }
            ++*outPointCount;
        }
    }
                           ...
                           polyVert_t *pointBuffer, markFragment_t *fragment)
{
    ...
    for (planeIndex = 0; planeIndex < clipPlaneCount; ++planeIndex) {
        R_ChopPolyBehindPlane(pointCount, clipPoints[activeBuffer], &pointCount, clipPoints[activeBuffer == 0 ? 1 : 0],
                              clipPlaneNormals[planeIndex], clipPlaneDistances[planeIndex], 0.5f);
        activeBuffer ^= 1;
        if (pointCount == 0)
            return qfalse;
    }
    ...
    for (pointIndex = 0; pointIndex < pointCount; ++pointIndex) {
        const renderer_mark_clip_vertex_t *clipPoint = &clipPoints[activeBuffer][pointIndex];
        polyVert_t *point = &pointBuffer[pointIndex];

        memcpy(point->xyz, clipPoint->xyz, sizeof(point->xyz));
        memcpy(point->lightmapCoords, clipPoint->lightmapCoords, sizeof(clipPoint->lightmapCoords));
    }
                          ...
                          int32_t maxSurfaces, int32_t *surfaceCount)
{
    ...
    if (tree->maxs[0] < mins[0] || tree->mins[0] > maxs[0] || tree->maxs[1] < mins[1] || tree->mins[1] > maxs[1] ||
    ...
    if (tree->childCount != 0) {
        for (index = 0; index < tree->childCount; ++index) {
            R_AABBTreeSurfaces_r(&tree->children[index], mins, maxs, surfaceBuffer, maxSurfaces, surfaceCount);
        }
        return;
    }
    ...
    for (index = 0; index < tree->surfaceCount; ++index) {
        ...
        if ((worldSurface->shader->surfaceParmFlags & SHADER_MARKS_DISABLED) != 0)
            continue;

        surface = (const renderer_lit_surface_t *)worldSurface->data;
        if (surface->boundsMax[0] < mins[0] || surface->boundsMin[0] > maxs[0] || surface->boundsMax[1] < mins[1] ||
            surface->boundsMin[1] > maxs[1] || surface->boundsMax[2] < mins[2] || surface->boundsMin[2] > maxs[2]) {
            continue;
        }
                    ...
                    int32_t *surfaceCount)
{
    ...
    for (groupIndex = 0; groupIndex < cell->cullGroupCount; ++groupIndex) {
        ...
        if (group->maxs[0] < mins[0] || group->mins[0] > maxs[0] || group->maxs[1] < mins[1] || group->mins[1] > maxs[1] ||
            group->maxs[2] < mins[2] || group->mins[2] > maxs[2]) {
            continue;
        }

        for (surfaceIndex = 0; surfaceIndex < group->surfaceCount; ++surfaceIndex) {
            msurface_t *worldSurface;
            const renderer_lit_surface_t *surface;
            ...
            for (existingIndex = 0; existingIndex < *surfaceCount; ++existingIndex) {
    ...
    R_AABBTreeSurfaces_r(cell->aabbTree, mins, maxs, surfaceBuffer, maxSurfaces, surfaceCount);
}
                     ...
                     int32_t *surfaceCount)
{
    while (node->cellIndex == R_WORLD_NODE_INTERNAL) {
        ...
        R_BoxSurfaces_r(node->data.node.children[0], mins, maxs, surfaceBuffer, maxSurfaces, surfaceCount);
        node = node->data.node.children[1];
    }

    if (node->cellIndex != R_WORLD_NODE_NO_CELL) {
        R_CellSurfaces(&tr.world->cells[node->cellIndex], mins, maxs, surfaceBuffer, maxSurfaces, surfaceCount);
    }
}
...
{
    msurface_t *surfaceBuffer[R_MARK_MAX_WORLD_SURFACES];
    renderer_mark_clip_vertex_t clipPoints[R_MARK_CLIP_BUFFER_COUNT][R_MARK_CLIP_MAX_VERTICES];
    vec3_t clipPlaneNormals[R_MARK_CLIP_MAX_VERTICES + R_MARK_EXTRA_CLIP_PLANES];
    float clipPlaneDistances[R_MARK_CLIP_MAX_VERTICES + R_MARK_EXTRA_CLIP_PLANES];
    ...
    if (pointCount <= 0 || pointCount > R_MARK_CLIP_MAX_VERTICES || points == NULL || projectionOrigin == NULL || projectionAxis == NULL ||
        maxPoints <= 0 || pointBuffer == NULL || maxFragments <= 0 || fragmentBuffer == NULL) {
        return 0;
    }

    ++tr.viewCount;
    for (component = 0; component < 3; ++component)
        rendererMarkProjectionPlane.components.normal[component] = -projectionAxis[0][component];
    ...
    for (index = 0; index < pointCount; ++index) {
        ...
        for (component = 0; component < 3; ++component) {
            ...
            projectedPoint[component] = pointComponent + projectionRadius * directionComponent;
            extendedPoint[component] = pointComponent + (projectionRadius * -20.0f) * directionComponent;
        }
    ...
    for (index = 0; index < clippedSourcePointCount; ++index) {
        const int32_t nextIndex = (index + 1) % clippedSourcePointCount;
        vec3_t edge;

        for (component = 0; component < 3; ++component)
            edge[component] = points[nextIndex][component] - points[index][component];

        clipPlaneNormals[index][0] = edge[1] * projectionAxis[0][2] - edge[2] * projectionAxis[0][1];
        clipPlaneNormals[index][1] = edge[2] * projectionAxis[0][0] - edge[0] * projectionAxis[0][2];
        clipPlaneNormals[index][2] = edge[0] * projectionAxis[0][1] - edge[1] * projectionAxis[0][0];
        (void)VectorNormalize(clipPlaneNormals[index]);

        clipPlaneDistances[index] = points[index][0] * clipPlaneNormals[index][0] + points[index][1] * clipPlaneNormals[index][1] +
                                    points[index][2] * clipPlaneNormals[index][2];
    }

    memcpy(clipPlaneNormals[clippedSourcePointCount], projectionAxis[0], sizeof(vec3_t));
    clipPlaneDistances[clippedSourcePointCount] =
        points[0][0] * projectionAxis[0][0] + points[0][1] * projectionAxis[0][1] + points[0][2] * projectionAxis[0][2] - projectionRadius;

    for (component = 0; component < 3; ++component) {
        clipPlaneNormals[clippedSourcePointCount + 1][component] = -projectionAxis[0][component];
    }
    clipPlaneDistances[clippedSourcePointCount + 1] = points[0][0] * clipPlaneNormals[clippedSourcePointCount + 1][0] +
                                                      points[0][1] * clipPlaneNormals[clippedSourcePointCount + 1][1] +
                                                      points[0][2] * clipPlaneNormals[clippedSourcePointCount + 1][2] - projectionRadius;
    clipPlaneCount = clippedSourcePointCount + R_MARK_EXTRA_CLIP_PLANES;

    R_BoxSurfaces_r(tr.world->nodes, boundsMin, boundsMax, surfaceBuffer, R_MARK_MAX_WORLD_SURFACES, &surfaceCount);

    for (component = 0; component < 3; ++component)
        rendererMarkProjectionPlane.components.normal[component] = -rendererMarkProjectionPlane.components.normal[component];
    ...
    for (index = 0; index < surfaceCount; ++index) {
        ...
        if (requestedShader != tr.defaultShader && worldSurface->shader->lightmapIndex >= 0) {
            fragmentShader = R_FindShader(requestedShader->name, worldSurface->shader->lightmapIndex, qtrue, R_MARK_SHADER_USAGE);
        } else {
            fragmentShader = requestedShader;
        }

        for (firstTriangleIndex = 0; firstTriangleIndex < surface->indexCount; firstTriangleIndex += 3) {
            const uint16_t vertexIndices[3] = {surface->indices[firstTriangleIndex], surface->indices[firstTriangleIndex + 1],
            ...
            for (corner = 0; corner < 3; ++corner) {
                const int32_t vertexOffset = (int32_t)vertexIndices[corner] - (int32_t)baseIndex;

                trianglePoints[corner] = &surface->positions[vertexOffset];
                triangleLightmapCoords[corner] = &surface->lightmapCoords[vertexOffset];
            }

            for (component = 0; component < 3; ++component) {
                edge0[component] = (*trianglePoints[0])[component] - (*trianglePoints[1])[component];
                edge1[component] = (*trianglePoints[2])[component] - (*trianglePoints[1])[component];
            }
            triangleNormal[0] = edge0[1] * edge1[2] - edge0[2] * edge1[1];
            triangleNormal[1] = edge0[2] * edge1[0] - edge0[0] * edge1[2];
            triangleNormal[2] = edge0[0] * edge1[1] - edge0[1] * edge1[0];
            (void)VectorNormalize(triangleNormal);

            facing = triangleNormal[0] * projectionAxis[0][0] + triangleNormal[1] * projectionAxis[0][1] +
                     triangleNormal[2] * projectionAxis[0][2];
            if (!(facing > 0.5f))
                continue;

            for (corner = 0; corner < 3; ++corner) {
                memcpy(clipPoints[0][corner].xyz, *trianglePoints[corner], sizeof(vec3_t));
                memcpy(clipPoints[0][corner].lightmapCoords, *triangleLightmapCoords[corner], sizeof(vec2_t));
            }

            fragment = &fragmentBuffer[fragmentCount];
            if (R_AddMarkFragment(3, clipPoints, clipPlaneCount, clipPlaneNormals, clipPlaneDistances, maxPoints - outputPointCount,
            ...
            if (fragment->numPoints > 0) {
                ...
                for (corner = 0; corner < fragment->numPoints; ++corner) {
                    polyVert_t *outputPoint = &pointBuffer[outputPointCount + corner];
                    vec3_t relativePoint;

                    for (component = 0; component < 3; ++component) {
                        relativePoint[component] = outputPoint->xyz[component] - projectionOrigin[component];
                    }
                    outputPoint->st[0] =
                        0.5f + textureScale * ((relativePoint[2] * projectionAxis[1][2] + relativePoint[1] * projectionAxis[1][1]) +
                                               relativePoint[0] * projectionAxis[1][0]);
                    outputPoint->st[1] =
                        0.5f + textureScale * ((relativePoint[0] * projectionAxis[2][0] + relativePoint[2] * projectionAxis[2][2]) +
            ...
            if (fragmentCount == maxFragments || outputPointCount > maxPoints - 3) {
