...
#define R_PICK_SHADER_TRACE_DISTANCE \
    262144.0f /* 0x48800000: the positive world-bound sentinel */
                       ...
                       char *contents, int32_t bufferSize)
{
    ...
    for (int32_t component = 0; component < 3; ++component) {
            ...
            start[component] +
            direction[component] * R_PICK_SHADER_TRACE_DISTANCE;
    }

    trace.fraction = 1.0f;
    CM_BoxTrace(&trace, start, end, vec3_origin, vec3_origin,
                CM_WORLD_MODEL, R_PICK_SHADER_CONTENTS_MASK, qfalse);

    if (trace.startsolid != 0 || trace.allsolid != 0 ||
        trace.fraction == 1.0f || trace.material == NULL) {
        return qfalse;
    }
    ...
    const char *const surfaceTypeName =
        surfaceType > SURFACE_TYPE_DEFAULT &&
            ...
            ? surfaceParms[surfaceType - 1].name
            : "^1default^7";
    ...
    const char *const solidityName =
        ((uint32_t)trace.contents & CONTENTS_SOLID) != 0U
            ? "solid"
            : "^3nonsolid^7";
    ...
    for (const surfaceParm_t *parm =
             &surfaceParms[SURFACE_PARM_GENERAL_FIRST];
         parm->name != NULL; ++parm) {
        if (((uint32_t)trace.surfaceFlags &
             parm->surfaceFlags) != 0U) {
            surfaceFlags[surfaceLength++] = ' ';
            char *const destination = surfaceFlags + surfaceLength;
            strncpy(destination, parm->name,
                    (size_t)(bufferSize - surfaceLength));
            if (surfaceFlags[bufferSize - 1] != '\0') {
                return qfalse;
            }
        ...
        if (((uint32_t)trace.contents & parm->contents) != 0U) {
            ...
            strncpy(destination, parm->name,
                    (size_t)(bufferSize - contentsLength));
            if (contents[bufferSize - 1] != '\0') {
                return qfalse;
            }
