...
enum {
    ...
    R_WATER_MAX_TEXELS =
        R_WATER_MAX_TEXTURE_DIMENSION * R_WATER_MAX_TEXTURE_DIMENSION,
    R_WATER_X86_SHIFT_COUNT_MASK = 31
};

#define R_WATER_TWO_PI_F 6.2831854820251465f /* 0x40c90fdb */
#define R_WATER_FFT_ANGLE_STEP_F \
    0.024543693289160728f /* 0x3cc90fdb: 2*pi/256 */
#define R_WATER_RANDOM_UNIT_F \
    0.000030517578125f /* 0x38000000: 1/32768 */
#define R_WATER_PHASE_SCALE_F \
    162.97465515136719f /* 0x4322f983: 1024/(2*pi) */
#define R_WATER_TIME_SCALE_F \
    0.0010000000474974513f /* 0x3a83126f: milliseconds to seconds */
...
#define R_WATER_EQ_FINE_EPSILON \
    0.0010000000474974513 /* 0x3f50624de0000000 */
...
static renderer_water_complex_t
    rendererWaterFftTrig[R_WATER_FFT_TABLE_SIZE];
         ...
         int32_t stride)
{
    ...
    for (int32_t index = 0; index < count; ++index) {
        const int32_t reversed =
            rendererWaterFftBitReverse[index] >> (8 - log2Count);
    ...
    while (span < count) {
        ...
        for (int32_t twiddleIndex = 0;
             twiddleIndex < span; ++twiddleIndex) {
            const renderer_water_complex_t twiddle =
                rendererWaterFftTrig[twiddleIndex << twiddleShift];

            for (int32_t index = twiddleIndex;
                 index < count; index += doubleSpan) {
                renderer_water_complex_t *lower = &data[index * stride];
                renderer_water_complex_t *upper =
                    &data[(index + span) * stride];
                const float productReal =
                    twiddle.real * upper->real -
                    twiddle.imaginary * upper->imaginary;
                const float productImaginary =
                    twiddle.imaginary * upper->real +
                    twiddle.real * upper->imaginary;
...
void GaussianRandom(float *real, float *imaginary)
{
    ...
    do {
        x = (float)(
            (long double)coduo_crt_rand() *
                (long double)R_WATER_RANDOM_UNIT_F * 2.0L -
            1.0L);
        y =
            (long double)coduo_crt_rand() *
                (long double)R_WATER_RANDOM_UNIT_F * 2.0L -
    ...
    const long double scale =
        sqrtl((-2.0L * logl(radiusSquared)) / radiusSquared);
...
{
    const float rowWaveStep = (float)(
        (long double)R_WATER_TWO_PI_F /
        ((long double)waterMap->textureHeight *
         waterMap->horizontalWorldLength));
    const float columnWaveStep = (float)(
        (long double)R_WATER_TWO_PI_F /
        ((long double)waterMap->textureWidth *
         waterMap->verticalWorldLength));
    const long double windVelocitySquared =
        (long double)waterMap->windVelocity * waterMap->windVelocity;
    const float windLengthScaleSquared = (float)(
        (windVelocitySquared / (long double)waterMap->gravity) *
        windVelocitySquared);
    ...
    for (int32_t row = -halfHeight; row < halfHeight; ++row) {
        const float rowWaveNumber =
            (float)((long double)row * rowWaveStep);
        const float rowWaveNumberSquared = (float)(
            (long double)rowWaveNumber * rowWaveNumber);

        for (int32_t column = -halfWidth;
             column < halfWidth; ++column) {
            const float columnWaveNumber =
                (float)((long double)column * columnWaveStep);
            float gaussianReal;
            ...
            const long double waveNumberSquared =
                (long double)columnWaveNumber * columnWaveNumber +
                rowWaveNumberSquared;
            const float angularFrequencySquared = (float)(
                sqrtl(waveNumberSquared) * waterMap->gravity);
            const long double windDotWave =
                (long double)columnWaveNumber *
                    waterMap->windDirection[1] +
                (long double)rowWaveNumber *
                    waterMap->windDirection[0];

            if (!(windDotWave <= 0.0L)) {
                const long double exponential = expl(
                    -1.0L /
                    (waveNumberSquared * windLengthScaleSquared));
                const long double waveNumberSixth =
                    (waveNumberSquared * waveNumberSquared) *
                    waveNumberSquared;
                const long double spectrum =
                    (((long double)waterMap->amplitude * exponential) /
                     waveNumberSixth) *
                    (windDotWave * windDotWave);
                const long double scale =
                    (long double)waterMap->amplitude *
                    sqrtl(0.5L * spectrum);

                waterMap->initialFrequencies[frequencyIndex].real =
                    (float)((long double)gaussianReal * scale);
                waterMap->initialFrequencies[frequencyIndex].imaginary =
                    (float)((long double)gaussianImaginary * scale);
                waterMap->angularFrequencies[frequencyIndex] =
                    sqrtf(angularFrequencySquared);
            } else {
            ...
            frequencyIndex = (int32_t)(
                (uint32_t)frequencyIndex + 1u);
        }
    }
}
    ...
    for (int32_t row = -halfHeight; row < halfHeight; ++row) {
        for (int32_t column = -halfWidth;
            ...
            const float angularFrequency =
                waterMap->angularFrequencies[index];

            if (angularFrequency == 0.0f) {
                frequencies[index].real = 0.0f;
                frequencies[index].imaginary = 0.0f;
            } else {
                const long double retainedPhase =
                    (long double)timeSeconds * angularFrequency *
                    R_WATER_PHASE_SCALE_F;
                const float storedPhase = (float)retainedPhase;
                const int32_t phase =
                    coduo_x87_fistp_i32((long double)storedPhase);
                const int32_t sineIndex = (int32_t)(
                    (uint32_t)phase & R_WATER_WAVE_TABLE_MASK);
                const int32_t cosineIndex = (int32_t)(
                    ((uint32_t)phase + 255u) &
                    R_WATER_WAVE_TABLE_MASK);

                frequencies[index].real =
                    waterMap->initialFrequencies[index].real *
                    tr.sinTable[cosineIndex];
                frequencies[index].imaginary =
                    waterMap->initialFrequencies[index].imaginary *
                    tr.sinTable[sineIndex];
            }
    ...
    const shader_water_map_t *waterMap)
{
    ...
    while ((1u <<
            (rowLog2 & R_WATER_X86_SHIFT_COUNT_MASK)) !=
           (uint32_t)waterMap->textureHeight) {
        rowLog2 += 1u;
    }
    while ((1u <<
            (columnLog2 & R_WATER_X86_SHIFT_COUNT_MASK)) !=
           (uint32_t)waterMap->textureWidth) {
        columnLog2 += 1u;
    }

    for (int32_t row = 0; row < waterMap->textureHeight; ++row) {
        FFT(&amplitudes[row * waterMap->textureWidth],
            (int32_t)columnLog2, 1);
    }
    for (int32_t column = 0; column < waterMap->textureWidth; ++column) {
        FFT(&amplitudes[column], (int32_t)rowLog2,
            waterMap->textureWidth);
    }
}
...
void WaterNormalsFromAmplitudes(uint32_t *normalPixels,
                                ...
                                const shader_water_map_t *waterMap)
{
    const int32_t texelCount = (int32_t)(
        (uint32_t)waterMap->textureWidth *
        (uint32_t)waterMap->textureHeight);
    const int32_t wrappedTexelCountSquared =
        (int32_t)((uint32_t)texelCount * (uint32_t)texelCount);
    const float normalZSquared = (float)wrappedTexelCountSquared;
    const uint32_t texelMask =
        (uint32_t)texelCount - 1u;
    ...
    for (int32_t index = 0; index < texelCount; ++index) {
        ...
        amplitudes[index].real = (float)sqrtl(
            (long double)imaginary * imaginary +
            (long double)real * real);
    }

    uint32_t currentIndex = 0;
    uint32_t previousColumnIndex =
        (uint32_t)waterMap->textureWidth - 1u;
    uint32_t previousRowIndex =
        (uint32_t)texelCount - (uint32_t)waterMap->textureWidth;
    ...
    for (int32_t row = 0; row < waterMap->textureHeight; ++row) {
        ...
        do {
            const long double columnDifference =
                (long double)amplitudes[previousColumnIndex].real -
                amplitudes[currentIndex].real;
            const long double rowDifference =
                (long double)amplitudes[previousRowIndex].real -
                amplitudes[currentIndex].real;
            const long double denominator = sqrtl(
                rowDifference * rowDifference +
                (columnDifference * columnDifference +
                 (long double)normalZSquared));
            const long double scale =
                (long double)R_WATER_NORMAL_SCALE_F / denominator;
            const int32_t normalZ = coduo_x87_fistp_i32(
                (long double)texelCount * scale);
            const int32_t normalY =
                coduo_x87_fistp_i32(rowDifference * scale);
            const int32_t normalX =
                coduo_x87_fistp_i32(columnDifference * scale);
            uint32_t packedNormal =
                UINT32_C(0x0000ff7f) + (uint32_t)normalZ;
        ...
        previousColumnIndex +=
            (uint32_t)waterMap->textureWidth;
        previousRowIndex =
            (rowStart + (uint32_t)texelCount) & texelMask;
    }
}
...
{
    if (first->textureWidth != second->textureWidth ||
        first->textureHeight != second->textureHeight ||
        first->horizontalWorldLength != second->horizontalWorldLength ||
        first->verticalWorldLength != second->verticalWorldLength ||
        fabsl((long double)first->amplitude - second->amplitude) >
            R_WATER_EQ_FINE_EPSILON ||
        fabsl((long double)first->gravity - second->gravity) >
            R_WATER_EQ_SCALAR_EPSILON ||
        fabsl((long double)first->windVelocity - second->windVelocity) >
            R_WATER_EQ_SCALAR_EPSILON) {
        return qfalse;
    }

    const long double firstLengthSquared =
        (long double)first->windDirection[0] * first->windDirection[0] +
        (long double)first->windDirection[1] * first->windDirection[1];
    const long double secondLengthSquared =
        (long double)second->windDirection[0] * second->windDirection[0] +
        (long double)second->windDirection[1] * second->windDirection[1];
    const long double directionDot =
        (long double)first->windDirection[0] * second->windDirection[0] +
        (long double)first->windDirection[1] * second->windDirection[1];
    const long double directionCosine = directionDot /
        (sqrtl(firstLengthSquared * secondLengthSquared) +
         R_WATER_EQ_DIRECTION_EPSILON);

    return fabsl(directionCosine - 1.0L) <= R_WATER_EQ_FINE_EPSILON
        ? qtrue : qfalse;
}
    ...
    const shader_water_map_t *configuration)
{
    ...
    if (mapIndex == R_WATER_MAX_MAPS) {
        ri.Error(ERR_DROP,
                 "\x15map uses more than %i waterMap textures",
                 R_WATER_MAX_MAPS);
    }

    shader_water_map_t *waterMap = &rendererWaterMaps[mapIndex];
    const int32_t texelCount = (int32_t)(
        (uint32_t)configuration->textureWidth *
        (uint32_t)configuration->textureHeight);
    const uint32_t frequencyBytes =
        (uint32_t)texelCount * 8u;
    const uint32_t allocationBytes =
        (uint32_t)texelCount * 12u;
    const uint32_t normalPixelBytes =
        (uint32_t)texelCount * 4u;
    const int32_t mapByteOffset = (int32_t)(
        (uint32_t)mapIndex * R_WATER_I386_MAP_BYTES);
    ...
    *waterMap = *configuration;
    waterMap->initialFrequencies =
        ri.Hunk_Alloc((size_t)allocationBytes);
    waterMap->angularFrequencies =
        (float *)((uint8_t *)waterMap->initialFrequencies + frequencyBytes);
    PickWaterFrequencies(waterMap);

    uint8_t *normalPixels =
        ri.Hunk_AllocateTempMemory((size_t)normalPixelBytes);
    memset(normalPixels, UINT8_MAX, (size_t)normalPixelBytes);
    waterMap->image = R_CreateImage(
        va("*water%i", mapByteOffset), normalPixels,
                           ...
                           int32_t time, int32_t textureUnit)
{
    ...
    WaterFrequenciesAtTime(amplitudes, waterMap,
                           (float)time * R_WATER_TIME_SCALE_F);
    ...
    qglTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0,
                     waterMap->image->width, waterMap->image->height,
                     GL_RGBA, GL_UNSIGNED_BYTE, normalPixels);
}
