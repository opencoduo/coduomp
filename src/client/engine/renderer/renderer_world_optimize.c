...
qboolean CanOptimizeShader(const shader_t *shader)
{
    ...
    for (int32_t stageIndex = 0;
        ...
        if ((stage->flags & SHADER_STAGE_PER_LIGHT) != 0)
            continue;
        if (stage->bundle[0].textureEnvMode == 0 ||
            stage->bundle[1].textureEnvMode == 0 ||
            stage->rgbGen != firstStage->rgbGen ||
                         ...
                         renderer_shader_surface_build_t *build)
{
    if (vertexCount >= UINT16_MAX + 1) {
        ri.Error(ERR_DROP,
                 "\x15surface with shader %s has more than 65,536 vertices",
                 shader->name);
    }

    build->optimized = CanOptimizeShader(shader);
    build->vertexBytes = (size_t)(
        (uint32_t)vertexCount *
        (uint32_t)sizeof(renderer_world_interleaved_vertex_t));
    ...
    if (build->optimized != qfalse) {
        ...
        if (glConfig.vertexBufferObjectAvailable != qfalse) {
            shader->optimizedVertexStorage.glBuffer = R_CreateBufferARB(
                GL_ARRAY_BUFFER_ARB, build->vertexBytes, NULL,
                GL_STATIC_DRAW_ARB);
            ...
            build->storageMode = R_AllocMemoryNV(
                R_STATIC_VERTEX_MEMORY_PRIMARY, build->vertexBytes,
                &shader->optimizedVertexStorage.address);
        } else if (glConfig.vertexArrayObjectATIAvailable != qfalse) {
            build->storageMode = R_AllocMemoryATI(
                R_STATIC_VERTEX_MEMORY_SECONDARY, build->vertexBytes,
                &shader->optimizedVertexStorageOffset);
            if (build->storageMode == R_STATIC_VERTEX_MEMORY_PRIMARY ||
                build->storageMode == R_STATIC_VERTEX_MEMORY_SECONDARY) {
                shader->optimizedBackend = SHADER_BACKEND_OPTIMIZED_ATI;
                shader->optimizedVertexStorage =
                    build->storageMode == R_STATIC_VERTEX_MEMORY_PRIMARY
        ...
        if (shader->optimizedBackend ==
            SHADER_BACKEND_OPTIMIZED_GENERIC) {
            build->storageMode = R_STATIC_VERTEX_MEMORY_HUNK;
            shader->optimizedVertexStorage.address =
                ri.Hunk_Alloc(build->vertexBytes);
        }
    } else {
    ...
    int32_t firstVertex)
{
    for (int32_t vertexIndex = 0;
        ...
        if (surface->normals != NULL) {
            memcpy(surface->normals[vertexIndex], source->normal,
                   sizeof(surface->normals[vertexIndex]));
        }
        memcpy(surface->texCoords[vertexIndex], source->st,
               sizeof(surface->texCoords[vertexIndex]));
        memcpy(surface->lightmapCoords[vertexIndex], source->lightmap,
               sizeof(surface->lightmapCoords[vertexIndex]));

        if (lightmapPlacement != NULL) {
            surface->lightmapCoords[vertexIndex][0] = (float)(
                (long double)lightmapPlacement->sScale *
                    (long double)
                        surface->lightmapCoords[vertexIndex][0] +
                (long double)lightmapPlacement->sOffset);
            surface->lightmapCoords[vertexIndex][1] = (float)(
                (long double)lightmapPlacement->tScale *
                    (long double)
                        surface->lightmapCoords[vertexIndex][1] +
                (long double)lightmapPlacement->tOffset);
        }

        memcpy(surface->colors[vertexIndex], source->color,
               sizeof(surface->colors[vertexIndex]));
    ...
    const uint32_t vertexVec3Bytes =
        (uint32_t)surface->vertexCount *
        (uint32_t)sizeof(*surface->normals);
    const uint32_t vertexVec2Bytes =
        (uint32_t)surface->vertexCount *
        (uint32_t)sizeof(*surface->texCoords);
    memcpy(tess.stageNormals, surface->normals,
           (size_t)vertexVec3Bytes);
    memcpy(tess.texCoords[0], surface->texCoords,
           (size_t)vertexVec2Bytes);
    memcpy(tess.xyz, surface->positions,
           (size_t)vertexVec3Bytes);
    for (int32_t index = 0; index < surface->indexCount; ++index) {
        tess.indexes[index] =
            (uint16_t)(surface->indices[index] - firstVertex);
    }

    RB_CalcTangentSpace();

    memcpy(surface->tangents, tess.stageTangents,
           (size_t)vertexVec3Bytes);
    memcpy(surface->bitangents, tess.stageBitangents,
           (size_t)vertexVec3Bytes);
    ...
    int32_t indexCount, const int16_t *indices)
{
    ...
    if (vertexCount <= 0 || indexCount <= 0) {
        ri.Error(ERR_DROP, "\x15" "Invalid empty triangle soup surface");
        return qfalse;
    }
    if (indices[0] != 0) {
    ...
    const uint32_t indexBytes =
        (uint32_t)indexCount * (uint32_t)sizeof(uint16_t);
    const uint32_t vertexPayloadBytes =
        (uint32_t)vertexCount *
        (uint32_t)(
            4U * sizeof(vec3_t) + 2U * sizeof(vec2_t) +
            sizeof(uint8_t[4]));
    const uint32_t payloadBytes =
        indexBytes + vertexPayloadBytes;
    const size_t allocationBytes =
        offsetof(renderer_world_mesh_surface_t, indices) +
        (size_t)payloadBytes;
    renderer_world_mesh_surface_t *surface =
        ri.Hunk_Alloc(allocationBytes);

    uint8_t *vertexStorage =
        (uint8_t *)surface->indices + indexBytes;
    const uint32_t vertexVec3Bytes =
        (uint32_t)vertexCount * (uint32_t)sizeof(vec3_t);
    const uint32_t vertexVec2Bytes =
        (uint32_t)vertexCount * (uint32_t)sizeof(vec2_t);
    ...
    surface->surfaceType =
        (renderer_surface_type_t)shader->optimizedBackend;
    ...
    LittleVertices_T2T2C4V3(
        vertices, vertexCount, surface, lightmapPlacement,
        build->firstVertex);
    ...
    case R_SURFACE_OPTIMIZED_NV:
        OptimizeVertices_T2T2C4V3_GenericOrNV(
            ...
            (renderer_world_interleaved_vertex_t *)(void *)(
                shader->optimizedVertexStorage.address + vertexOffset));
        break;

    case R_SURFACE_OPTIMIZED_ARB:
        OptimizeVertices_T2T2C4V3_ARB(
            surface, shader->optimizedVertexStorage.glBuffer,
            vertexOffset);
        break;

    case R_SURFACE_OPTIMIZED_ATI:
        OptimizeVertices_T2T2C4V3_ATI(
            surface, shader->optimizedVertexStorage.atiObjectBuffer,
    ...
    renderer_world_interleaved_vertex_t *vertices)
{
    for (int32_t vertexIndex = 0;
        ...
        memcpy(vertices[vertexIndex].lightmapCoord,
               surface->lightmapCoords[vertexIndex],
               sizeof(vertices[vertexIndex].lightmapCoord));
        memcpy(vertices[vertexIndex].color,
               surface->colors[vertexIndex],
               sizeof(vertices[vertexIndex].color));
        memcpy(vertices[vertexIndex].position,
               surface->positions[vertexIndex],
               sizeof(vertices[vertexIndex].position));
    }
}
    ...
    uint32_t objectBuffer, size_t vertexOffset)
{
    const uint32_t vertexBytes =
        ...
        (uint32_t)sizeof(renderer_world_interleaved_vertex_t);
    renderer_world_interleaved_vertex_t *vertices =
        ri.Hunk_AllocateTempMemory((size_t)vertexBytes);
    ...
    uint32_t vertexBuffer, size_t vertexOffset)
{
    ...
    qglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, (intptr_t)vertexOffset,
                        (intptr_t)vertexBytes, vertices);
...
void R_RefreshOptimizedWorldSurfaces_ARB(void)
{
    ...
    for (int32_t surfaceIndex = 0;
         surfaceIndex < tr.world->numsurfaces; ++surfaceIndex) {
        msurface_t *worldSurface =
            &tr.world->surfaces[surfaceIndex];
        renderer_world_mesh_surface_t *surface =
        ...
        OptimizeVertices_T2T2C4V3_ARB(
            surface, worldSurface->shader->optimizedVertexStorage.glBuffer,
            (size_t)firstVertex *
                sizeof(renderer_world_interleaved_vertex_t));
    }
}
...
void R_IncrementalRefreshOptimizedWorldSurfaces_ARB(void)
{
    ...
    for (;;) {
        ...
        msurface_t *worldSurface =
            &tr.world->surfaces[tr.worldRefreshSurfaceIndex];
        renderer_world_mesh_surface_t *surface =
            (renderer_world_mesh_surface_t *)worldSurface->data;
        if (surface->surfaceType != R_SURFACE_OPTIMIZED_ARB)
            continue;
                  ...
                  const msurface_t *worldSurface)
{
    ...
    const uint32_t colorBytes =
        (uint32_t)surface->vertexCount *
        (uint32_t)sizeof(*surface->colors);
