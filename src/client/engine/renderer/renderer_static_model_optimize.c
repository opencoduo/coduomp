...
typedef union renderer_static_model_vertex_staging_u {
    renderer_static_model_t2v3_vertex_t t2v3[R_STATIC_MODEL_VERTEX_STAGING_COUNT];
    renderer_static_model_t2n3v3_vertex_t t2n3v3[R_STATIC_MODEL_VERTEX_STAGING_COUNT];
...
{
    return (size + (R_STATIC_VERTEX_MEMORY_ALIGNMENT - 1U)) & ~(size_t)(R_STATIC_VERTEX_MEMORY_ALIGNMENT - 1U);
}
...
void R_OptimizeSModelSurfGeneric(renderer_static_model_surface_t *surface)
{
    ...
    if ((surfaceFlags & R_SMODEL_T2V3_SURFACE_FLAGS_MASK) == 0) {
        ...
        renderer_static_model_t2v3_vertex_t *optimized = ri.Hunk_Alloc((size_t)surface->vertexCount * sizeof(*optimized));
        surface->optimized.vertices = (uint8_t *)optimized;

        for (uint16_t vertexIndex = 0; vertexIndex < surface->vertexCount; ++vertexIndex) {
            memcpy(optimized[vertexIndex].texCoord, surface->texCoords[vertexIndex], sizeof(optimized[vertexIndex].texCoord));
            memcpy(optimized[vertexIndex].position, surface->vertices[vertexIndex], sizeof(optimized[vertexIndex].position));
        }
        return;
    }
    ...
    if ((surfaceFlags & R_SMODEL_T2N3V3_SURFACE_FLAGS_MASK) == 0) {
        ...
        renderer_static_model_t2n3v3_vertex_t *optimized = ri.Hunk_Alloc((size_t)surface->vertexCount * sizeof(*optimized));
        surface->optimized.vertices = (uint8_t *)optimized;

        for (uint16_t vertexIndex = 0; vertexIndex < surface->vertexCount; ++vertexIndex) {
            memcpy(optimized[vertexIndex].texCoord, surface->texCoords[vertexIndex], sizeof(optimized[vertexIndex].texCoord));
            memcpy(optimized[vertexIndex].normal, surface->normals[vertexIndex], sizeof(optimized[vertexIndex].normal));
            memcpy(optimized[vertexIndex].position, surface->vertices[vertexIndex], sizeof(optimized[vertexIndex].position));
        }
    }
}
...
void R_OptimizeSModelSurfNV(renderer_static_model_surface_t *surface)
{
    ...
    if ((surfaceFlags & R_SMODEL_T2V3_SURFACE_FLAGS_MASK) == 0) {
        ...
        surface->storageSource =
            R_AllocMemoryNV(R_STATIC_VERTEX_MEMORY_PRIMARY, (size_t)surface->vertexCount * sizeof(renderer_static_model_t2v3_vertex_t),
                            &surface->optimized.vertices);

        renderer_static_model_t2v3_vertex_t *optimized = (renderer_static_model_t2v3_vertex_t *)surface->optimized.vertices;
        for (uint16_t vertexIndex = 0; vertexIndex < surface->vertexCount; ++vertexIndex) {
            memcpy(optimized[vertexIndex].texCoord, surface->texCoords[vertexIndex], sizeof(optimized[vertexIndex].texCoord));
            memcpy(optimized[vertexIndex].position, surface->vertices[vertexIndex], sizeof(optimized[vertexIndex].position));
        }
    ...
    if ((surfaceFlags & R_SMODEL_T2N3V3_SURFACE_FLAGS_MASK) == 0) {
        ...
        surface->storageSource =
            R_AllocMemoryNV(R_STATIC_VERTEX_MEMORY_PRIMARY, (size_t)surface->vertexCount * sizeof(renderer_static_model_t2n3v3_vertex_t),
                            &surface->optimized.vertices);

        renderer_static_model_t2n3v3_vertex_t *optimized = (renderer_static_model_t2n3v3_vertex_t *)surface->optimized.vertices;
        for (uint16_t vertexIndex = 0; vertexIndex < surface->vertexCount; ++vertexIndex) {
            memcpy(optimized[vertexIndex].texCoord, surface->texCoords[vertexIndex], sizeof(optimized[vertexIndex].texCoord));
            memcpy(optimized[vertexIndex].normal, surface->normals[vertexIndex], sizeof(optimized[vertexIndex].normal));
            memcpy(optimized[vertexIndex].position, surface->vertices[vertexIndex], sizeof(optimized[vertexIndex].position));
...
qboolean R_OptimizeSModelSurfATI(renderer_static_model_surface_t *surface)
{
    ...
    const uint32_t surfaceFlags = surface->shader->surfaceFlags;
    const size_t alignedIndexBytes = coduomp_align_static_model_bytes((size_t)surface->indexCount * sizeof(*surface->indices));
    size_t vertexBytes;
    ...
    const size_t allocationBytes = vertexBytes + (glConfig.elementArrayATIAvailable ? alignedIndexBytes : 0U);
    size_t allocationOffset = 0;
    surface->storageSource = R_AllocMemoryATI(R_STATIC_VERTEX_MEMORY_PRIMARY, allocationBytes, &allocationOffset);
    if (surface->storageSource == R_STATIC_VERTEX_MEMORY_NONE)
        return qfalse;

    const renderer_static_vertex_memory_base_t bufferBase =
        surface->storageSource == R_STATIC_VERTEX_MEMORY_PRIMARY ? tr.staticVertexMemoryPrimary : tr.staticVertexMemorySecondary;
    surface->optimized.atiObjectBuffer = bufferBase.atiObjectBuffer;
    surface->storageSource = R_STATIC_VERTEX_MEMORY_HUNK;

    if (glConfig.elementArrayATIAvailable) {
        surface->backend.ati.vertexOffset = (uint32_t)(allocationOffset + alignedIndexBytes);
        surface->backend.ati.indexOffset = (uint32_t)allocationOffset;
        qglUpdateObjectBufferATI(surface->optimized.atiObjectBuffer, surface->backend.ati.indexOffset,
                                 (int32_t)((size_t)surface->indexCount * sizeof(*surface->indices)), surface->indices, GL_PRESERVE_ATI);
    } else {
    ...
    qglUpdateObjectBufferATI(surface->optimized.atiObjectBuffer, surface->backend.ati.vertexOffset, (int32_t)vertexBytes, &staging,
                             GL_PRESERVE_ATI);
    return qtrue;
}
...
qboolean R_OptimizeSModelSurfARB(renderer_static_model_surface_t *surface)
{
    ...
    surface->backend.arb.indexBuffer =
        R_CreateBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, alignedIndexBytes, surface->indices, GL_STATIC_DRAW_ARB);
    ...
    surface->optimized.vertexBuffer = R_CreateBufferARB(GL_ARRAY_BUFFER_ARB, vertexBytes, &staging, GL_STATIC_DRAW_ARB);
