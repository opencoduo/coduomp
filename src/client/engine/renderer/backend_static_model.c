...
static void RB_SetStaticModelShowColor(
    int32_t indexCount, rb_static_model_show_color_t colorType)
{
    ...
    if (colorType == RB_STATIC_MODEL_SHOW_COLOR_T2V3) {
        qglColor3f(tr.identityLight * 0.5f,
                   tr.identityLight * 0.21875f,
                   tr.identityLight);
    } else {
        qglColor3f(tr.identityLight * 0.25f,
                   tr.identityLight * 0.25f,
                   tr.identityLight);
    }
}
...
static void RB_DrawStaticModelPointer(
    renderer_static_model_surface_t *surface, qboolean hasNormals,
    qboolean colorsFromStage, const char *logMessage)
{
    const uint8_t *vertices = surface->optimized.vertices;
    const int32_t vertexStride =
        hasNormals ? (int32_t)sizeof(renderer_static_model_t2n3v3_vertex_t)
                   : (int32_t)sizeof(renderer_static_model_t2v3_vertex_t);
    ...
    GL_ClientState(GLS_CLIENT_TEXCOORD0_ARRAY |
                   (hasNormals ? GLS_CLIENT_NORMAL_ARRAY : 0) |
    ...
    if (hasNormals) {
        qglNormalPointer(
            ...
            vertices +
                offsetof(renderer_static_model_t2n3v3_vertex_t, normal));
        qglVertexPointer(
            3, GL_FLOAT, vertexStride,
            vertices +
                offsetof(renderer_static_model_t2n3v3_vertex_t, position));
    } else {
        qglVertexPointer(
            3, GL_FLOAT, vertexStride,
            vertices +
                offsetof(renderer_static_model_t2v3_vertex_t, position));
    }
    ...
    for (int32_t stageIndex = 0;
        ...
        GL_DrawElements(GL_TRIANGLES, surface->indexCount,
                        GL_UNSIGNED_SHORT, surface->indices);
    }
    ...
    if (r_showtris->integer != 0) {
        ...
        RB_SetStaticModelShowColor(
            surface->indexCount,
            colorsFromStage ? RB_STATIC_MODEL_SHOW_COLOR_T2V3
                            : RB_STATIC_MODEL_SHOW_COLOR_BLUE);
        GL_DrawElements(GL_TRIANGLES, surface->indexCount,
                        GL_UNSIGNED_SHORT, surface->indices);
...
static void RB_DrawStaticModelARB(
    renderer_static_model_surface_t *surface, qboolean hasNormals,
    const char *logMessage)
{
    ...
    if (r_showtris->integer != 0) {
        ...
        RB_BindStaticModelShowStage(
            tr.showTrisShader->stages[0]);
        RB_SetStaticModelShowColor(
            surface->indexCount, RB_STATIC_MODEL_SHOW_COLOR_BLUE);
        GL_DrawElements(GL_TRIANGLES, surface->indexCount,
                        GL_UNSIGNED_SHORT, NULL);
        qglDepthRange(0.0, 1.0);
    }
...
void RB_SurfaceStaticModel(renderer_surface_t *surfaceData)
{
    renderer_static_model_surface_t *surface =
        (renderer_static_model_surface_t *)surfaceData;
    if (r_logFile->integer != 0) {
        GLimp_LogComment(
            va("--- RB_SurfaceStaticModel( %s ) ---\n",
               tess.shader->name));
    }

    const int32_t firstVertex = tess.vertexCount;
    const uint32_t firstComponent = (uint32_t)firstVertex *
                                    (uint32_t)tess.vertexComponentCount;
    memcpy(&tess.xyz[firstComponent],
           surface->vertices,
           (size_t)surface->vertexCount * sizeof(surface->vertices[0]));
    memcpy(&tess.texCoords[R_TESS_BASE_TEXCOORD_SET][firstVertex],
           surface->texCoords,
    ...
    if ((tess.shader->surfaceFlags &
        ...
        memcpy(&tess.stageNormals[firstVertex], surface->normals,
               (size_t)surface->vertexCount *
                   sizeof(surface->normals[0]));
        tess.requiresVertexBasis = qtrue;
    }

    for (uint16_t index = 0; index < surface->indexCount; ++index) {
        const uint32_t destinationIndex =
            (uint32_t)tess.indexCount + (uint32_t)index;
        tess.indexes[destinationIndex] = (uint16_t)(
            (uint32_t)surface->indices[index] +
            (uint32_t)firstVertex);
    }
    tess.indexCount = (int32_t)(
        (uint32_t)tess.indexCount + (uint32_t)surface->indexCount);
    tess.vertexCount = (int32_t)(
        (uint32_t)tess.vertexCount + (uint32_t)surface->vertexCount);
}
...
{
    renderer_cached_static_model_surface_t *cachedSurface =
        (renderer_cached_static_model_surface_t *)surfaceData;
    if (r_logFile->integer != 0) {
        GLimp_LogComment(
            va("--- RB_SurfaceStaticModelCached( %s ) ---\n",
               tess.shader->name));
    }
...
void RB_SurfaceStaticModelT2V3_ARB(renderer_surface_t *surfaceData)
{
    renderer_static_model_surface_t *surface =
        (renderer_static_model_surface_t *)surfaceData;
    RB_DrawStaticModelARB(
        surface, qfalse,
        "--- RB_SurfaceStaticModelT2V3_ARB ---\n");
}
...
void RB_SurfaceStaticModelT2N3V3_ARB(renderer_surface_t *surfaceData)
{
    ...
    RB_DrawStaticModelARB(
        surface, qtrue,
        "--- RB_SurfaceStaticModelT2N3V3_ARB ---\n");
}
...
void RB_SurfaceStaticModelATI(renderer_surface_t *surfaceData)
{
    ...
    if (hasNormals) {
        qglArrayObjectATI(
            GL_NORMAL_ARRAY, 3, GL_FLOAT, vertexStride,
            objectBuffer,
            vertexOffset +
                (uint32_t)offsetof(
                    renderer_static_model_t2n3v3_vertex_t, normal));
        qglArrayObjectATI(
            GL_VERTEX_ARRAY, 3, GL_FLOAT, vertexStride,
            objectBuffer,
            vertexOffset +
                (uint32_t)offsetof(
                    renderer_static_model_t2n3v3_vertex_t, position));
    } else {
        qglArrayObjectATI(
            GL_VERTEX_ARRAY, 3, GL_FLOAT, vertexStride,
            objectBuffer,
            vertexOffset +
                (uint32_t)offsetof(
                    renderer_static_model_t2v3_vertex_t, position));
    }

    if (glConfig.elementArrayATIAvailable != qfalse) {
        qglEnable(GL_ELEMENT_ARRAY_ATI);
        qglArrayObjectATI(
            GL_ELEMENT_ARRAY_ATI, 1, GL_UNSIGNED_SHORT, 0,
            objectBuffer, surface->backend.ati.indexOffset);
    }
    ...
    for (int32_t stageIndex = 0;
        ...
        if (!hasNormals)
            RB_SetStaticModelStageColor(stage);
        GL_State(stage->stateBits);
        RB_SetupMultitextureATI(stage, objectBuffer,
                                texCoordOffsets, vertexStride);

        if (glConfig.elementArrayATIAvailable != qfalse) {
            GL_DrawElementArrayATI(GL_TRIANGLES,
                                   surface->indexCount);
        } else {
            GL_DrawElements(GL_TRIANGLES, surface->indexCount,
                            GL_UNSIGNED_SHORT, surface->indices);
    ...
    if (r_showtris->integer != 0) {
        ...
        RB_SetupMultitextureATI(stage, objectBuffer,
                                texCoordOffsets, vertexStride);
        RB_SetStaticModelShowColor(
            surface->indexCount, RB_STATIC_MODEL_SHOW_COLOR_T2V3);
...
void RB_SurfaceStaticModelT2V3_NV(renderer_surface_t *surfaceData)
{
    renderer_static_model_surface_t *surface =
        (renderer_static_model_surface_t *)surfaceData;
    RB_DrawStaticModelPointer(
        surface, qfalse, qtrue,
        "--- RB_SurfaceStaticModelT2V3_NV ---\n");
}
...
void RB_SurfaceStaticModelT2N3V3_NV(renderer_surface_t *surfaceData)
{
    ...
    RB_DrawStaticModelPointer(
        surface, qtrue, qfalse,
        "--- RB_SurfaceStaticModelT2N3V3_NV ---\n");
}
