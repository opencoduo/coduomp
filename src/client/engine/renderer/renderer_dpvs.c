...
static const vec4_t visibleSurfaceDebugColor =
    {0.0f, 1.0f, 0.0f, 1.0f}; /* original 0x0058fbb8 */
static const vec4_t portalDebugColor =
    {0.0f, 1.0f, 1.0f, 0.25f}; /* original 0x00591380 */
static const vec4_t cullGroupDebugColor =
    {0.75f, 0.75f, 0.0f, 1.0f}; /* original 0x0058fbf8 */
static const vec4_t staticModelDebugColor =
    {0.75f, 0.75f, 0.75f, 1.0f}; /* original 0x0058fc78 */
static const vec4_t modelBoundsDebugColor =
    {0.0f, 1.0f, 1.0f, 1.0f}; /* original 0x0058fc28 */
static const vec4_t acceptedTreeDebugColor =
    {1.0f, 1.0f, 0.0f, 1.0f}; /* original 0x0058fbe8 */
static const vec4_t clippedLeafDebugColor =
    {1.0f, 0.69999998807907104f, 0.0f, 1.0f}; /* original 0x0058fca8 */
static const vec4_t culledEntityDebugColor =
    {1.0f, 0.0f, 0.0f, 1.0f}; /* original 0x0058fba8 */
static const vec4_t visibleEntityDebugColor =
    {0.0f, 1.0f, 0.0f, 1.0f}; /* original 0x0058fbb8 */
static const vec4_t acceptedBevelDebugColor =
    {0.0f, 0.75f, 0.75f, 1.0f}; /* original 0x0058fc38 */
static const vec4_t rejectedBevelDebugColor =
    {0.0f, 0.5f, 0.5f, 1.0f}; /* original 0x0058fc48 */
...
void R_SetupDPVSFrustumPlanes(void)
{
    for (int32_t planeIndex = 0;
         ...
         ++planeIndex) {
        renderer_dpvs_plane_t *dpvsPlane =
            &rendererDpvsFrustumPlanes[planeIndex];
        const renderer_frustum_plane_t *viewPlane =
...
{
    float combined[4][4];
    const float (*worldMatrix)[4] =
        (const float (*)[4])tr.viewParms.world.modelMatrix;
    const float (*projectionMatrix)[4] =
        (const float (*)[4])tr.viewParms.projectionMatrix;

    for (int32_t column = 0; column < 4; ++column) {
        for (int32_t row = 0; row < 4; ++row) {
            combined[row][column] = (float)(
                (((long double)worldMatrix[row][3] *
                       projectionMatrix[3][column] +
                   (long double)worldMatrix[row][0] *
                       projectionMatrix[0][column]) +
                  (long double)worldMatrix[row][1] *
                      projectionMatrix[1][column]) +
                 (long double)projectionMatrix[2][column] *
                     worldMatrix[row][2]);
        }
    }
    ...
    for (int32_t row = 0; row < 4; ++row) {
        for (int32_t column = 0; column < 4; ++column) {
            rendererDpvsWorldViewProjectionMatrix[row][column] =
                combined[column][row];
        }
    }
}
...
{
    static const float nearPlaneOffset =
        0.10000000149011612f; /* 0x3dcccccd */
    static const float planeEpsilon =
    ...
    rendererDpvsNearPlane.normal[0] =
        tr.viewParms.orientation.axis[0][0];
    rendererDpvsNearPlane.normal[1] =
        tr.viewParms.orientation.axis[0][1];
    rendererDpvsNearPlane.normal[2] =
        tr.viewParms.orientation.axis[0][2];
    rendererDpvsNearPlane.distance = (float)(
        (((long double)rendererDpvsNearPlane.normal[2] *
              rendererDpvsViewOrigin[2] +
          (long double)rendererDpvsNearPlane.normal[1] *
              rendererDpvsViewOrigin[1]) +
         (long double)rendererDpvsNearPlane.normal[0] *
             rendererDpvsViewOrigin[0]) -
    ...
    rendererDpvsActiveNearPlane = tr.viewParms.isMirror != qfalse
        ? &tr.viewParms.portalPlane
        : &rendererDpvsNearPlane;

    const float farPlaneDistance = RE_GetFarPlaneDist();
    if (farPlaneDistance > 0.0f) {
        rendererDpvsFarPlane.normal[0] =
            -tr.viewParms.orientation.axis[0][0];
        rendererDpvsFarPlane.normal[1] =
            -tr.viewParms.orientation.axis[0][1];
        rendererDpvsFarPlane.normal[2] =
            -tr.viewParms.orientation.axis[0][2];
        rendererDpvsFarPlane.distance = (float)(
            (((long double)rendererDpvsFarPlane.normal[2] *
                  rendererDpvsViewOrigin[2] +
              (long double)rendererDpvsFarPlane.normal[1] *
                  rendererDpvsViewOrigin[1]) +
             (long double)rendererDpvsFarPlane.normal[0] *
                 rendererDpvsViewOrigin[0]) -
...
{
    const int32_t xBound =
        plane->sideOffsets[0] == R_DPVS_SIDE_X_POSITIVE_OFFSET ? 0 : 1;
    const int32_t yBound =
        plane->sideOffsets[1] == R_DPVS_SIDE_Y_POSITIVE_OFFSET ? 0 : 1;
    const int32_t zBound =
        plane->sideOffsets[2] == R_DPVS_SIDE_Z_POSITIVE_OFFSET ? 0 : 1;
    const long double cornerDistance =
        (long double)plane->normal[2] * bounds[zBound][2] +
        (long double)plane->normal[1] * bounds[yBound][1] +
        (long double)plane->normal[0] * bounds[xBound][0];
                       ...
                       int32_t planeCount, int32_t planeIndex)
{
    ...
    if (rendererDpvsActiveFarPlane != NULL &&
        R_BoxBehindPlane(bounds, rendererDpvsActiveFarPlane)) {
        return qtrue;
    }

    for (int32_t occluderIndex = 0;
         occluderIndex < rendererDpvsOccluderCount; ++occluderIndex) {
        renderer_occluder_t *occluder =
            rendererDpvsOccluders[occluderIndex];
        if (planeIndex >= occluder->cullPlaneLimit)
            continue;

        occluder->cullPlaneLimit = INT32_MAX;
        int32_t activePlaneIndex;
        for (activePlaneIndex = 0;
             activePlaneIndex < occluder->activePlaneCount;
             ++activePlaneIndex) {
            /*
             ...
             */
            if (!R_BoxBehindPlane(
                    bounds, &occluder->activePlanes[activePlaneIndex])) {
                break;
            }
        }
                             ...
                             int32_t planeCount, int32_t planeIndex)
{
    ...
    for (int32_t occluderIndex = 0;
        ...
        for (activePlaneIndex = 0;
             ...
             ++activePlaneIndex) {
            const renderer_dpvs_plane_t *plane =
                &occluder->activePlanes[activePlaneIndex];
            const int32_t xBound =
                plane->sideOffsets[0] == R_DPVS_SIDE_X_POSITIVE_OFFSET
                    ? 0 : 1;
            const int32_t yBound =
...
void R_AddSkySurfacesDPVS(void)
{
    ...
    for (int32_t surfaceIndex = 0;
         surfaceIndex < tr.world->skySurfaceCount;
         ++surfaceIndex) {
        msurface_t *worldSurface =
            tr.world->skySurfaces[surfaceIndex];
        renderer_lit_surface_t *surface =
...
{
    static const float coronaBoundsScale =
        1.7320507764816284f; /* 0x3fddb3d7, nominally sqrt(3) */
    ...
    for (int32_t coronaIndex = 0;
         coronaIndex < tr.world->coronaCount;
         ++coronaIndex) {
        const renderer_world_corona_t *corona =
            &tr.world->coronas[coronaIndex];
        vec3_t viewDirection;
        vec3_t bounds[2];

        viewDirection[0] = tr.viewParms.orientation.origin[0] -
                           corona->origin[0];
        viewDirection[1] = tr.viewParms.orientation.origin[1] -
                           corona->origin[1];
        viewDirection[2] = tr.viewParms.orientation.origin[2] -
                           corona->origin[2];
        ...
        const long double boundsRadiusRaw =
            (long double)corona->scale * coronaBoundsScale;
        for (int32_t component = 0; component < 3; ++component) {
            bounds[0][component] = (float)(
                (long double)corona->origin[component] - boundsRadiusRaw);
            bounds[1][component] = (float)(
                boundsRadiusRaw + (long double)corona->origin[component]);
        }
        ...
        for (int32_t component = 0; component < 3; ++component) {
            entity.origin[component] = (float)(
                (long double)viewDirection[component] * coronaViewOffset +
                (long double)corona->origin[component]);
        }
        ...
        if (viewDirection[2] > corona->zFadeOut) {
            ...
            const long double fadedAlphaRaw =
                (long double)corona->color[3] *
                ((long double)corona->zCutoff - viewDirection[2]) /
                ((long double)corona->zCutoff - corona->zFadeOut);
            entity.shaderRGBA[3] =
                coduo_fp_to_u8_extended(fadedAlphaRaw);
        }
...
int32_t R_CellForCamera(void)
{
    ...
    if (tr.world == NULL)
        ri.Error(ERR_DROP, "\x15" "R_CellForCamera: bad model");

    node = tr.world->nodes;
    while (node->contents == R_WORLD_NODE_NO_CELL) {
        const cplane_t *plane = node->data.node.plane;
        const long double distance =
            (((long double)plane->normal[2] * rendererDpvsViewOrigin[2] +
              (long double)plane->normal[1] * rendererDpvsViewOrigin[1]) +
                                ...
                                int32_t planeCount)
{
    ...
    while (planesRemaining != 0) {
        ...
        while (verticesRemaining != 0) {
            const long double distance =
                (long double)planes->normal[2] *
                    (long double)(*vertex)[2] +
                (long double)planes->normal[1] *
                    (long double)(*vertex)[1] +
                (long double)planes->normal[0] *
                    (long double)(*vertex)[0];
                            ...
                            vec3_t *outputVertices)
{
    ...
    if (inputCount < 0 || inputCount > R_PORTAL_CLIP_VERTEX_CAPACITY) {
        ri.Error(ERR_DROP,
                 "R_ChopPortalWinding: invalid input vertex count %i",
                 inputCount);
        *vertexCount = 0;
        return NULL;
    }
    ...
    for (int32_t vertexIndex = 0;
        ...
        const long double distanceRaw =
            (long double)plane->normal[2] * (*vertex)[2] +
            (long double)plane->normal[0] * (*vertex)[0] +
            (long double)plane->normal[1] * (*vertex)[1] -
    ...
    for (int32_t vertexIndex = 0;
        ...
        if (side == R_PORTAL_CLIP_ON) {
            ...
            if (outputCount >= R_PORTAL_CLIP_VERTEX_CAPACITY) {
                ri.Error(ERR_DROP,
                         "R_ChopPortalWinding: clipped portal exceeds %i vertices",
                         R_PORTAL_CLIP_VERTEX_CAPACITY);
                *vertexCount = 0;
                return NULL;
            }
            memcpy(outputVertices[outputCount], inputVertices[vertexIndex],
                   sizeof(vec3_t));
            ++outputCount;
            /*
        ...
        if (nextSide != R_PORTAL_CLIP_ON && nextSide != side) {
            ...
            const int32_t nextIndex =
                (vertexIndex + 1) % inputCount;
            const long double fraction =
                (long double)distances[vertexIndex] /
                ((long double)distances[vertexIndex] -
                 distances[vertexIndex + 1]);

            for (int32_t component = 0; component < 3; ++component) {
                outputVertices[outputCount][component] = (float)(
                    ((long double)inputVertices[nextIndex][component] -
                     inputVertices[vertexIndex][component]) * fraction +
    ...
    int32_t *outPlaneCount)
{
    ...
    *outPlaneCount = portal->vertexCount;
    vertices = R_ChopPortalWinding(
        portalPlane, portal->vertices, outPlaneCount, clipVertices[0]);
    if (*outPlaneCount == 0)
        return NULL;

    const long double cameraPlaneDistance =
        (long double)portal->plane.distance -
        ((long double)rendererDpvsViewOrigin[2] *
             portal->plane.normal[2] +
         (long double)rendererDpvsViewOrigin[1] *
             portal->plane.normal[1] +
         (long double)rendererDpvsViewOrigin[0] *
             portal->plane.normal[0]);
    /*
    ...
    if (cameraPlaneDistance < (long double)nearPortalDistance) {
        const uint32_t planeBytes =
            (uint32_t)planeCount * 20u;
        renderer_dpvs_plane_t *planeCopy =
            ri.Hunk_AllocateTempMemory((size_t)planeBytes);
    ...
    if (rendererDpvsActiveFarPlane != NULL) {
        vec3_t *output = vertices == clipVertices[0]
            ? clipVertices[1] : clipVertices[0];
        vertices = R_ChopPortalWinding(rendererDpvsActiveFarPlane,
                                       vertices, outPlaneCount, output);
        if (*outPlaneCount == 0)
            return NULL;
    }
    ...
    for (int32_t planeIndex = 0;
        ...
        vec3_t *output = vertices == clipVertices[0]
            ? clipVertices[1] : clipVertices[0];
        vertices = R_ChopPortalWinding(&planes[planeIndex],
                                       vertices, outPlaneCount, output);
        if (*outPlaneCount == 0)
            return NULL;
    }
    ...
    if (r_showportals->integer != 0) {
        R_AddDebugPolygon(portalDebugColor, *outPlaneCount,
                          (const vec3_t *)vertices);
    }

    return R_PortalClipPlanesInternal(vertices, outPlaneCount,
                                      *outPlaneCount);
}
                           ...
                           uint32_t dlightBits)
{
    ...
    for (uint32_t portalsRemaining = (uint32_t)cell->portalCount;
         portalsRemaining != 0;
         --portalsRemaining, ++portal) {
        ...
        const long double cameraDistance =
            ...
            (long double)rendererDpvsViewOrigin[0] *
                portal->plane.normal[0];
        if (cameraDistance >
            (long double)portal->plane.distance + planeEpsilon) {
            continue;
        }
        ...
        for (int32_t occluderIndex = 0;
             occluderIndex < rendererDpvsOccluderCount;
             ++occluderIndex) {
            const renderer_occluder_t *occluder =
                rendererDpvsOccluders[occluderIndex];
            if (R_PortalBehindAllPlanes(
                    portal, occluder->activePlanes,
                    occluder->activePlaneCount)) {
                occluded = qtrue;
                break;
            }
        }
        ...
        renderer_dpvs_plane_t *clippedPlanes =
            R_PortalClipPlanes(portal, sourcePlane, planes,
                               planeCount, &clippedPlaneCount);
        if (clippedPlaneCount == 0)
            continue;

        portal->recursionActive = qtrue;
        R_RecursivePortalWalk(portal->cell, &portal->plane,
                              clippedPlanes, clippedPlaneCount,
                              dlightBits);
                       ...
                       int32_t planeCount, uint32_t dlightBits)
{
    ...
    if (tree->childCount != 0) {
        for (int32_t childIndex = 0;
             ...
             ++childIndex) {
            R_AddAABBTreeSurfaces_r(&tree->children[childIndex],
                                    planes, planeCount,
                                    dlightBits, 0);
        }
    } else {
        for (int32_t surfaceIndex = 0;
             ...
             ++surfaceIndex) {
            R_AddTrianglesSurface(&tree->surfaces[surfaceIndex],
                                  dlightBits, planes,
                                  planeCount, 0);
        }
    }
...
{
    for (int32_t vertexIndex = 0;
         vertexIndex < occluder->vertexCount;
         ++vertexIndex) {
                        ...
                        int32_t planeCount)
{
    ...
    for (int32_t occluderIndex = 0;
        ...
        if (R_CullOccluderByPlane(occluder,
                                  rendererDpvsActiveNearPlane)) {
            continue;
        }
        ...
        if (rendererDpvsOccluderCount == R_DPVS_MAX_ACTIVE_OCCLUDERS) {
            ri.Error(ERR_DROP,
                     "\x15Too many active occluder brushes");
        }
        rendererDpvsOccluders[rendererDpvsOccluderCount] = occluder;
        rendererDpvsOccluderCount = (int32_t)(
            (uint32_t)rendererDpvsOccluderCount + 1u);

        occluder->cullPlaneLimit = INT32_MAX;
        occluder->activePlaneCount = rendererDpvsActivePlaneCount;
        occluder->activePlanes =
            &rendererDpvsActivePlanes[rendererDpvsActivePlaneCount];

        for (int32_t authoredPlaneIndex = 0;
             authoredPlaneIndex < occluder->planeCount;
             ++authoredPlaneIndex) {
            renderer_dpvs_plane_t *plane =
                &occluder->planes[authoredPlaneIndex];
            ...
            if (cameraDistance > plane->distance) {
                ...
                if (rendererDpvsActivePlaneCount ==
                    R_DPVS_MAX_ACTIVE_OCCLUDER_PLANES) {
                    ri.Error(ERR_DROP,
                             "\x15Too many active occluder faces");
                }
                rendererDpvsActivePlanes[rendererDpvsActivePlaneCount] =
                    *plane;
                rendererDpvsActivePlaneCount = (int32_t)(
                    (uint32_t)rendererDpvsActivePlaneCount + 1u);
            }
        }

        for (int32_t edgeIndex = 0;
             edgeIndex < occluder->edgeCount;
             ++edgeIndex) {
            ...
            vec3_t first;
            vec3_t second;
            const vec3_t *firstVertex =
                edge->vertices[cameraSide == R_DPVS_CAMERA_BEHIND_PLANE];
            const vec3_t *secondVertex = edge->vertices[cameraSide];
            for (int32_t component = 0; component < 3; ++component) {
                first[component] =
                    (*firstVertex)[component] -
                    rendererDpvsViewOrigin[component];
                second[component] =
            ...
            renderer_dpvs_plane_t *activePlane =
                &rendererDpvsActivePlanes[rendererDpvsActivePlaneCount];
            activePlane->normal[0] = (float)(
                (long double)second[1] * first[2] -
                (long double)second[2] * first[1]);
            activePlane->normal[1] = (float)(
                (long double)second[2] * first[0] -
                (long double)second[0] * first[2]);
            activePlane->normal[2] = (float)(
                (long double)second[0] * first[1] -
                (long double)second[1] * first[0]);
            VectorNormalizeFast(activePlane->normal);
            activePlane->distance = (float)(
                ((long double)rendererDpvsViewOrigin[2] *
                     activePlane->normal[2] +
                 (long double)rendererDpvsViewOrigin[1] *
                     activePlane->normal[1]) +
                (long double)rendererDpvsViewOrigin[0] *
                    activePlane->normal[0]);
            R_SetPlaneSidesDPVS(activePlane);
            rendererDpvsActivePlaneCount = (int32_t)(
                (uint32_t)rendererDpvsActivePlaneCount + 1u);
        }

        occluder->activePlaneCount = (int32_t)(
            (uint32_t)rendererDpvsActivePlaneCount -
            (uint32_t)occluder->activePlaneCount);
    }
}
...
static void R_XModelWorldBounds(const trRefEntity_t *entity,
                                vec3_t mins, vec3_t maxs)
{
    ...
    for (int32_t axisIndex = 0; axisIndex < 3; ++axisIndex) {
        ...
        axisLengthSquared[axisIndex] = (float)(
            ((long double)(*axis)[0] * (*axis)[0] +
             (long double)(*axis)[1] * (*axis)[1]) +
            (long double)(*axis)[2] * (*axis)[2]);
    }
    ...
    const long double boundsScaleRaw =
        sqrtl((long double)maximumAxisLengthSquared);
    const XModelInfo *model =
        entity->e.dobj->models[0]->info;

    for (int32_t component = 0; component < 3; ++component) {
        mins[component] = (float)(
            (long double)model->mins[component] * boundsScaleRaw +
            (long double)entity->e.origin[component]);
        maxs[component] = (float)(
            (long double)model->maxs[component] * boundsScaleRaw +
            (long double)entity->e.origin[component]);
    }
}
...
static void R_BModelWorldBounds(const trRefEntity_t *entity,
                                vec3_t mins, vec3_t maxs)
{
    ...
    const bmodel_t *model =
        tr.models[entity->e.hModel]->bmodel;
    ...
    for (int32_t cornerIndex = 0; cornerIndex < 8; ++cornerIndex) {
        ...
        localPoint[0] = (cornerIndex & 1) != 0
            ? model->bounds[1][0] : model->bounds[0][0];
        localPoint[1] = (cornerIndex & 2) != 0
            ? model->bounds[1][1] : model->bounds[0][1];
        localPoint[2] = (cornerIndex & 4) != 0
            ? model->bounds[1][2] : model->bounds[0][2];

        for (int32_t component = 0; component < 3; ++component) {
            float transformed = (float)(
                (long double)localPoint[0] *
                    entity->e.axis[0][component] +
                (long double)entity->e.origin[component]);
            transformed = (float)(
                (long double)localPoint[1] *
                    entity->e.axis[1][component] +
                (long double)transformed);
            worldPoint[component] = (float)(
                (long double)localPoint[2] *
                    entity->e.axis[2][component] +
                (long double)transformed);
        }
...
static qboolean R_XModelIsHuge(const vec3_t mins, const vec3_t maxs)
{
    ...
    enum { R_HUGE_MODEL_REQUIRED_AXES = 2 };
    int32_t hugeAxisCount = 0;

    for (int32_t component = 0; component < 3; ++component) {
        if ((long double)maxs[component] - mins[component] >
            hugeModelExtent)
            ++hugeAxisCount;
    }
    return hugeAxisCount >= R_HUGE_MODEL_REQUIRED_AXES
        ? qtrue : qfalse;
}
...
static void R_AddModelToCell(trRefEntity_t *entity,
                             const vec3_t mins, const vec3_t maxs,
                             renderer_world_cell_t *cell)
{
    ...
    if (rendererDpvsCellEntityLinkCount ==
        R_DPVS_MAX_CELL_ENTITY_LINKS) {
        ri.Error(ERR_DROP,
    ...
    for (renderer_cell_entity_link_t *link = cell->entityLinks;
         link != NULL;
         link = link->next) {
...
static int32_t R_FilterModelIntoCells_r(
    const mnode_t *node, trRefEntity_t *entity,
    const vec3_t mins, const vec3_t maxs)
{
    while (node->cellIndex == R_WORLD_NODE_INTERNAL) {
        ...
        if (side != R_MODEL_FILTER_SIDE_CROSS) {
            node = node->data.node.children[
                side - R_MODEL_FILTER_SIDE_FRONT];
            continue;
        }
        ...
        if (plane->type < R_MODEL_FILTER_AXIAL_PLANE_COUNT) {
            ...
            firstCell = R_FilterModelIntoCells_r(
                node->data.node.children[R_MODEL_FILTER_SIDE_BACK - 1],
                entity, mins, backMaxs);

            if (!(frontMins[axis] < maxs[axis]))
                return firstCell;

            secondCell = R_FilterModelIntoCells_r(
                node->data.node.children[R_MODEL_FILTER_SIDE_FRONT - 1],
                entity, frontMins, maxs);
        } else {
            firstCell = R_FilterModelIntoCells_r(
                node->data.node.children[R_MODEL_FILTER_SIDE_FRONT - 1],
                entity, mins, maxs);
        ...
        return firstCell == secondCell
            ? firstCell : R_WORLD_NODE_INTERNAL;
    }

    if (node->cellIndex >= 0) {
        R_AddModelToCell(entity, mins, maxs,
                         &tr.world->cells[node->cellIndex]);
    }
    return node->cellIndex;
}
...
void R_FilterModelsIntoCells(const renderer_dpvs_plane_t *planes,
                             int32_t planeCount)
{
    const qboolean filterBrushModels =
        r_cullBModels->integer != 0 && r_drawBModels->integer != 0;
    const qboolean filterXModels =
        r_cullXModels->integer != 0 && r_drawXModels->integer != 0;
    ...
    for (int32_t entityIndex = 0;
         entityIndex < tr.refdef.num_entities;
         ++entityIndex) {
        ...
        if (entity->e.reType == RT_MODEL &&
            R_XModelIsHuge(bounds[0], bounds[1]) != qfalse) {
            entity->cullState = CULL_IN;
        } else if (R_FilterModelIntoCells_r(
                       tr.world->nodes, entity,
                       bounds[0], bounds[1]) == R_WORLD_NODE_NO_CELL) {
            entity->cullState = CULL_IN;
        }
...
{
    renderer_occluder_t
        *activeOccluders[R_DPVS_MAX_ACTIVE_OCCLUDERS];
    ...
    if (cameraCellIndex >= 0) {
        renderer_world_cell_t *cell =
            &tr.world->cells[cameraCellIndex];
        if (r_singlecell->integer != 0) {
            rendererDpvsActiveFarPlane = NULL;
            R_AddCellSurfaces(cell, rendererDpvsFrustumPlanes,
                              R_FRUSTUM_PLANE_COUNT, dlightBits);
            R_AddCellCullGroups(cell, rendererDpvsFrustumPlanes,
                                R_FRUSTUM_PLANE_COUNT, dlightBits);
        } else {
            R_RecursivePortalWalk(cell, &rendererDpvsNearPlane,
                                  rendererDpvsFrustumPlanes,
                                  R_FRUSTUM_PLANE_COUNT,
                                  dlightBits);
        }
        ...
        for (int32_t cellIndex = 0;
             cellIndex < tr.world->cellCount;
             ++cellIndex) {
            R_AddCellSurfaces(&tr.world->cells[cellIndex],
                              rendererDpvsFrustumPlanes,
                              R_FRUSTUM_PLANE_COUNT, dlightBits);
        }
        for (int32_t groupIndex = 0;
             groupIndex < tr.world->cullGroupCount;
             ++groupIndex) {
            R_AddCullGroupDPVS(&tr.world->cullGroups[groupIndex],
                               rendererDpvsFrustumPlanes,
                               R_FRUSTUM_PLANE_COUNT, dlightBits);
        }
    }
    ...
    for (int32_t cellIndex = 0;
         cellIndex < tr.world->cellCount;
         ++cellIndex) {
        for (renderer_cell_entity_link_t *link =
                 tr.world->cells[cellIndex].entityLinks;
             link != NULL;
             link = link->next) {
            trRefEntity_t *entity = link->entity;
            if (entity->e.reType == RT_MODEL &&
                r_showCullXModels->integer !=
                    R_SHOW_CULL_MODE_CELL_LINKS) {
                continue;
            }
            if (entity->e.reType == RT_BRUSH_MODEL &&
                r_showCullBModels->integer !=
                    R_SHOW_CULL_MODE_CELL_LINKS) {
                continue;
            }
    ...
    vec3_t *vertices, int32_t *planeCount, int32_t vertexCount)
{
    ...
    const uint32_t allocationPlaneCount =
        (uint32_t)vertexCount +
        (allocateBevels ? R_PORTAL_BEVEL_COUNT : 0u);
    const uint32_t allocationBytes =
        allocationPlaneCount * (uint32_t)sizeof(renderer_dpvs_plane_t);
    renderer_dpvs_plane_t *outputPlanes =
        ri.Hunk_AllocateTempMemory((size_t)allocationBytes);
    ...
    if (allocateBevels) {
        ...
        const int32_t unrolledVertexCount =
            vertexCount >= 4 ? vertexCount & ~3 : 0;
        ...
        for (int32_t vertexIndex = 0;
             vertexIndex < unrolledVertexCount;
             vertexIndex += 4) {
            {
                const vec3_t *vertex = &vertices[vertexIndex];
                const float clipW = (float)(
                    (((long double)
                          rendererDpvsWorldViewProjectionMatrix[3][0] *
                          (*vertex)[0] +
                      (long double)
                          rendererDpvsWorldViewProjectionMatrix[3][2] *
                          (*vertex)[2]) +
                     (long double)
                         rendererDpvsWorldViewProjectionMatrix[3][1] *
                         (*vertex)[1]) +
                    (long double)
                        rendererDpvsWorldViewProjectionMatrix[3][3]);

                if (!(clipW > 0.0f)) {
                    const float inverseW =
                        (float)(1.0L / (long double)clipW);
                    const float projectedX = (float)(
                        ((((long double)
                               rendererDpvsWorldViewProjectionMatrix[0][0] *
                               (*vertex)[0] +
                           (long double)
                               rendererDpvsWorldViewProjectionMatrix[0][2] *
                               (*vertex)[2]) +
                          (long double)
                              rendererDpvsWorldViewProjectionMatrix[0][1] *
                              (*vertex)[1]) +
                         (long double)
                             rendererDpvsWorldViewProjectionMatrix[0][3]) *
                        (long double)inverseW);
                    const float projectedY = (float)(
                        ((((long double)
                               rendererDpvsWorldViewProjectionMatrix[1][0] *
                               (*vertex)[0] +
                           (long double)
                               rendererDpvsWorldViewProjectionMatrix[1][2] *
                               (*vertex)[2]) +
                          (long double)
                              rendererDpvsWorldViewProjectionMatrix[1][1] *
                              (*vertex)[1]) +
                         (long double)
                             rendererDpvsWorldViewProjectionMatrix[1][3]) *
                        (long double)inverseW);
        ...
        for (int32_t vertexIndex = unrolledVertexCount;
        ...
        for (int32_t cornerIndex = 0;
             cornerIndex < R_PORTAL_BEVEL_COUNT;
             ++cornerIndex) {
            const float projectedX = cornerIndex < 2
                ? minimumX : maximumX;
            const float projectedY =
                cornerIndex == 1 || cornerIndex == 2
                    ? minimumY : maximumY;
            float homogeneous[4];

            for (int32_t component = 0; component < 4; ++component) {
                homogeneous[component] = (float)(
                    ((long double)
                         rendererDpvsInverseWorldViewProjectionMatrix[1]
                             [component] * projectedY +
                     (long double)
                         rendererDpvsInverseWorldViewProjectionMatrix[0]
                             [component] * projectedX) +
                    (long double)rendererDpvsInverseWorldViewProjectionMatrix
                        [3][component]);
            }
            const float inverseW =
                (float)(1.0L / (long double)homogeneous[3]);
            bevelCorners[cornerIndex][0] = (float)(
                (long double)homogeneous[0] * inverseW);
            bevelCorners[cornerIndex][1] = (float)(
                (long double)homogeneous[1] * inverseW);
            bevelCorners[cornerIndex][2] = (float)(
                (long double)homogeneous[2] * inverseW);
        }
        memcpy(bevelCorners[R_PORTAL_BEVEL_COUNT], bevelCorners[0],
               sizeof(vec3_t));
    }
    ...
    for (int32_t vertexIndex = 0;
         vertexIndex < vertexCount;
         ++vertexIndex) {
        relativeVertices[vertexIndex][0] =
            vertices[vertexIndex][0] - rendererDpvsViewOrigin[0];
        relativeVertices[vertexIndex][1] =
            vertices[vertexIndex][1] - rendererDpvsViewOrigin[1];
        relativeVertices[vertexIndex][2] =
            vertices[vertexIndex][2] - rendererDpvsViewOrigin[2];
    }
    memcpy(relativeVertices[vertexCount], relativeVertices[0],
           sizeof(vec3_t));
    ...
    for (int32_t edgeIndex = 0;
        ...
        edgeNormals[edgeIndex][0] = (float)(
            (long double)(*next)[1] * (*current)[2] -
            (long double)(*next)[2] * (*current)[1]);
        edgeNormals[edgeIndex][1] = (float)(
            (long double)(*next)[2] * (*current)[0] -
            (long double)(*next)[0] * (*current)[2]);
        edgeNormals[edgeIndex][2] = (float)(
            (long double)(*next)[0] * (*current)[1] -
            (long double)(*next)[1] * (*current)[0]);
    ...
    if (r_portalbevels->value > 0.0f) {
        ...
        for (int32_t cornerIndex = 0;
             cornerIndex <= R_PORTAL_BEVEL_COUNT;
             ++cornerIndex) {
            relativeBevelCorners[cornerIndex][0] =
                bevelCorners[cornerIndex][0] - rendererDpvsViewOrigin[0];
            relativeBevelCorners[cornerIndex][1] =
                bevelCorners[cornerIndex][1] - rendererDpvsViewOrigin[1];
            relativeBevelCorners[cornerIndex][2] =
                bevelCorners[cornerIndex][2] - rendererDpvsViewOrigin[2];
        }

        for (int32_t bevelIndex = 0;
             bevelIndex < R_PORTAL_BEVEL_COUNT;
             ++bevelIndex) {
            ...
            for (int32_t edgeIndex = 0;
                ...
                const long double dot =
                    ((long double)edgeNormals[edgeIndex][0] * bevelNormal[0] +
                     (long double)edgeNormals[edgeIndex][2] * bevelNormal[2]) +
            ...
            if (rejected) {
                if ((r_showportals->integer & 2) != 0) {
                    R_AddDebugLine(bevelCorners[bevelIndex],
                                   bevelCorners[bevelIndex + 1],
                                   rejectedBevelDebugColor);
                }
                continue;
            }
            ...
            if (r_showportals->integer != 0) {
                R_AddDebugLine(bevelCorners[bevelIndex],
                               bevelCorners[bevelIndex + 1],
                               acceptedBevelDebugColor);
            }

            renderer_dpvs_plane_t *plane =
                &outputPlanes[outputPlaneCount++];
    ...
    for (int32_t edgeIndex = 0;
        ...
        if (edgeIndex < unrolledEdgeCount && (edgeIndex & 3) == 0) {
            ...
            lengthSquared =
                ((long double)edgeNormals[edgeIndex][2] *
                     edgeNormals[edgeIndex][2] +
                 (long double)edgeNormals[edgeIndex][0] *
                     edgeNormals[edgeIndex][0]) +
                (long double)edgeNormals[edgeIndex][1] *
                    edgeNormals[edgeIndex][1];
        } else {
        ...
        renderer_dpvs_plane_t *plane =
            &outputPlanes[outputPlaneCount++];
        memcpy(plane->normal, edgeNormals[edgeIndex],
               sizeof(plane->normal));
