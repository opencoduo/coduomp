...
#define RB_DIAGNOSTIC_TRIS_DEPTH_FAR \
    0.10000000149011612 /* original double 0x3fb99999a0000000 */
#define RB_DIAGNOSTIC_NORMAL_DEPTH_FAR \
    0.9990000128746033 /* original double 0x3feff7cee0000000 */
...
void DrawTris(shaderCommands_t *tessellation)
{
    ...
    if (r_showtris->integer <= 2 &&
        (tessellation->entity == &backEnd.entity2D ||
         tessellation->entity == &tr.worldEntity)) {
        return;
    }
    ...
    tessellation->activeStageCount =
        tr.showTrisShader->numUnfoggedPasses;

    if ((r_showtris->integer & 1) != 0) {
        qglDepthRange(RB_DIAGNOSTIC_DEPTH_NEAR,
                      RB_DIAGNOSTIC_TRIS_DEPTH_FAR);
    }
...
void DrawNormals(shaderCommands_t *tessellation)
{
    ...
    qglDepthRange(RB_DIAGNOSTIC_DEPTH_NEAR,
                  RB_DIAGNOSTIC_NORMAL_DEPTH_FAR);
    GL_Bind(tr.defaultImage);
    RB_glColor3f(1.0f, 1.0f, 1.0f);

    if (tessellation->stageTangentsValid == qfalse ||
        tessellation->stageBitangentsValid == qfalse) {
        RB_CalcTangentSpace();
    }

    if (r_shownormals->integer == 1) {
        qglDepthRange(RB_DIAGNOSTIC_DEPTH_NEAR,
                      RB_DIAGNOSTIC_DEPTH_NEAR);
    }

    GL_State(GLS_DEPTHMASK_TRUE | GLS_POLYMODE_LINE);
    RB_glBegin(GL_LINES);

    for (vertexIndex = 0; vertexIndex < tessellation->vertexCount;
         ++vertexIndex) {
        const float *xyz =
            &tessellation->xyz[vertexIndex *
                               tessellation->vertexComponentCount];
        vec3_t endpoint;
        ...
        if (tessellation->stageTangentsValid != qfalse &&
            ...
            const vec3_t *bitangent =
                &tessellation->stageBitangents[vertexIndex];
            ...
            RB_glVertex3fv(xyz);
            endpoint[0] =
                ((*bitangent)[0] + (*bitangent)[0]) + xyz[0];
            endpoint[1] =
                ((*bitangent)[1] + (*bitangent)[1]) + xyz[1];
            endpoint[2] =
                ((*bitangent)[2] + (*bitangent)[2]) + xyz[2];
            RB_glVertex3fv(endpoint);
        ...
        RB_glVertex3fv(xyz);
        endpoint[0] =
            (tessellation->stageNormals[vertexIndex][0] +
             tessellation->stageNormals[vertexIndex][0]) + xyz[0];
        endpoint[1] =
            (tessellation->stageNormals[vertexIndex][1] +
             tessellation->stageNormals[vertexIndex][1]) + xyz[1];
        endpoint[2] =
            (tessellation->stageNormals[vertexIndex][2] +
             tessellation->stageNormals[vertexIndex][2]) + xyz[2];
        RB_glVertex3fv(endpoint);

        if ((vertexIndex & (RB_DIAGNOSTIC_LINE_BATCH_VERTICES - 1)) ==
            RB_DIAGNOSTIC_LINE_BATCH_VERTICES - 1) {
            RB_glEnd();
            RB_glBegin(GL_LINES);
        }
    }
...
void DrawColoredNormals(shaderCommands_t *tessellation)
{
    ...
    for (vertexIndex = 0; vertexIndex < tessellation->vertexCount;
        ...
        const float *xyz =
            &tessellation->xyz[vertexIndex *
                               tessellation->vertexComponentCount];
        const int32_t greenNormalIndex =
            vertexIndex + 1 < tessellation->vertexCount
                ? vertexIndex + 1 : tessellation->vertexCount - 1;
        const int32_t blueNormalIndex =
            vertexIndex + 2 < tessellation->vertexCount
                ? vertexIndex + 2 : tessellation->vertexCount - 1;
        vec3_t endpoint;

        /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
        RB_glColor3f(
            (tessellation->stageNormals[vertexIndex][0] + 1.0f) * 0.5f,
            (tessellation->stageNormals[greenNormalIndex][0] + 1.0f) *
                0.5f,
            (tessellation->stageNormals[blueNormalIndex][0] + 1.0f) *
                0.5f);
        RB_glVertex3fv(xyz);
        endpoint[0] =
            (tessellation->stageNormals[vertexIndex][0] +
