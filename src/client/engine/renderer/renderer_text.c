...
static const float r_consoleByteToUnit =
    0.0039215688593685627f; /* 0x3b808081, approximately 1/255 */
...
static const float r_consoleIconBaselineHeight =
    38.40000152587890625f; /* 0x4219999a, 48 * 0.8 */
                      ...
                      float fixedAdvance, int32_t limit)
{
    ...
    const float finalScale =
        fixedAdvance == 0.0f ? scale * font->glyphScale : fixedAdvance;
    ...
    while (*cursor != '\0' && glyphCount < limit) {
        ...
        if (fixedAdvance == 0.0f) {
            long double advance =
                (long double)R_GetGlyphHorizAdvance(font, character);
...
{
    RE_Text_PaintWithCursor(x, y, fontHandle, scale, color, text,
                            -1, 0, fixedAdvance, limit, textStyle);
}
...
static qboolean R_Text_GetConsoleString(const uint16_t *encoded,
                                        int32_t *remainingCount,
                                        const char **outText,
                                        const uint16_t **outNext,
                                        float color[4])
{
    ...
    *outText = rendererConsoleTextBuffer;
    ...
    while (sourceIndex < *remainingCount) {
        ...
        } else if (control == R_CONSOLE_ICON_RED ||
                   control == R_CONSOLE_ICON_WIDTH ||
                   control == R_CONSOLE_ICON_HEIGHT ||
...
static void R_Text_GetConsoleIcon(const uint16_t *encoded,
                                  int32_t *remainingCount,
                                  const uint16_t **cursor,
                                  float scale,
                                  float *width, float *height,
                                  int32_t *shaderHandle, float color[4])
{
    ...
    *shaderHandle = 0;
    ...
    while (sourceIndex < *remainingCount) {
        ...
        case R_CONSOLE_ICON_SHADER: {
            ...
            /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered engine boundary input and state before use. */
            while (control == R_CONSOLE_ICON_SHADER && value != 0 &&
                   sourceIndex < *remainingCount - 1) {
                             ...
                             int32_t encodedCount)
{
    ...
    while (cursor != NULL) {
        const char *text;
        const qboolean hasIcon =
            R_Text_GetConsoleString(cursor, &encodedCount,
                                    &text, &cursor, NULL);

        if (text != NULL && *text != '\0') {
            if (fixedAdvance == 0.0f) {
                width += (float)RE_Text_Width(text, fontHandle, scale,
                                             fixedAdvance, encodedCount);
            } else {
        ...
        if (hasIcon != qfalse) {
            ...
            R_Text_GetConsoleIcon(cursor, &encodedCount, &cursor, scale,
...
static void R_Text_PaintConsoleIcon(float x, float y,
                                    float width, float height,
                                    int32_t shaderHandle)
{
    ri.AdjustFrom640(&x, &y, &width, &height);
    RE_StretchPic(x, y, width, height,
                          ...
                          int32_t textStyle)
{
    ...
    while (cursor != NULL) {
        ...
        const qboolean hasIcon =
            R_Text_GetConsoleString(cursor, &encodedCount,
                                    &text, &cursor, currentColor);

        if (text != NULL && *text != '\0') {
            RE_Text_Paint(x + xOffset, y, fontHandle, scale,
                          currentColor, text, fixedAdvance, 0, textStyle);
            if (hasIcon != qfalse) {
                xOffset += (float)RE_Text_Width(
                    text, fontHandle, scale, fixedAdvance, encodedCount);
            }
        }
        ...
        if (hasIcon != qfalse) {
            ...
            R_Text_GetConsoleIcon(cursor, &encodedCount, &cursor, scale,
                                  &iconWidth, &iconHeight,
                                  &shaderHandle, currentColor);
            RE_SetColor(currentColor);
            iconY = y -
                (iconHeight + scale * r_consoleIconBaselineHeight) * 0.5f;
            R_Text_PaintConsoleIcon(x + xOffset, iconY,
                                    iconWidth, iconHeight, shaderHandle);
            xOffset += iconWidth;
        }
    }
}
                             ...
                             int32_t limit, int32_t textStyle)
{
    ...
    commandSize =
        (uint32_t)offsetof(text_paint_command_t, text) + textLength + 1u;
    ...
    command->scale = scale;
    command->color.components[0] =
        (uint8_t)(color[0] * r_textColorByteScale);
    command->color.components[1] =
        (uint8_t)(color[1] * r_textColorByteScale);
    command->color.components[2] =
        (uint8_t)(color[2] * r_textColorByteScale);
    command->color.components[3] =
        (uint8_t)(color[3] * r_textColorByteScale);
