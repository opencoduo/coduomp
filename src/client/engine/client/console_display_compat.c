...
enum { CODUOMP_CONSOLE_REFERENCE_HEIGHT = 1080 };
...
float coduomp_console_canvas_scale_compat(void)
{
    ...
    return (float)cls.rendererConfig.vidHeight /
           (float)CODUOMP_CONSOLE_REFERENCE_HEIGHT;
}
...
static float coduomp_console_virtual_width(void)
{
    ...
    return (float)cls.rendererConfig.vidWidth /
           coduomp_console_canvas_scale_compat();
}
...
static void coduomp_console_check_resize_compat(void)
{
    ...
    int32_t newLineWidth =
        (int32_t)(coduomp_console_virtual_width() /
                  (float)CODUOMP_CONSOLE_GLYPH_WIDTH) -
        CODUOMP_CONSOLE_HORIZONTAL_MARGIN_GLYPHS;
...
static void coduomp_console_draw_glyph_compat(float x, float y,
                                               int32_t character,
                                               const float color[4])
{
    ...
    rendererExports.TextPaint(
        x, y + (float)CODUOMP_CONSOLE_GLYPH_HEIGHT,
        CODUOMP_CONSOLE_TEXT_FONT, CODUOMP_CONSOLE_TEXT_SCALE,
        color, text, (float)CODUOMP_CONSOLE_GLYPH_WIDTH,
...
static void coduomp_console_draw_input_compat(float visibleHeight)
{
    ...
    const int32_t maximumVisible =
        con.lineWidth > 4 ? con.lineWidth - 4 : 1;
    int32_t firstCharacter = 0;
    const float y =
        visibleHeight - (float)CODUOMP_CONSOLE_INPUT_BOTTOM_OFFSET;
    ...
    coduomp_console_draw_glyph_compat(
        (float)CODUOMP_CONSOLE_GLYPH_WIDTH, y, ']', color);
    rendererExports.TextPaintWithCursor(
        (float)(CODUOMP_CONSOLE_GLYPH_WIDTH * 2),
        y + (float)CODUOMP_CONSOLE_GLYPH_HEIGHT,
        CODUOMP_CONSOLE_TEXT_FONT, CODUOMP_CONSOLE_TEXT_SCALE,
        color, &con_inputField.buffer[firstCharacter],
        con_inputField.cursor - firstCharacter,
        key_overstrikeMode != qfalse
            ? CODUOMP_CONSOLE_FIXED_OVERSTRIKE_CURSOR
            : CODUOMP_CONSOLE_FIXED_INSERT_CURSOR,
        (float)CODUOMP_CONSOLE_GLYPH_WIDTH,
        maximumVisible, CODUOMP_CONSOLE_TEXT_STYLE);
}
...
static void coduomp_console_draw_solid_compat(float fraction)
{
    ...
    float stockBackgroundHeight =
        fraction * (float)CODUOMP_CONSOLE_STOCK_VIRTUAL_HEIGHT;
    ...
    if (stockBackgroundHeight >
        (float)CODUOMP_CONSOLE_STOCK_VIRTUAL_HEIGHT) {
        stockBackgroundHeight =
            (float)CODUOMP_CONSOLE_STOCK_VIRTUAL_HEIGHT;
    }

    con.xAdjust =
        (float)CODUOMP_CONSOLE_GLYPH_WIDTH * canvasScale;
    con.visiblePixelHeight =
        (int32_t)(visibleHeight * canvasScale);
    ...
    if (backgroundHeight < 1) {
        ...
        SCR_DrawPic(0.0f, 0.0f, 640.0f, (float)backgroundHeight,
                    cls.consoleShader);
    }

    {
        const vec4_t separatorColor = {
            0.0f, 0.0f, 0.0f, CODUOMP_CONSOLE_SEPARATOR_ALPHA
        };
        SCR_FillRect(0.0f, (float)backgroundHeight,
                     640.0f, 2.0f, separatorColor);
    }

    {
        ...
        rendererExports.TextPaint(
            virtualWidth -
                (float)(versionLength * CODUOMP_CONSOLE_GLYPH_WIDTH),
            visibleHeight - 2.0f,
            CODUOMP_CONSOLE_TEXT_FONT, CODUOMP_CONSOLE_TEXT_SCALE,
            color, version, (float)CODUOMP_CONSOLE_GLYPH_WIDTH,
            0, CODUOMP_CONSOLE_TEXT_STYLE);
    }

    rowCount =
        ((int32_t)visibleHeight - CODUOMP_CONSOLE_GLYPH_WIDTH) /
        CODUOMP_CONSOLE_GLYPH_WIDTH;
    ...
    if (con.displayLine != con.currentLine) {
        ...
        for (int32_t column = 0; column < con.lineWidth;
             column += CODUOMP_CONSOLE_BACKSCROLL_STEP) {
            coduomp_console_draw_glyph_compat(
                (float)((column + 2) * CODUOMP_CONSOLE_GLYPH_WIDTH),
                (float)y, '^', color);
        }
...

    {
        ...
        for (int32_t row = 0;
             row < rowCount && displayLine >= 0;
             ++row, --displayLine, y -= CODUOMP_CONSOLE_GLYPH_HEIGHT) {
            const uint16_t *text;
            int32_t textRow;
            ...
            rendererExports.TextConsolePaint(
                (float)CODUOMP_CONSOLE_GLYPH_WIDTH,
                (float)(y + CODUOMP_CONSOLE_GLYPH_WIDTH),
                CODUOMP_CONSOLE_TEXT_FONT, CODUOMP_CONSOLE_TEXT_SCALE,
                color, text, (float)CODUOMP_CONSOLE_GLYPH_WIDTH,
                con.lineWidth, CODUOMP_CONSOLE_TEXT_STYLE);
        }
    }
