...
{
    vec3_t viewOrigin = {cl.inputState.clientLerpOrigin[0], cl.inputState.clientLerpOrigin[1], cl.inputState.clientLerpOrigin[2]};
    viewOrigin[2] += cl.snap.ps.viewHeightCurrent - 1.0f;
    AddLeanToPosition(viewOrigin, cl.snap.ps.viewAngles[1], cl.snap.ps.leanFraction, 16.0f, 20.0f);

    const vec3_t entityOrigin = {entity->pos.trBase[0], entity->pos.trBase[1], entity->pos.trBase[2]};
    vec3_t direction = {entityOrigin[0] - viewOrigin[0], entityOrigin[1] - viewOrigin[1], entityOrigin[2] - viewOrigin[2]};
    ...
    vec3_t farSideOffset = {nearSideOffset[0] * 18.0f, nearSideOffset[1] * 18.0f, nearSideOffset[2] * 18.0f};
    for (int32_t component = 0; component < 3; ++component)
        nearSideOffset[component] *= 10.0f;

    const float centerHeight = entity->animMovetype != 0 ? 16.0f : 40.0f;
    vec3_t target = {entityOrigin[0], entityOrigin[1], entityOrigin[2] + centerHeight};
    trace_t trace;

    trace.fraction = 1.0f;
    CM_BoxTrace(&trace, viewOrigin, target, NULL, NULL, CM_WORLD_MODEL, CONTENTS_SOLID, qfalse);
...
{
    entityState_t *const entity = &cl.parseEntities[(uint32_t)cl.parseEntitySequence & (CODUO_PARSE_RING_COUNT - 1)];

    if (unchanged != qfalse) {
        *entity = *oldEntity;
    } else if (MSG_ReadDeltaEntity(message, oldEntity, entity, newNumber) != qfalse) {
        return;
    }

    /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered engine boundary input and state before use. */
    if ((entity->eType == ET_PLAYER || entity->eType == ET_PLAYER_CORPSE) && (uint32_t)entity->clientNum >= (uint32_t)MAX_CLIENTS) {
        Com_Error(ERR_DROP,
                  "\x15"
                  "CL_DeltaEntity: invalid client number %i",
                  entity->clientNum);
        return;
    }

    if (clc.onlyVisibleClients != qfalse && entity->number < MAX_CLIENTS) {
        if (isEntVisible(entity) != qfalse) {
            cl_entityLastVisibleTime[entity->number] = frame->serverTime;
            entity->eFlags &= ~CL_ENTITY_FLAG_NODRAW;
        } else if (cl_entityLastVisibleTime[entity->number] < (int32_t)((uint32_t)frame->serverTime - CL_ENTITY_RECENTLY_VISIBLE_MSEC)) {
            entity->eFlags |= CL_ENTITY_FLAG_NODRAW;
        }
    }
...
{
    clientState_t *const client = &cl.parseClients[(uint32_t)cl.parseClientSequence & (CODUO_PARSE_RING_COUNT - 1)];

    if (unchanged != qfalse) {
        *client = *oldClient;
    } else if (MSG_ReadDeltaClient(message, oldClient, client, newNumber) != qfalse) {
        return;
    }
...
static const entityState_t *CL_OldPacketEntity(const clSnapshot_t *oldFrame, int32_t oldIndex)
{
    ...
    return &cl.parseEntities[((uint32_t)oldFrame->firstEntitySequence + (uint32_t)oldIndex) & (CODUO_PARSE_RING_COUNT - 1)];
}
...
void CL_ParsePacketEntities(msg_t *message, const clSnapshot_t *oldFrame, clSnapshot_t *newFrame)
{
    ...
    int32_t oldIndex = 0;
    const entityState_t *oldEntity = CL_OldPacketEntity(oldFrame, oldIndex);
    int32_t oldNumber = oldEntity != NULL ? oldEntity->number : CL_PARSE_OLD_NUMBER_END;

    int32_t newNumber = MSG_ReadBits(message, CL_PACKET_ENTITY_NUMBER_BITS);
    while (newNumber != CL_PACKET_ENTITY_END) {
        ...
        while (oldNumber < newNumber) {
            ...
            oldNumber = oldEntity != NULL ? oldEntity->number : CL_PARSE_OLD_NUMBER_END;
        }
        ...
        if (oldNumber == newNumber) {
            ...
            CL_DeltaEntity(message, newFrame, newNumber, oldEntity, qfalse);
            ...
            CL_DeltaEntity(message, newFrame, newNumber, &cl.entityBaselines[newNumber], qfalse);
        }

        newNumber = MSG_ReadBits(message, CL_PACKET_ENTITY_NUMBER_BITS);
    }
...
static const clientState_t *CL_OldPacketClient(const clSnapshot_t *oldFrame, int32_t oldIndex)
{
    /* NOT_FROM_ORIGINAL_SOURCE: typed source-level spelling of the repeated
    ...
    return &cl.parseClients[((uint32_t)oldFrame->firstClientSequence + (uint32_t)oldIndex) & (CODUO_PARSE_RING_COUNT - 1)];
}
...
void CL_ParsePacketClients(msg_t *message, const clSnapshot_t *oldFrame, clSnapshot_t *newFrame)
{
    ...
    int32_t oldIndex = 0;
    const clientState_t *oldClient = CL_OldPacketClient(oldFrame, oldIndex);
    int32_t oldNumber = oldClient != NULL ? oldClient->clientNum : CL_PARSE_OLD_NUMBER_END;

    while (MSG_ReadBit(message) != 0) {
        const int32_t newNumber = MSG_ReadBits(message, CL_PACKET_CLIENT_NUMBER_BITS);
        if (message->readcount > message->cursize) {
            Com_Error(ERR_DROP, "\x15"
        ...
        while (oldNumber < newNumber) {
            ...
            oldNumber = oldClient != NULL ? oldClient->clientNum : CL_PARSE_OLD_NUMBER_END;
        }
        ...
        if (oldNumber == newNumber) {
            ...
            CL_DeltaClient(message, newFrame, newNumber, oldClient, qfalse);
            ...
            CL_DeltaClient(message, newFrame, newNumber, &nullClient, qfalse);
        }
    }
...
void CL_ParseSnapshot(msg_t *message)
{
    ...
    if (deltaDistance == 0) {
        ...
        newSnapshot.deltaNum = (int32_t)((uint32_t)newSnapshot.messageNum - (uint32_t)deltaDistance);
    }
    ...
    if (newSnapshot.deltaNum <= 0) {
        ...
        oldSnapshot = &cl.snapshots[(uint32_t)newSnapshot.deltaNum & CL_SNAPSHOT_BACKUP_MASK];
        ...
        } else if ((int32_t)((uint32_t)cl.parseEntitySequence - (uint32_t)oldSnapshot->firstEntitySequence) >
                   CODUO_PARSE_RING_COUNT - CL_SNAPSHOT_PARSE_SAFETY_MARGIN) {
            Com_DPrintf("Delta parseEntitiesNum too old.\n");
        } else if ((int32_t)((uint32_t)cl.parseClientSequence - (uint32_t)oldSnapshot->firstClientSequence) >
                   CODUO_PARSE_RING_COUNT - CL_SNAPSHOT_PARSE_SAFETY_MARGIN) {
    ...
    MSG_ReadDeltaPlayerstate(message, oldSnapshot != NULL ? &oldSnapshot->ps : NULL, &newSnapshot.ps);

    /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered engine boundary input and state before use. */
    if ((uint32_t)newSnapshot.ps.psClientNum >= (uint32_t)MAX_CLIENTS) {
        Com_Error(ERR_DROP,
                  "\x15"
                  "CL_ParseSnapshot: invalid player client number %i",
    ...
    if ((int32_t)((uint32_t)newSnapshot.messageNum - (uint32_t)invalidateMessageNum) >= CODUO_SNAPSHOT_BACKUP_COUNT) {
        invalidateMessageNum = newSnapshot.messageNum - (CODUO_SNAPSHOT_BACKUP_COUNT - 1);
    }
    while (invalidateMessageNum < newSnapshot.messageNum) {
        cl.snapshots[(uint32_t)invalidateMessageNum & CL_SNAPSHOT_BACKUP_MASK].valid = qfalse;
    ...
    for (int32_t packetAge = 0; packetAge < CODUO_SNAPSHOT_BACKUP_COUNT; ++packetAge, --packetSequence) {
        const clOutPacket_t *const packet = &cl.outPackets[(uint32_t)packetSequence & CL_SNAPSHOT_BACKUP_MASK];
    ...
    cl.snapshots[(uint32_t)cl.snap.messageNum & CL_SNAPSHOT_BACKUP_MASK] = cl.snap;
...
void CL_ParseGamestate(msg_t *message)
{
    ...
    for (;;) {
        const clServerMessageCommand_t command = (clServerMessageCommand_t)MSG_ReadByte(message);
        ...
        if (command == CL_SVC_CONFIGSTRING) {
            ...
            /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered engine boundary input and state before use. */
            if (configstringIndex >= CS_EFFECTS && configstringIndex < CS_FX && strcspn(value, ".") >= FX_EFFECT_TEMPLATE_NAME_CAPACITY) {
                Com_Error(ERR_DROP, "\x15"
                                    "CL_ParseGamestate: effect name is too long");
            }

            const size_t valueBytes = strlen(value) + 1;
            if ((size_t)cl.gameState.dataCount + valueBytes > MAX_GAMESTATE_CHARS) {
                Com_Error(ERR_DROP, "\x15MAX_GAMESTATE_CHARS exceeded");
            }

            cl.gameState.stringOffsets[configstringIndex] = cl.gameState.dataCount;
            memcpy(&cl.gameState.stringData[cl.gameState.dataCount], value, valueBytes);
            cl.gameState.dataCount += (int32_t)valueBytes;
            continue;
        }

        if (command == CL_SVC_BASELINE) {
            const int32_t entityNumber = MSG_ReadBits(message, CL_PACKET_ENTITY_NUMBER_BITS);
            ...
            (void)MSG_ReadDeltaEntity(message, &nullEntity, &cl.entityBaselines[entityNumber], entityNumber);
            continue;
        }
    ...
    const char *advertisedName = Info_ValueForKey(serverInfo, "sv_hostname");
    if (advertisedName[0] == '\0')
        advertisedName = Info_ValueForKey(serverInfo, "hostname");
    if (advertisedName[0] != '\0')
        serverDisplayName = advertisedName;
    const qboolean namespaceChanged = coduomp_server_namespace_activate(
        &clc.serverAddress, serverDisplayName,
        sv_running->integer == 0 && clc.demoPlayback == qfalse && cl_updateStarted == qfalse ? qtrue : qfalse);
    ...
    const qboolean cachedRootPaks = coduomp_server_namespace_cache_referenced_paks();

    if (sv_running->integer == 0) {
        if (namespaceChanged != qfalse || cachedRootPaks != qfalse)
            FS_Restart(clc.checksumFeed);
        else if (fs_game->modified != qfalse || clc.checksumFeed != fs_checksumFeed)
...
void CL_ParseDownload(msg_t *message)
{
    ...
    if (blockNumber == -1) {
        ...
        strncpy(cls.staticDownload.originalDownloadName, cls.staticDownload.downloadName,
                sizeof(cls.staticDownload.originalDownloadName) - 1);
        cls.staticDownload.originalDownloadName[sizeof(cls.staticDownload.originalDownloadName) - 1] = '\0';

        const char *const redirectedUrl = MSG_ReadString(message);
        strncpy(cls.staticDownload.downloadName, redirectedUrl, sizeof(cls.staticDownload.downloadName) - 1);
        cls.staticDownload.downloadName[sizeof(cls.staticDownload.downloadName) - 1] = '\0';
        ...
        if ((clc.downloadFlags & CL_WWW_DOWNLOAD_OPEN_URL) != 0) {
            ...
            if (cl_updateStarted == qfalse || NET_CompareAdrSigned(&clc.serverAddress, &cls.autoUpdateServer) != 0) {
                Com_Printf("Refusing external open for package download\n");
        ...
        if (strstr(clc.badChecksumList, va("@%s", cls.staticDownload.originalDownloadName)) != NULL) {
            Com_Printf("refusing redirect to %s by server "
                       "(bad checksum)\n",
                       cls.staticDownload.downloadName);
            CL_AddReliableCommand("wwwdl fail");
            clc.wwwDownloadAborting = qtrue;
        ...
        if (coduomp_server_namespace_build_download_path(cls.staticDownload.downloadTempName, localTempPath, sizeof(localTempPath)) ==
            qfalse) {
            CL_AddReliableCommand("wwwdl fail");
            clc.wwwDownloadAborting = qtrue;
        ...
        strncpy(cls.staticDownload.downloadTempName, localTempPath, sizeof(cls.staticDownload.downloadTempName) - 1);
        cls.staticDownload.downloadTempName[sizeof(cls.staticDownload.downloadTempName) - 1] = '\0';
        if (DL_BeginDownload(cls.staticDownload.downloadTempName, cls.staticDownload.downloadName) == qfalse) {
            CL_AddReliableCommand("wwwdl fail");
            clc.wwwDownloadAborting = qtrue;
            Com_Printf("Failed to initialize download for '%s'\n", cls.staticDownload.downloadName);
        }

        if ((clc.downloadFlags & CL_WWW_DOWNLOAD_DISCONNECT) != 0) {
            CL_AddReliableCommand("wwwdl bbl8r");
    ...
    if (blockLength < 0) {
        Com_Error(ERR_DROP,
                  "\x15"
                  "CL_ParseDownload: invalid block length %i",
                  blockLength);
    }
    ...
    if (clc.downloadBlock != blockNumber) {
        ...
        Com_DPrintf("CL_ParseDownload: Sending retransmitt request "
    ...
    if (clc.downloadFile == 0) {
        ...
        clc.downloadFile = coduomp_server_namespace_open_download_write(cls.staticDownload.downloadTempName);
        if (clc.downloadFile == 0) {
    ...
    if (blockLength != 0) {
        (void)FS_Write(blockData, blockLength, clc.downloadFile);
    }
    ...
    if (clc.downloadFile != 0) {
        ...
        clc.downloadFile = 0;
        coduomp_server_namespace_rename_download(cls.staticDownload.downloadTempName, cls.staticDownload.downloadName);
    }
...
{
    const char *const systemInfo = &cl.gameState.stringData[cl.gameState.stringOffsets[CL_SYSTEMINFO_CONFIGSTRING]];

    cl.serverId = coduo_crt_atoi(Info_ValueForKey(systemInfo, "sv_serverid"));
    memset(cl_entityLastVisibleTime, 0, sizeof(cl_entityLastVisibleTime));
    ...
    const char *const loadedPakChecksums = Info_ValueForKey(systemInfo, "sv_paks");
    const char *const loadedPakNames = Info_ValueForKey(systemInfo, "sv_pakNames");
    FS_PureServerSetLoadedPaks(loadedPakChecksums, loadedPakNames);

    const char *const referencedPakChecksums = Info_ValueForKey(systemInfo, "sv_referencedPaks");
    const char *const referencedPakNames = Info_ValueForKey(systemInfo, "sv_referencedPakNames");
    FS_PureServerSetReferencedPaks(referencedPakChecksums, referencedPakNames);
    ...
    cl_connectedToPureServer = (qboolean)Cvar_VariableValue("sv_pure");
}
...
void CL_ParseCommandString(msg_t *message)
{
    ...
    Q_strncpyz(clc.serverCommands[(uint32_t)sequence & (CODUO_RELIABLE_COMMAND_COUNT - 1)], command, CODUO_RELIABLE_COMMAND_CAPACITY);
}
...
void CL_ParseServerMessage(msg_t *message)
{
    ...
    static const char *const commandNames[] = {"svc_bad",      "svc_nop",           "svc_gamestate", "svc_configstring",
                                               "svc_baseline", "svc_serverCommand", "svc_download",  "svc_snapshot"};
    ...
    MSG_Init(&decompressed, decompressedData, (int32_t)sizeof(decompressedData));
    const int32_t decompressedSize = MSG_ReadBitsCompress(message->data + message->readcount, decompressed.data,
                                                          message->cursize - message->readcount, (int32_t)sizeof(decompressedData));
    /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered engine boundary input and state before use. */
    ...
    for (;;) {
        ...
        int32_t command;
        if (decompressed.readcount < decompressed.cursize) {
            command = decompressed.data[decompressed.readcount++];
        } else {
        ...
        if (cl_shownet->integer >= 2) {
            ...
            if ((uint32_t)command < sizeof(commandNames) / sizeof(commandNames[0])) {
