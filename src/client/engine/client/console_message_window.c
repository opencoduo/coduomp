...
static int32_t con_gameMessageStartTimes[CON_STANDARD_MESSAGE_CAPACITY]; /* 0x04e3cc4c */
static int32_t con_gameMessageEndTimes[CON_STANDARD_MESSAGE_CAPACITY]; /* 0x04e3cc6c */
static int32_t con_gameMessageLineIndices[CON_STANDARD_MESSAGE_CAPACITY]; /* 0x04e3cc8c */
static int32_t con_boldMessageStartTimes[CON_STANDARD_MESSAGE_CAPACITY]; /* 0x04e3ccd0 */
static int32_t con_boldMessageEndTimes[CON_STANDARD_MESSAGE_CAPACITY]; /* 0x04e3ccf0 */
static int32_t con_boldMessageLineIndices[CON_STANDARD_MESSAGE_CAPACITY]; /* 0x04e3cd10 */
static int32_t con_subtitleStartTimes[CON_STANDARD_MESSAGE_CAPACITY]; /* 0x04e3cd54 */
static int32_t con_subtitleEndTimes[CON_STANDARD_MESSAGE_CAPACITY]; /* 0x04e3cd74 */
static int32_t con_subtitleLineIndices[CON_STANDARD_MESSAGE_CAPACITY]; /* 0x04e3cd94 */

/* Original Win32 mini-console arrays are the three 100-dword regions at
 * 0x04e3cdd8, 0x04e3cf68, and 0x04e3d0f8. */
static int32_t con_miniconStartTimes[CON_MINICON_MAX_LINES]; /* 0x04e3cdd8 */
static int32_t con_miniconEndTimes[CON_MINICON_MAX_LINES]; /* 0x04e3cf68 */
static int32_t con_miniconLineIndices[CON_MINICON_MAX_LINES]; /* 0x04e3d0f8 */
...
void Con_OneTimeInit(void)
{
    ...
    Con_InitMessageWindow(&con_gameMessageWindow, con_gameMessageStartTimes, con_gameMessageEndTimes, con_gameMessageLineIndices,
                          CON_STANDARD_MESSAGE_CAPACITY, CON_STANDARD_VISIBLE_LINES, CON_STANDARD_SCROLL_MSEC, CON_STANDARD_FADE_IN_MSEC,
                          CON_STANDARD_FADE_OUT_MSEC);
    Con_InitMessageWindow(&con_boldGameMessageWindow, con_boldMessageStartTimes, con_boldMessageEndTimes, con_boldMessageLineIndices,
                          CON_STANDARD_MESSAGE_CAPACITY, CON_STANDARD_VISIBLE_LINES, CON_STANDARD_SCROLL_MSEC, CON_STANDARD_FADE_IN_MSEC,
                          CON_STANDARD_FADE_OUT_MSEC);
    Con_InitMessageWindow(&con_subtitleWindow, con_subtitleStartTimes, con_subtitleEndTimes, con_subtitleLineIndices,
                          CON_STANDARD_MESSAGE_CAPACITY, CON_STANDARD_VISIBLE_LINES, CON_STANDARD_SCROLL_MSEC, CON_STANDARD_FADE_IN_MSEC,
                          CON_STANDARD_FADE_OUT_MSEC);
    ...
    Con_InitMessageWindow(&con_miniConsoleWindow, con_miniconStartTimes, con_miniconEndTimes, con_miniconLineIndices,
                          con_miniconLines->integer, 0, 0, 0, 0);
...
void Con_UpdateMessageWindowLine(console_message_window_t *window, qboolean advanceLine, int32_t messageTime)
{
    ...
    window->lineEndTimes[slot] = (int32_t)(Sys_Milliseconds() + (uint32_t)messageTime);
    window->lineIndices[slot] = con.currentLine;

    if (advanceLine == qfalse || window->lineCapacity <= 0)
        return;

    window->activeLineIndex = (window->activeLineIndex + 1) % window->lineCapacity;

    for (int32_t visibleLine = 0; visibleLine < window->visibleLineCount; ++visibleLine) {
        slot = (window->activeLineIndex + visibleLine) % window->lineCapacity;

        const int32_t now = (int32_t)Sys_Milliseconds();
        const int32_t endTime = window->lineEndTimes[slot];
        if ((int32_t)((uint32_t)endTime - (uint32_t)window->fadeOutTime) > now) {
            const uint32_t lineDuration = (uint32_t)endTime - (uint32_t)window->lineStartTimes[slot];
            const uint32_t adjustedStart = Sys_Milliseconds() + (uint32_t)window->fadeOutTime - lineDuration;
            window->lineStartTimes[slot] = (int32_t)adjustedStart;
            window->lineEndTimes[slot] = (int32_t)(Sys_Milliseconds() + (uint32_t)window->fadeOutTime);
        }
    }
}
...
void Con_Linefeed(console_message_destination_t destination, int32_t messageTime)
{
    ...
    } else if ((int32_t)((uint32_t)con.currentLine - (uint32_t)con.displayLine) >= con.totalLines) {
