...
void Con_DrawInput(void)
{
    ...
    const int32_t y =
        con.visiblePixelHeight - CON_INPUT_BOTTOM_OFFSET;

    rendererExports.SetColor(con_miniconColor);
    SCR_DrawSmallChar(FastRound(con.xAdjust), y, ']');
    Field_Draw(&con_inputField,
               FastRound(con.xAdjust) + CON_SMALL_CHAR_WIDTH, y);
}
...
{
    int32_t pixelHeight =
        (int32_t)((float)cls.rendererConfig.vidHeight * fraction);
    if (pixelHeight <= 0)
        return;
    ...
    con.xAdjust =
        ...
        (float)CON_SMALL_CHAR_WIDTH;

    int32_t backgroundHeight =
        (int32_t)(fraction * (float)CON_VIRTUAL_HEIGHT - 2.0f);
    if (backgroundHeight < 1) {
        backgroundHeight = 0;
    } else {
        SCR_DrawPic(0.0f, 0.0f, (float)CON_VIRTUAL_WIDTH,
                    (float)backgroundHeight, cls.consoleShader);
    }

    const vec4_t separatorColor = {
        0.0f, 0.0f, 0.0f, CONSOLE_SEPARATOR_ALPHA
    };
    SCR_FillRect(0.0f, (float)backgroundHeight,
                 (float)CON_VIRTUAL_WIDTH, 2.0f, separatorColor);
    ...
    SCR_DrawSmallStringExt(
        cls.rendererConfig.vidWidth -
            versionLength * CON_SMALL_CHAR_WIDTH,
        pixelHeight - CON_VERSION_BASELINE_OFFSET, version, color);

    int32_t rowCount =
        (pixelHeight - CON_SMALL_CHAR_WIDTH) / CON_SMALL_CHAR_WIDTH;
    ...
    if (con.displayLine != con.currentLine) {
        ...
        for (int32_t column = 0; column < con.lineWidth;
             column += CON_BACKSCROLL_INDICATOR_STEP) {
            SCR_DrawSmallChar(
                (int32_t)con.xAdjust +
                    column * CON_SMALL_CHAR_WIDTH +
                    CON_SMALL_CHAR_WIDTH,
                y, '^');
        }
    ...
    const float inverseXScale =
        (float)CON_VIRTUAL_WIDTH /
        (float)cls.rendererConfig.vidWidth;
    const float inverseYScale =
        (float)CON_VIRTUAL_HEIGHT /
        (float)cls.rendererConfig.vidHeight;
    for (int32_t row = 0;
         row < rowCount && displayLine >= 0;
         ++row, --displayLine, y -= CON_SMALL_CHAR_HEIGHT) {
        if (con.currentLine - displayLine >= con.totalLines)
            continue;

        const int32_t textRow = displayLine % con.totalLines;
        const uint16_t *text =
            &con.text[textRow * con.lineWidth];
        rendererExports.TextConsolePaint(
            (float)(int32_t)con.xAdjust * inverseXScale,
            (float)(y + CON_SMALL_CHAR_WIDTH) * inverseYScale,
            CON_TEXT_FONT, inverseYScale * CONSOLE_TEXT_SCALE,
            color, text, inverseXScale * (float)CON_SMALL_CHAR_WIDTH,
            con.lineWidth, CON_TEXT_STYLE);
    }
