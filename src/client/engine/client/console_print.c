...
static qboolean coduomp_console_add_encoded_cell(uint16_t cell)
{
    ...
    if (con.lineWidth <= 0 || con.totalLines <= 0 || con.lineCursor < 0 ||
        con.lineCursor >= con.lineWidth) {
        return qfalse;
    }
    ...
    cellIndex = (uint64_t)(uint32_t)row * (uint32_t)con.lineWidth +
                (uint32_t)con.lineCursor;
...
{
    const uint16_t cell =
        (uint16_t)(((uint16_t)tag << 8) | (uint16_t)value);
                            ...
                            int32_t firstComponentTag)
{
    for (int32_t component = 0; component < 3; ++component) {
        ...
        int32_t byteValue = coduo_fp_to_i32_extended(
            (long double)color[component] *
            (long double)CON_COLOR_COMPONENT_BYTE_MAX);
        ...
        const uint8_t tag =
            (uint8_t)(firstComponentTag + component);
...
{
    int32_t color =
        initialColor < 0 ? CON_DEFAULT_COLOR : initialColor;

    while (*text != '\0') {
        const int32_t character = (int32_t)(int8_t)*text;

        if (character == '^' && text[1] != '\0' &&
    ...
    int32_t messageTime, int32_t lineWidth, int32_t initialColor)
{
    ...
    if (destination == CON_DEST_GAME_MESSAGE ||
        destination == CON_DEST_BOLD_GAME_MESSAGE) {
        ...
        if (visibleLength > lineWidth) {
            ...
            const int32_t lineCount = (int32_t)ceil(
                (double)visibleLengthFloat / (double)lineWidth);
            balancedLineWidth = (int32_t)rint(
                (double)visibleLengthFloat / (double)lineCount);
        }
    }
    ...
    while (*cursor != '\0') {
        ...
        while (wordLength < lineWidth &&
               cursor[wordLength] > (uint8_t)' ') {
            ++wordLength;
        }
        if (wordLength != lineWidth &&
            con.lineCursor + wordLength > lineWidth) {
        ...
        if (con.lineCursor == 0 && character == ' ' &&
            lineWrapped != qfalse) {
            continue;
        }

        const int32_t row = con.currentLine % con.totalLines;
        con.text[row * con.lineWidth + con.lineCursor] =
            (uint16_t)(((uint16_t)color << 8) | character);
        ++con.lineCursor;

        if (con.lineCursor >= lineWidth ||
            (con.lineCursor >= balancedLineWidth && character == ' ')) {
            Con_Linefeed(destination, messageTime);
            lineWrapped = qtrue;
        }
    }
                     ...
                     int32_t messageTime, int32_t lineWidth)
{
    ...
    if (messageTime == 0) {
        ...
        case CON_DEST_GAME_MESSAGE:
            messageTime =
                FastRound(con_gameMessageTime->value * 1000.0f);
            break;
        case CON_DEST_BOLD_GAME_MESSAGE:
            messageTime =
                FastRound(con_boldGameMessageTime->value * 1000.0f);
            break;
    ...
    if (destination == CON_DEST_GAME_MESSAGE ||
        ...
        while (newline != NULL) {
            ...
            color = CL_ConsolePrint_AddLine(
                line, destination, messageTime, lineWidth, color);
            text = newline + 1;
            newline = strchr(text, '\n');
        }
    }
    ...
    (void)CL_ConsolePrint_AddLine(
        text, destination, messageTime, lineWidth, color);
}
    ...
    const float *victimColor, int32_t messageTime)
{
    ...
    /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered engine boundary input and state before use. */
    if (attackerName[0] != '\0') {
        CL_AddConsoleInfoColor(attackerColor,
                               CON_DEATH_COLOR_FIRST_TAG);
        CL_AddConsoleInfoChar(attackerName, CON_DEFAULT_COLOR);
        CL_AddConsoleInfoChar(" ", CON_DEFAULT_COLOR);
    }

    CL_AddConsoleInfoColor(victimColor,
                           CON_DEATH_VICTIM_COLOR_FIRST_TAG);

    const uint16_t widthCell =
        (uint16_t)(
            (uint16_t)coduo_fp_to_i32_extended(
                (long double)weaponIconWidth *
                (long double)CON_DEATH_ICON_SIZE_SCALE) |
    ...
    if (destination != CON_DEST_MINICONSOLE) {
        ...
        Con_UpdateNotifyLine(CON_DEST_MINICONSOLE, qfalse,
                             messageTime);
    }
    con.lastMessageDestination = destination;
}
...
void PbMsgToScreen(const char *prefix, const char *message)
{
    ...
    CL_ConsolePrint(va("%s: %s", prefix, message),
                    CON_DEST_GAME_MESSAGE, 0, 0);
}
