...
{
    static const char sourceStringEdRelativePath[] =
        "../source_data/string_resources/subtitle.st";
    static const char localStringEdPath[] =
    ...
    FS_BuildOSPath(
        fs_homepath->string, sourceStringEdRelativePath, "",
        sourceStringEdOSPath);
    sourceStringEdOSPath[
        strlen(sourceStringEdOSPath) - 1] = '\0';

    FILE *const writableProbe =
        fopen(sourceStringEdOSPath, "r+");
    ...
    FS_BuildOSPath(
        fs_basepath->string, fs_currentGameDir,
        localStringEdPath, localStringEdOSPath);
    ...
    char **const files = FS_ListFilteredFiles(
        "soundaliases", "csv", NULL, &fileCount);
    if (fileCount == 0) {
        Com_Printf(
    ...
    for (int32_t fileIndex = 0;
         ...
         ++fileIndex) {
        Com_sprintf(
            aliasPath, (int32_t)sizeof(aliasPath),
            "soundaliases/%s", files[fileIndex]);
        Com_LocalizeSoundAliasCsvFile(aliasPath, NULL);
        Hunk_ClearToMarkTemp();
    }

    FS_FreeFileList(files);
    COM_WriteFinalStringEdFile(
        localStringEdOSPath, sourceStringEdOSPath);
