...
static const char SOUND_ALIAS_TYPE_MISMATCH_WARNING_EXEMPT_FILE[] = "temp.wav";
...
void Com_LoadSoundAliasSounds(sndAliasBank_t bank)
{
    ...
    for (int32_t aliasIndex = 0; aliasIndex < aliasCount; ++aliasIndex) {
        ...
        if (alias->type == SND_ALIAS_TYPE_LOADED) {
            ...
            for (int32_t priorIndex = 0; priorIndex < aliasIndex; ++priorIndex) {
                ...
                if (warnedTypeMismatch == qfalse &&
                    coduo_crt_stricmp(alias->soundFile, SOUND_ALIAS_TYPE_MISMATCH_WARNING_EXEMPT_FILE) != 0) {
                    Com_Printf("WARNING: sound file '%s' used as streamed "
                               "in alias '%s' and loaded in alias '%s'\n",
                               alias->soundFile, prior->aliasName, alias->aliasName);
            ...
            if (alias->soundFileInfo == NULL) {
                Com_Printf("WARNING: loaded sound file 'sound/%s' "
                           "couldn't be read\n",
                           alias->soundFile);
                ++missingSoundCount;
            }
        ...
        if (alias->type == SND_ALIAS_TYPE_STREAMED) {
            ...
            for (int32_t priorIndex = 0; priorIndex < aliasIndex; ++priorIndex) {
                ...
                if (prior->type == SND_ALIAS_TYPE_STREAMED) {
                    alias->streamedFileExists = prior->streamedFileExists;
                    sharedSound = qtrue;
                    break;
                }
                if (warnedTypeMismatch == qfalse &&
                    ...
                    Com_Printf("WARNING: sound file '%s' used as streamed "
                               "in alias '%s' and loaded in alias '%s'\n",
                               alias->soundFile, alias->aliasName, prior->aliasName);
                    warnedTypeMismatch = qtrue;
                }
            }
            ...
            int32_t fileHandle;
            (void)FS_FOpenFileRead(va("sound/%s", alias->soundFile), &fileHandle, qfalse);
            ...
            if (fileHandle != 0) {
                ...
                Com_Printf("^1WARNING: streamed sound 'sound/%s' not found\n", alias->soundFile);
                ++missingSoundCount;
            }
        }
    }
    ...
    if (missingSoundCount != 0 && mss_errorOnMissing->integer != 0) {
        const errorParm_t errorCode = bank == SND_ALIAS_BANK_COMMON ? ERR_FATAL : ERR_DROP;
        /* NOT_FROM_ORIGINAL_SOURCE: validate this recovered engine boundary input and state before use. */
        Com_Error(errorCode, "%i sound file(s) are missing or in a bad format\n", missingSoundCount);
    }
}
...
void Com_UnloadSoundAliasSounds(sndAliasBank_t bank)
{
    ...
    for (int32_t aliasIndex = 0; aliasIndex < aliasCount; ++aliasIndex) {
        ...
        for (int32_t priorIndex = 0; priorIndex < aliasIndex; ++priorIndex) {
            ...
            if (prior->type == SND_ALIAS_TYPE_LOADED && prior->soundFile == alias->soundFile) {
                sharedSound = qtrue;
                break;
            }
        }
