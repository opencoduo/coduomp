...
static void coduomp_miniaudio_set_error(const char *operation, ma_result result)
{
    (void)snprintf(coduomp_miniaudio_error, sizeof(coduomp_miniaudio_error), "%s: %s", operation, ma_result_description(result));
}
...
static int16_t coduomp_miniaudio_decode_ima_nibble(uint8_t nibble, int32_t *predictor, int32_t *stepIndex)
{
...
static qboolean coduomp_miniaudio_decode_ima(const snd_alias_sound_file_t *soundFile, void **outPcm, uint32_t *outFrameCount)
{
    const uint32_t blockSize = soundFile->blockSize;
    if (soundFile->data == NULL || soundFile->dataLength == 0 || blockSize < CODUOMP_MINIAUDIO_IMA_HEADER_BYTES)
        return qfalse;

    const uint32_t framesPerBlock = 1u + (blockSize - CODUOMP_MINIAUDIO_IMA_HEADER_BYTES) * 2u;
    const uint32_t blockCount = (soundFile->dataLength + blockSize - 1u) / blockSize;
    const uint64_t capacityFrames = (uint64_t)framesPerBlock * blockCount;
    if (capacityFrames == 0 || capacityFrames > (uint64_t)UINT32_MAX / sizeof(int16_t))
        return qfalse;

    int16_t *const output = malloc((size_t)capacityFrames * sizeof(*output));
    if (output == NULL)
        return qfalse;
    ...
    while (inputOffset + CODUOMP_MINIAUDIO_IMA_HEADER_BYTES <= soundFile->dataLength) {
        ...
        for (uint32_t offset = CODUOMP_MINIAUDIO_IMA_HEADER_BYTES; offset < currentBlockSize; ++offset) {
            const uint8_t packed = block[offset];
            if (outputFrames < capacityFrames) {
                output[outputFrames++] = coduomp_miniaudio_decode_ima_nibble(packed & 15u, &predictor, &stepIndex);
            }
            if (outputFrames < capacityFrames) {
                output[outputFrames++] = coduomp_miniaudio_decode_ima_nibble(packed >> 4, &predictor, &stepIndex);
            }
        }
...
static void coduomp_miniaudio_pan_process(ma_node *node, const float **inputFrames, ma_uint32 *inputFrameCount, float **outputFrames,
                                          ma_uint32 *outputFrameCount)
{
    coduomp_miniaudio_pan_node_t *const pan = (coduomp_miniaudio_pan_node_t *)node;
...
static ma_node_vtable coduomp_miniaudio_pan_vtable = {coduomp_miniaudio_pan_process, NULL, 1, 1, 0};
...
static void coduomp_miniaudio_3d_sample_reset(coduomp_miniaudio_3d_sample_t *sample)
{
...
qboolean coduomp_miniaudio_provider_init(int32_t sampleRate)
{
    ...
    const ma_result result = ma_engine_init(&config, &coduomp_miniaudio_engine);
    if (result != MA_SUCCESS) {
        coduomp_miniaudio_set_error("could not open Miniaudio device", result);
        return qfalse;
    }
    coduomp_miniaudio_engine_initialized = qtrue;
    (void)snprintf(coduomp_miniaudio_error, sizeof(coduomp_miniaudio_error), "%s", "no Miniaudio error");
    return qtrue;
}
...
void coduomp_miniaudio_provider_shutdown(void)
{
    ...
    memset(&coduomp_miniaudio_engine, 0, sizeof(coduomp_miniaudio_engine));
...
coduomp_miniaudio_3d_sample_t *coduomp_miniaudio_3d_sample_create(void)
{
    if (!coduomp_miniaudio_engine_initialized)
        return NULL;
    coduomp_miniaudio_3d_sample_t *const sample = calloc(1, sizeof(*sample));
    if (sample != NULL) {
...
void coduomp_miniaudio_3d_sample_destroy(coduomp_miniaudio_3d_sample_t *sample)
{
    if (sample == NULL)
        return;
...
qboolean coduomp_miniaudio_3d_sample_set_info(coduomp_miniaudio_3d_sample_t *sample, const snd_alias_sound_file_t *soundFile)
{
    if (sample == NULL || soundFile == NULL || !coduomp_miniaudio_engine_initialized || soundFile->channelCount != 1 ||
        soundFile->sampleRate == 0 || soundFile->data == NULL || soundFile->dataLength == 0)
        return qfalse;

    coduomp_miniaudio_3d_sample_reset(sample);
    ma_format format;
    const void *pcmData = soundFile->data;
    if (soundFile->formatTag == CODUOMP_MINIAUDIO_WAVE_FORMAT_PCM && soundFile->bitsPerSample == 8) {
        format = ma_format_u8;
        sample->frameCount = soundFile->dataLength;
    } else if (soundFile->formatTag == CODUOMP_MINIAUDIO_WAVE_FORMAT_PCM && soundFile->bitsPerSample == 16 &&
               soundFile->dataLength % sizeof(int16_t) == 0) {
        format = ma_format_s16;
        sample->frameCount = soundFile->dataLength / (uint32_t)sizeof(int16_t);
    } else if (soundFile->formatTag == CODUOMP_MINIAUDIO_WAVE_FORMAT_IMA_ADPCM) {
        format = ma_format_s16;
        if (!coduomp_miniaudio_decode_ima(soundFile, &sample->ownedPcm, &sample->frameCount)) {
            (void)snprintf(coduomp_miniaudio_error, sizeof(coduomp_miniaudio_error), "%s", "could not decode Miniaudio 3D sample");
            return qfalse;
        }
        pcmData = sample->ownedPcm;
    } else {
        (void)snprintf(coduomp_miniaudio_error, sizeof(coduomp_miniaudio_error), "%s", "unsupported Miniaudio 3D sample format");
        return qfalse;
    }

    ma_audio_buffer_config bufferConfig = ma_audio_buffer_config_init(format, 1, sample->frameCount, pcmData, NULL);
    bufferConfig.sampleRate = soundFile->sampleRate;
    ma_result result = ma_audio_buffer_init(&bufferConfig, &sample->buffer);
    if (result != MA_SUCCESS) {
        coduomp_miniaudio_set_error("could not initialize Miniaudio sample buffer", result);
        coduomp_miniaudio_3d_sample_reset(sample);
        return qfalse;
    }
    ...
    result = ma_node_init(ma_engine_get_node_graph(&coduomp_miniaudio_engine), &nodeConfig, NULL, (ma_node *)&sample->panNode);
    if (result != MA_SUCCESS) {
        coduomp_miniaudio_set_error("could not initialize Miniaudio Fast 2D node", result);
        coduomp_miniaudio_3d_sample_reset(sample);
        return qfalse;
    }
    ...
    result = ma_node_attach_output_bus((ma_node *)&sample->panNode, 0, ma_engine_get_endpoint(&coduomp_miniaudio_engine), 0);
    if (result != MA_SUCCESS) {
        coduomp_miniaudio_set_error("could not attach Miniaudio Fast 2D node", result);
        coduomp_miniaudio_3d_sample_reset(sample);
        return qfalse;
    }

    ma_sound_config soundConfig = ma_sound_config_init_2(&coduomp_miniaudio_engine);
    ...
    result = ma_sound_init_ex(&coduomp_miniaudio_engine, &soundConfig, &sample->sound);
    if (result != MA_SUCCESS) {
        coduomp_miniaudio_set_error("could not initialize Miniaudio 3D voice", result);
        coduomp_miniaudio_3d_sample_reset(sample);
        return qfalse;
    }
...
void coduomp_miniaudio_3d_sample_forget_sound(coduomp_miniaudio_3d_sample_t *sample, const snd_alias_sound_file_t *soundFile)
{
    if (sample != NULL && sample->soundFile == soundFile)
        coduomp_miniaudio_3d_sample_reset(sample);
}

void coduomp_miniaudio_3d_sample_set_channel_gains(coduomp_miniaudio_3d_sample_t *sample, float left, float right)
{
    if (sample == NULL)
        return;
...
void coduomp_miniaudio_3d_sample_set_playback_rate(coduomp_miniaudio_3d_sample_t *sample, int32_t playbackRate, int32_t baseRate)
{
    if (sample == NULL || !sample->soundInitialized || playbackRate <= 0 || baseRate <= 0)
        return;
...
void coduomp_miniaudio_3d_sample_set_loop_count(coduomp_miniaudio_3d_sample_t *sample, int32_t loopCount)
{
    if (sample != NULL && sample->soundInitialized)
        ma_sound_set_looping(&sample->sound, loopCount == 0 ? MA_TRUE : MA_FALSE);
}

void coduomp_miniaudio_3d_sample_end(coduomp_miniaudio_3d_sample_t *sample)
{
    if (sample == NULL || !sample->soundInitialized)
        return;
    (void)ma_sound_stop(&sample->sound);
...
void coduomp_miniaudio_3d_sample_stop(coduomp_miniaudio_3d_sample_t *sample)
{
    if (sample != NULL && sample->soundInitialized)
        (void)ma_sound_stop(&sample->sound);
}

void coduomp_miniaudio_3d_sample_resume(coduomp_miniaudio_3d_sample_t *sample)
{
    if (sample != NULL && sample->soundInitialized && sample->active)
        (void)ma_sound_start(&sample->sound);
}

void coduomp_miniaudio_3d_sample_start(coduomp_miniaudio_3d_sample_t *sample)
{
    if (sample == NULL || !sample->soundInitialized)
        return;
...
qboolean coduomp_miniaudio_3d_sample_is_active(coduomp_miniaudio_3d_sample_t *sample)
{
    return sample != NULL && sample->soundInitialized && sample->active && !ma_sound_at_end(&sample->sound) ? qtrue : qfalse;
}

uint32_t coduomp_miniaudio_3d_sample_frame_count(const coduomp_miniaudio_3d_sample_t *sample)
{
    return sample != NULL ? sample->frameCount : 0;
}

uint32_t coduomp_miniaudio_3d_sample_cursor_frame(const coduomp_miniaudio_3d_sample_t *sample)
{
    if (sample == NULL || !sample->soundInitialized)
        return 0;
    ma_uint64 cursor = 0;
    if (ma_sound_get_cursor_in_pcm_frames(&sample->sound, &cursor) != MA_SUCCESS)
        return 0;
    return cursor <= UINT32_MAX ? (uint32_t)cursor : UINT32_MAX;
}

void coduomp_miniaudio_3d_sample_seek_frame(coduomp_miniaudio_3d_sample_t *sample, uint32_t frame)
{
    if (sample == NULL || !sample->soundInitialized)
        return;
