...
#elif defined(__i386__) || defined(__x86_64__)
    __cpuid_count(leaf, 0, registers[0], registers[1], registers[2], registers[3]);
#else
    (void)leaf;
    registers[0] = 0;
    registers[1] = 0;
...
static qboolean Sys_CpuHas3DNow(void)
{
    ...
    return (registers[3] & UINT32_C(0x80000000)) != 0 ? qtrue : qfalse;
}
...
static qboolean Sys_CpuHasSse(void)
{
    ...
    return (registers[3] & (UINT32_C(1) << 25)) != 0 ? qtrue : qfalse;
}
...
static qboolean Sys_CpuHasMmx(void)
{
    ...
    Sys_ReadCpuid(SYS_CPUID_FEATURES_LEAF, registers);
    return (registers[3] & (UINT32_C(1) << 23)) != 0 ? qtrue : qfalse;
}
#endif
...
{
    const double rounded = rint((double)value + sysRoundHalfBias);
...
double Sys_GetCpuFrequencyMHz(void)
{
    ...
    return (double)((cyclesEnd - cyclesStart) * (uint64_t)performanceFrequency.QuadPart) * 0.000001 /
    ...
    if (sysctlbyname("hw.cpufrequency_max", &frequencyHz, &frequencySize, NULL, 0) != 0) {
        frequencySize = sizeof(frequencyHz);
        if (sysctlbyname("hw.cpufrequency", &frequencyHz, &frequencySize, NULL, 0) != 0) {
            return 0.0;
        }
    }
    return (double)frequencyHz * 0.000001;
#elif defined(__i386__) || defined(__x86_64__)
    struct timespec delay = {0, SYS_CPU_MEASUREMENT_MSEC * 1000000L};
    ...
    elapsedSeconds = (double)(counterEnd.tv_sec - counterStart.tv_sec) + (double)(counterEnd.tv_nsec - counterStart.tv_nsec) * 0.000000001;
...
int32_t Sys_GetPhysicalMemoryMB(void)
{
    ...
    if (kernel32 != NULL) {
        getMemoryStatusEx = (global_memory_status_ex_t)(uintptr_t)GetProcAddress(kernel32, "GlobalMemoryStatusEx");
    }
    ...
    if (availableVirtualBytes < SYS_MIN_AVAILABLE_VIRTUAL_MEMORY) {
        ...
        if (MessageBoxA(NULL, body, title, SYS_LOW_MEMORY_DIALOG_FLAGS) != SYS_DIALOG_RESULT_YES) {
    ...
    if (sysctlbyname("hw.memsize", &totalPhysicalBytes, &memorySize, NULL, 0) != 0) {
    ...
    const float physicalMegabytes = (float)totalPhysicalBytes * (1.0f / SYS_BYTES_PER_MEGABYTE);
    int32_t roundedMegabytes = Sys_RoundPositiveFloatToInt(physicalMegabytes);
    ...
    if (roundedMegabytes < 0 || (uint64_t)roundedMegabytes * SYS_BYTES_PER_MEGABYTE != totalPhysicalBytes ||
...
{
    return sysPhysicalMemoryMB <= SYS_LOW_MEMORY_THRESHOLD_BYTES ? qtrue : qfalse;
}
...
qboolean Sys_DetectSSESupport(void)
{
    ...
    if (__get_cpuid(1, &eax, &ebx, &ecx, &edx) != 0 && (edx & bit_SSE) != 0) {
