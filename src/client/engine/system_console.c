...
#if defined(_WIN32)
static console_input_field_t sysConsoleHistory[SYS_CONSOLE_HISTORY_COUNT]; /* original 0x009d0920 */
static int32_t sysConsoleHistoryHead; /* original 0x0389fde4 */
static int32_t sysConsoleHistoryLine; /* original 0x0389fde8 */
static int32_t sysConsoleCompletionMatchCount; /* original 0x009d5020 */
static char sysConsoleCompletionShortestMatch[CON_COMPLETION_MATCH_SIZE];
    /* original 0x009d5028 */
static uint32_t sysConsoleTextLength; /* original 0x009d5428 */
static int32_t sysConsoleCompletionEnumerationIndex; /* original 0x009d542c */
static console_input_field_t sysConsoleField; /* original 0x009d5430 */
static int32_t sysConsoleCompletionMatchIndex; /* original 0x009d554c */
static char sysConsoleCompletionString[CON_COMPLETION_MATCH_SIZE];
    /* original 0x009d5550 */
...
static WNDPROC sysConsoleInputWndProc;          /* original 0x009d5f98 */
static qboolean sysConsoleErrorFlash; /* original 0x009d5f9c */
static int32_t sysConsoleCompletionPrefixLength; /* original 0x009d5fa0 */
...
LRESULT CALLBACK ConWndProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        (void)SetTimer(window, SYS_CONSOLE_ERROR_TIMER_ID, SYS_CONSOLE_ERROR_TIMER_MSEC, NULL);
        break;
    ...
    case WM_SIZE: {
        ...
        (void)SetWindowPos((HWND)sysConsoleOutputWindow, NULL, SYS_CONSOLE_SIDE_INSET, SYS_CONSOLE_OUTPUT_TOP,
                           width - SYS_CONSOLE_CONTROL_WIDTH_INSET, height - SYS_CONSOLE_OUTPUT_HEIGHT_INSET, 0);
        (void)SetWindowPos((HWND)sysConsoleInputWindow, NULL, SYS_CONSOLE_SIDE_INSET, height - SYS_CONSOLE_INPUT_BOTTOM_INSET,
                           width - SYS_CONSOLE_CONTROL_WIDTH_INSET, SYS_CONSOLE_INPUT_HEIGHT, 0);
        ...
        scaledButtonWidth *= (long double)0.001612903201021254f;
        scaledButtonWidth *= (long double)72.0f;
        const int32_t buttonWidth = (int32_t)scaledButtonWidth;
        const int32_t buttonY = height - SYS_CONSOLE_BUTTON_BOTTOM_INSET;

        (void)SetWindowPos(sysConsoleCopyButton, NULL, SYS_CONSOLE_SIDE_INSET, buttonY, buttonWidth, SYS_CONSOLE_BUTTON_HEIGHT, 0);
        (void)SetWindowPos(sysConsoleClearButton, NULL, buttonWidth + SYS_CONSOLE_BUTTON_GAP, buttonY, buttonWidth,
                           SYS_CONSOLE_BUTTON_HEIGHT, 0);
        (void)SetWindowPos(sysConsoleQuitButton, NULL, width - buttonWidth - SYS_CONSOLE_CONTROL_WIDTH_INSET, buttonY, buttonWidth,
                           SYS_CONSOLE_BUTTON_HEIGHT, 0);
    ...
    case WM_ACTIVATE:
        ...
        if (dedicated != NULL && dedicated->integer == 0 && com_viewlog != NULL) {
            if (com_viewlog->integer == SYS_CONSOLE_VISIBLE && HIWORD((DWORD_PTR)wParam) != 0) {
                (void)Cvar_Set2("viewlog", "2", qtrue);
            } else if (com_viewlog->integer == SYS_CONSOLE_MINIMIZED && HIWORD((DWORD_PTR)wParam) == 0) {
    ...
    case WM_CLOSE:
        if (dedicated != NULL && dedicated->integer != 0) {
            ...
            Sys_QueEvent(0, SE_CONSOLE, 0, 0, (int32_t)strlen(command) + 1, command);
            return 0;
        }
    ...
    case WM_CTLCOLORSTATIC: {
        ...
        if (control == (HWND)sysConsoleErrorWindow) {
            ...
            (void)SetTextColor(deviceContext, sysConsoleErrorFlash != qfalse ? errorFlashText : 0);
    ...
    case WM_TIMER:
        if (wParam == SYS_CONSOLE_ERROR_TIMER_ID) {
            sysConsoleErrorFlash = sysConsoleErrorFlash == qfalse ? qtrue : qfalse;
            if (sysConsoleErrorWindow != NULL) {
                (void)InvalidateRect((HWND)sysConsoleErrorWindow, NULL, FALSE);
            }
        }
        break;
    ...
    case WM_COMMAND:
        ...
        case SYS_CONSOLE_COPY_BUTTON_ID:
            (void)SendMessageA((HWND)sysConsoleOutputWindow, EM_SETSEL, 0, -1);
            (void)SendMessageA((HWND)sysConsoleOutputWindow, WM_COPY, 0, 0);
            break;
        ...
        case SYS_CONSOLE_CLEAR_BUTTON_ID:
            (void)SendMessageA((HWND)sysConsoleOutputWindow, EM_SETSEL, 0, -1);
            (void)SendMessageA((HWND)sysConsoleOutputWindow, EM_REPLACESEL, 0, (LPARAM) "");
...
{
    const int32_t prefixLength = (int32_t)strlen(sysConsoleCompletionString);

    if (Q_stricmpn(sysConsoleCompletionString, candidate, prefixLength) != 0) {
        return;
    }

    if (sysConsoleCompletionEnumerationIndex == sysConsoleCompletionMatchIndex) {
        Q_strncpyz(sysConsoleCompletionShortestMatch, candidate, CON_COMPLETION_MATCH_SIZE);
    }
...
static void Sys_ConsoleFindMatches(const char *candidate)
{
    ...
    int32_t index = 0;
    while (index < CON_COMPLETION_MATCH_SIZE - 1 && candidate[index] != '\0') {
        if (coduo_crt_tolower((int32_t)(int8_t)sysConsoleCompletionShortestMatch[index]) !=
...
static void Sys_ConsoleConcatRemaining(const char *source, const char *separator)
{
    ...
    Q_strcat(sysConsoleField.buffer, CON_INPUT_BUFFER_SIZE, remaining + strlen(separator));
}
...
{
    if (Q_stricmpn(sysConsoleCompletionShortestMatch, candidate, sysConsoleCompletionPrefixLength) == 0) {
        Sys_Print(va("  ^9%s^0\n", candidate));
    }
}
...
static void Sys_ConsoleCompleteCommand(qboolean showMatches)
{
    ...
    if (coduo_crt_strnicmp(command, "pb_", 3) == 0) {
        ...
        Q_strncpyz(punkBusterCommand, command, CON_INPUT_BUFFER_SIZE);
        ...
        Com_sprintf(sysConsoleField.buffer, CON_INPUT_BUFFER_SIZE, "%s", punkBusterCommand);
        sysConsoleField.cursor = (int32_t)strlen(sysConsoleField.buffer);
        return;
    }

    if (sysConsoleCompletionPrefixLength == 0) {
        Cmd_TokenizeString(sysConsoleField.buffer);
        Q_strncpyz(sysConsoleCompletionString, Cmd_Argv(0), CON_COMPLETION_MATCH_SIZE);
        if (sysConsoleCompletionString[0] == '/' || sysConsoleCompletionString[0] == '\\') {
            const size_t shiftedLength = strlen(sysConsoleCompletionString);
            memmove(sysConsoleCompletionString, &sysConsoleCompletionString[1], shiftedLength);
        }
        ...
        Com_sprintf(sysConsoleField.buffer, CON_INPUT_BUFFER_SIZE, "%s", sysConsoleCompletionShortestMatch);

        if (sysConsoleCompletionMatchCount == 1) {
            if (Cmd_Argc() == 1) {
                Q_strcat(sysConsoleField.buffer, CON_INPUT_BUFFER_SIZE, " ");
            } else {
                Sys_ConsoleConcatRemaining(originalField.buffer, sysConsoleCompletionString);
            }
            sysConsoleField.cursor = (int32_t)strlen(sysConsoleField.buffer);
        } else {
            sysConsoleField.cursor = (int32_t)strlen(sysConsoleField.buffer);
            sysConsoleCompletionPrefixLength = sysConsoleField.cursor;
            Sys_ConsoleConcatRemaining(originalField.buffer, sysConsoleCompletionString);
            showMatches = qtrue;
        }
    } else if (sysConsoleCompletionMatchCount != 1) {
        char previousMatch[CON_COMPLETION_MATCH_SIZE];
        Q_strncpyz(previousMatch, sysConsoleCompletionShortestMatch, CON_COMPLETION_MATCH_SIZE);

        ++sysConsoleCompletionMatchIndex;
        if (sysConsoleCompletionMatchIndex == sysConsoleCompletionMatchCount) {
            sysConsoleCompletionMatchIndex = 0;
        }
        ...
        Com_sprintf(sysConsoleField.buffer, CON_INPUT_BUFFER_SIZE, "%s", sysConsoleCompletionShortestMatch);
        sysConsoleField.cursor = (int32_t)strlen(sysConsoleField.buffer);
        Sys_ConsoleConcatRemaining(originalField.buffer, previousMatch);
    }

    if (sysConsoleCompletionMatchCount == 1) {
        sysConsoleCompletionPrefixLength = (int32_t)strlen(sysConsoleCompletionShortestMatch);
    }

    if (showMatches != qfalse && sysConsoleCompletionMatchCount > 0) {
        console_input_field_t displayedField = sysConsoleField;
        displayedField.buffer[sysConsoleCompletionPrefixLength] = '\0';
...
LRESULT CALLBACK InputLineWndProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message) {
    case WM_CHAR:
        (void)GetWindowTextA((HWND)sysConsoleInputWindow, sysConsoleField.buffer, CON_INPUT_BUFFER_SIZE);
        (void)SendMessageA((HWND)sysConsoleInputWindow, EM_GETSEL, 0, (LPARAM)&sysConsoleField.cursor);
        sysConsoleField.scroll = 0;

        if (wParam == '\r') {
            const size_t queuedLength = strlen(sysConsoleInputLine);
            /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
            if (queuedLength > SYS_CONSOLE_INPUT_APPEND_LIMIT) {
                Sys_Print("WARNING: console input queue is full\n");
                return 0;
            }
            strncat(sysConsoleInputLine, sysConsoleField.buffer, (size_t)SYS_CONSOLE_INPUT_APPEND_LIMIT - queuedLength);

            char *const lineEnd = &sysConsoleInputLine[strlen(sysConsoleInputLine)];
            lineEnd[0] = '\n';
            lineEnd[1] = '\0';

            (void)SetWindowTextA((HWND)sysConsoleInputWindow, "");
            ...
            sysConsoleHistory[sysConsoleHistoryHead % SYS_CONSOLE_HISTORY_COUNT] = sysConsoleField;
        ...
        if (wParam == '\t') {
            ...
            (void)SetWindowTextA((HWND)sysConsoleInputWindow, sysConsoleField.buffer);
            (void)SendMessageA((HWND)sysConsoleInputWindow, EM_SETSEL, (WPARAM)sysConsoleField.cursor, (LPARAM)sysConsoleField.cursor);
            return 0;
        }
    ...
    case WM_KEYDOWN:
        if (wParam == VK_UP) {
            if (sysConsoleHistoryHead - sysConsoleHistoryLine < SYS_CONSOLE_HISTORY_COUNT && sysConsoleHistoryLine > 0) {
        ...
        sysConsoleField = sysConsoleHistory[sysConsoleHistoryLine % SYS_CONSOLE_HISTORY_COUNT];
        (void)SetWindowTextA((HWND)sysConsoleInputWindow, sysConsoleField.buffer);
        (void)SendMessageA((HWND)sysConsoleInputWindow, EM_SETSEL, (WPARAM)sysConsoleField.cursor, (LPARAM)sysConsoleField.cursor);
        sysConsoleCompletionPrefixLength = 0;
        return 0;
    ...
    case WM_KILLFOCUS:
        if ((HWND)wParam == (HWND)sysConsoleWindow || (HWND)wParam == (HWND)sysConsoleErrorWindow) {
            (void)SetFocus((HWND)sysConsoleInputWindow);
            return 0;
        }
        break;
    ...
    return CallWindowProcA(sysConsoleInputWndProc, window, message, wParam, lParam);
}
#endif
...
void Sys_CreateConsole(void)
{
    ...
    static const char consoleClassName[] = "CoD United Offensive WinConsole";
    static const char consoleWindowTitle[] = "CoD United Offensive Console";
    static const DWORD consoleWindowStyle = WS_POPUP | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX;
    static const DWORD consoleInputStyle = WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL;
    static const DWORD consoleButtonStyle = WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON;
    static const DWORD consoleOutputStyle = WS_CHILD | WS_VISIBLE | WS_BORDER | WS_VSCROLL | ES_MULTILINE | ES_AUTOVSCROLL | ES_READONLY;
    ...
    consoleClass.hIcon = LoadIconA(instance, MAKEINTRESOURCEA(SYS_CONSOLE_ICON_RESOURCE_ID));
    ...
    consoleClass.hbrBackground = (HBRUSH)(INT_PTR)(COLOR_MENU + 1);
    consoleClass.lpszClassName = consoleClassName;
    if (RegisterClassA(&consoleClass) == 0)
        return;
    sysConsoleClassRegistered = qtrue;

    RECT windowRect = {0, 0, SYS_CONSOLE_CLIENT_WIDTH, SYS_CONSOLE_CLIENT_HEIGHT};
    (void)AdjustWindowRect(&windowRect, consoleWindowStyle, FALSE);

    HDC desktopContext = GetDC(GetDesktopWindow());
    const int32_t screenWidth = GetDeviceCaps(desktopContext, HORZRES);
    const int32_t screenHeight = GetDeviceCaps(desktopContext, VERTRES);
    (void)ReleaseDC(GetDesktopWindow(), desktopContext);

    sysConsoleWidth = windowRect.right - windowRect.left + 1;
    sysConsoleHeight = windowRect.bottom - windowRect.top + 1;
    sysConsoleWindow =
        CreateWindowExA(0, consoleClassName, consoleWindowTitle, consoleWindowStyle, (screenWidth - SYS_CONSOLE_CENTERING_WIDTH) / 2,
                        (screenHeight - SYS_CONSOLE_CLIENT_HEIGHT) / 2, sysConsoleWidth, sysConsoleHeight, NULL, NULL, instance, NULL);
    if (sysConsoleWindow == NULL) {
        Sys_DestroyConsole();
        return;
    }
    ...
    const int32_t fontHeight = -MulDiv(SYS_CONSOLE_FONT_POINT_SIZE, GetDeviceCaps(consoleContext, LOGPIXELSY), SYS_CONSOLE_POINTS_PER_INCH);
    sysConsoleFont = CreateFontA(fontHeight, 0, 0, 0, FW_LIGHT, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                                 CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, FIXED_PITCH | FF_MODERN, "Courier New");
    (void)ReleaseDC((HWND)sysConsoleWindow, consoleContext);

    sysConsoleInputWindow = CreateWindowExA(WS_EX_CLIENTEDGE, "edit", NULL, consoleInputStyle, SYS_CONSOLE_INPUT_X, SYS_CONSOLE_INPUT_Y,
                                            SYS_CONSOLE_INPUT_WIDTH, SYS_CONSOLE_INPUT_HEIGHT, (HWND)sysConsoleWindow,
                                            (HMENU)(INT_PTR)SYS_CONSOLE_INPUT_CONTROL_ID, instance, NULL);

    sysConsoleCopyButton =
        CreateWindowExA(0, "button", NULL, consoleButtonStyle, SYS_CONSOLE_COPY_BUTTON_X, SYS_CONSOLE_BUTTON_Y, SYS_CONSOLE_BUTTON_WIDTH,
                        SYS_CONSOLE_BUTTON_HEIGHT, (HWND)sysConsoleWindow, (HMENU)(INT_PTR)SYS_CONSOLE_COPY_BUTTON_ID, instance, NULL);
    (void)SendMessageA(sysConsoleCopyButton, WM_SETTEXT, 0, (LPARAM) "copy");

    sysConsoleClearButton =
        CreateWindowExA(0, "button", NULL, consoleButtonStyle, SYS_CONSOLE_CLEAR_BUTTON_X, SYS_CONSOLE_BUTTON_Y, SYS_CONSOLE_BUTTON_WIDTH,
                        SYS_CONSOLE_BUTTON_HEIGHT, (HWND)sysConsoleWindow, (HMENU)(INT_PTR)SYS_CONSOLE_CLEAR_BUTTON_ID, instance, NULL);
    (void)SendMessageA(sysConsoleClearButton, WM_SETTEXT, 0, (LPARAM) "clear");

    sysConsoleQuitButton =
        CreateWindowExA(0, "button", NULL, consoleButtonStyle, SYS_CONSOLE_QUIT_BUTTON_X, SYS_CONSOLE_BUTTON_Y, SYS_CONSOLE_BUTTON_WIDTH,
                        SYS_CONSOLE_BUTTON_HEIGHT, (HWND)sysConsoleWindow, (HMENU)(INT_PTR)SYS_CONSOLE_QUIT_BUTTON_ID, instance, NULL);
    (void)SendMessageA(sysConsoleQuitButton, WM_SETTEXT, 0, (LPARAM) "quit");

    sysConsoleOutputWindow = CreateWindowExA(WS_EX_CLIENTEDGE, "edit", NULL, consoleOutputStyle, SYS_CONSOLE_OUTPUT_X, SYS_CONSOLE_OUTPUT_Y,
                                             SYS_CONSOLE_OUTPUT_WIDTH, SYS_CONSOLE_OUTPUT_HEIGHT, (HWND)sysConsoleWindow,
                                             (HMENU)(INT_PTR)SYS_CONSOLE_OUTPUT_CONTROL_ID, instance, NULL);
    (void)SendMessageA((HWND)sysConsoleOutputWindow, WM_SETFONT, (WPARAM)sysConsoleFont, 0);

    /* SetWindowLongPtr is the pointer-width-safe spelling of the original
     * i386 SetWindowLongA(GWL_WNDPROC) operation. */
    sysConsoleInputWndProc = (WNDPROC)SetWindowLongPtrA((HWND)sysConsoleInputWindow, GWLP_WNDPROC, (LONG_PTR)InputLineWndProc);
    (void)SendMessageA((HWND)sysConsoleInputWindow, WM_SETFONT, (WPARAM)sysConsoleFont, 0);
...
void Sys_DestroyConsole(void)
{
    ...
    if (sysConsoleClassRegistered != qfalse) {
        (void)UnregisterClassA("CoD United Offensive WinConsole", (HINSTANCE)sysApplicationInstance);
...
void Sys_Print(const char *text)
{
    ...
    while (text[sourceIndex] != '\0' && outputLength < SYS_CONSOLE_PRINT_BUFFER_CAPACITY - 1) {
        ...
        if (character == '\n' && text[sourceIndex + 1] == '\r') {
            ...
            output[outputLength++] = '\r';
            output[outputLength++] = '\n';
        } else if (character == '^' && text[sourceIndex + 1] != '\0' && text[sourceIndex + 1] != '^' && text[sourceIndex + 1] >= '0' &&
    ...
    if (sysConsoleTextLength > (uint32_t)SYS_CONSOLE_TEXT_RETAIN_LIMIT) {
    ...
    SendMessageA((HWND)sysConsoleOutputWindow, EM_REPLACESEL, 0, (LPARAM)output);
#else
...
static qboolean WIN_UsesNtAltTabHotkey(void)
{
    ...
    const char *const name = architecture != NULL && architecture->string != NULL ? architecture->string : "";
...
void WIN_DisableAltTab(void)
{
    ...
    if (WIN_UsesNtAltTabHotkey() != qfalse) {
        ...
        BOOL previousState;
        SystemParametersInfoA(SPI_SETSCREENSAVERRUNNING, TRUE, &previousState, 0);
    }
#endif
...
void WIN_EnableAltTab(void)
{
    ...
    if (WIN_UsesNtAltTabHotkey() != qfalse) {
        ...
        BOOL previousState;
        SystemParametersInfoA(SPI_SETSCREENSAVERRUNNING, FALSE, &previousState, 0);
    }
#endif
...
static int32_t MapKey(uint32_t virtualKey, uint32_t keyData)
{
    ...
    _Static_assert(_Alignof(win32_foreign_key_t) == 1, "Win32 foreign-key pair alignment changed");
    _Static_assert(offsetof(win32_foreign_key_t, windowsCharacter) == 0x00, "Win32 foreign-key input byte moved");
    _Static_assert(offsetof(win32_foreign_key_t, engineCharacter) == 0x01, "Win32 foreign-key output byte moved");
    _Static_assert(sizeof(win32_foreign_key_t) == 0x02, "Win32 foreign-key pair stride changed");
#endif
    static const win32_foreign_key_t foreignKeys[] = {{181, 128}, {191, 129}, {223, 130}, {224, 131}, {225, 132}, {228, 133}, {229, 134},
                                                      {230, 135}, {231, 136}, {232, 137}, {233, 138}, {236, 139}, {241, 140}, {242, 141},
                                                      {243, 142}, {246, 143}, {248, 144}, {249, 145}, {250, 146}, {252, 147}};
    const qboolean extended = (qboolean)((keyData >> WIN32_EXTENDED_KEY_SHIFT) & 1u);

    if (((keyData >> WIN32_SCAN_CODE_SHIFT) & WIN32_SCAN_CODE_MASK) == WIN32_GRAVE_SCAN_CODE) {
        return '~';
    }
    ...
    case VK_TAB:
        return K_TAB;
    case VK_CLEAR:
        return extended != qfalse ? 0 : K_KP_5;
    case VK_RETURN:
        return extended != qfalse ? K_KP_ENTER : K_ENTER;
    case VK_SHIFT:
        return K_SHIFT;
    case VK_CONTROL:
        return K_CTRL;
    ...
    case VK_SPACE:
        return K_SPACE;
    case VK_PRIOR:
        return extended != qfalse ? K_PGUP : K_KP_PGUP;
    case VK_NEXT:
        return extended != qfalse ? K_PGDN : K_KP_PGDN;
    case VK_END:
        return extended != qfalse ? K_END : K_KP_END;
    case VK_HOME:
        return extended != qfalse ? K_HOME : K_KP_HOME;
    case VK_LEFT:
        return extended != qfalse ? K_LEFTARROW : K_KP_LEFTARROW;
    case VK_UP:
        return extended != qfalse ? K_UPARROW : K_KP_UPARROW;
    case VK_RIGHT:
        return extended != qfalse ? K_RIGHTARROW : K_KP_RIGHTARROW;
    case VK_DOWN:
        return extended != qfalse ? K_DOWNARROW : K_KP_DOWNARROW;
    case VK_INSERT:
        return extended != qfalse ? K_INS : K_KP_INS;
    case VK_DELETE:
        return extended != qfalse ? K_DEL : K_KP_DEL;
    case VK_NUMPAD0:
        return K_KP_INS;
    case VK_NUMPAD1:
        return K_KP_END;
    ...
    if (virtualKey >= 'A' && virtualKey <= 'Z') {
        return (int32_t)(extended != qfalse ? virtualKey : virtualKey + ('a' - 'A'));
    }
    if (virtualKey >= VK_F1 && virtualKey <= VK_F12)
        return K_F1 + (int32_t)(virtualKey - VK_F1);

    int32_t key = (int32_t)(MapVirtualKeyA(virtualKey, WIN32_MAP_VIRTUAL_KEY_TO_CHARACTER) & WIN32_SCAN_CODE_MASK);
    if (key <= WIN32_ASCII_LIMIT)
        return key;

    for (size_t index = 0; index < sizeof(foreignKeys) / sizeof(foreignKeys[0]); ++index) {
...
LRESULT CALLBACK MainWndProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (message == registeredMouseWheelMessage) {
        const int32_t key = (int32_t)wParam > 0 ? K_MWHEELUP : K_MWHEELDOWN;
    ...
    case WM_CREATE:
        ...
        win_allowAltTab = Cvar_Get("win_allowalttab", CODUOMP_WIN_ALLOW_ALT_TAB_DEFAULT, CVAR_ARCHIVE | CVAR_LATCH);
        registeredMouseWheelMessage = RegisterWindowMessageA("MSWHEEL_ROLLMSG");

        if (r_fullscreen->integer == 1 && win_allowAltTab->integer == 0) {
            WIN_DisableAltTab();
        } else {
            WIN_EnableAltTab();
        }
    ...
    case WM_MOVE:
        if (r_fullscreen->integer == 0) {
            ...
            const int32_t x = (int32_t)(int16_t)LOWORD((DWORD_PTR)lParam) + border.left;
            const int32_t y = (int32_t)(int16_t)HIWORD((DWORD_PTR)lParam) + border.top;
    ...
    case WM_ACTIVATE:
        AppActivate(LOWORD((DWORD_PTR)wParam) != WA_INACTIVE ? qtrue : qfalse, (qboolean)HIWORD((DWORD_PTR)wParam));
        break;
    ...
    case WM_SYSKEYDOWN:
        if (wParam == VK_RETURN) {
            if (cls.state != CA_LOADING && r_fullscreen != NULL) {
                Cvar_SetValue("r_fullscreen", r_fullscreen->integer == 0 ? 1.0f : 0.0f);
    ...
    case WM_MOUSEWHEEL: {
        const int32_t key = (int16_t)HIWORD((DWORD_PTR)wParam) > 0 ? K_MWHEELUP : K_MWHEELDOWN;
        Sys_QueEvent(sysMsgTime, SE_KEY, key, qtrue, 0, NULL);
        Sys_QueEvent(sysMsgTime, SE_KEY, key, qfalse, 0, NULL);
        break;
    }
    ...
    case WM_POWERBROADCAST:
        if (wParam == WIN_POWER_QUERY_SUSPEND || wParam == WIN_POWER_QUERY_STANDBY) {
...
{
    strncpy(sysConsoleErrorText, text, SYS_CONSOLE_ERROR_TEXT_CAPACITY - 1);
    ...
    sysConsoleErrorWindow = CreateWindowExA(0, "static", NULL, WS_CHILD | WS_VISIBLE | SS_SUNKEN, 6, 5, 606, 60, (HWND)sysConsoleWindow,
                                            (HMENU)(uintptr_t)10, (HINSTANCE)sysApplicationInstance, NULL);
    SendMessageA((HWND)sysConsoleErrorWindow, WM_SETFONT, (WPARAM)sysConsoleFont, 0);
    SetWindowTextA((HWND)sysConsoleErrorWindow, sysConsoleErrorText);
    DestroyWindow((HWND)sysConsoleInputWindow);
    sysConsoleInputWindow = NULL;
    MessageBoxA(NULL, text, Sys_LocalizeString("WIN_ERROR"), SYS_ERROR_DIALOG_TYPE);
#else
