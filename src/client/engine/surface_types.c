...
#define SURFACE_TYPE_INFO(type_, name_, hasContents_, contents_) \
    {(name_), (hasContents_), (uint32_t)(type_) << SURFACE_TYPE_SHIFT, (contents_)}
...
const surfaceParm_t surfaceParms[] = {SURFACE_TYPE_INFO(SURFACE_TYPE_BARK, "bark", qfalse, 0),
                                      ...
                                      SURFACE_TYPE_INFO(SURFACE_TYPE_FOLIAGE, "foliage", qtrue, CONTENTS_FOLIAGE),
                                      SURFACE_TYPE_INFO(SURFACE_TYPE_GLASS, "glass", qtrue, CONTENTS_GLASS),
                                      ...
                                      SURFACE_TYPE_INFO(SURFACE_TYPE_WATER, "water", qtrue, CONTENTS_WATER),
     ...
     * surfaceFlags or contents mask is present in the collision trace. */
                                      {"opaqueglass", qfalse, (uint32_t)SURFACE_TYPE_GLASS << SURFACE_TYPE_SHIFT, 0},
...
_Static_assert(sizeof(surfaceParms) == 55 * sizeof(surfaceParms[0]), "surface-parm table row count changed");
#endif
...
surfaceType_t CL_SurfaceTypeFromName(const char *name)
{
    ...
    for (size_t index = 0; index < SURFACE_TYPE_INFO_COUNT; ++index) {
        if (Q_strcasecmp(name, surfaceParms[index].name) == 0) {
            return (surfaceType_t)((surfaceParms[index].surfaceFlags >> SURFACE_TYPE_SHIFT) & SURFACE_TYPE_MASK);
        }
    }
...
{
    if (surfaceType > SURFACE_TYPE_DEFAULT && surfaceType < SURFACE_TYPE_COUNT) {
