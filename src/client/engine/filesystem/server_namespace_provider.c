...
static qboolean coduomp_stock_server_namespace_activate(const netadr_t *address, const char *serverName, qboolean eligibleRemoteServer)
{
    (void)address;
    (void)serverName;
...
static int32_t coduomp_stock_server_namespace_append_cached_mods(char *listBuffer, int32_t bufferSize)
{
    (void)listBuffer;
    (void)bufferSize;
    return 0;
...
static const char *coduomp_stock_server_namespace_root(const char *ordinaryHomeRoot)
{
    return ordinaryHomeRoot;
}

static qboolean coduomp_stock_server_namespace_allows(const searchpath_t *searchpath)
{
    (void)searchpath;
    return qtrue;
}
