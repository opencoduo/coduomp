#ifndef QCOMMON_ENTITY_EVENT_TYPES_H
#define QCOMMON_ENTITY_EVENT_TYPES_H

/*
 * Shared player/entity event protocol domain. The Windows cgame pointer array
 * at 0x30082428..0x30082773 (published through the pointer global at
 * 0x30082774), the Windows game module, and the Linux game module all publish
 * the same ordered 211 event names and use these values as playerState and
 * entityState event ids. The Linux name corpus is contiguous at file offsets
 * 0x997c0..0x9a7b9. EV_MAX_EVENTS is the array extent, not a transmitted event
 * value; the last valid event is EV_GRENADE_SUICIDE (210).
 */
...
typedef char entity_event_abi_size[sizeof(entityEvent_t) == 4 ? 1 : -1];
