                              ...
                              int32_t inputLength)
{
    ...
    for (uint32_t index = 0;
         index != (uint32_t)inputLength; ++index) {
        Huff_offsetTransmit(&msgHuff.compressor, input[index], output,
                            &bitOffset);
    }
    return (bitOffset + HUFFMAN_BYTE_BIT_MASK) >>
           HUFFMAN_BYTE_OFFSET_SHIFT;
}
                             ...
                             int32_t inputLength, int32_t outputCapacity)
{
    ...
    if (inputLength < 0 || outputCapacity < 0 ||
        inputLength > INT32_MAX / HUFFMAN_BITS_PER_BYTE) {
        return HUFFMAN_TRANSFORM_ERROR;
    }

    const int64_t inputBits =
        (int64_t)inputLength * HUFFMAN_BITS_PER_BYTE;
    int32_t bitOffset = 0;
