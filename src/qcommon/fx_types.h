...
enum {
    FX_EFFECT_TEMPLATE_NAME_CAPACITY = 64
};
...
typedef char q_fx_bolt_entity_offset[offsetof(sfx_bolt_info_t, entityNum) == 0x00 ? 1 : -1];
typedef char q_fx_bolt_bone_offset[offsetof(sfx_bolt_info_t, boneIndex) == 0x04 ? 1 : -1];
typedef char q_fx_bolt_size[sizeof(sfx_bolt_info_t) == 0x08 ? 1 : -1];
