 ...
 * 0x080f0940. */
const char *const net_profileSocketNames[NET_PROFILE_SOCKET_NAME_COUNT] = {
    "client", "server"
};
...
void Netchan_Init(int32_t qport)
{
    ...
    net_qport = Cvar_Get(
        "net_qport", va("%i", (int32_t)((uint32_t)qport & UINT16_MAX)),
        CVAR_INIT);
...
void Netchan_TransmitNextFragment(netchan_t *channel)
{
    ...
    MSG_WriteLong(
        &message,
        (int32_t)((uint32_t)channel->outgoingSequence | UINT32_C(0x80000000)));
    if (channel->sock == NS_CLIENT) {
        MSG_WriteShort(&message, net_qport->integer);
    }

    fragmentLength = NETCHAN_FRAGMENT_SIZE;
    if (channel->unsentFragmentStart + NETCHAN_FRAGMENT_SIZE >
        channel->unsentLength) {
        fragmentLength =
            channel->unsentLength - channel->unsentFragmentStart;
    }

    MSG_WriteShort(&message, channel->unsentFragmentStart);
    MSG_WriteShort(&message, fragmentLength);
    MSG_WriteData(&message,
                  channel->unsentBuffer + channel->unsentFragmentStart,
                  fragmentLength);

    NET_SendPacket(channel->sock, message.cursize, message.data,
    ...
    channel->unsentFragmentStart += fragmentLength;
    if (channel->unsentFragmentStart == channel->unsentLength &&
        fragmentLength != NETCHAN_FRAGMENT_SIZE) {
...
qboolean Netchan_Process(netchan_t *channel, msg_t *message)
{
    ...
    if (showpackets->integer != 0) {
        if (fragmented != qfalse) {
            Com_Printf("%s recv %4i : s=%i fragment=%i,%i\n",
                       ...
                       message->cursize, sequence, fragmentStart,
                       fragmentLength);
        } else {
            Com_Printf("%s recv %4i : s=%i\n",
                       net_profileSocketNames[channel->sock],
                       message->cursize, sequence);
        }
    }
    ...
    if (fragmentLength < 0 ||
        message->readcount + fragmentLength > message->cursize ||
        (uint32_t)channel->fragmentLength + (uint32_t)fragmentLength >
            (uint32_t)MAX_MSGLEN) {
        if (showdrop->integer != 0 || showpackets->integer != 0) {
    ...
    memcpy(channel->fragmentBuffer + channel->fragmentLength,
           message->data + message->readcount, (size_t)fragmentLength);
    ...
    memcpy(message->data + NETCHAN_SEQUENCE_HEADER_BYTES,
           channel->fragmentBuffer, (size_t)channel->fragmentLength);
    message->cursize =
        channel->fragmentLength + NETCHAN_SEQUENCE_HEADER_BYTES;
    channel->fragmentLength = 0;
