...
extern const char *const net_profileSocketNames[NET_PROFILE_SOCKET_NAME_COUNT];

void NetProf_PrepProfiling(netProfileInfo_t **profile);
void NetProf_AddPacket(netProfileStream_t *stream, int32_t bytes, qboolean fragmented);
void NetProf_NewSendPacket(netchan_t *channel, int32_t bytes, qboolean fragmented);
void NetProf_NewRecievePacket(netchan_t *channel, int32_t bytes, qboolean fragmented);
