                          ...
                          int32_t sourcePosOffset)
{
    ...
    do {
        ...
        if (fileCodeStart != NULL &&
            address > (uintptr_t)fileCodeStart) {
            break;
        }
    } while (qtrue);
    ...
    PrintSourcePos(
        channel, script_sourceFiles[fileIndex].filename,
                          ...
                          const char *detail)
{
    ...
    for (int32_t stackIndex = script_callStackDepth - 1;
         stackIndex >= 0; --stackIndex) {
        Com_PrintMessage(channel, "called from:\n");
        Scr_PrintPrevCodePos(channel,
                             script_callStackCodepos[stackIndex], 0);
    }
    Com_PrintMessage(channel, "************************************\n");
}
                  ...
                  const char *message, const char *detail)
{
    ...
    int32_t errorCode =
        script_forceErrorReport == qfalse ? ERR_SCRIPT
                                          : ERR_DROP;
    Com_Error(errorCode,
