...
{
    size_t newHighTempBytes = (hunk.highTemp + size + SCRIPT_PARSE_HUNK_ALIGNMENT_MASK) & ~(size_t)SCRIPT_PARSE_HUNK_ALIGNMENT_MASK;
    hunk.highTemp = newHighTempBytes;

    if ((int32_t)(hunk.lowTemp + newHighTempBytes) > (int32_t)hunk.totalSize) {
        Com_Meminfo_f();
        Com_Error(ERR_DROP, "\x15Hunk_AllocateTempMemoryHigh: failed on %i", (int32_t)size);
    }
...
scr_ast_node_t *node6(uintptr_t word0, uintptr_t word1, uintptr_t word2, uintptr_t word3, uintptr_t word4, uintptr_t word5, uintptr_t word6)
{
    scr_ast_node_t *node = coduomp_script_parse_allocate(offsetof(scr_ast_node_t, payload) + sizeof(node->payload.forStatement));
...
{
    scr_ast_list_item_t *item = coduomp_script_parse_allocate(sizeof(*item));
    scr_ast_list_t *list;
