                          ...
                          uint32_t paramCount)
{
    ...
    VariableValue *stackBase =
        &script_valueStackTop[-(int32_t)paramCount];
    ...
    if (script_callStackDepth < SCRIPT_CALL_STACK_COUNT) {
        ...
        thread = VM_Execute(script_valueStackTop, codePos, thread,
                            thread, stackBase);
...
#endif
    uint16_t thread = VM_ExecuteThread(
        script_levelHandle,
        codePos,
        paramCount);
...
void VM_SetTime(void)
{
    ...
    uint16_t timeSlot = FindVariable(
