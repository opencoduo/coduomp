...
static void coduomp_script_usage_read_stack_entry_value(const VariableStackBufferEntry *entry, VariableValue *value)
{
...
static int32_t coduomp_script_usage_collect_code_positions(const VariableStackBuffer *frame,
                                                           script_codepos_t codePositions[SCRIPT_CALL_STACK_COUNT])
{
    ...
    for (uint16_t entryIndex = 0; entryIndex < frame->size; ++entryIndex) {
        const VariableStackBufferEntry *const entry = &frame->entries[entryIndex];
        if (entry->type == SCRIPT_USAGE_STACK_ENTRY_CODEPOS) {
            savedCodePositions[savedCodePositionCount % SAVED_CODE_POSITION_CAPACITY] = (script_codepos_t)entry->payload;
            savedCodePositionCount++;
        }
    }

    const uint32_t retainedSavedCount =
        savedCodePositionCount < SAVED_CODE_POSITION_CAPACITY ? savedCodePositionCount : SAVED_CODE_POSITION_CAPACITY;
    ...
    for (uint32_t index = 0; index < retainedSavedCount; ++index) {
        ...
        codePositions[index + 1] = savedCodePositions[savedIndex % SAVED_CODE_POSITION_CAPACITY];
    }
...
static VariableStackBuffer *coduomp_script_usage_stack_frame_for_node(uint16_t handle)
{
    VariableValue value;
...
static const char *coduomp_script_usage_entity_type_name(const script_class_map_entry_t *usage)
{
    return usage->name;
}
#endif
...
int32_t ThreadInfoCompare(const void *left, const void *right)
{
    ...
    int32_t sharedDepth = leftStack->depth < rightStack->depth ? leftStack->depth : rightStack->depth;
    ...
    for (int32_t index = 0; index < sharedDepth; ++index) {
        ...
        if (leftCodePos != rightCodePos) {
            ...
            uint32_t difference = (uint32_t)(uintptr_t)leftCodePos - (uint32_t)(uintptr_t)rightCodePos;
            int32_t result;
    ...
    uint32_t difference = (uint32_t)leftStack->depth - (uint32_t)rightStack->depth;
    int32_t result;
...
float Scr_GetEntryUsage(script_variable_type_t type, VariableUnion value)
{
    ...
    if (type == SCRIPT_VAR_OBJECT &&
        ...
        return Scr_GetObjectUsage(object) / (script_variableNodes[object].payload.halves.valueOrRefCount + 1.0f);
    }
...
long double Scr_GetEntryUsageForNode(script_variable_node_t *node)
{
    return 1.0f + Scr_GetEntryUsage((script_variable_type_t)(node->packedTypeIndex & SCRIPT_VARIABLE_NODE_TYPE_MASK), node->payload);
}
#endif

float Scr_GetEndonUsage(uint16_t localVars)
{
    uint16_t pauseEntry = FindObjectVariable(script_pauseArrayHandle, localVars);
...
#if EMULATE_X87 && defined(LINUX_BEHAVIOR)
        float entryUsage =
            Scr_GetEntryUsage((script_variable_type_t)(script_variableNodes[child].packedTypeIndex & SCRIPT_VARIABLE_NODE_TYPE_MASK),
                              script_variableNodes[child].payload);
        usage = x87f_store_f32(x87f_add(x87f_load_f32(usage), x87f_add(x87f_load_f32(1.0f), x87f_load_f32(entryUsage))));
#else
    ...
    *endonUsage = Scr_GetEndonUsage(frame->localId);
    ...
    while (remaining != 0) {
        ...
        if (type == SCRIPT_USAGE_STACK_ENTRY_CODEPOS) {
            ...
            coduomp_script_usage_read_stack_entry_value(entry - 2, &priorValue);
            uint16_t localVars = (uint16_t)priorValue.payload;
            ...
            *endonUsage += Scr_GetEndonUsage(localVars);
            ...
            coduomp_script_usage_read_stack_entry_value(entry - 1, &value);
...
{
    script_usage_stack_signature_t *signatures = Z_MallocInternal((size_t)SCRIPT_VARIABLE_NODE_COUNT * sizeof(*signatures));
    int32_t signatureCount = 0;

    for (uint32_t handle = 1; handle < SCRIPT_VARIABLE_NODE_COUNT; ++handle) {
        script_variable_node_t *node = &script_variableNodes[handle];

        if ((node->packedTypeIndex & SCRIPT_USAGE_STACK_NODE_BITS) == 0 ||
            (node->packedTypeIndex & SCRIPT_VARIABLE_NODE_TYPE_MASK) != SCRIPT_USAGE_STACK_SIGNATURE_NODE_TYPE) {
            continue;
        }

        script_usage_stack_signature_t *signature = &signatures[signatureCount++];
        VariableStackBuffer *frame = coduomp_script_usage_stack_frame_for_node((uint16_t)handle);
        const int32_t depth = coduomp_script_usage_collect_code_positions(frame, signature->codePosStack);
        signature->variableUsage = Scr_GetThreadUsage(frame, &signature->endonUsage);
        signature->depth = depth;
    }

    coduo_qsort(signatures, signatureCount, sizeof(*signatures), ThreadInfoCompare);
    ...
    while (index < signatureCount) {
        ...
        do {
            ...
            index++;
        } while (index < signatureCount && ThreadInfoCompare(signature, &signatures[index]) == 0);

#if defined(__x86_64__)
        coduo_x87_truncation_control_t conversionControl;
        int32_t variableUsageInteger = CODUO_X87_TRUNCATE_I32_FIRST(&conversionControl, (long double)variableUsage);
        int32_t endonUsageInteger = CODUO_X87_TRUNCATE_I32_NEXT(&conversionControl, (long double)endonUsage);
#else
        int32_t variableUsageInteger = (int32_t)variableUsage;
        int32_t endonUsageInteger = (int32_t)endonUsage;
#endif
        Com_Printf("count: %d, var usage: %d, endon usage: %d\n", count, variableUsageInteger, endonUsageInteger);
        Scr_PrintPrevCodePos(SCRIPT_USAGE_PRINT_CHANNEL, signature->codePosStack[0], 0);
        for (int32_t stackIndex = 1; stackIndex < signature->depth; ++stackIndex) {
            Com_Printf("called from:\n");
            Scr_PrintPrevCodePos(SCRIPT_USAGE_PRINT_CHANNEL, signature->codePosStack[stackIndex], 0);
        }
    }
    ...
    for (uint32_t usageIndex = 0; usageIndex < script_entityTypeUsageCount; ++usageIndex) {
        uint16_t classEntry = FindVariable(script_entityTypeClassMapRoot, usageIndex);
        ...
        for (uint16_t child = FindNextSibling(classObject); child != 0; child = FindNextSibling(child)) {
            count++;
            variableUsage += Scr_GetObjectUsage(script_variableNodes[child].payload.halves.valueOrRefCount);
        }

#if defined(__x86_64__)
        int32_t variableUsageInteger = CODUO_X87_TRUNCATE_I32((long double)variableUsage);
#else
        int32_t variableUsageInteger = (int32_t)variableUsage;
#endif
        Com_Printf("ent type '%s'... count: %d, var usage: %d\n",
                   coduomp_script_usage_entity_type_name(&script_entityTypeUsageRecords[usageIndex]), count, variableUsageInteger);
    }
...
void Scr_DumpScriptThreads(void)
{
    script_usage_stack_signature_t *signatures = malloc((size_t)SCRIPT_VARIABLE_NODE_COUNT * sizeof(*signatures));
    if (signatures == NULL) {
        SCRIPT_OUT_OF_MEMORY();
        return;
    }
    memset(signatures, 0, (size_t)SCRIPT_VARIABLE_NODE_COUNT * sizeof(*signatures));
    ...
    for (uint32_t handle = 1; handle < SCRIPT_VARIABLE_NODE_COUNT; ++handle) {
        ...
        const VariableStackBuffer *frame = (const VariableStackBuffer *)node->payload.valuePayload;
        script_usage_stack_signature_t *signature = &signatures[signatureCount++];
        signature->variableUsage = Scr_GetThreadUsage(frame, &signature->endonUsage);
        signature->depth = coduomp_script_usage_collect_code_positions(frame, signature->codePosStack);
    }

    coduo_qsort(signatures, (size_t)signatureCount, sizeof(*signatures), ThreadInfoCompare);
    ...
    for (int32_t first = 0; first < signatureCount;) {
        ...
        do {
            ...
            ++count;
            ++next;
        } while (next < signatureCount && ThreadInfoCompare(signature, &signatures[next]) == 0);
    ...
    for (uint32_t usageIndex = 0; usageIndex < script_entityTypeUsageCount; ++usageIndex) {
        ...
        Com_Printf("ent type '%s'... count: %d, var usage: %d\n", script_entityTypeUsageRecords[usageIndex].name, count,
                   (int32_t)variableUsage);
    }
    Com_Printf("********************************\n");
}
#endif
