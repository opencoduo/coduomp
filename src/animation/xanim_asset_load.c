...
static _Noreturn void xanim_load_fail(const xanim_load_cursor_t *cursor,
                                     const char *reason)
{
    Com_Error(ERR_DROP, "\x15" "malformed xanim '%s': %s",
...
static const uint8_t *xanim_load_take(xanim_load_cursor_t *cursor,
                                      size_t size)
{
...
static const char *xanim_load_take_string(xanim_load_cursor_t *cursor,
                                          size_t *size)
{
    const uint8_t *terminator = memchr(cursor->position, '\0',
                                       cursor->remaining);
    if (terminator == NULL) {
...
static const char *xanim_load_take_interned_string(
    xanim_load_cursor_t *cursor, size_t *size)
{
 ...
 * XAnimLoadFile. */
static qboolean xanim_load_part_bit_is_set(const uint8_t *partBits,
                                              int32_t partIndex)
{
    return (partBits[partIndex >> 3] &
...
void ReadQuat(const int16_t packed[3],
              xanim_int16_vec4_t *out)
{
    ...
    remainingBits -=
        (uint32_t)(int32_t)out->components[2] *
        (uint32_t)(int32_t)out->components[2];
    remainingBits -=
        (uint32_t)(int32_t)out->components[1] *
        (uint32_t)(int32_t)out->components[1];
    remainingBits -=
        (uint32_t)(int32_t)out->components[0] *
        (uint32_t)(int32_t)out->components[0];
    int32_t remaining;
    memcpy(&remaining, &remainingBits, sizeof(remaining));
    out->components[3] = remaining > 0
        ? (int16_t)(int32_t)floor(sqrt((double)remaining) + 0.5)
        : 0;
}

/* CoDUOMP.exe 0x0049cdf0..0x0049ce38 and coduo_lnxded
 * 0x080c1966..0x080c19f3 agree; ReadQuat2 is the exact Mac symbol. */
void ReadQuat2(const int16_t packed[1],
               xanim_int16_vec2_t *out)
{
    out->components[0] = packed[0];
    uint32_t remainingBits =
        XANIM_PACKED_UNIT_VECTOR_REMAINDER -
        (uint32_t)(int32_t)out->components[0] *
            (uint32_t)(int32_t)out->components[0];
    int32_t remaining;
    memcpy(&remaining, &remainingBits, sizeof(remaining));
    out->components[1] = remaining > 0
        ? (int16_t)(int32_t)floor(sqrt((double)remaining) + 0.5)
        : 0;
}
 ...
 * quaternion reads. */
static void xanim_load_read_vec2(xanim_load_cursor_t *cursor,
                                 qboolean negate,
                                 xanim_int16_vec2_t *out)
{
    int16_t packed;
...
/* NOT_FROM_ORIGINAL_SOURCE: local factoring of XAnimLoadFile's packed
 * quaternion reads. */
static void xanim_load_read_vec4(xanim_load_cursor_t *cursor,
                                 qboolean negate,
                                 xanim_int16_vec4_t *out)
{
    int16_t packed[3];
...
static void xanim_load_fix_vec2_continuity(
    xanim_int16_vec2_t *frames, uint16_t frameCount)
{
    for (int32_t index = 1; index < frameCount; ++index) {
        ...
        int32_t dot =
            (int32_t)previous->components[0] * current->components[0] +
...
/* NOT_FROM_ORIGINAL_SOURCE: local factoring of the quaternion sign-continuity
 * pass at 0x004961e0 and 0x004968f0. */
static void xanim_load_fix_vec4_continuity(
    xanim_int16_vec4_t *frames, uint16_t frameCount)
{
    for (int32_t index = 1; index < frameCount; ++index) {
        ...
        int32_t dot =
            (int32_t)previous->components[0] * current->components[0] +
            (int32_t)previous->components[1] * current->components[1] +
            (int32_t)previous->components[2] * current->components[2] +
            (int32_t)previous->components[3] * current->components[3];
...
static size_t xanim_load_rotation_stream_size(
    uint16_t frameCount, uint16_t totalFrameCount,
    qboolean smallFrameKeys)
{
    size_t size = offsetof(xanim_rotation_stream_t, tail) +
                  sizeof(uint16_t);

    if (frameCount < totalFrameCount) {
        size_t keyedSize = offsetof(xanim_rotation_stream_t, tail) +
            (smallFrameKeys ? (size_t)frameCount + 1U
                            : (size_t)frameCount * sizeof(uint16_t));
        if (keyedSize > size) {
            size = keyedSize;
        }
    }
...
static size_t xanim_load_translation_stream_size(
    uint16_t frameCount, uint16_t totalFrameCount,
    qboolean smallFrameKeys)
{
    size_t size = offsetof(xanim_translation_stream_t, key) +
                  sizeof(uint16_t);

    if (frameCount < totalFrameCount) {
        size_t keyedSize = offsetof(xanim_translation_stream_t, key) +
            (smallFrameKeys ? (size_t)frameCount + 1U
                            : (size_t)frameCount * sizeof(uint16_t));
        if (keyedSize > size) {
            size = keyedSize;
...
static uint16_t xanim_load_key_value(const uint8_t *keys, size_t index,
                                    qboolean smallFrameKeys)
{
...
static void xanim_load_validate_keys(xanim_load_cursor_t *cursor,
                                     const uint8_t *keys,
                                     uint16_t frameCount,
...
static void xanim_load_copy_rotation_keys(
    xanim_rotation_stream_t *stream, xanim_load_cursor_t *cursor,
    uint16_t frameCount, uint16_t totalFrameCount,
    qboolean smallFrameKeys)
{
    if (frameCount >= totalFrameCount) {
    ...
    size_t bytes = smallFrameKeys
        ? (size_t)frameCount
        : (size_t)frameCount * sizeof(uint16_t);
    const uint8_t *keys = xanim_load_take(cursor, bytes);
    xanim_load_validate_keys(cursor, keys, frameCount, totalFrameCount,
                             smallFrameKeys);
...
/* NOT_FROM_ORIGINAL_SOURCE: source-level factoring of the two repeated key
 * copy branches in XAnimLoadFile. */
static void xanim_load_copy_translation_keys(
    xanim_translation_stream_t *stream, xanim_load_cursor_t *cursor,
    uint16_t frameCount, uint16_t totalFrameCount,
    qboolean smallFrameKeys)
{
    if (frameCount >= totalFrameCount) {
...
static void xanim_load_parse_rotation_stream(
    xanim_load_cursor_t *cursor, xanim_rotation_stream_t **streamOut,
    uint16_t totalFrameCount, qboolean smallFrameKeys,
    qboolean compressedVec2, qboolean negate, xanim_asset_alloc_fn alloc)
{
    ...
    if (frameCount == 1) {
        if (compressedVec2) {
            ...
            stream = alloc(offsetof(xanim_rotation_stream_t, tail) +
                           sizeof(uint16_t));
    ...
    stream = alloc(xanim_load_rotation_stream_size(
        frameCount, totalFrameCount, smallFrameKeys));
    xanim_load_copy_rotation_keys(
        stream, cursor, frameCount, totalFrameCount, smallFrameKeys);
    if (compressedVec2) {
        ...
        for (int32_t index = 0; index < frameCount; ++index) {
            ...
            xanim_load_read_vec2(
                cursor, index == 0 ? negate : qfalse, &frames[index]);
        }
 ...
 * parser repeated for delta motion and every animated part. */
static void xanim_load_parse_translation_stream(
    xanim_load_cursor_t *cursor, xanim_translation_stream_t **streamOut,
    uint16_t totalFrameCount, qboolean smallFrameKeys,
    xanim_asset_alloc_fn alloc)
{
    ...
    stream = alloc(xanim_load_translation_stream_size(
        frameCount, totalFrameCount, smallFrameKeys));
    xanim_load_copy_translation_keys(
        stream, cursor, frameCount, totalFrameCount, smallFrameKeys);
                              ...
                              xanim_asset_alloc_fn alloc)
{
    ...
    xanim_notetrack_t *noteTracks = alloc(
        (size_t)noteTrackCount * sizeof(noteTracks[0]) +
        sizeof(noteTracks[0]) + sizeof(noteTracks[0].nameHandle));
    ...
    for (int32_t index = 0; index < noteTrackCount; ++index) {
        ...
        noteTracks[index].nameHandle = SL_GetStringOfLen(
            name, XANIM_NOTETRACK_STRING_USER, nameSize,
            XANIM_NOTETRACK_STRING_TYPE);
        uint16_t frame = xanim_load_read_unsigned_short(&noteCursor);

        noteTracks[index].time =
            record->frameCountMinusOne != 0
                ? (float)frame / (float)record->frameCountMinusOne
                : 0.0f;
    }

    noteTracks[noteTrackCount].nameHandle = SL_GetStringOfLen(
        "end", XANIM_NOTETRACK_STRING_USER, sizeof("end"),
        XANIM_NOTETRACK_STRING_TYPE);
...
void XAnimFreeMemory(fileData_t *entry)
{
    ...
    if (xanim_rootTreeHandle != 0) {
        uint16_t nameHandle = SL_GetLowercaseString_(
            entry->name, 0, 7);
...
{
    for (uint16_t child = FindNextSibling(xanim_rootTreeHandle);
...
{
    fileData_t *entry =
        FS_GetDataForFile("xanim", animName, "");

    if (entry == NULL) {
        Com_Error(ERR_DROP, "\x15" "Cannot precache 'xanim/%s'.",
                  animName);
        return;
    }
    ...
    if (strlen(animName) > sizeof(path) - sizeof("xanim/")) {
        Com_Error(ERR_DROP, "\x15" "XAnim path is too long");
        return;
    }
    ...
    xanim_load_cursor_t cursor = {
        (const uint8_t *)fileBuffer, (size_t)fileLength, animName
    };
    ...
    size_t partNameStorageSize =
        ((size_t)partCount + 1U) * sizeof(uint16_t);
    ...
    uint8_t hasDeltaMotion =
        (uint8_t)((flags & XANIM_FILE_DELTA_MOTION_FLAG) >> 1);
    ...
    qboolean smallFrameKeys =
        totalFrameCount <= XANIM_SMALL_FRAME_KEY_LIMIT;

    record->frameCountMinusOne = (uint16_t)(totalFrameCount - 1U);
    record->frequency = record->frameCountMinusOne == 0
        ? 0.0f
        : record->frameRate / (float)record->frameCountMinusOne;

    if (hasDeltaMotion != 0) {
        xanim_part_stream_pair_t *deltaMotion =
            alloc(sizeof(*deltaMotion));

        record->deltaMotion = deltaMotion;
        xanim_load_parse_rotation_stream(
            &cursor, &deltaMotion->rotation, totalFrameCount,
            smallFrameKeys, qtrue, qfalse, alloc);
        xanim_load_parse_translation_stream(
            &cursor, &deltaMotion->translation, totalFrameCount,
            smallFrameKeys, alloc);
    }
    ...
    memcpy(compressedRotationBits, xanim_load_take(&cursor, bitsetSize),
           bitsetSize);
    ...
    for (int32_t index = 0; index < partCount; ++index) {
        ...
        partNames[index + 1] = SL_GetStringOfLen(
            name, 0, nameSize, XANIM_PART_NAME_STRING_TYPE);
    }

    size_t partStreamStorageSize =
        (size_t)partCount * sizeof(record->partStreamPairs[0]);
    record->partStreamPairs = alloc(partStreamStorageSize);
    for (int32_t index = 0; index < partCount; ++index) {
        xanim_part_stream_pair_t *streams =
            &record->partStreamPairs[index];
        qboolean compressedRotation = xanim_load_part_bit_is_set(
            compressedRotationBits, index);

        xanim_load_parse_rotation_stream(
            &cursor, &streams->rotation, totalFrameCount, smallFrameKeys,
            compressedRotation,
        ...
        xanim_load_parse_translation_stream(
            &cursor, &streams->translation, totalFrameCount,
            smallFrameKeys, alloc);
    }

    (void)ReadNoteTracks(animName, cursor.position, cursor.remaining,
                         record, alloc);
    FS_FreeFile(fileBuffer);
                       ...
                       XModel **models, int32_t modelCount)
{
    ...
    size_t remapSize = (size_t)(
        (uint32_t)animPartCount + DOBJ_PART_REMAP_PREFIX_SIZE);
    uint32_t remapStorage[(remapSize + sizeof(uint32_t) - 1U) /
                          sizeof(uint32_t)];
    XAnimToXModel *remap = (XAnimToXModel *)remapStorage;
    uint8_t *animToDObjPart = remap->boneIndex;

    Com_Memset(remap->partBits, 0,
               sizeof(remap->partBits));
    for (int32_t animPartIndex = animPartCount - 1;
         animPartIndex >= 0; --animPartIndex) {
        animToDObjPart[animPartIndex] =
            XANIM_PART_REMAP_SENTINEL;
    }

    int32_t sourcePartIndex = 0;
    for (int32_t modelIndex = 0; modelIndex < modelCount;
         ++modelIndex) {
        const uint16_t *modelPartNames =
            XModelBoneNames(models[modelIndex]);
        int32_t modelPartCount =
            XModelNumBones(models[modelIndex]);

        for (int32_t modelPartIndex = 0;
             modelPartIndex < modelPartCount; ++modelPartIndex) {
            ...
            for (int32_t animPartIndex = animPartCount - 1;
                ...
                if (animToDObjPart[animPartIndex] ==
                    XANIM_PART_REMAP_SENTINEL) {
                    animToDObjPart[animPartIndex] =
                        (uint8_t)sourcePartIndex;
                    remap->partBits[sourcePartIndex >> 3] |=
                        (uint8_t)(1U << (sourcePartIndex & 7));
                }
                break;
            }
            ++sourcePartIndex;
