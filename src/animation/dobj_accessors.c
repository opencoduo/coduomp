...
qboolean DObjSkelAreBonesUpToDate(const DObj *obj, const uint32_t *partBits)
{
    ...
    for (int32_t wordIndex = 0; wordIndex < DOBJ_PART_BITSET_WORD_COUNT; ++wordIndex) {
        if ((~storage->blockedPartBits[wordIndex] & partBits[wordIndex]) != 0) {
            return qfalse;
        }
    }
    return qtrue;
...
{
    const uint8_t *blockedPartBytes = (const uint8_t *)obj->evaluationStorage->blockedPartBits;
...
{
    return (obj->collisionSkipModelMask & (1U << (modelIndex & 31))) != 0 ? qtrue : qfalse;
}
...
{
    return (int32_t)(sizeof(dobj_eval_storage_t) + (size_t)obj->boneCount * sizeof(DObjAnimMat) * 3U);
}
...
{
    return obj->skeletonCacheKey == key && obj->evaluationStorage != NULL ? qtrue : qfalse;
}
...
int32_t DObjGetLodForDist(const DObj *obj, int32_t modelIndex, float distance)
{
...
{
    /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
    return obj != NULL && obj->evaluationStorage != NULL ? &obj->evaluationStorage->partSpans[obj->boneCount].evalParts.parts[0] : NULL;
}
...
qboolean DObjSetRotTransIndex(DObj *obj, const uint8_t *partBits, int32_t boneIndex)
{
    /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
    if (obj == NULL || obj->evaluationStorage == NULL || partBits == NULL || (uint32_t)boneIndex >= obj->boneCount) {
        return qfalse;
    }

    int32_t byteIndex = boneIndex >> 3;
    uint8_t bit = (uint8_t)(1U << (boneIndex & 7));
    uint8_t *blockedPartBytes = (uint8_t *)obj->evaluationStorage->blockedPartBits;

    if ((partBits[byteIndex] & bit) == 0 || (blockedPartBytes[byteIndex] & bit) != 0) {
        return qfalse;
    }
...
qboolean DObjSetControlRotTransIndex(DObj *obj, const uint8_t *partBits, int32_t boneIndex)
{
    /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
    if (obj == NULL || obj->evaluationStorage == NULL || partBits == NULL || (uint32_t)boneIndex >= obj->boneCount) {
        return qfalse;
...
{
    for (int32_t modelIndex = 0; modelIndex < obj->modelCount; ++modelIndex) {
        if ((XModelGetContents(obj->models[modelIndex]) & contentsMask) != 0) {
            return qtrue;
        }
    }
    return qfalse;
