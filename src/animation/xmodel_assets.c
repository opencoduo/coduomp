...
enum {
    ...
    XMODEL_BONE_USAGE_CAPACITY = XSURFACE_BONE_USAGE_WORD_COUNT * 32,
...
static XSurfaceTriangle xmodel_defaultTriangles[XMODEL_DEFAULT_SURFACE_COUNT];
static XSurfaceRigidVert xmodel_defaultRigidVertices[XMODEL_DEFAULT_VERTEX_COUNT];
...
static size_t coduo_xmodel_validation_remaining(const coduo_xmodel_validation_stream_t *stream)
{
    return (size_t)(stream->end - stream->cursor);
}

static qboolean coduo_xmodel_validation_take(coduo_xmodel_validation_stream_t *stream, size_t size, const uint8_t **bytes)
{
...
static qboolean coduo_xmodel_validation_u8(coduo_xmodel_validation_stream_t *stream, uint8_t *value)
{
    const uint8_t *bytes;
    if (coduo_xmodel_validation_take(stream, sizeof(*value), &bytes) == qfalse) {
        return qfalse;
    }
    *value = bytes[0];
    return qtrue;
...
static qboolean coduo_xmodel_validation_i16(coduo_xmodel_validation_stream_t *stream, int16_t *value)
{
    const uint8_t *bytes;
    if (coduo_xmodel_validation_take(stream, sizeof(*value), &bytes) == qfalse) {
        return qfalse;
...
static qboolean coduo_xmodel_validation_u32(coduo_xmodel_validation_stream_t *stream, uint32_t *value)
{
    const uint8_t *bytes;
    if (coduo_xmodel_validation_take(stream, sizeof(*value), &bytes) == qfalse) {
        return qfalse;
...
static qboolean coduo_xmodel_validation_c_string(coduo_xmodel_validation_stream_t *stream, size_t *length)
{
...
static uint16_t coduo_xmodel_validation_index(const uint8_t *bytes, size_t index)
{
    uint16_t value;
...
static qboolean coduo_xmodel_validate_blend(coduo_xmodel_validation_stream_t *stream)
{
    int16_t boneIndex;
    if (coduo_xmodel_validation_i16(stream, &boneIndex) == qfalse || boneIndex < 0 || boneIndex >= XMODEL_MAX_BONES) {
        return qfalse;
    }
    return coduo_xmodel_validation_take(stream, sizeof(vec3_t), NULL);
}

static qboolean coduo_xmodel_validate_surface(coduo_xmodel_validation_stream_t *stream)
{
    uint8_t tileMode;
    int16_t vertexCount;
    ...
    if (coduo_xmodel_validation_u8(stream, &tileMode) == qfalse || coduo_xmodel_validation_i16(stream, &vertexCount) == qfalse ||
        coduo_xmodel_validation_i16(stream, &triangleCount) == qfalse || coduo_xmodel_validation_i16(stream, &stripCount) == qfalse ||
        coduo_xmodel_validation_i16(stream, &singleBoneIndex) == qfalse) {
        return qfalse;
    }
    (void)tileMode;

    if (vertexCount < 0 || triangleCount < 0 || stripCount < 0 || (triangleCount == INT16_MAX && (triangleCount & 1) != 0)) {
        return qfalse;
    }

    if (singleBoneIndex == XMODEL_NO_SINGLE_BONE) {
        if (coduo_xmodel_validation_i16(stream, &weightedPointCount) == qfalse ||
            coduo_xmodel_validation_i16(stream, &compactWeightedVertexCount) == qfalse || weightedPointCount < 0 ||
            compactWeightedVertexCount < 0 || compactWeightedVertexCount > vertexCount) {
            return qfalse;
        }
    } else if (singleBoneIndex < 0 || singleBoneIndex >= XMODEL_MAX_BONES) {
        return qfalse;
    }
    ...
    for (int32_t strip = 0; strip < stripCount; ++strip) {
        ...
        if (coduo_xmodel_validation_u8(stream, &stripVertexCount) == qfalse || stripVertexCount < XMODEL_TRIANGLE_INDEX_COUNT ||
            coduo_xmodel_validation_take(stream, (size_t)stripVertexCount * sizeof(uint16_t), &indices) == qfalse) {
            return qfalse;
        }

        for (size_t index = 0; index < stripVertexCount; ++index) {
            if (coduo_xmodel_validation_index(indices, index) >= (uint16_t)vertexCount) {
                return qfalse;
            }
        }
        ...
        for (int32_t vertex = XMODEL_TRIANGLE_INDEX_COUNT; vertex < stripVertexCount; vertex += 2) {
            uint16_t next = coduo_xmodel_validation_index(indices, (size_t)vertex);
            ...
            uint16_t next2 = coduo_xmodel_validation_index(indices, (size_t)vertex + 1u);
    ...
    if (singleBoneIndex == XMODEL_NO_SINGLE_BONE) {
        ...
        for (int32_t vertex = 0; vertex < vertexCount; ++vertex) {
            ...
            if (coduo_xmodel_validation_take(stream, sizeof(vec3_t) + sizeof(vec2_t), NULL) == qfalse ||
                coduo_xmodel_validation_i16(stream, &additiveWeightCount) == qfalse || additiveWeightCount < 0 ||
                coduo_xmodel_validate_blend(stream) == qfalse) {
                return qfalse;
            }
            if (additiveWeightCount == 0) {
                ++zeroAdditiveVertices;
            } else if (coduo_xmodel_validation_take(stream, sizeof(float), NULL) == qfalse) {
                return qfalse;
            }
            if (totalAdditivePoints > weightedPointCount - additiveWeightCount) {
                return qfalse;
            }
            totalAdditivePoints += additiveWeightCount;
        }
        if (zeroAdditiveVertices != compactWeightedVertexCount || totalAdditivePoints != weightedPointCount) {
            return qfalse;
        }
        for (int32_t point = 0; point < weightedPointCount; ++point) {
            if (coduo_xmodel_validate_blend(stream) == qfalse || coduo_xmodel_validation_take(stream, sizeof(float), NULL) == qfalse) {
                return qfalse;
            }
        }
    } else if (coduo_xmodel_validation_take(stream, (size_t)vertexCount * (sizeof(XSurfaceRigidVert) + sizeof(vec2_t)), NULL) == qfalse) {
        return qfalse;
    }
...
static qboolean coduo_xmodel_validate_surfs_file(const uint8_t *data, size_t size)
{
    if (data == NULL) {
        return qfalse;
    }
    ...
    if (coduo_xmodel_validation_i16(&stream, &version) == qfalse || version != XMODEL_ASSET_VERSION ||
        coduo_xmodel_validation_i16(&stream, &surfaceCount) == qfalse || surfaceCount < 0) {
        return qfalse;
    }
...
static qboolean coduo_xmodel_validate_parts_file(const uint8_t *data, size_t size)
{
    if (data == NULL) {
        return qfalse;
    }
    ...
    int16_t rootPartCount;
    if (coduo_xmodel_validation_i16(&stream, &version) == qfalse || version != XMODEL_ASSET_VERSION ||
        coduo_xmodel_validation_i16(&stream, &childPartCount) == qfalse || coduo_xmodel_validation_i16(&stream, &rootPartCount) == qfalse ||
        childPartCount < 0 || rootPartCount < 0) {
        return qfalse;
    }

    const int32_t totalPartCount = (int32_t)childPartCount + (int32_t)rootPartCount;
    if (totalPartCount <= 0 || totalPartCount > XMODEL_MAX_BONES) {
        return qfalse;
    }

    for (int32_t partIndex = rootPartCount; partIndex < totalPartCount; ++partIndex) {
        uint8_t parentIndex;
        if (coduo_xmodel_validation_u8(&stream, &parentIndex) == qfalse || parentIndex >= partIndex ||
            coduo_xmodel_validation_take(&stream, sizeof(vec3_t) + 3u * sizeof(int16_t), NULL) == qfalse) {
            return qfalse;
        }
    }
    ...
    for (int32_t partIndex = 0; partIndex < totalPartCount; ++partIndex) {
        if (coduo_xmodel_validation_c_string(&stream, NULL) == qfalse ||
            coduo_xmodel_validation_take(&stream, 2u * sizeof(vec3_t), NULL) == qfalse) {
            return qfalse;
        }
    }
    return coduo_xmodel_validation_take(&stream, (size_t)totalPartCount, NULL);
}

static qboolean coduo_xmodel_validate_model_file(const uint8_t *data, size_t size)
{
    if (data == NULL) {
        return qfalse;
    }
    ...
    for (int32_t lod = 0; lod < XMODEL_LOD_COUNT; ++lod) {
        size_t nameLength;
        if (coduo_xmodel_validation_take(&stream, sizeof(float), NULL) == qfalse ||
            coduo_xmodel_validation_c_string(&stream, &nameLength) == qfalse || nameLength >= XMODEL_PATH_BUFFER_SIZE) {
            return qfalse;
        }
    ...
    if (coduo_xmodel_validation_u32(&stream, &modelFileCount) == qfalse ||
        coduo_xmodel_validation_u32(&stream, &collisionSurfaceCount) == qfalse || collisionSurfaceCount > INT32_MAX) {
        return qfalse;
    }
    (void)modelFileCount;

    for (uint32_t surface = 0; surface < collisionSurfaceCount; ++surface) {
        uint32_t triangleCount;
        uint32_t basePoseIndex;
        uint32_t contents;
        uint32_t surfaceFlags;
        if (coduo_xmodel_validation_u32(&stream, &triangleCount) == qfalse || triangleCount > INT32_MAX ||
            triangleCount > coduo_xmodel_validation_remaining(&stream) / sizeof(XModelCollTri) ||
            coduo_xmodel_validation_take(&stream, (size_t)triangleCount * sizeof(XModelCollTri), NULL) == qfalse ||
            coduo_xmodel_validation_take(&stream, 2u * sizeof(vec3_t), NULL) == qfalse ||
            coduo_xmodel_validation_u32(&stream, &basePoseIndex) == qfalse || coduo_xmodel_validation_u32(&stream, &contents) == qfalse ||
            coduo_xmodel_validation_u32(&stream, &surfaceFlags) == qfalse || basePoseIndex >= XMODEL_MAX_BONES) {
            return qfalse;
        }
        (void)contents;
        (void)surfaceFlags;
    ...
    for (int32_t lod = 0; lod < XMODEL_LOD_COUNT; ++lod) {
        ...
        int16_t surfaceCount;
        if (coduo_xmodel_validation_i16(&stream, &surfaceCount) == qfalse || surfaceCount < 0) {
            return qfalse;
        }
        for (int32_t surface = 0; surface < surfaceCount; ++surface) {
...
static qboolean coduo_xmodel_loaded_references_fit(const XModelInfo *model, const XModelLodInfo *lod, const XModelSurfsData *surfs)
{
    const int32_t boneCount = model->parts->data.xmodelParts->partNameTableSlot->partNameTable->count;
    ...
    for (int32_t surfaceIndex = 0; surfaceIndex < surfs->surfaceCount; ++surfaceIndex) {
        ...
        for (int32_t boneIndex = boneCount; boneIndex < XMODEL_BONE_USAGE_CAPACITY; ++boneIndex) {
            if ((surface->boneUsage[boneIndex >> 5] & (UINT32_C(1) << (boneIndex & 31))) != 0) {
                return qfalse;
            }
        }
    }
...
static qboolean coduo_xmodel_loaded_collision_fits(const XModelInfo *model)
{
    const int32_t boneCount = model->parts->data.xmodelParts->partNameTableSlot->partNameTable->count;
    for (int32_t surfaceIndex = 0; surfaceIndex < model->collisionSurfaceCount; ++surfaceIndex) {
        const int32_t basePoseIndex = model->collisionSurfaces[surfaceIndex].basePoseIndex;
...
void ReadBlend(XSurface *surface, XSimpleBlendInfo *blend, const uint8_t **position)
{
    ...
    if (boneIndex < 0 || boneIndex >= XMODEL_MAX_BONES) {
        Com_Error(ERR_DROP, "\x15"
                            "XModel blend has an invalid bone index");
        return;
    }
    ((uint8_t *)surface->boneUsage)[boneIndex >> XMODEL_BONE_USAGE_SHIFT] |= (uint8_t)(1U << (boneIndex & XMODEL_BONE_USAGE_MASK));
    blend->boneMatrixOffset = (uint32_t)(int32_t)boneIndex << XMODEL_BONE_INDEX_SHIFT;
...
void XSurfaceUnstrip(const XStripInfo *strips, XSurfaceTriangle *triangles)
{
    ...
    for (int32_t strip = 0; strip < strips->stripCount; ++strip) {
        ...
        for (int32_t vertex = XMODEL_TRIANGLE_INDEX_COUNT; vertex < vertexCount; vertex += 2) {
 ...
 * scratch arrays. */
static void coduo_xmodel_unstrip_serialized(const uint8_t *stripData, int32_t stripCount, XSurfaceTriangle *triangles)
{
    for (int32_t strip = 0; strip < stripCount; ++strip) {
...
void XModelReadSurface(XSurface *surface, const uint8_t **position, void *(*alloc)(size_t size))
{
    ...
    if (singleBoneIndex == XMODEL_NO_SINGLE_BONE) {
        ...
        uint32_t fullBlendVertexCount = (uint32_t)(int32_t)surface->vertexCount - (uint32_t)(int32_t)compactWeightedVertexCount;
        uint32_t blendPrimaryBytes = fullBlendVertexCount * (uint32_t)sizeof(XSurfaceBlendVert) +
                                     (uint32_t)(int32_t)compactWeightedVertexCount * (uint32_t)sizeof(XSurfaceBlendVertNoWeight);
        surface->vertexData.blendPrimaryStream = alloc((size_t)blendPrimaryBytes);
        surface->weightedPoints = alloc((size_t)((uint32_t)(int32_t)weightedPointCount * (uint32_t)sizeof(surface->weightedPoints[0])));
        surface->texCoords = alloc((size_t)((uint32_t)(int32_t)surface->vertexCount * (uint32_t)sizeof(surface->texCoords[0])));
    } else {
        surface->boneIndex = (int16_t)((uint32_t)(int32_t)singleBoneIndex << XMODEL_BONE_INDEX_SHIFT);
        /* The prepass and ReadBlend-equivalent index policy prove this mask
         * lane before the retained original store. */
        ((uint8_t *)surface->boneUsage)[singleBoneIndex >> XMODEL_BONE_USAGE_SHIFT] |=
            (uint8_t)(1U << (singleBoneIndex & XMODEL_BONE_USAGE_MASK));
        surface->vertexData.rigidVertices =
            alloc((size_t)((uint32_t)(int32_t)surface->vertexCount * (uint32_t)sizeof(surface->vertexData.rigidVertices[0])));
    ...
    for (int32_t vertex = 0; vertex < surface->vertexCount; ++vertex) {
        if (singleBoneIndex == XMODEL_NO_SINGLE_BONE) {
            XSurfaceBlendVert *weightedVertex = (XSurfaceBlendVert *)(void *)weightedVertexCursor;
            ...
            weightedVertex->additiveWeightCount = XModelReadStreamInt16(&cursor);
            ReadBlend(surface, &primaryBlend, &cursor);
            weightedVertex->blend = primaryBlend;

            if (weightedVertex->additiveWeightCount == 0) {
                weightedVertexCursor += sizeof(XSurfaceBlendVertNoWeight);
            } else {
                weightedVertex->primaryWeight = XModelReadStreamFloat(&cursor);
    ...
    for (int32_t point = 0; point < weightedPointCount; ++point) {
        ...
        surface->weightedPoints[point].weight = XModelReadStreamFloat(&cursor);
    }

    qboolean appendPaddingTriangle = (surface->triangleCount & 1) != 0 ? qtrue : qfalse;
    if (appendPaddingTriangle != qfalse) {
        ...
        surface->triangleCount = (int16_t)(uint16_t)((uint16_t)surface->triangleCount + 1u);
    }
    int32_t triangleAllocCount = surface->triangleCount;
    surface->triangles = alloc((size_t)((uint32_t)triangleAllocCount * (uint32_t)sizeof(surface->triangles[0])));
    coduo_xmodel_unstrip_serialized(stripData, stripCount, surface->triangles);
...
{
    const XModelPartsData *parts = model->info->parts->data.xmodelParts;
    ...
    const float packedQuatScale = 3.0518509447574615e-05f; /* It is effectively the binary32 representation of 1.0 / 32767.0. */
    ...
    if (remaining > 0) {
        do {
            ...
            localRotation[0] = parts->baseRotations[childIndex].components[0] * packedQuatScale;
            localRotation[1] = parts->baseRotations[childIndex].components[1] * packedQuatScale;
            localRotation[2] = parts->baseRotations[childIndex].components[2] * packedQuatScale;
            localRotation[3] = parts->baseRotations[childIndex].components[3] * packedQuatScale;

            QuatMultiply(localRotation, basePose[partIndex - slot->parentPartDeltas[childIndex]].axis[0], basePose[partIndex].axis[0]);
            ++childIndex;
            ++partIndex;
            --remaining;
        } while (remaining > 0);
    ...
    if (remaining > 0) {
        do {
            ...
            const DObjSkelMat *parent = &basePose[partIndex - slot->parentPartDeltas[childIndex]];

            XModelExpandQuatToAxis(pose->axis[0]);
            XSurfaceTransformPoint43(parts->baseTranslations[childIndex], parent, pose->origin);
            ++childIndex;
            ++partIndex;
            --remaining;
        } while (remaining > 0);
...
int32_t XModelTraceLine(const XModel *model, trace_t *trace, const DObjSkelMat *basePose, const vec3_t start, const vec3_t end,
                        int32_t contentsMask)
{
    ...
    for (int32_t surfaceIndex = 0; surfaceIndex < collision->collisionSurfaceCount; ++surfaceIndex) {
        const XModelCollSurf *surface = &collision->collisionSurfaces[surfaceIndex];
        if ((contentsMask & surface->contents) == 0) {
            continue;
        }

        const DObjSkelMat *pose = &basePose[surface->basePoseIndex];
        vec3_t offsetStart = {
        ...
        if (CM_TraceLineSkipsBox(localStart, localEnd, surface->expandedMins, surface->expandedMaxs, trace->fraction) != qfalse) {
            continue;
        }
        ...
        for (int32_t facetIndex = 0; facetIndex < surface->numCollTris; ++facetIndex) {
            const XModelCollTri *facet = &surface->collTris[facetIndex];
            const XModelCollTriPlane *plane0 = &facet->planes[0];
            ...
            const long double endDistanceRaw =
                ((long double)localEnd[0] * (long double)plane0->normal[0] + (long double)localEnd[2] * (long double)plane0->normal[2]) +
                (long double)localEnd[1] * (long double)plane0->normal[1] - (long double)plane0->distance;
             ...
             * start point. */
            const long double startDistanceRaw = ((long double)localStart[0] * (long double)plane0->normal[0] +
                                                  (long double)localStart[2] * (long double)plane0->normal[2]) +
                                                 (long double)localStart[1] * (long double)plane0->normal[1] -
            ...
            const long double distanceDenominator = (long double)startDistance - (long double)endDistance;
            /* 0x0049f265..0x0049f284 stores hitFraction, but compares its
             * retained quotient against the current trace fraction. */
            const long double hitFractionRaw = ((long double)startDistance - (long double)planeOffsetEpsilon) / distanceDenominator;
            ...
            const long double planeFractionRaw = (long double)startDistance / distanceDenominator;
            vec3_t hitPoint;
            hitPoint[0] = (float)((long double)delta[0] * planeFractionRaw + (long double)localStart[0]);
            hitPoint[1] = (float)((long double)delta[1] * planeFractionRaw + (long double)localStart[1]);
            const long double hitPointZRaw = (long double)delta[2] * planeFractionRaw + (long double)localStart[2];
            hitPoint[2] = (float)hitPointZRaw;
            const XModelCollTriPlane *plane1 = &facet->planes[1];
            ...
            const long double barycentric0Raw =
                ((hitPointZRaw * (long double)plane1->normal[2] + (long double)hitPoint[1] * (long double)plane1->normal[1]) +
                 (long double)hitPoint[0] * (long double)plane1->normal[0]) -
                (long double)plane1->distance;
            float barycentric0 = (float)barycentric0Raw;
            if (barycentric0Raw < (long double)barycentricMin || barycentric0 > barycentricMax) {
                continue;
            }

            const XModelCollTriPlane *plane2 = &facet->planes[2];
            ...
            const long double barycentric1Raw =
                (((long double)hitPoint[2] * (long double)plane2->normal[2] + (long double)hitPoint[1] * (long double)plane2->normal[1]) +
                 (long double)hitPoint[0] * (long double)plane2->normal[0]) -
                (long double)plane2->distance;
            if (barycentric1Raw < (long double)barycentricMin ||
                (long double)barycentric0 + barycentric1Raw > (long double)barycentricMax) {
                continue;
            }
    ...
    vec3_t worldNormal;
    XSurfaceTransformNormal43(trace->normal, &basePose[hitPart], worldNormal);
 ...
 */
int32_t XModelTraceLine(const XModel *model, trace_t *traceState, const DObjSkelMat *basePose, const vec3_t start, const vec3_t end,
                        int32_t contentMask)
{
    const XModel *entry = model;
    ...
    for (int32_t surfaceIndex = 0; surfaceIndex < collision->collisionSurfaceCount; ++surfaceIndex) {
        ...
        if (CM_TraceLineSkipsBox(localStart, localEnd, surface->expandedMins, surface->expandedMaxs, traceState->fraction) != qfalse) {
            continue;
        }
        ...
        for (int32_t facetIndex = 0; facetIndex < surface->numCollTris; ++facetIndex) {
            ...
            float endDist =
                localEnd[0] * plane0->normal[0] + localEnd[1] * plane0->normal[1] + localEnd[2] * plane0->normal[2] - plane0->distance;

            if (endDist >= 0.0f) {
                continue;
            }

            float startDist = localStart[0] * plane0->normal[0] + localStart[1] * plane0->normal[1] + localStart[2] * plane0->normal[2] -
                              plane0->distance;
            ...
            float hitFraction = (startDist - planeOffsetEpsilon) / (startDist - endDist);
            ...
            const XModelCollTriPlane *plane1 = &facet->planes[1];
            float bary0 =
                hitPoint[0] * plane1->normal[0] + hitPoint[1] * plane1->normal[1] + hitPoint[2] * plane1->normal[2] - plane1->distance;
            ...
            const XModelCollTriPlane *plane2 = &facet->planes[2];
            float bary1 =
                hitPoint[0] * plane2->normal[0] + hitPoint[1] * plane2->normal[1] + hitPoint[2] * plane2->normal[2] - plane2->distance;
            if (bary1 < barycentricMin || bary0 + bary1 > barycentricMax) {
                continue;
            }
...
#if defined(WINDOWS_BEHAVIOR)
qboolean XModelGetStaticBounds(const XModel *model, axis_t transform, vec3_t mins, vec3_t maxs)
{
    const XModelInfo *collision = model->info;
    ...
    for (int32_t surfaceIndex = 0; surfaceIndex < collision->collisionSurfaceCount; ++surfaceIndex) {
        const XModelCollSurf *surface = &collision->collisionSurfaces[surfaceIndex];
        for (int32_t corner = 0; corner < XMODEL_BOUNDS_CORNER_COUNT; ++corner) {
            vec3_t localCorner = {
                (corner & 1) == 0 ? surface->expandedMaxs[0] : surface->expandedMins[0],
                (corner & 2) == 0 ? surface->expandedMaxs[1] : surface->expandedMins[1],
                (corner & 4) == 0 ? surface->expandedMaxs[2] : surface->expandedMins[2],
            };
            ...
            vec3_t transformedCorner = {
                (float)(((long double)localCorner[0] * transform[0][0] + (long double)localCorner[2] * transform[2][0]) +
                        (long double)localCorner[1] * transform[1][0]),
                (float)(((long double)localCorner[0] * transform[0][1] + (long double)localCorner[2] * transform[2][1]) +
                        (long double)localCorner[1] * transform[1][1]),
                (float)(((long double)localCorner[2] * transform[2][2] + (long double)localCorner[1] * transform[1][2]) +
...
qboolean XModelGetStaticBounds(const XModel *model, axis_t transform, vec3_t mins, vec3_t maxs)
{
    ...
    for (int32_t surfaceIndex = 0; surfaceIndex < collision->collisionSurfaceCount; ++surfaceIndex) {
        ...
        for (int32_t corner = 0; corner < XMODEL_BOUNDS_CORNER_COUNT; ++corner) {
            ...
            localCorner[0] = (corner & 1) == 0 ? surface->expandedMaxs[0] : surface->expandedMins[0];
            localCorner[1] = (corner & 2) == 0 ? surface->expandedMaxs[1] : surface->expandedMins[1];
            localCorner[2] = (corner & 4) == 0 ? surface->expandedMaxs[2] : surface->expandedMins[2];

            /* C11 cannot add element qualification through the axis_t array
             * typedef implicitly; the matrix remains read-only. */
            MatrixTransformVector(localCorner, (const vec3_t *)transform, transformedCorner);
...
#if defined(WINDOWS_BEHAVIOR)
void XSurfaceGetVerts(const XSurface *surface, const DObjSkelMat *basePose, vec3_t *outVerts, vec2_t *outTexCoords, vec3_t *outNormals)
{
    ...
    if (outTexCoords != NULL) {
        ...
        uint32_t texCoordBytes = (uint32_t)(int32_t)surface->vertexCount * (uint32_t)sizeof(surface->texCoords[0]);
        memcpy(outTexCoords, surface->texCoords, (size_t)texCoordBytes);
    }

    if (surface->boneIndex == XMODEL_NO_SINGLE_BONE) {
        const uint8_t *primaryVertexCursor = surface->vertexData.blendPrimaryStream;
        ...
        while (verticesRemaining > 0) {
            const XSurfaceBlendVert *primary = (const XSurfaceBlendVert *)(const void *)primaryVertexCursor;
            const DObjSkelMat *primaryPose =
                (const DObjSkelMat *)(const void *)((const uint8_t *)basePose + primary->blend.boneMatrixOffset);

            if (outNormal != NULL) {
                (*outNormal)[0] = (float)(((long double)primary->normal[2] * primaryPose->axis[2][0] +
                                           (long double)primary->normal[1] * primaryPose->axis[1][0]) +
                                          (long double)primary->normal[0] * primaryPose->axis[0][0]);
                (*outNormal)[1] = (float)(((long double)primary->normal[2] * primaryPose->axis[2][1] +
                                           (long double)primary->normal[0] * primaryPose->axis[0][1]) +
                                          (long double)primary->normal[1] * primaryPose->axis[1][1]);
                (*outNormal)[2] = (float)(((long double)primary->normal[2] * primaryPose->axis[2][2] +
                                           (long double)primary->normal[0] * primaryPose->axis[0][2]) +
                                          (long double)primary->normal[1] * primaryPose->axis[1][2]);
                ++outNormal;
            }
            const float *primaryPoint = primary->blend.position;
            (*outVert)[0] =
                (float)((((long double)primaryPoint[2] * primaryPose->axis[2][0] + (long double)primaryPoint[1] * primaryPose->axis[1][0]) +
                         (long double)primaryPoint[0] * primaryPose->axis[0][0]) +
                        (long double)primaryPose->origin[0]);
            (*outVert)[1] =
                (float)((((long double)primaryPoint[0] * primaryPose->axis[0][1] + (long double)primaryPoint[2] * primaryPose->axis[2][1]) +
                         (long double)primaryPoint[1] * primaryPose->axis[1][1]) +
                        (long double)primaryPose->origin[1]);
            (*outVert)[2] =
                (float)((((long double)primaryPoint[0] * primaryPose->axis[0][2] + (long double)primaryPoint[2] * primaryPose->axis[2][2]) +
                         (long double)primaryPoint[1] * primaryPose->axis[1][2]) +
            ...
            if (primary->additiveWeightCount > 0) {
                ...
                int32_t additiveWeightsRemaining = primary->additiveWeightCount;
                ...
                do {
                    ...
                    const vec3_t point = {additivePoint->blend.position[0], additivePoint->blend.position[1],
                                          additivePoint->blend.position[2]};
                    const long double weight = additivePoint->weight;
                    (*outVert)[0] =
                        (float)(((((long double)point[1] * additivePose->axis[1][0] + (long double)point[2] * additivePose->axis[2][0]) +
                                  (long double)point[0] * additivePose->axis[0][0]) +
                                 (long double)additivePose->origin[0]) *
                                    weight +
                                (long double)(*outVert)[0]);
                    (*outVert)[1] =
                        (float)(((((long double)point[1] * additivePose->axis[1][1] + (long double)point[0] * additivePose->axis[0][1]) +
                                  (long double)point[2] * additivePose->axis[2][1]) +
                                 (long double)additivePose->origin[1]) *
                                    weight +
                                (long double)(*outVert)[1]);
                    (*outVert)[2] =
                        (float)(((((long double)point[1] * additivePose->axis[1][2] + (long double)point[0] * additivePose->axis[0][2]) +
                                  (long double)point[2] * additivePose->axis[2][2]) +
                ...
                primaryVertexCursor += sizeof(XSurfaceBlendVertNoWeight);
            }
            ++outVert;
            --verticesRemaining;
        }
    ...
    const DObjSkelMat *rigidPose = (const DObjSkelMat *)(const void *)((const uint8_t *)basePose + surface->boneIndex);
    const XSurfaceRigidVert *rigidVertex = surface->vertexData.rigidVertices;
    vec3_t *outVert = outVerts;
    vec3_t *outNormal = outNormals;
    ...
    while (verticesRemaining > 0) {
        if (outNormal != NULL) {
            ...
            (*outNormal)[0] = (float)(((long double)normal[1] * rigidPose->axis[1][0] + (long double)normal[0] * rigidPose->axis[0][0]) +
                                      (long double)normal[2] * rigidPose->axis[2][0]);
            (*outNormal)[1] = (float)(((long double)normal[2] * rigidPose->axis[2][1] + (long double)normal[1] * rigidPose->axis[1][1]) +
                                      (long double)normal[0] * rigidPose->axis[0][1]);
            (*outNormal)[2] = (float)(((long double)normal[2] * rigidPose->axis[2][2] + (long double)normal[1] * rigidPose->axis[1][2]) +
                                      (long double)normal[0] * rigidPose->axis[0][2]);
            ++outNormal;
        }
        const float *point = rigidVertex->position;
        (*outVert)[0] = (float)((((long double)point[1] * rigidPose->axis[1][0] + (long double)point[2] * rigidPose->axis[2][0]) +
                                 (long double)point[0] * rigidPose->axis[0][0]) +
                                (long double)rigidPose->origin[0]);
        (*outVert)[1] = (float)((((long double)point[0] * rigidPose->axis[0][1] + (long double)point[2] * rigidPose->axis[2][1]) +
                                 (long double)point[1] * rigidPose->axis[1][1]) +
                                (long double)rigidPose->origin[1]);
        (*outVert)[2] = (float)((((long double)point[0] * rigidPose->axis[0][2] + (long double)point[2] * rigidPose->axis[2][2]) +
...
void XSurfaceGetVerts(const XSurface *surface, const DObjSkelMat *basePose, vec3_t *outVerts, vec2_t *outTexCoords, vec3_t *outNormals)
{
    ...
    if (XSurfaceGetBoneIndex(surface) == XMODEL_NO_SINGLE_BONE) {
        const uint8_t *primaryVertexCursor = surface->vertexData.blendPrimaryStream;
        const XSurfaceWeightedPoint *additiveVerts = (const XSurfaceWeightedPoint *)surface->weightedPoints;

        int32_t remainingVertices = surface->vertexCount;
        int32_t vertexIndex = 0;
        while (remainingVertices > 0) {
            const XSurfaceBlendVert *primary = (const XSurfaceBlendVert *)primaryVertexCursor;
            const DObjSkelMat *primaryPose = (const DObjSkelMat *)((const uint8_t *)basePose + primary->blend.boneMatrixOffset);

            if (outNormals != NULL) {
                XSurfaceTransformNormal43(primary->normal, primaryPose, outNormals[vertexIndex]);
            }

            XSurfaceTransformPoint43(primary->blend.position, primaryPose, outVerts[vertexIndex]);
            ...
            if (primary->additiveWeightCount > 0) {
                ...
                do {
                    const XSurfaceWeightedPoint *additive = additiveVerts++;
                    const DObjSkelMat *additivePose = (const DObjSkelMat *)((const uint8_t *)basePose + additive->blend.boneMatrixOffset);

                    XSurfaceAccumulateWeightedPoint43(additive->blend.position, additive->weight, additivePose, outVerts[vertexIndex]);
                    --remainingWeights;
                } while (remainingWeights > 0);

                primaryVertexCursor += sizeof(XSurfaceBlendVert);
            } else {
                primaryVertexCursor += sizeof(XSurfaceBlendVertNoWeight);
            }
    ...
    const DObjSkelMat *rigidPose = (const DObjSkelMat *)((const uint8_t *)basePose + surface->boneIndex);
    const XSurfaceRigidVert *rigidVerts = surface->vertexData.rigidVertices;

    int32_t remainingVertices = surface->vertexCount;
    int32_t vertexIndex = 0;
    while (remainingVertices > 0) {
        if (outNormals != NULL) {
            XSurfaceTransformNormal43(rigidVerts[vertexIndex].normal, rigidPose, outNormals[vertexIndex]);
        }

        XSurfaceTransformPoint43(rigidVerts[vertexIndex].position, rigidPose, outVerts[vertexIndex]);
        ++vertexIndex;
        --remainingVertices;
    }
}
...
XModelSurfs *XModelSurfsCloneSurfs(const XModelSurfs *modelSurfs, void *(*alloc)(size_t size))
{
    ...
    clone->surfaces = alloc((size_t)((uint32_t)(int32_t)clone->surfaceCount * (uint32_t)sizeof(clone->surfaces[0])));
    memcpy(clone->surfaces, source->surfaces, (size_t)((uint32_t)(int32_t)clone->surfaceCount * (uint32_t)sizeof(clone->surfaces[0])));
...
XModelSurfs *XModelSurfsPrecache(const char *name, void *(*alloc)(size_t size))
{
    XModelSurfs *entry = (XModelSurfs *)FS_GetDataForFile("xmodelsurfs", name, "");
    ...
    if (strlen(name) > sizeof(path) - sizeof("xmodelsurfs/")) {
        Com_Error(ERR_DROP, "\x15"
                            "XModel surface path is too long");
        return NULL;
    }
    ...
    if (coduo_xmodel_validate_surfs_file(fileBuffer, (size_t)fileLength) == qfalse) {
        FS_FreeFile(fileBuffer);
        Com_Error(ERR_DROP,
                  "\x15"
                  "Malformed xmodelsurfs asset '%s'",
                  name);
        return NULL;
    }
    ...
    if (version != XMODEL_ASSET_VERSION) {
        ...
        Com_Error(ERR_DROP,
                  "\x15"
                  "xmodelsurfs '%s' out of date (version %d, "
                  "expecting %d)",
    ...
    surfs->surfaces = alloc((size_t)((uint32_t)(int32_t)surfaceCount * (uint32_t)sizeof(surfs->surfaces[0])));
    XSurface *surfaces = alloc((size_t)((uint32_t)(int32_t)surfaceCount * (uint32_t)sizeof(surfaces[0])));
...
fileData_t *XModelPartsPrecache(const char *name, void *(*alloc)(size_t size))
{
    fileData_t *entry = FS_GetDataForFile("xmodelparts", name, "");
    ...
    if (coduo_xmodel_validate_parts_file(fileBuffer, (size_t)fileLength) == qfalse) {
        FS_FreeFile(fileBuffer);
        Com_Error(ERR_DROP,
                  "\x15"
                  "Malformed xmodelparts asset '%s'",
                  name);
        return NULL;
    }
    ...
    if (version != XMODEL_ASSET_VERSION) {
        ...
        Com_Error(ERR_DROP,
                  "\x15"
                  "xmodelparts '%s' out of date (version %d, "
                  "expecting %d)",
                  name, version, XMODEL_ASSET_VERSION);
        return NULL;
    }
    ...
    uint16_t totalPartCountBits = (uint16_t)((uint16_t)rootPartCount + (uint16_t)childPartCount);
    int16_t totalPartCount;
    memcpy(&totalPartCount, &totalPartCountBits, sizeof(totalPartCount));
    uint32_t partNameTableBytes = (uint32_t)(sizeof(XModelPartNameTable) - sizeof(((XModelPartNameTable *)0)->handles[0])) +
                                  (uint32_t)(int32_t)totalPartCount * (uint32_t)sizeof(((XModelPartNameTable *)0)->handles[0]);
    XModelPartNameTable *partNameTable = alloc((size_t)partNameTableBytes);
    partNameTable->count = totalPartCount;

    if (totalPartCount > XMODEL_MAX_BONES) {
        Com_Error(ERR_DROP,
                  "\x15"
                  "xmodel '%s' has more than %d bones",
                  name, XMODEL_MAX_BONES);
        return NULL;
    }

    uint32_t partNameSlotBytes = (uint32_t)(sizeof(XModelPartNameTableSlot) - sizeof(((XModelPartNameTableSlot *)0)->parentPartDeltas[0])) +
                                 (uint32_t)(int32_t)childPartCount * (uint32_t)sizeof(((XModelPartNameTableSlot *)0)->parentPartDeltas[0]);
    ...
    uint32_t rotationBytes = childPartCountBits * (uint32_t)sizeof(xanim_int16_vec4_t);
    uint32_t translationBytes = childPartCountBits * (uint32_t)sizeof(vec3_t);
    ...
    uint32_t stockI386UnusedAllocationBytes = (uint32_t)(0x68 - sizeof(xmodel_parts_data_stock_i386_layout_t)) +
                                              childPartCountBits * (uint32_t)(0x18 - sizeof(xanim_int16_vec4_t) - sizeof(vec3_t));
    uint32_t partsAllocationBytes = (uint32_t)sizeof(XModelPartsData) + rotationBytes + translationBytes + stockI386UnusedAllocationBytes;
    ...
    parts->baseTranslations =
        childPartCount != 0
            ? (vec3_t *)((uint8_t *)parts->baseRotations + (intptr_t)childPartCount * (intptr_t)sizeof(parts->baseRotations[0]))
            : NULL;
    parts->partStateIndices = alloc((size_t)(uint32_t)(int32_t)totalPartCount);
    parts->partCollisions = alloc((size_t)((uint32_t)(int32_t)totalPartCount * (uint32_t)sizeof(parts->partCollisions[0])));

    for (int32_t partIndex = rootPartCount; partIndex < totalPartCount; ++partIndex) {
        int32_t childIndex = partIndex - rootPartCount;
        slot->parentPartDeltas[childIndex] = (uint8_t)(partIndex - *cursor++);
        parts->baseTranslations[childIndex][0] = XModelReadStreamFloat(&cursor);
        parts->baseTranslations[childIndex][1] = XModelReadStreamFloat(&cursor);
        parts->baseTranslations[childIndex][2] = XModelReadStreamFloat(&cursor);
        ...
        ReadQuat(packedRotation, &parts->baseRotations[childIndex]);
    }

    for (int32_t partIndex = 0; partIndex < totalPartCount; ++partIndex) {
        size_t partNameSize = strlen((const char *)cursor) + 1U;
        partNameTable->handles[partIndex] = SL_GetStringOfLen((const char *)cursor, 0, partNameSize, XMODEL_PART_NAME_STRING_TYPE);
        cursor += partNameSize;

        XModelPartColl *collision = &parts->partCollisions[partIndex];
        ...
        const float centerSumX = (float)((long double)collision->maxs[0] + collision->mins[0]);
        const float centerSumY = (float)((long double)collision->maxs[1] + collision->mins[1]);
        const float centerSumZ = (float)((long double)collision->maxs[2] + collision->mins[2]);
        collision->center[0] = (float)((long double)centerSumX * (long double)0.5f);
        collision->center[1] = (float)((long double)centerSumY * (long double)0.5f);
        collision->center[2] = (float)((long double)centerSumZ * (long double)0.5f);
        const float deltaX = (float)((long double)collision->maxs[0] - collision->center[0]);
#if defined(WINDOWS_BEHAVIOR)
        ...
        const long double deltaY = (long double)collision->maxs[1] - collision->center[1];
        const long double deltaZ = (long double)collision->maxs[2] - collision->center[2];
        /* CoDUOMP.exe 0x0049df7a..0x0049df95 folds Z, Y, X. */
        collision->radiusSq = (float)((deltaZ * deltaZ + deltaY * deltaY) + (long double)deltaX * (long double)deltaX);
#else
        /* Linux stores all three deltas as binary32 before folding X, Y, Z;
         * coduo_lnxded 0x080c320b..0x080c3287. */
        const float deltaY = (float)((long double)collision->maxs[1] - collision->center[1]);
        const float deltaZ = (float)((long double)collision->maxs[2] - collision->center[2]);
        collision->radiusSq = (float)(((long double)deltaX * (long double)deltaX + (long double)deltaY * (long double)deltaY) +
                                      (long double)deltaZ * (long double)deltaZ);
#endif
    }

    memcpy(parts->partStateIndices, cursor, (size_t)(uint32_t)(int32_t)totalPartCount);
    FS_FreeFile(fileBuffer);
...
const uint8_t *XModelLoadConfigFile(const char *name, const uint8_t *cursor, XModelConfig *header)
{
    int16_t version = XModelReadStreamInt16(&cursor);
    if (version != XMODEL_ASSET_VERSION) {
        Com_Error(ERR_DROP,
                  "\x15"
                  "xmodel '%s' out of date (version %d, "
                  "expecting %d)",
                  name, version, XMODEL_ASSET_VERSION);
        return NULL;
    }
...
const uint8_t *XModelLoadCollData(const uint8_t *cursor, XModelInfo *collision, void *(*alloc)(size_t size))
{
    ...
    collision->collisionSurfaceCount = (int32_t)XModelReadStreamUint32(&cursor);
    ...
    collision->collisionSurfaces =
        alloc((size_t)((uint32_t)collision->collisionSurfaceCount * (uint32_t)sizeof(collision->collisionSurfaces[0])));
    for (int32_t surfaceIndex = 0; surfaceIndex < collision->collisionSurfaceCount; ++surfaceIndex) {
        XModelCollSurf *surface = &collision->collisionSurfaces[surfaceIndex];
        surface->numCollTris = (int32_t)XModelReadStreamUint32(&cursor);
        surface->collTris = alloc((size_t)((uint32_t)surface->numCollTris * (uint32_t)sizeof(surface->collTris[0])));

        for (int32_t facetIndex = 0; facetIndex < surface->numCollTris; ++facetIndex) {
            XModelCollTri *facet = &surface->collTris[facetIndex];
            for (int32_t planeIndex = 0; planeIndex < XMODEL_COLLISION_FACET_PLANE_COUNT; ++planeIndex) {
                facet->planes[planeIndex].normal[0] = XModelReadStreamFloat(&cursor);
                facet->planes[planeIndex].normal[1] = XModelReadStreamFloat(&cursor);
                facet->planes[planeIndex].normal[2] = XModelReadStreamFloat(&cursor);
                facet->planes[planeIndex].distance = XModelReadStreamFloat(&cursor);
            }
        }

        surface->expandedMins[0] = (float)((long double)XModelReadStreamFloat(&cursor) - boundsEpsilon);
        surface->expandedMins[1] = (float)((long double)XModelReadStreamFloat(&cursor) - boundsEpsilon);
        surface->expandedMins[2] = (float)((long double)XModelReadStreamFloat(&cursor) - boundsEpsilon);
        surface->expandedMaxs[0] = (float)((long double)XModelReadStreamFloat(&cursor) + boundsEpsilon);
        surface->expandedMaxs[1] = (float)((long double)XModelReadStreamFloat(&cursor) + boundsEpsilon);
        surface->expandedMaxs[2] = (float)((long double)XModelReadStreamFloat(&cursor) + boundsEpsilon);
        surface->basePoseIndex = (int32_t)XModelReadStreamUint32(&cursor);
        surface->contents = (int32_t)(XModelReadStreamUint32(&cursor) & XMODEL_COLLISION_CONTENTS_MASK);
        surface->surfaceFlags = (int32_t)XModelReadStreamUint32(&cursor);
...
{
    return FS_GetDataForFile("xmodel", name, "") != NULL ? qtrue : qfalse;
}

/* Source: CoDUOMP.exe 0x0049e270..0x0049e614. */
XModel *XModelPrecache(const char *name, xmodel_load_mode_t loadMode, void *(*alloc)(size_t size), void *(*optionalAlloc)(size_t size))
{
    ...
    if (collision == NULL) {
        ...
        if (coduo_xmodel_validate_model_file(fileBuffer, (size_t)fileLength) == qfalse) {
            FS_FreeFile(fileBuffer);
            Com_Error(ERR_DROP,
                      "\x15"
                      "Malformed xmodel asset '%s'",
                      name);
            return NULL;
        }

        const uint8_t *cursor = XModelLoadConfigFile(name, fileBuffer, &header);
        if (cursor == NULL) {
            FS_FreeFile(fileBuffer);
            return NULL;
        }
        const uint8_t *fileEnd = (const uint8_t *)fileBuffer + (size_t)fileLength;

        uint32_t lodNameBytes = 0;
        for (int32_t lodIndex = 0; lodIndex < XMODEL_LOD_COUNT; ++lodIndex) {
            lodNameBytes += (uint32_t)strlen(header.lods[lodIndex].name) + 1u;
        }

        uint32_t modelInfoBytes = (uint32_t)sizeof(*collision) + lodNameBytes;
        ...
        for (int32_t lodIndex = 0; lodIndex < XMODEL_LOD_COUNT; ++lodIndex) {
            ...
            lod->surfaceNameTable = alloc((size_t)((uint32_t)(int32_t)lod->surfaceCount * (uint32_t)sizeof(lod->surfaceNameTable[0])));
            for (int32_t surfaceIndex = 0; surfaceIndex < lod->surfaceCount; ++surfaceIndex) {
                ...
                lod->surfaceNameTable[surfaceIndex] = SL_GetString_(surfaceName, 0, XMODEL_SURFACE_NAME_STRING_TYPE);
            }
            lod->distance = header.lods[lodIndex].distance;
        }
    ...
    collision->parts = XModelPartsPrecache(collision->lodRecords[0].name, alloc);
    ...
    if (coduo_xmodel_loaded_collision_fits(collision) == qfalse) {
        Com_Error(ERR_DROP,
                  "\x15"
                  "XModel '%s' has an invalid collision bone",
                  name);
        return NULL;
    }
    ...
    for (int32_t lodIndex = 0; lodIndex < XMODEL_LOD_COUNT && collision->lodRecords[lodIndex].name[0] != '\0'; ++lodIndex) {
        XModelLodInfo *lod = &collision->lodRecords[lodIndex];
        ...
        if (coduo_xmodel_loaded_references_fit(collision, lod, lod->surfs->surfs) == qfalse) {
            Com_Error(ERR_DROP,
                      "\x15"
                      "XModel '%s' has invalid surface references",
                      name);
            return NULL;
        }

        if (xmodel_optimizeEnabled != qfalse && loadMode == XMODEL_LOAD_SURFACES_PREPROCESSED) {
            xmodel_compat_optimize_loaded_surfs(lod->surfs->surfs, optionalAlloc);
        }
    }
    return entry;
}
