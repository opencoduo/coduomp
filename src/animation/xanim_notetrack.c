...
uint16_t XAnimGetNextNotifyTime(XAnimEntry *entry, XAnimInfo *node, float time)
{
    ...
    xanim_notetrack_t *notetracks = entry->payload.leafAsset->data.xanimParts->noteTracks;
    xanim_notetrack_t *best = NULL;
    float bestTime = 2.0f;

    for (xanim_notetrack_t *cursor = notetracks; cursor->nameHandle != 0; ++cursor) {
    ...
    if (best == NULL) {
        ...
        uint32_t byteDelta = UINT32_C(0) - (uint32_t)(uintptr_t)(const void *)notetracks;
