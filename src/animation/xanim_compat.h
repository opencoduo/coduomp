...
{
    return offsetof(XAnimTree, poolNodeHandles) + nodeCount * sizeof(uint16_t) + nodeCount * sizeof(xanim_runtime_tree_tail_span_t) +
           sizeof(uint16_t);
}
...
static inline uint8_t *coduo_xanim_part_remap_table_bytes(XAnimTree *runtimeTree, int32_t selector)
{
    size_t nodeCount = runtimeTree->sourceTree->nodeCount;
    uint8_t *tailBytes = (uint8_t *)&runtimeTree->poolNodeHandles[nodeCount];
 ...
 * tables. */
static inline uint16_t coduo_xanim_part_remap_handle_load(const uint8_t *tableBytes, size_t nodeIndex)
{
    uint16_t handle;
 ...
 * tables. */
static inline void coduo_xanim_part_remap_handle_store(uint8_t *tableBytes, size_t nodeIndex, uint16_t handle)
{
...
static inline uint8_t *coduo_xanim_part_remap_generation_bytes(uint8_t *tableBytes, size_t nodeCount)
{
