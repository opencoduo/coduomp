...
static uint8_t coduomp_dobj_ownership_mask(int32_t slot,
                                          uint8_t ownership)
{
...
{
    return (dobjAllocState[slot >> 2] &
            coduomp_dobj_ownership_mask(slot, DOBJ_ANY_OWNERSHIP)) != 0
               ? qtrue
               : qfalse;
}

/* NOT_FROM_ORIGINAL_SOURCE: typed factoring of the repeated packed-bit set. */
static void coduomp_dobj_set_ownership(int32_t slot,
                                      uint8_t ownership)
{
    dobjAllocState[slot >> 2] = (uint8_t)(
        dobjAllocState[slot >> 2] |
        coduomp_dobj_ownership_mask(slot, ownership));
}

/* NOT_FROM_ORIGINAL_SOURCE: typed factoring of the repeated packed-bit clear. */
static void coduomp_dobj_clear_ownership(int32_t slot,
                                        uint8_t ownership)
{
    dobjAllocState[slot >> 2] = (uint8_t)(
        dobjAllocState[slot >> 2] &
        (uint8_t)~coduomp_dobj_ownership_mask(slot, ownership));
}

void Com_InitDObj(void)
{
    Com_Memset(dobjAllocState, 0, sizeof(dobjAllocState));
    Com_Memset(clientDObjHandleByEntity, 0,
               sizeof(clientDObjHandleByEntity));
    Com_Memset(serverDObjHandleByEntity, 0,
               sizeof(serverDObjHandleByEntity));
...
int32_t Com_GetFreeDObjIndex(void)
{
    ...
    for (slot = dobjLastAllocatedIndex + 1;
         slot < DOBJ_POOL_COUNT; ++slot) {
                            ...
                            qboolean releaseRuntimeTree)
{
    ...
    const int32_t slot =
        (int16_t)clientDObjHandleByEntity[entityNum];
                            ...
                            qboolean releaseRuntimeTree)
{
    ...
    const int32_t slot =
        (int16_t)serverDObjHandleByEntity[entityNum];
...
void Com_MirrorServerDObjsToClient(void)
{
    for (int32_t entityNum = 0;
         entityNum < DOBJ_SERVER_HANDLE_COUNT; ++entityNum) {
...
_Static_assert(sizeof(serverDObjHandleByEntity) == 0x800,
               "server DObj handle table size changed");
_Static_assert(sizeof(dobjAllocState) == 0x100,
               "packed DObj allocation-state size changed");
_Static_assert(sizeof(clientDObjHandleByEntity) == 0x900,
               "client DObj handle table size changed");
#endif
