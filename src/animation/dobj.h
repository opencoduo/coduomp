...
void DObjCreate(const DObjModel *models, uint16_t modelCount, XAnimTree *runtimeTree, DObj *obj, uint16_t gameId);
void DObjFree(DObj *obj, qboolean releaseRuntimeTree);
void DObjBuildTracePartRemap(DObj *obj);
void DObjGetHierarchyBits(DObj *obj, int32_t boneIndex, uint32_t *partBits);
...
void DObjTraceModelParts(const DObj *obj, const vec3_t start, const vec3_t end, int32_t contentsMask, dobj_trace_result_t *trace);
