...
void XAnimCalcRelDeltaParts(XAnimParts *record, float weight, float *delta, float startTime, float endTime);
void XAnimCalcAbsDeltaParts(XAnimParts *record, float weight, float *delta, float time);
void XAnimCalcDeltaTree(XAnimTree *tree, uint32_t animIndex, float weight, float *delta, qboolean clear, qboolean normalize);
