...
{
    const int32_t componentPairs[3][2] = {{1, 2}, {2, 0}, {0, 1}};
    ...
    for (int32_t axis = 0; axis < 3; ++axis) {
        if (angles[axis] != 0.0f) {
            ...
            radians =
                x87f_store_f64(x87f_div(x87f_mul(x87f_load_f32(angles[axis]), x87f_load_f32(3.1415927410125732f)), x87f_load_f64(180.0)));
#else
            radians =
                x87f_store_f64(x87f_div(x87f_mul(x87f_load_f32(angles[axis]), x87f_load_f64(3.141592653589793)), x87f_load_f64(180.0)));
#endif
#else
#if defined(WINDOWS_BEHAVIOR)
            radians = (double)((long double)angles[axis] * (long double)3.1415927410125732f / 180.0L);
#else
            radians = (double)((long double)angles[axis] * (long double)3.141592653589793 / 180.0L);
#endif
#endif
...
#if EMULATE_X87
            rotated[first] = x87f_store_f32(x87f_sub(x87f_mul(x87f_load_f32(previous[first]), x87f_load_f64(cosine)),
                                                     x87f_mul(x87f_load_f32(previous[second]), x87f_load_f64(sine))));
            rotated[second] = x87f_store_f32(x87f_add(x87f_mul(x87f_load_f32(previous[second]), x87f_load_f64(cosine)),
                                                      x87f_mul(x87f_load_f32(previous[first]), x87f_load_f64(sine))));
#else
            rotated[first] =
                (float)((long double)previous[first] * (long double)cosine - (long double)previous[second] * (long double)sine);
            rotated[second] =
                (float)((long double)previous[second] * (long double)cosine + (long double)previous[first] * (long double)sine);
#endif
        }
