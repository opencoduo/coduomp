                            ...
                            const vec3_t lineEnd, vec3_t projected)
{
    ...
    for (int32_t lane = 0; lane < 3; ++lane) {
        pointDelta[lane] = x87f_store_f32(x87f_sub(
            x87f_load_f32(point[lane]), x87f_load_f32(lineStart[lane])));
        direction[lane] = x87f_store_f32(x87f_sub(
            x87f_load_f32(lineEnd[lane]), x87f_load_f32(lineStart[lane])));
    }
#else
    for (int32_t lane = 0; lane < 3; ++lane) {
        pointDelta[lane] = (float)((long double)point[lane] -
                                   lineStart[lane]);
        direction[lane] = (float)((long double)lineEnd[lane] -
                                  lineStart[lane]);
    }
#endif
...
#if EMULATE_X87
    projection = x87f_store_f32(x87f_add(
        x87f_add(
            x87f_mul(x87f_load_f32(direction[2]),
                     x87f_load_f32(pointDelta[2])),
            x87f_mul(x87f_load_f32(direction[1]),
                     x87f_load_f32(pointDelta[1]))),
        x87f_mul(x87f_load_f32(direction[0]),
                 x87f_load_f32(pointDelta[0]))));
    for (int32_t lane = 0; lane < 3; ++lane) {
        projected[lane] = x87f_store_f32(x87f_add(
            x87f_load_f32(lineStart[lane]),
            x87f_mul(x87f_load_f32(projection),
                     x87f_load_f32(direction[lane]))));
    }
#else
    projection = (float)(
        ((long double)direction[2] * pointDelta[2] +
         (long double)direction[1] * pointDelta[1]) +
        (long double)direction[0] * pointDelta[0]);
    for (int32_t lane = 0; lane < 3; ++lane) {
        projected[lane] = (float)(
            (long double)lineStart[lane] +
            (long double)projection * direction[lane]);
    }
#endif
}
                            ...
                            const vec3_t lineEnd, vec3_t projected)
{
    ...
    for (int32_t lane = 0; lane < 3; ++lane) {
        ...
        const x87f projection = x87f_add(
            x87f_add(
                x87f_mul(x87f_load_f32(pointDelta[0]),
                         x87f_load_f32(direction[0])),
                x87f_mul(x87f_load_f32(pointDelta[1]),
                         x87f_load_f32(direction[1]))),
            x87f_mul(x87f_load_f32(pointDelta[2]),
                     x87f_load_f32(direction[2])));
        projected[lane] = x87f_store_f32(x87f_add(
            x87f_load_f32(lineStart[lane]),
            x87f_mul(projection, x87f_load_f32(direction[lane]))));
#else
        const long double projection =
            ((long double)pointDelta[0] * direction[0] +
             (long double)pointDelta[1] * direction[1]) +
            (long double)pointDelta[2] * direction[2];
        projected[lane] = (float)(
            (long double)lineStart[lane] +
            projection * direction[lane]);
#endif
    }
}
