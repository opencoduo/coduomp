                               ...
                               const bg_anim_script_t *script)
{
    ...
    for (index = 0; index < script->conditionCount; ++index) {
        ...
        if (bgAnimConditionTypes[type].mode == ANIM_CONDMODE_BITMASK) {
            if (((uint32_t)condition->value[0] &
                 clientInfo->conditionWords[type][0]) != 0 ||
                ((uint32_t)condition->value[1] &
                 clientInfo->conditionWords[type][1]) != 0) {
                continue;
            }
            return qfalse;
        }
        ...
        if (bgAnimConditionTypes[type].mode == ANIM_CONDMODE_EQUAL &&
            clientInfo->conditionWords[type][0] !=
                                    ...
                                    const bg_anim_script_list_t *scriptList)
{
    ...
    if ((uint32_t)clientNum >= (uint32_t)MAX_CLIENTS) {
        Com_Error(ERR_DROP,
                  "\x15" "BG_FirstValidItem: invalid client number %i",
                  clientNum);
        return NULL;
    }
                             ...
                             int32_t value, qboolean checkConversion)
{
    ...
    if ((uint32_t)clientNum >= (uint32_t)MAX_CLIENTS) {
        Com_Error(ERR_DROP,
                  "\x15" "BG_UpdateConditionValue: invalid client number %i",
                  clientNum);
        return;
    }
    ...
    if (checkConversion &&
        ...
        clientInfo->conditionWords[conditionType][0] = 0;
        clientInfo->conditionWords[conditionType][1] = 0;
        clientInfo->conditionWords[conditionType][
            coduo_int32_sar((uint32_t)value, 5)] |=
            UINT32_C(1) << ((uint32_t)value & UINT32_C(31));
    } else {
                             ...
                             qboolean convertBitset)
{
    ...
    for (bit = 0; bit < BG_ANIM_CONDITION_BIT_COUNT; ++bit) {
        if ((clientInfo->conditionWords[conditionType][bit >> 5] &
             (UINT32_C(1) << ((uint32_t)bit & UINT32_C(31)))) != 0) {
            return bit;
        }
    }
    return 0;
