...
void PM_CrashLand(void)
{
...
#if EMULATE_X87
    {
        ...
        const x87f halfNegativeGravity = x87f_mul(x87f_load_f32(0.5f), negativeGravity);
        const x87f previousVelocityRegister = x87f_load_f32(previousVerticalVelocity);
        const x87f fallDistance = x87f_sub(x87f_load_f32(pml.previousOrigin[2]), x87f_load_f32(ps->psOrigin[2]));
        const x87f discriminant = x87f_sub(x87f_mul(previousVelocityRegister, previousVelocityRegister),
                                           x87f_mul(x87f_mul(fallDistance, halfNegativeGravity), x87f_load_f32(4.0f)));
        ...
        const x87f impactTime = x87f_div(x87f_sub(x87f_neg(previousVelocityRegister), x87f_sqrt(discriminant)),
                                         x87f_add(halfNegativeGravity, halfNegativeGravity));
        landingVelocity = x87f_neg(x87f_add(previousVelocityRegister, x87f_mul(impactTime, negativeGravity)));
        fallHeight =
            x87f_store_f32(x87f_div(x87f_mul(landingVelocity, landingVelocity), x87f_add(x87f_load_f32(gravity), x87f_load_f32(gravity))));
    }
#else
    const long double gravityWide = (long double)ps->gravity;
    const long double negativeGravity = -gravityWide;
    const long double halfNegativeGravity = 0.5L * negativeGravity;
    const long double previousVelocityRegister = (long double)previousVerticalVelocity;
    const long double fallDistance = (long double)pml.previousOrigin[2] - (long double)ps->psOrigin[2];
    const long double discriminant = previousVelocityRegister * previousVelocityRegister - (fallDistance * halfNegativeGravity) * 4.0L;
    ...
    const long double impactTime =
        (-previousVelocityRegister - coduo_x87_sqrtl(discriminant)) / (halfNegativeGravity + halfNegativeGravity);
    landingVelocity = -(previousVelocityRegister + impactTime * negativeGravity);
    /* 0x30009daf reloads the rounded FST copy of gravity for this denominator. */
    fallHeight = (float)((landingVelocity * landingVelocity) / (2.0L * (long double)gravity));
#endif

    if (move->debugMove != 0) {
#if EMULATE_X87
        Com_Printf("landing vel: %.1f fall height: %.1f\n", x87f_store_f64(landingVelocity), (double)fallHeight);
#else
    ...
    if (!(maxHeight <= minHeightForRange)) {
        ...
        if (!(minHeightForSign < 0.0f)) {
            ...
            if (!(fallHeight <= minHeightForFall) && ((uint8_t)pml.groundTrace.surfaceFlags & SURF_NODAMAGE) == 0) {
...
#if !EMULATE_X87
                        long double numerator = (long double)fallHeight - minHeightForNumerator;
#endif
                        float maxHeightForDenominator = bg_fallDamageMaxHeight.value;
                        float minHeightForDenominator = bg_fallDamageMinHeight.value;
#if !EMULATE_X87
                        long double denominator = (long double)maxHeightForDenominator - minHeightForDenominator;
#endif
#if EMULATE_X87
                        fallDamage = (int32_t)(uint32_t)x87f_store_i64_trunc(
                            x87f_mul(x87f_div(x87f_sub(x87f_load_f32(fallHeight), x87f_load_f32(minHeightForNumerator)),
                                              x87f_sub(x87f_load_f32(maxHeightForDenominator), x87f_load_f32(minHeightForDenominator))),
                                     x87f_load_f32(PM_CRASH_DAMAGE_SCALE)));
#else
                        fallDamage = coduo_fp_to_i32_extended(numerator / denominator * PM_CRASH_DAMAGE_SCALE);
#endif
                        if (fallDamage < 0) {
                            fallDamage = 0;
...
#if EMULATE_X87
        fallDamage =
            (int32_t)(uint32_t)x87f_store_i64_trunc(x87f_mul(x87f_load_i32(fallDamage), x87f_load_f32(PM_CRASH_WATERLEVEL_TWO_SCALE)));
#else
        fallDamage = coduo_fp_to_i32_extended((long double)fallDamage * (long double)PM_CRASH_WATERLEVEL_TWO_SCALE);
#endif
    }
    ...
    if (!(fallHeight <= PM_CRASH_EVENT_HEIGHT_BASE)) {
        ...
        landingEventParm = (int32_t)(uint32_t)x87f_store_i64_trunc(
            x87f_mul(x87f_add(x87f_mul(x87f_sub(x87f_load_f32(fallHeight), x87f_load_f32(PM_CRASH_EVENT_HEIGHT_BASE)),
        ...
        landingEventParm = coduo_fp_to_i32_extended(
            (((long double)fallHeight - (long double)PM_CRASH_EVENT_HEIGHT_BASE) * (long double)PM_CRASH_EVENT_HEIGHT_SCALE +
    ...
    if (fallDamage != 0) {
        ...
        if (landingAnimPs->legsTimer == 0) {
            ...
            BG_AnimScriptEvent(landingAnimPs, ANIM_EVENT_LAND, qfalse, qtrue);
            move = pm;
        }
        ...
        if (fallDamage < 100 && ((uint8_t)pml.groundTrace.surfaceFlags & SURF_SLICK) == 0) {
            int32_t stunTime = fallDamage * PM_CRASH_STUN_DAMAGE_MSEC + PM_CRASH_STUN_BASE_MSEC;
            float speedScale;
...
#if EMULATE_X87
                speedScale = x87f_store_f32(
                    x87f_sub(x87f_load_f32(PM_CRASH_STUN_SPEED_HIGH),
                             x87f_mul(x87f_mul(x87f_sub(x87f_load_i32(stunTime), x87f_load_f32((float)PM_CRASH_STUN_BASE_MSEC)),
                                               x87f_load_f32(PM_CRASH_STUN_MSEC_TO_SEC)),
                                      x87f_load_f32(PM_CRASH_STUN_SPEED_SPAN))));
#else
                speedScale = (float)((long double)PM_CRASH_STUN_SPEED_HIGH -
                                     ((long double)stunTime - (long double)PM_CRASH_STUN_BASE_MSEC) *
                                         (long double)PM_CRASH_STUN_MSEC_TO_SEC * (long double)PM_CRASH_STUN_SPEED_SPAN);
#endif
            }
            ...
            velocityXPs->velocity[0] = (float)(liveSpeedScale * velocityXPs->velocity[0]);
            playerState_t *velocityYPs = move->ps;
            velocityYPs->velocity[1] = (float)((long double)speedScale * velocityYPs->velocity[1]);
            playerState_t *velocityZPs = move->ps;
            velocityZPs->velocity[2] = (float)((long double)speedScale * velocityZPs->velocity[2]);
        } else {
            playerState_t *velocityXPs = move->ps;
            velocityXPs->velocity[0] = (float)((long double)velocityXPs->velocity[0] * PM_CRASH_VELOCITY_DAMP);
            playerState_t *velocityYPs = move->ps;
            velocityYPs->velocity[1] = (float)((long double)velocityYPs->velocity[1] * PM_CRASH_VELOCITY_DAMP);
            playerState_t *velocityZPs = move->ps;
            velocityZPs->velocity[2] = (float)((long double)velocityZPs->velocity[2] * PM_CRASH_VELOCITY_DAMP);
        }
...
void PM_CrashLand(void)
{
...
#if EMULATE_X87
    discriminant =
        x87f_store_f32(x87f_sub(x87f_mul(x87f_load_f32(fallVelocity), x87f_load_f32(fallVelocity)),
                                x87f_mul(x87f_mul(x87f_load_f32(halfNegativeGravity), x87f_load_f32(4.0f)), x87f_load_f32(fallDistance))));
#else
    discriminant = (float)((long double)fallVelocity * (long double)fallVelocity -
                           ((long double)halfNegativeGravity * 4.0L) * (long double)fallDistance);
#endif
...
#if EMULATE_X87
    impactTime =
        x87f_store_f32(x87f_div(x87f_sub(x87f_neg(x87f_load_f32(fallVelocity)), x87f_load_f64(CoduoLibm_Sqrt((double)discriminant))),
                                x87f_add(x87f_load_f32(halfNegativeGravity), x87f_load_f32(halfNegativeGravity))));
    impactVelocity = x87f_store_f32(
        x87f_neg(x87f_add(x87f_mul(x87f_load_f32(impactTime), x87f_neg(x87f_load_f32(gravity))), x87f_load_f32(fallVelocity))));
    fallHeight = x87f_store_f32(x87f_div(x87f_mul(x87f_load_f32(impactVelocity), x87f_load_f32(impactVelocity)),
                                         x87f_add(x87f_load_f32(gravity), x87f_load_f32(gravity))));
#else
    impactTime = (float)((-(long double)fallVelocity - (long double)CoduoLibm_Sqrt((double)discriminant)) /
                         ((long double)halfNegativeGravity + (long double)halfNegativeGravity));
    impactVelocity = (float)(-((long double)impactTime * -(long double)gravity + (long double)fallVelocity));
    fallHeight = (float)(((long double)impactVelocity * (long double)impactVelocity) / ((long double)gravity + (long double)gravity));
#endif
    ...
    if (maximumDamageHeight <= minimumDamageHeight || minimumDamageHeight < 0.0f) {
        Com_Printf("bg_fallDamageMaxHeight and bg_fallDamageMinHeight "
                   "have bad values\n");
        damagePercent = 0;
    } else if (fallHeight <= minimumDamageHeight || (pml.groundTrace.surfaceFlags & SURF_NODAMAGE) != 0 || pm->ps->pmType >= PM_TYPE_DEAD) {
        damagePercent = 0;
...
#if EMULATE_X87
        damagePercent =
            x87f_store_i32_trunc(x87f_mul(x87f_div(x87f_sub(x87f_load_f32(fallHeight), x87f_load_f32(minimumDamageHeight)),
                                                   x87f_sub(x87f_load_f32(maximumDamageHeight), x87f_load_f32(minimumDamageHeight))),
                                          x87f_load_f32(100.0f)));
#else
        damagePercent = coduo_fp_to_i32_extended(((long double)fallHeight - (long double)minimumDamageHeight) /
                                                 ((long double)maximumDamageHeight - (long double)minimumDamageHeight) * 100.0L);
#endif
        if (damagePercent < 0) {
            damagePercent = 0;
...
#if EMULATE_X87
        damagePercent = x87f_store_i32_trunc(x87f_mul(x87f_load_i32(damagePercent), x87f_load_f32(0.5f)));
#else
        damagePercent = coduo_fp_to_i32_extended((long double)damagePercent * 0.5L);
#endif
    }
    ...
    if (fallHeight <= PM_CRASH_EVENT_HEIGHT_BASE) {
        ...
        stepEvent = x87f_store_i32_trunc(x87f_add(
            x87f_mul(x87f_div(x87f_sub(x87f_load_f32(fallHeight), x87f_load_f32(PM_CRASH_EVENT_HEIGHT_BASE)), x87f_load_f32(26.0f)),
                     x87f_load_f32(PM_CRASH_EVENT_PARM_SCALE)),
            x87f_load_f32(PM_CRASH_EVENT_PARM_SCALE)));
#else
        stepEvent = coduo_fp_to_i32_extended(((long double)fallHeight - (long double)PM_CRASH_EVENT_HEIGHT_BASE) / 26.0L *
                                                 (long double)PM_CRASH_EVENT_PARM_SCALE +
                                             (long double)PM_CRASH_EVENT_PARM_SCALE);
#endif
    }
    ...
    if (damagePercent == 0) {
        if (fallHeight > PM_CRASH_MINOR_HEIGHT) {
            if (fallHeight < PM_CRASH_MEDIUM_HEIGHT) {
                ...
                BG_AddPredictableEventToPlayerstate(PM_HardLandingForSurface(), stepEvent, pm->ps);
            }
        }
        return;
    }
    ...
    if (damagePercent < 100 && (pml.groundTrace.surfaceFlags & SURF_SLICK) == 0) {
        int32_t stunTime = damagePercent * PM_CRASH_STUN_DAMAGE_MSEC + PM_CRASH_STUN_BASE_MSEC;
        float speedScale;
        ...
        if (stunTime < PM_CRASH_STUN_BASE_MSEC + 1) {
            ...
            speedScale = x87f_store_f32(x87f_sub(
                ...
                x87f_mul(x87f_div(x87f_sub(x87f_load_i32(stunTime), x87f_load_f32((float)PM_CRASH_STUN_BASE_MSEC)), x87f_load_f32(1000.0f)),
                         x87f_load_f32(PM_CRASH_STUN_SPEED_SPAN))));
#else
            speedScale = (float)((long double)PM_CRASH_STUN_SPEED_HIGH - ((long double)stunTime - (long double)PM_CRASH_STUN_BASE_MSEC) /
                                                                             1000.0L * (long double)PM_CRASH_STUN_SPEED_SPAN);
#endif
        } else {
            speedScale = PM_CRASH_STUN_SPEED_LOW;
        }
    ...
    BG_AddPredictableEventToPlayerstate(PM_DamageLandingForSurface(), damagePercent, pm->ps);
}
#endif
