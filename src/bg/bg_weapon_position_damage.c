...
static void bg_compat_add_view_kick_angles(pm_weapon_angle_state_t *state, vec3_t angles, float fraction)
{
#if EMULATE_X87
    angles[0] = x87f_store_f32(x87f_add(
        x87f_mul(x87f_mul(x87f_load_f32(fraction), x87f_load_f32(state->viewKickPitch)), x87f_load_f32(WEAPON_VIEWKICK_PITCH_SCALE)),
        x87f_load_f32(angles[0])));
    angles[1] = x87f_store_f32(x87f_sub(x87f_load_f32(angles[1]), x87f_mul(x87f_load_f32(fraction), x87f_load_f32(state->viewKickYaw))));
    angles[2] = x87f_store_f32(
        x87f_add(x87f_mul(x87f_mul(x87f_load_f32(fraction), x87f_load_f32(state->viewKickYaw)), x87f_load_f32(WEAPON_VIEWKICK_ROLL_SCALE)),
                 x87f_load_f32(angles[2])));
#else
    angles[0] += fraction * state->viewKickPitch * WEAPON_VIEWKICK_PITCH_SCALE;
    angles[1] -= fraction * state->viewKickYaw;
    angles[2] += fraction * state->viewKickYaw * WEAPON_VIEWKICK_ROLL_SCALE;
#endif
}
#endif
...
void BG_CalculateWeaponPosition_DamageKick(pm_weapon_angle_state_t *state, vec3_t angles)
{
    ...
    long double scale = ((long double)ads + (long double)1.0f) * (long double)0.5f;
    ...
    if (ps->adsFraction != 0.0f) {
        ...
        if (weaponInfo->adsOverlayReticle != 0) {                 /* [wi+0x284] */
            scale *= (long double)1.0f - (long double)0.75f * (long double)ps->adsFraction;
        }
    }

    /* 0x3001554c..0x30015551: kickAge = time - viewKickStartTime (signed ms), formed in
     * integer registers then FILD'd to float. */
    int32_t kickAgeMs = coduo_int32_from_bits((uint32_t)state->time - (uint32_t)viewKickStartTime);
    ...
    if (kickAge < attackDuration) {
        ...
        envelope = ((long double)2.0f - (long double)fabsf(storedR)) * r;
    } else {
        ...
        const long double remaining = (long double)1.0f - (kickAge - attackDuration) / (long double)attackWindow;
        ...
        envelope = (long double)1.0f - ((long double)2.0f - (long double)fabsf(storedQ)) * q;
    }
    ...
    angles[0] = (float)((long double)angles[0] + (long double)0.5f * kick * (long double)state->viewKickPitch);
    angles[1] = (float)((long double)angles[1] - kick * (long double)state->viewKickYaw);
    angles[2] = (float)((long double)angles[2] + (long double)0.5f * kick * (long double)state->viewKickYaw);
#else
    playerState_t *ps;
    ...
    scale = x87f_store_f32(
        x87f_add(x87f_mul(x87f_load_f32(adsFraction), x87f_load_f32(WEAPON_VIEWKICK_ADS_BASE)), x87f_load_f32(WEAPON_VIEWKICK_ADS_BASE)));
#else
    scale = adsFraction * WEAPON_VIEWKICK_ADS_BASE + WEAPON_VIEWKICK_ADS_BASE;
#endif
    ...
    if (adsFraction != 0.0f && weapon->adsOverlayReticle != 0) {
        ...
        scale = x87f_store_f32(
            x87f_mul(x87f_load_f32(scale),
                     x87f_sub(x87f_load_f32(1.0f), x87f_mul(x87f_load_f32(adsFraction), x87f_load_f32(WEAPON_VIEWKICK_ADS_REDUCTION)))));
#else
    ...
    if (elapsed < fadeInTime) {
#if EMULATE_X87
        const float leanArg = x87f_store_f32(x87f_div(x87f_load_f32(elapsed), x87f_load_f32(fadeInTime)));
        bg_compat_add_view_kick_angles(state, angles,
                                       x87f_store_f32(x87f_mul(bg_compat_get_lean_fraction_x87(leanArg), x87f_load_f32(scale))));
#else
        bg_compat_add_view_kick_angles(state, angles, GetLeanFraction(elapsed / fadeInTime) * scale);
#endif
    } else {
        ...
        fadeOutFraction = x87f_store_f32(x87f_sub(
            x87f_load_f32(1.0f), x87f_div(x87f_sub(x87f_load_f32(elapsed), x87f_load_f32(fadeInTime)), x87f_load_f32(fadeOutTime))));
#else
        fadeOutFraction = 1.0f - (elapsed - fadeInTime) / fadeOutTime;
#endif
        if (fadeOutFraction > 0.0f) {
            ...
            fadeOutLean = x87f_store_f32(x87f_sub(x87f_load_f32(1.0f), bg_compat_get_lean_fraction_x87(leanArg)));
#else
            fadeOutLean = 1.0f - GetLeanFraction(1.0f - fadeOutFraction);
#endif
            bg_compat_add_view_kick_angles(state, angles, fadeOutLean * scale);
        }
    }
#endif
}
