...
void BG_AnimUpdatePlayerStateConditions(pmove_t *pmove)
{
    ...
    if ((uint32_t)clientNum >= (uint32_t)MAX_CLIENTS) {
        Com_Error(ERR_DROP,
                  "\x15" "BG_AnimUpdatePlayerStateConditions: "
                  "invalid client number %i",
                  clientNum);
        return;
    }
    BG_UpdateConditionValue(clientNum, ANIM_COND_WEAPON,
                            player->currentWeapon, qtrue);
    BG_UpdateConditionValue(
        clientNum, ANIM_COND_WEAPONCLASS,
        BG_GetInfoForWeapon(player->currentWeapon)->weaponClass, qtrue);
    BG_UpdateConditionValue(
        clientNum, ANIM_COND_WEAPON_POSITION,
        (player->entityStateFlags & EF_ADS) != 0
    ...
    if ((player->entityStateFlags & EF_IN_VEHICLE) != 0) {
        ...
        if (vehiclePosition == BG_VEHICLE_POSITION_FIRST) {
            BG_UpdateConditionValue(clientNum, ANIM_COND_MOUNTED,
                                    ANIM_MOUNT_VEHICLE_DRIVER, qtrue);
        } else if (vehiclePosition == BG_VEHICLE_POSITION_SECOND) {
            BG_UpdateConditionValue(clientNum, ANIM_COND_MOUNTED,
                                    ANIM_MOUNT_VEHICLE_GUNNER, qtrue);
        } else if (vehiclePosition >= BG_VEHICLE_POSITION_FIRST_PASSENGER &&
                   vehiclePosition <= BG_VEHICLE_POSITION_LAST_PASSENGER) {
            BG_UpdateConditionValue(clientNum, ANIM_COND_MOUNTED,
                                    vehiclePosition + 1, qtrue);
        }

        vehicleType = player->vehicleType;
        if (vehicleType == VEHICLE_TYPE_TANK) {
            BG_UpdateConditionValue(clientNum, ANIM_COND_VEHICLE,
                                    ANIM_VEHICLE_TANK, qtrue);
        } else if (vehicleType == VEHICLE_TYPE_4_WHEEL) {
            BG_UpdateConditionValue(clientNum, ANIM_COND_VEHICLE,
                                    ANIM_VEHICLE_JEEP, qtrue);
        } else if (vehicleType == VEHICLE_TYPE_ARTILLERY) {
            BG_UpdateConditionValue(clientNum, ANIM_COND_VEHICLE,
                                    ANIM_VEHICLE_FLAK88, qtrue);
        } else {
            bg_compat_player_state_vehicle_type_error(vehicleType);
        }

        BG_UpdateConditionValue(clientNum, ANIM_COND_VEHICLE_MOTION,
                                player->vehicleMotion, qtrue);
    } else {
        BG_UpdateConditionValue(
            clientNum, ANIM_COND_MOUNTED,
            (player->entityStateFlags & EF_FORCED_STANCE_MASK) != 0
                ? ANIM_MOUNT_MG42
                : ANIM_MOUNT_UNUSED,
            qtrue);
        BG_UpdateConditionValue(clientNum, ANIM_COND_VEHICLE,
                                ANIM_VEHICLE_UNUSED, qtrue);
        BG_UpdateConditionValue(clientNum, ANIM_COND_VEHICLE_MOTION,
                                ANIM_VEHICLE_MOTION_UNUSED, qtrue);
    }

    BG_UpdateConditionValue(clientNum, ANIM_COND_UNDERHAND,
                            player->viewAngles[0] > 0.0f, qtrue);

    inputCondition =
        (pmove->command.buttons & BG_PMOVE_INPUT_CONDITION_BIT) != 0;
    if ((player->playerStateFlags &
         BG_PLAYER_STATE_SUPPRESS_INPUT_CONDITION) != 0) {
        inputCondition = qfalse;
    }
    BG_UpdateConditionValue(clientNum, ANIM_COND_FIRING, inputCondition,
                            qtrue);
}
                             ...
                             clientInfo_t *clientInfo)
{
    ...
    const bg_static_animation_t *animation =
        &bgs.animationTable.entries[animationIndex];
    ...
    if ((uint32_t)clientNum >= (uint32_t)MAX_CLIENTS) {
        Com_Error(ERR_DROP,
                  "\x15" "BG_AnimPlayerConditions: invalid client number %i",
                  clientNum);
        return;
    }
    BG_UpdateConditionValue(clientNum, ANIM_COND_WEAPON, weapon, qtrue);
    BG_UpdateConditionValue(clientNum, ANIM_COND_WEAPONCLASS,
                            BG_GetInfoForWeapon(weapon)->weaponClass, qtrue);
    BG_UpdateConditionValue(
        clientNum, ANIM_COND_WEAPON_POSITION,
        (entityFlags & EF_ADS) != 0 ? ANIM_WEAPON_POSITION_ADS
                                    : ANIM_WEAPON_POSITION_HIP,
        qtrue);
