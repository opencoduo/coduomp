    ...
    if (sharedCapIndex < 0) {
        if (weaponInfo->clipRequired != 0) {
            return coduo_int32_from_bits(
                (uint32_t)bg_ammoClipSizes[clipIndex] -
                (uint32_t)ps->clips[clipIndex]);
        }
        return coduo_int32_from_bits(
            (uint32_t)bg_ammoTypeMax[ammoIndex] -
            (uint32_t)ps->ammo[ammoIndex]);
    }

    remaining = bg_sharedAmmoCapSizes[sharedCapIndex];
    for (candidate = 1; candidate <= bg_numWeapons;
        ...
        if (candidateInfo->clipRequired != 0) {
            ...
            if (seenClip[index] == 0) {
                remaining = coduo_int32_from_bits(
                    (uint32_t)remaining - (uint32_t)ps->clips[index]);
                seenClip[index] = 1;
            }
        } else {
            ...
            if (seenAmmo[index] == 0) {
                remaining = coduo_int32_from_bits(
                    (uint32_t)remaining - (uint32_t)ps->ammo[index]);
                seenAmmo[index] = 1;
            }
        }
    }
...
int32_t BG_GetTotalAmmoReserve(const playerState_t *ps, int32_t weapon)
{
    ...
    if (sharedCapIndex < 0) {
        return weaponInfo->clipRequired != 0
                   ? ps->clips[weaponInfo->clipIndex]
    ...
    for (candidate = 1; candidate <= bg_numWeapons;
        ...
        if (candidateInfo->clipRequired != 0) {
            int32_t index = candidateInfo->clipIndex;
            if (seenClip[index] == 0) {
                total = coduo_int32_from_bits(
                    (uint32_t)total + (uint32_t)ps->clips[index]);
                seenClip[index] = 1;
            }
        } else {
            int32_t index = candidateInfo->ammoIndex;
            if (seenAmmo[index] == 0) {
                total = coduo_int32_from_bits(
                    (uint32_t)total + (uint32_t)ps->ammo[index]);
                seenAmmo[index] = 1;
            }
        }
    }
                                ...
                                int32_t preferredSlot)
{
    ...
    if (weaponInfo->stackable == 0 ||
        (uint32_t)slot - 1u >= WEAPSLOT_LAST_DROPPABLE) {
        return WEAPSLOT_NONE;
    }
    ...
    return occupant == 0 || bg_weaponInfos[occupant]->stackable != 0
               ? slot
               : WEAPSLOT_NONE;
}
        ...
        if (weaponInfo->stackable == 0) {
            ...
            while (candidate <= bg_numWeapons) {
                if (weaponInfo->stackable != 0) {
                    ...
                    (void)ignoredSlot;
                    if ((ps->weaponBits[coduo_int32_sar(
                             (uint32_t)candidate, 5)] &
                         (1u << ((uint32_t)candidate & 31u))) != 0 &&
...
qboolean BG_CanItemBeGrabbed(const entityState_t *itemState,
                             const playerState_t *ps, int32_t traceMode)
{
    ...
    if (itemIndex < BG_FIRST_PICKUP_ITEM || itemIndex >= BG_PICKUP_ITEM_COUNT) {
        BG_WEAPON_ERROR("\x15" "BG_CanItemBeGrabbed: index out of range");
    }
    ...
    switch (item->type) {
    case IT_WEAPON:
        if ((ps->weaponBits[coduo_int32_sar((uint32_t)weapon, 5)] &
             (1u << ((uint32_t)weapon & 31u))) != 0) {
    ...
    case IT_HEALTH:
        return ps->stats[STAT_HEALTH] < ps->stats[STAT_MAX_HEALTH]
                   ...
                   : qfalse;
    case IT_BAD:
        BG_WEAPON_ERROR("\x15" "BG_CanItemBeGrabbed: IT_BAD");
        return qfalse;
    default:
        return qfalse;
    }
