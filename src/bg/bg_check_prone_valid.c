...
#define BG_PRONE_DEBUG_LINE(lineStart, lineEnd, lineColor) \
    bg_compat_prone_debug_line((lineStart), (lineEnd), (lineColor))

int BG_CheckProneValid(int clientNum, const vec3_t origin, float radius,
                       float height, float yaw, float *groundOffset,
                       float *pitchDown, float *pitchUp,
                       qboolean skipInitialTrace, qboolean allowFallback,
                       const vec3_t groundNormal, pm_trace_fn_t traceFunc,
                       ...
                       pm_entity_type_fn_t entityTypeFunc)
{
    ...
    if (bg_compat_prone_debug_enabled() != qfalse) {
        ...
        bg_compat_prone_debug_box(debugMins, debugMaxs,
                                  BG_PRONE_DEBUG_MEDIUM_CYAN);
    }

    /* NEG/SBB/AND/ADD at 0x30006ead..0x30006eb7 selects exactly these masks. */
    contentMask = useAltContentMask ? BG_PRONE_CONTENTS_ALT
                                    : BG_PRONE_CONTENTS_DEFAULT;
    ...
    if (skipInitialTrace == qfalse) {
        ...
        if ((((uint32_t)trace.contents & BG_PRONE_BLOCKING_CONTENTS) != 0u &&
             trace.startsolid != 0) ||
            trace.allsolid != 0) {
            return 0;
        }
    ...
    if (allowFallback != qfalse && groundNormal != NULL &&
        groundNormal[2] < BG_PRONE_GROUND_NORMAL_MIN_Z) {
        return 0;
    }
    ...
    if (checkForwardClearance != qfalse) {
        ...
        start[2] = origin[2] + BG_PRONE_CLEARANCE_HEIGHT;
                         ...
                         (long double)forward[2] *
                             BG_PRONE_FORWARD_CLEARANCE);
        traceFunc(&trace, start, mins, maxs, end, clientNum,
                  (int32_t)contentMask);
    ...
    const float proneReach = (float)((long double)proneLength -
                                     BG_PRONE_TRACE_RADIUS);
#else
    const long double proneReach =
        (long double)proneLength - BG_PRONE_TRACE_RADIUS;
#endif
    end[0] = (float)((long double)start[0] +
                     (long double)proneReach * (long double)forward[0]);
    end[1] = (float)((long double)start[1] +
                     (long double)proneReach * (long double)forward[1]);
    end[2] = (float)((long double)start[2] +
                     (long double)proneReach * (long double)forward[2]);
    traceFunc(&trace, start, mins, maxs, end, clientNum,
              (int32_t)contentMask);

    if (bg_compat_prone_debug_enabled() != qfalse) {
        bg_compat_prone_debug_circle(start, BG_PRONE_TRACE_RADIUS, right,
                                     BG_PRONE_DEBUG_MEDIUM_CYAN);
    }
    if (bg_compat_prone_debug_enabled() != qfalse) {
        bg_compat_prone_debug_circle(start, BG_PRONE_TRACE_RADIUS, up,
                                     BG_PRONE_DEBUG_MEDIUM_CYAN);
    }
    ...
    if (trace.fraction < BG_PRONE_TRACE_FULL_FRACTION) {
        ...
        frontClearance = (float)(
            (long double)proneReach * (long double)trace.fraction +
            BG_PRONE_TRACE_RADIUS);
        if ((long double)frontClearance <
            (long double)radius + BG_PRONE_CLEARANCE_MARGIN) {
            BG_PRONE_DEBUG_LINE(start, trace.endpos, BG_PRONE_DEBUG_RED);
            return 0;
        }

        minimumForwardClearance =
            (long double)height * BG_PRONE_GROUND_NORMAL_MIN_Z +
            BG_PRONE_FORWARD_PROBE;
        if ((long double)frontClearance <
            (long double)minimumForwardClearance) {
            float traceDistance;
            ...
            end[2] = (float)((long double)end[2] +
                             BG_PRONE_RAISED_RETRY_HEIGHT);
            ...
            if (trace.fraction < BG_PRONE_TRACE_FULL_FRACTION) {
                ...
                if ((long double)frontClearance <
                    (long double)minimumForwardClearance) {
                    BG_PRONE_DEBUG_LINE(start, trace.endpos,
                                        BG_PRONE_DEBUG_RED);
                    return 0;
                }
                BG_PRONE_DEBUG_LINE(start, trace.endpos,
     ...
     * facing direction. */
    end[0] = (float)((long double)origin[0] +
                     (long double)forward[0] * BG_PRONE_FORWARD_PROBE);
    end[1] = (float)((long double)origin[1] +
                     (long double)forward[1] * BG_PRONE_FORWARD_PROBE);
#if defined(LINUX_BEHAVIOR)
    /* Linux 0x21fe6..0x22004 stores origin+forward*24 before adding height. */
    end[2] = (float)((long double)origin[2] +
                     (long double)forward[2] * BG_PRONE_FORWARD_PROBE);
...
#if defined(WINDOWS_BEHAVIOR)
    const float groundTraceDepth = (float)(
        ((long double)radius * BG_PRONE_GROUND_TRACE_SCALE +
         (long double)height) - BG_PRONE_TRACE_RADIUS);
#else
    const long double groundTraceDepth =
        ((long double)radius * BG_PRONE_GROUND_TRACE_SCALE +
         (long double)height) - BG_PRONE_TRACE_RADIUS;
#endif
    end[2] = (float)((long double)start[2] -
                     (long double)groundTraceDepth);
    traceFunc(&trace, start, mins, maxs, end, clientNum,
              (int32_t)contentMask);
    if (trace.fraction == BG_PRONE_TRACE_FULL_FRACTION) {
        BG_PRONE_DEBUG_LINE(start, trace.endpos, BG_PRONE_DEBUG_RED);
    ...
    if (blockedForward != qfalse) {
        float slopeDrop = (float)(
            (long double)groundTraceDepth * (long double)trace.fraction +
            BG_PRONE_TRACE_RADIUS);

        if ((long double)frontClearance - (long double)slopeDrop <
            (long double)slopeDrop * BG_PRONE_MAX_SLOPE_RATIO) {
            BG_PRONE_DEBUG_LINE(frontHit, middleHit, BG_PRONE_DEBUG_RED);
            goto fallback_failure;
        }

        BG_PRONE_DEBUG_LINE(frontHit, middleHit,
                            BG_PRONE_DEBUG_MEDIUM_CYAN);
#if defined(LINUX_BEHAVIOR)
        ...
        delta[0] = (float)((long double)delta[0] +
                           (long double)forward[0] * BG_PRONE_TRACE_RADIUS);
        delta[1] = (float)((long double)delta[1] +
                           (long double)forward[1] * BG_PRONE_TRACE_RADIUS);
        delta[2] = (float)((long double)delta[2] +
                           (long double)forward[2] * BG_PRONE_TRACE_RADIUS);
        delta[2] = (float)((long double)delta[2] + BG_PRONE_TRACE_RADIUS);
#else
        delta[0] = (float)(
            ((long double)frontHit[0] - middleHit[0]) +
            (long double)forward[0] * BG_PRONE_TRACE_RADIUS);
        delta[1] = (float)(
            ((long double)frontHit[1] - middleHit[1]) +
            (long double)forward[1] * BG_PRONE_TRACE_RADIUS);
        ...
        delta[2] = (float)((long double)deltaZ +
                           (long double)forward[2] * BG_PRONE_TRACE_RADIUS +
                           BG_PRONE_TRACE_RADIUS);
#endif
         ...
         * reloading X and Y for the midpoint averages. */
                         ...
                         ((long double)proneReach - BG_PRONE_FORWARD_PROBE) *
                             (long double)delta[0]);
        end[1] = (float)((long double)start[1] +
                         ((long double)proneReach - BG_PRONE_FORWARD_PROBE) *
                             (long double)delta[1]);
                         ...
                         ((long double)proneReach - BG_PRONE_FORWARD_PROBE) *
                             (long double)delta[2]);
        end[0] = (float)(((long double)proneReach * forward[0] +
                          (long double)origin[0] + end[0]) *
                         BG_PRONE_MIDPOINT_SCALE);
        end[1] = (float)(((long double)proneReach * forward[1] +
                          (long double)origin[1] + end[1]) *
                         BG_PRONE_MIDPOINT_SCALE);
#else
        /* Windows 0x300077cf stores only the Y connector term before the outer
         * sum; the X connector term remains in x87. */
        const float connectMid1 = (float)(
            ((long double)proneReach - BG_PRONE_FORWARD_PROBE) * delta[1] +
            start[1]);
        end[2] = (float)(
            start[2] + ((long double)proneReach - BG_PRONE_FORWARD_PROBE) *
                           delta[2]);
        end[0] = (float)(
            ((long double)proneReach * forward[0] + origin[0] +
             (((long double)proneReach - BG_PRONE_FORWARD_PROBE) * delta[0] +
              start[0])) * BG_PRONE_MIDPOINT_SCALE);
        end[1] = (float)(
            ((long double)proneReach * forward[1] + origin[1] + connectMid1) *
            BG_PRONE_MIDPOINT_SCALE);
#endif
        ...
        if (trace.fraction < BG_PRONE_TRACE_FULL_FRACTION) {
            ...
            start[2] = (float)((long double)trace.endpos[2] +
                               BG_PRONE_CONNECT_RETRY_HEIGHT);
            end[2] = (float)((long double)end[2] +
                             BG_PRONE_CONNECT_RETRY_HEIGHT);
            traceFunc(&trace, start, mins, maxs, end, clientNum,
                      (int32_t)contentMask);
     ...
     * Windows uses one multiply by 2.0f. */
        ...
        (((long double)frontHit[2] - middleHit[2]) +
         ((long double)frontHit[2] - middleHit[2]) + radius));
#else
    end[2] = (float)(
        (long double)frontHit[2] -
        (((long double)frontHit[2] - middleHit[2]) *
             BG_PRONE_FRONT_DROP_SCALE +
         radius));
#endif
    traceFunc(&trace, start, mins, maxs, end, clientNum,
              (int32_t)contentMask);
    ...
    if (trace.fraction < BG_PRONE_TRACE_FULL_FRACTION &&
        (((uint32_t)trace.contents & BG_PRONE_BLOCKING_CONTENTS) != 0u)) {
        goto fallback_failure;
    }
    if (trace.normal[2] < BG_PRONE_GROUND_NORMAL_MIN_Z) {
        return 0;
    ...
    start[2] = origin[2] + height;
                     ...
                     (long double)radius * BG_PRONE_REAR_TRACE_SCALE);
    traceFunc(&trace, start, mins, maxs, end, clientNum,
              (int32_t)contentMask);
    if (trace.fraction == BG_PRONE_TRACE_FULL_FRACTION) {
        BG_PRONE_DEBUG_LINE(start, trace.endpos, BG_PRONE_DEBUG_RED);
    ...
    if (fabsl(((long double)rearHit[2] - (long double)origin[2]) -
              BG_PRONE_TRACE_RADIUS) >
        (long double)BG_PRONE_MAX_GROUND_DELTA) {
    ...
    delta[2] = middleHit[2] - rearHit[2];
    {
        ...
        if (pitchDelta < BG_PRONE_MIN_PITCH_DELTA ||
            pitchDelta > BG_PRONE_MAX_PITCH_DELTA) {
            BG_PRONE_DEBUG_LINE(rearHit, middleHit,
                                BG_PRONE_DEBUG_MAGENTA);
            BG_PRONE_DEBUG_LINE(middleHit, frontHit,
                                BG_PRONE_DEBUG_MAGENTA);
            goto fallback_failure;
        }
    }
    ...
    if (bg_compat_prone_debug_enabled() != qfalse) {
        bg_compat_prone_debug_circle(rearHit, BG_PRONE_TRACE_RADIUS, right,
                                     BG_PRONE_DEBUG_MEDIUM_CYAN);
    }
    if (bg_compat_prone_debug_enabled() != qfalse) {
        bg_compat_prone_debug_circle(rearHit, BG_PRONE_TRACE_RADIUS, up,
                                     BG_PRONE_DEBUG_MEDIUM_CYAN);
    }
    if (bg_compat_prone_debug_enabled() != qfalse) {
        bg_compat_prone_debug_circle(middleHit, BG_PRONE_TRACE_RADIUS, right,
                                     BG_PRONE_DEBUG_MEDIUM_CYAN);
    }
    if (bg_compat_prone_debug_enabled() != qfalse) {
        bg_compat_prone_debug_circle(middleHit, BG_PRONE_TRACE_RADIUS, up,
                                     BG_PRONE_DEBUG_MEDIUM_CYAN);
    }
    if (bg_compat_prone_debug_enabled() != qfalse) {
        bg_compat_prone_debug_circle(frontHit, BG_PRONE_TRACE_RADIUS, right,
                                     BG_PRONE_DEBUG_MEDIUM_CYAN);
    }
    if (bg_compat_prone_debug_enabled() != qfalse) {
        bg_compat_prone_debug_circle(frontHit, BG_PRONE_TRACE_RADIUS, up,
                                     BG_PRONE_DEBUG_MEDIUM_CYAN);
    }
