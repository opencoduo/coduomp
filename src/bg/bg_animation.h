...
extern bg_indexed_string_t bgAnimConditionAliases[ANIM_COND_COUNT * BG_ANIM_CONDITION_VALUE_COUNT];
extern bg_condition_bits_t bgAnimConditionAliasBits[ANIM_COND_COUNT * BG_ANIM_CONDITION_VALUE_COUNT];
...
void BG_ParseConditionBits(char **parse, bg_indexed_string_t *values, int32_t conditionIndex, bg_condition_bits_t *bits);
qboolean BG_ParseConditions(char **parse, bg_anim_script_t *script);
void BG_ParseCommands(char **parse, bg_anim_script_t *script, bg_static_animation_t *animations);
void BG_AnimParseAnimScript(bg_static_animation_table_t *table, bg_runtime_animation_t *runtimeAnimations, int32_t *runtimeAnimationCount);
...
void BG_PlayerAnimation(const entityState_t *entity, clientInfo_t *clientInfo);
...
extern const char *const BG_ControllerTagNames[CLIENT_INFO_SPINE_CONTROL_COUNT];
void BG_Player_DoControllersInternal(clientInfo_t *clientInfo, const entityState_t *entity, vec3_t output[8]);
void BG_Player_DoControllers(void *dobjOwner, const entityState_t *entity, uint32_t *partBits, clientInfo_t *clientInfo);
void BG_PlayerAnimation_VerifyAnim(XAnimTree *animTree, bg_anim_slot_t *slot);
void BG_SetNewAnimation(clientInfo_t *clientInfo, bg_anim_slot_t *slot, uint32_t animationWord, qboolean forceDeathRestart);
void BG_RunLerpFrameRate(clientInfo_t *clientInfo, bg_anim_slot_t *slot, uint32_t animationWord, const entityState_t *entity);
