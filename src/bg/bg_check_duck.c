...
#define PM_RESET_TORSO_FOR_VIEWHEIGHT_CHANGE(ps_) do {                      \
    if ((ps_)->weaponState != WEAPON_STATE_DEPLOYING &&                        \
        (ps_)->weaponState != WEAPON_STATE_BREAKING_DOWN &&                       \
...
void PM_CheckDuck(void)
{
    ...
    if (ps->pmType >= PM_TYPE_DEAD) {
        ...
        ps = move->ps;
        move->trace = (ps->playerStateFlags & PMF_PRONE) != 0
                        ? move->trace2 : move->trace3;
        ps->entityStateFlags |= EF_STANCE_VALID;
        PM_ViewHeightAdjust();
        return;
    }
    ...
    if ((stanceEntityFlags & EF_RESTRICTED_MASK) != 0) {
        qboolean forceProne =
            (stanceEntityFlags & EF_FORCE_PRONE) != 0;
        qboolean forceCrouch =
            (stanceEntityFlags & EF_FORCE_CROUCH) != 0;
    ...
    if ((inputStanceFlags & PMF_ADS) != 0 &&
        BG_PM_WEAPON_INFO(ps->currentWeapon)->weaponClass == WEAPCLASS_LMG) {
        goto stance_selected;
    }

    if ((inputStanceFlags & PMF_LADDER) != 0 &&
        (move->command.wbuttons & PM_WBUTTON_STANCE_MASK) != 0) {
    ...
    if ((move->command.wbuttons & PM_WBUTTON_PRONE) != 0) {
        ...
        if ((ps->playerStateFlags & PMF_PRONE) == 0) {
            ...
            if (ps->groundEntityNum != ENTITYNUM_NONE && move->waterlevel == 0) {
                canProne = BG_CheckProne(
                    ps->psClientNum, ps->psOrigin, move->maxs[0], 30.0f,
                    ps->viewAngles[1], &ps->torsoHeight, &ps->torsoPitch,
                    &ps->waistPitch, qfalse, qtrue, NULL, move->trace3,
                    move->trace2, qfalse, 60.0f, move->entityType) != 0;
            ...
            if (!canProne) {
                ...
                if (ps->groundEntityNum != ENTITYNUM_NONE) {
                    ...
                    if ((move->command.wbuttons & PM_WBUTTON_STANCE_LATCH) == 0) {
                        ps = move->ps;
                        PM_APPEND_STANCE_EVENT(
                            ps, (ps->playerStateFlags & PMF_DUCKED) != 0
                                    ? PM_EVENT_PRONE_BLOCKED
    ...
    if ((move->command.wbuttons & PM_WBUTTON_CROUCH) != 0) {
        ...
        if ((ps->playerStateFlags & PMF_PRONE) != 0) {
            trace_t trace;
            move->maxs[2] = 50.0f;
            int32_t traceMask = coduo_int32_from_bits(
                (uint32_t)move->traceMask & ~CONTENTS_BODY);
            int32_t passEntityNum = ps->psClientNum;
            pm_trace_fn_t traceCallback = move->trace3;
            traceCallback(&trace, ps->psOrigin, move->mins, move->maxs,
                          ps->psOrigin, passEntityNum, traceMask);
...
stance_selected:
        ...
        if ((ps->playerStateFlags & PMF_PRONE) != 0) {
            if (ps->viewHeightTarget == ps->standViewHeight) {
                ...
                if (bg_compat_pmove_prone_debug_trace_enabled() != qfalse) {
                    (void)BG_CheckProne(
                        ps->psClientNum, ps->psOrigin, move->maxs[0], 30.0f,
                        ps->viewAngles[1], NULL, NULL, NULL, qfalse,
                        ps->groundEntityNum != ENTITYNUM_NONE, NULL, move->trace3,
                        move->trace2, qfalse, 60.0f, move->entityType);
            ...
            ps->viewHeightTarget = (ps->playerStateFlags & PMF_DUCKED) != 0
                                    ? ps->crouchViewHeight
                                    : ps->standViewHeight;
        }
    }
    ...
    if (ps->viewHeightTarget == ps->crouchViewHeight) {
        ...
        move->maxs[2] = (ps->playerStateFlags & (PMF_PRONE | PMF_DUCKED)) == 0
                          ? ps->playerMaxs[2] : 50.0f;
...

    {
...
#if EMULATE_X87
        raised[2] = x87f_store_f32(x87f_add(
            x87f_load_f32(ps->psOrigin[2]), x87f_load_f32(10.0f)));
#else
        ...
        traceCallback(&trace, ps->psOrigin, move->mins, move->maxs, raised,
                      passEntityNum, traceMask);
        ...
        traceCallback(&trace, firstEnd, move->mins, move->maxs, ps->psOrigin,
                      passEntityNum, traceMask);
...
#if EMULATE_X87
        lowered[2] = x87f_store_f32(x87f_sub(
            x87f_load_f32(loweredZPs->psOrigin[2]),
            x87f_load_f32(0.25f)));
#else
        ...
        passEntityNum = ps->psClientNum;
        traceCallback = move->trace2;
        traceCallback(&trace, ps->psOrigin, move->mins, move->maxs, lowered,
                      passEntityNum, traceMask);
        ...
        if (!trace.startsolid && trace.fraction < 1.0f) {
            ...
            ps = move->ps;
            targetPitch = (float)PitchForYawOnNormal(ps->proneDirection,
                                                     trace.normal);
            ps = move->ps;
        ...
        float pitchInput;
#if EMULATE_X87
        pitchInput = x87f_store_f32(x87f_sub(
            x87f_load_f32(ps->proneDirectionPitch),
            x87f_load_f32(ps->viewAngles[0])));
#else
        pitchInput = (float)((long double)ps->proneDirectionPitch -
                             (long double)ps->viewAngles[0]);
#endif
        ...
        if (pitchDelta < -45.0f) {
#if EMULATE_X87
            ps->proneTorsoPitch = x87f_store_f32(x87f_sub(
                x87f_load_f32(ps->viewAngles[0]), x87f_load_f32(45.0f)));
#else
            ps->proneTorsoPitch =
                (float)((long double)ps->viewAngles[0] - 45.0L);
#endif
        } else if (pitchDelta > 45.0f) {
#if EMULATE_X87
            ps->proneTorsoPitch = x87f_store_f32(x87f_add(
                x87f_load_f32(ps->viewAngles[0]), x87f_load_f32(45.0f)));
#else
            ps->proneTorsoPitch =
                (float)((long double)ps->viewAngles[0] + 45.0L);
