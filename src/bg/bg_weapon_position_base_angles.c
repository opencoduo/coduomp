...
void BG_CalculateWeaponPosition_BaseAngles(pm_weapon_angle_state_t *state, vec3_t angles)
{
    ...
    if (weapon->adsEnabled != 0) {
        ...
        angles[0] = (float)((long double)weapon->adsPitchOffset * (long double)ps->adsFraction + (long double)angles[0]);
#elif EMULATE_X87
        angles[0] = x87f_store_f32(
            x87f_add(x87f_mul(x87f_load_f32(ps->adsFraction), x87f_load_f32(weapon->adsPitchOffset)), x87f_load_f32(angles[0])));
#else
