...
/* NOT_FROM_ORIGINAL_SOURCE: temporary client reload assertion; server modules leave this unset. */
void (*coduomp_reload_event_observer)(const playerState_t *ps, int32_t event, int32_t eventParm);
                                         ...
                                         playerState_t *ps)
{
    ...
    if (coduomp_reload_event_observer != NULL) {
        coduomp_reload_event_observer(ps, (uint8_t)event, (uint8_t)eventParm);
    }
