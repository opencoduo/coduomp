...
static bg_static_animation_t *bg_compat_slot_animation(const bg_anim_slot_t *slot)
{
...
#else
    return (bg_static_animation_t *)(void *)((uint8_t *)bgAnimStaticTable + slot->animationOffset);
#endif
}

/* NOT_FROM_ORIGINAL_SOURCE: host-safe animation-offset mutator. */
static void bg_compat_set_slot_animation(bg_anim_slot_t *slot, bg_static_animation_t *animation)
{
    if (animation == NULL) {
...
#else
        slot->animationOffset = (uint32_t)((uint8_t *)animation - (uint8_t *)bgAnimStaticTable);
#endif
    }
}
...
void BG_PlayerAnimation_VerifyAnim(XAnimTree *animTree, bg_anim_slot_t *slot)
{
    const uint16_t animsIndex = Scr_GetAnimsIndex(bgAnimStaticTable->animTreeHandle);

    if (slot->animationWord != 0) {
        const uint32_t anim = bg_compat_make_xanim(animsIndex, (uint16_t)slot->animationWord) & ~ANIM_TOGGLEBIT;
...
static qboolean bg_compat_anim_slot_is_legs(const clientInfo_t *ci, const bg_anim_slot_t *slot)
{
...
/* NOT_FROM_ORIGINAL_SOURCE: source-level factoring of original BG_SetNewAnimation (0x1ce5f); no standalone original body. */
static uint32_t bg_compat_anim_slot_xanim(uint16_t animTreeIndex, uint32_t animationWord)
{
    return bg_compat_make_xanim(animTreeIndex, (uint16_t)animationWord) & ~ANIM_TOGGLEBIT;
}
...
static void bg_compat_copy_anim_slot_origin(bg_anim_slot_t *slot, const entityState_t *entity)
{
...
static float bg_compat_initial_loop_time(const clientInfo_t *ci, int duration)
{
    ...
    const x87f phase = x87f_add(x87f_div(x87f_load_i32(bg_compat_animation_time() % duration), x87f_load_i32(duration)),
                                x87f_mul(x87f_load_i32(ci->clientNum), x87f_load_f32(0.36f)));
    const int32_t phaseInteger = (int32_t)(uint32_t)x87f_store_i64_trunc(phase);
    return x87f_store_f32(x87f_sub(phase, x87f_load_i32(phaseInteger)));
#else
    const float phase = x87f_store_f32(x87f_add(x87f_mul(x87f_load_i32(ci->clientNum), x87f_load_f32(0.36f)),
                                                x87f_div(x87f_load_i32(bg_compat_animation_time() % duration), x87f_load_i32(duration))));
    /* 0x1d19c..0x1d1a8: phase - (float)(int)phase kept 80-bit (fistp-trunc,
     * fild back, subtract), one store. */
    return x87f_store_f32(x87f_sub(x87f_load_f32(phase), x87f_load_i32(x87f_store_i32_trunc(x87f_load_f32(phase)))));
#endif
#elif defined(WINDOWS_BEHAVIOR)
    const double phase = (double)(bg_compat_animation_time() % duration) / duration + (double)ci->clientNum * 0.36f;
    const int32_t phaseInteger = coduo_fp_to_i32_extended((long double)phase);
    return (float)(phase - phaseInteger);
#else
    const float phase = (float)((long double)ci->clientNum * (long double)0.36f +
                                (long double)(bg_compat_animation_time() % duration) / (long double)duration);
    const int32_t phaseInteger = coduo_fp_to_i32_extended((long double)phase);
...
static qboolean bg_compat_animation_stance_changed(const clientInfo_t *ci, uint32_t oldAnimation, uint32_t newAnimation)
{
    qboolean oldValue;
    qboolean newValue;
...
void BG_SetNewAnimation(clientInfo_t *ci, bg_anim_slot_t *slot, uint32_t animationWord, qboolean forceDeathRestart)
{
    ...
    if ((uint32_t)bgAnimStaticTable->entryCount <= animationIndex) {
        bg_compat_update_anim_slot_range_error(bgAnimStaticTable->entryCount, animationIndex);
    }
    ...
    if (animationIndex == 0) {
        ...
        if (isLegsSlot && bg_compat_animation_stance_changed(ci, oldAnimationWord, animationIndex)) {
            ci->animTransitionTime = bg_compat_animation_time() + BG_ANIM_TRANSITION_PAD_MS;
        }
    }
    ...
    if (oldAnimation == NULL && isLegsSlot) {
        ...
        if (currentAnimation == NULL || slot->blendTime < 1) {
            if (currentAnimation == NULL || currentAnimation->moveSpeed == 0) {
                minimumBlendTime =
                    (oldAnimation == NULL || oldAnimation->moveSpeed == 0) ? BG_ANIM_BLEND_IDLE_MS : BG_ANIM_BLEND_MOVING_OUT_MS;
            } else {
                minimumBlendTime = BG_ANIM_BLEND_MOVING_IN_MS;
            }
        }

        const int transitionBlendTime = ci->animTransitionTime - bg_compat_animation_time();
    ...
    if (currentAnimation != NULL && currentAnimation->moveSpeed != 0) {
        const uint32_t newAnim = bg_compat_anim_slot_xanim(animTreeIndex, animationWord);

        if (trap_XAnimIsLooped(newAnim)) {
            const uint32_t oldAnim = bg_compat_anim_slot_xanim(animTreeIndex, oldAnimationWord);

            if (oldAnimation == NULL || oldAnimation->moveSpeed == 0 || !trap_XAnimIsLooped(oldAnim)) {
                int duration = BG_ANIM_DEFAULT_CYCLE_MS;

                if (bg_compat_animation_is_primitive(oldAnim)) {
                    duration =
                        bg_compat_animation_get_length(anims, (uint16_t)(oldAnimationWord & ~ANIM_TOGGLEBIT)) + BG_ANIM_LENGTH_PAD_MS;
                }
    ...
    if (oldAnimation != NULL) {
        ...
        bg_compat_animation_clear_goal_weight(animTree, oldAnim, (float)slot->blendTime * BG_ANIM_MILLISECONDS_TO_SECONDS);
    }

    if (animationIndex == 0) {
        if (!isLegsSlot) {
            float blendSeconds = (float)slot->blendTime * BG_ANIM_MILLISECONDS_TO_SECONDS;

            trap_XAnimSetCompleteGoalWeight(
                animTree, bg_compat_make_xanim(bgs.resolvedTorsoAnimHandle.treeIndex, bgs.resolvedTorsoAnimHandle.animIndex), 0.0f,
                blendSeconds, 1.0f, 0, 0);
            blendSeconds = (float)slot->blendTime * BG_ANIM_MILLISECONDS_TO_SECONDS;
            trap_XAnimSetCompleteGoalWeight(
                animTree, bg_compat_make_xanim(bgs.resolvedLegsAnimHandle.treeIndex, bgs.resolvedLegsAnimHandle.animIndex), 1.0f,
                blendSeconds, 1.0f, 0, 0);
        }
        ...
        if (bg_compat_animation_debug_value() == 1) {
            Com_Printf("Anim-%s: %i, %s, (blend time) %i\n", isLegsSlot ? "legs " : "torso", animationIndex,
                       bgAnimStaticTable->entries[animationIndex].name, slot->blendTime);
        }
        return;
    }
    ...
    if ((currentAnimation->flags & BG_ANIM_ENTRY_DEATH) == 0) {
        const qboolean setStartTime = currentAnimation->moveSpeed != 0 && bg_compat_animation_get_weight(animTree, newAnim) == 0.0f;
        const float blendSeconds = (float)slot->blendTime * BG_ANIM_MILLISECONDS_TO_SECONDS;

        trap_XAnimSetCompleteGoalWeight(animTree, newAnim, 1.0f, blendSeconds, 1.0f, (uint16_t)currentAnimation->usedByScript, !isLegsSlot);
        if (setStartTime) {
        ...
        if (!forceDeathRestart) {
            (void)trap_XAnimSetCompleteGoalWeightKnobAll(animTree, newAnim,
                                                         bg_compat_make_xanim(bgs.rootAnimHandle.treeIndex, bgs.rootAnimHandle.animIndex),
                                                         1.0f, 0.0f, 1.0f, 0, 0);
            bg_compat_animation_set_time(animTree, newAnim, 1.0f);
        } else {
            const float blendSeconds = (float)slot->blendTime * BG_ANIM_MILLISECONDS_TO_SECONDS;
            trap_XAnimSetCompleteGoalWeight(animTree, newAnim, 1.0f, blendSeconds, 1.0f, 0, 0);
        }
    }

    if (!isLegsSlot) {
        float blendSeconds = (float)slot->blendTime * BG_ANIM_MILLISECONDS_TO_SECONDS;
        trap_XAnimSetCompleteGoalWeight(animTree,
                                        bg_compat_make_xanim(bgs.resolvedTorsoAnimHandle.treeIndex, bgs.resolvedTorsoAnimHandle.animIndex),
                                        1.0f, blendSeconds, 1.0f, (uint16_t)currentAnimation->usedByScript, 0);
        blendSeconds = (float)slot->blendTime * BG_ANIM_MILLISECONDS_TO_SECONDS;
        trap_XAnimSetCompleteGoalWeight(animTree,
                                        bg_compat_make_xanim(bgs.resolvedLegsAnimHandle.treeIndex, bgs.resolvedLegsAnimHandle.animIndex),
                                        0.01f, blendSeconds, 1.0f, (uint16_t)currentAnimation->usedByScript, 0);
    }

    if (bg_compat_animation_debug_value() == 1) {
        Com_Printf("Anim-%s: %i, %s, (blend time) %i\n", isLegsSlot ? "legs " : "torso", animationIndex, currentAnimation->name,
                   slot->blendTime);
    }
}
...
void BG_RunLerpFrameRate(clientInfo_t *ci, bg_anim_slot_t *slot, uint32_t animationWord, const entityState_t *entity)
{
    bg_static_animation_t *slotAnimation = bg_compat_slot_animation(slot);
    const qboolean usesVerticalDelta = slotAnimation != NULL && (slotAnimation->flags & BG_ANIM_ENTRY_VERTICAL_MOTION) != 0;
    XAnimTree *animTree = ci->animTree;
    const uint16_t animTreeIndex = Scr_GetAnimsIndex(bgAnimStaticTable->animTreeHandle);

    if (animationWord != slot->animationWord || (bg_compat_slot_animation(slot) == NULL && (animationWord & ~ANIM_TOGGLEBIT) != 0)) {
        BG_SetNewAnimation(ci, slot, animationWord, (entity->eFlags >> 10) & 1u);
    }
    ...
    if (animation->moveSpeed == 0 || slot->lastUpdateTime == 0) {
        ...
        if (usesVerticalDelta) {
            moveDistance = fabsf(slot->lastOrigin[2] - entity->pos.trBase[2]);
        } else {
            moveDistance = VectorDistance(slot->lastOrigin, entity->pos.trBase);
        }
...
#if EMULATE_X87
        const float moveSpeed = x87f_store_f32(
            x87f_div(x87f_load_f32(moveDistance), x87f_mul(x87f_load_i32(bg_compat_animation_sample_time() - slot->lastUpdateTime),
                                                           x87f_load_f32(BG_ANIM_MILLISECONDS_TO_SECONDS))));
        /* 0x1d6e4..0x1d6ec: moveSpeed / (fild)animation->moveSpeed. */
        slot->animRate = x87f_store_f32(x87f_div(x87f_load_f32(moveSpeed), x87f_load_i32(animation->moveSpeed)));
#elif defined(WINDOWS_BEHAVIOR)
        const float moveSpeed = (float)((double)moveDistance / ((double)(bg_compat_animation_sample_time() - slot->lastUpdateTime) *
                                                                (double)BG_ANIM_MILLISECONDS_TO_SECONDS));

        slot->animRate = (float)((double)moveSpeed / (double)animation->moveSpeed);
#else
        const float moveSpeed =
            (float)((long double)moveDistance / ((long double)(bg_compat_animation_sample_time() - slot->lastUpdateTime) *
                                                 (long double)BG_ANIM_MILLISECONDS_TO_SECONDS));

        slot->animRate = (float)((long double)moveSpeed / (long double)animation->moveSpeed);
#endif
        slot->lastUpdateTime = bg_compat_animation_sample_time();
        bg_compat_copy_anim_slot_origin(slot, entity);

        if (slot->animRate < BG_ANIM_RATE_MIN) {
            if (slot->animRate >= BG_ANIM_RATE_ZERO_EPSILON || !usesVerticalDelta) {
            ...
            if ((animation->flags & BG_ANIM_ENTRY_VERTICAL_MOTION) == 0) {
                if (animation->moveSpeed >= BG_ANIM_FAST_MOVE_SPEED_MIN) {
                    ...
                    const float maxRate =
                        BG_ANIM_RATE_CAP_SLOW_MOVE - (float)(animation->moveSpeed - BG_ANIM_RATE_SPEED_BASE) / BG_ANIM_RATE_SPEED_RANGE;
        ...
        if (bg_compat_animation_debug_value() == 2) {
            Com_Printf("MoveSpeed: %s, %i, %4.4f : %1.4f\n", bgAnimStaticTable->entries[animationWord & ~ANIM_TOGGLEBIT].name,
                       bgAnimStaticTable->entries[animationWord & ~ANIM_TOGGLEBIT].moveSpeed, moveSpeed, slot->animRate);
        }
    }

    if (slot->animationWord != 0) {
        const uint32_t anim = bg_compat_anim_slot_xanim(animTreeIndex, slot->animationWord);
