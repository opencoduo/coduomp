...
pack_t *FS_LoadZipFile(const char *zipFile, const char *basename)
{
    ...
    if (stat(zipFile, &archiveStatus) == 0) {
        archiveSizeUnsupported =
            !S_ISREG(archiveStatus.st_mode) || archiveStatus.st_size < 0 ||
                    (uintmax_t)archiveStatus.st_size >
                        FS_MAX_PAK_ARCHIVE_BYTES
                ? qtrue
                : qfalse;
    } else {
    ...
    if (archiveSizeUnsupported != qfalse) {
        Com_Printf(
            "WARNING: refusing pak '%s': archive size is unavailable or "
    ...
    void *archive;
    uint32_t entryCount;
    if (filesystem_compat_archive_open_catalog(
            zipFile, &archive, &entryCount) == qfalse) {
        return NULL;
    }
    ...
    if (entryCount != 0 &&
        filesystem_compat_archive_go_to_first(archive) != FS_ARCHIVE_OK) {
    ...
    for (uint32_t index = 0; index < entryCount; ++index) {
        ...
        if (filesystem_compat_archive_get_current_info(
                archive, &fileInfo, filename,
                sizeof(filename)) != FS_ARCHIVE_OK) {
        ...
        if (fileInfo.filenameLength >= sizeof(filename)) {
            Com_Printf(
                "WARNING: refusing pak '%s' with an overlong entry name\n",
                zipFile);
            fs_packFiles -= (int32_t)entryCount;
            filesystem_compat_archive_close_catalog(archive);
            return NULL;
        ...
        if (fileInfo.uncompressedSize > FS_MAX_PAK_ENTRY_BYTES) {
            Com_Printf(
                "WARNING: refusing pak '%s': entry '%s' exceeds "
        ...
        if (coduo_compat_path_is_safe_relative(filename) == qfalse) {
            Com_Printf(
                "WARNING: refusing pak '%s' with unsafe entry '%s'\n",
                zipFile, filename);
            fs_packFiles -= (int32_t)entryCount;
            filesystem_compat_archive_close_catalog(archive);
            return NULL;
    ...
    fileInPack_t *const packFiles = Z_MallocInternal(
        (size_t)entryCount * sizeof(*packFiles) + namesByteCount);
    char *nameCursor = (char *)(packFiles + entryCount);
    uint32_t *const checksums = Z_MallocInternal(
        (size_t)entryCount * sizeof(*checksums));

    int32_t hashSize = 1;
    while ((uint32_t)hashSize <= entryCount &&
           hashSize < FS_PACK_HASH_MAXIMUM) {
        hashSize <<= 1;
    }

    pack_t *const pack = Z_MallocInternal(
        sizeof(*pack) +
        (size_t)hashSize * sizeof(*pack->hashTable));
    ...
    if (basenameLength > 4 &&
        Q_stricmp(pack->pakBasename + basenameLength - 4,
                  ".pk3") == 0) {
    ...
    for (uint32_t index = 0; index < entryCount; ++index) {
        ...
        Q_strlwr(filename);
        const uint32_t hash =
            FS_HashFileName(filename, pack->hashSize);
        ...
        const int32_t zipPosition =
            filesystem_compat_archive_get_current_position(archive);
    ...
    const int32_t checksumBytes =
        (int32_t)(checksumCount * sizeof(*checksums));
    pack->checksum =
        (int32_t)Com_BlockChecksum(checksums, checksumBytes);
    pack->pureChecksum = (int32_t)Com_BlockChecksumKey(
        checksums, checksumBytes, fs_checksumFeed);
    Z_FreeInternal(checksums);
