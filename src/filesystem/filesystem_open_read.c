 ...
 * 0x08061e5f. */
static void filesystem_compat_mark_pack_references(
    pack_t *pack, const char *qpath)
{
    const size_t length = strlen(qpath);

    /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
    if (pack->generalReference == 0 &&
        (length < FS_SHADER_EXTENSION_LENGTH ||
         Q_stricmp(qpath + length - FS_SHADER_EXTENSION_LENGTH,
                   ".shader") != 0) &&
        (length < FS_SHORT_EXTENSION_LENGTH ||
         Q_stricmp(qpath + length - FS_SHORT_EXTENSION_LENGTH,
                   ".txt") != 0) &&
        (length < FS_SHORT_EXTENSION_LENGTH ||
         Q_stricmp(qpath + length - FS_SHORT_EXTENSION_LENGTH,
                   ".cfg") != 0) &&
        (length < FS_CONFIG_EXTENSION_LENGTH ||
         Q_stricmp(qpath + length - FS_CONFIG_EXTENSION_LENGTH,
                   ".config") != 0) &&
        strstr(qpath, "levelshots") == NULL &&
        (length < FS_SHORT_EXTENSION_LENGTH ||
         Q_stricmp(qpath + length - FS_SHORT_EXTENSION_LENGTH,
                   ".bot") != 0) &&
        (length < FS_ARENA_EXTENSION_LENGTH ||
         Q_stricmp(qpath + length - FS_ARENA_EXTENSION_LENGTH,
                   ".arena") != 0) &&
        (length < FS_MENU_EXTENSION_LENGTH ||
         Q_stricmp(qpath + length - FS_MENU_EXTENSION_LENGTH,
                   ".menu") != 0) &&
    ...
    if (pack->gameModuleReference == 0 &&
        FS_ShiftedStrStr(qpath, "{usmgskesve~><4jrr", -6) != NULL) {
        pack->gameModuleReference = 1;
    }
    if (pack->cgameModuleReference == 0 &&
        FS_ShiftedStrStr(qpath, "wqaeicogaoraz:80fnn", -2) != NULL) {
        pack->cgameModuleReference = 1;
    }
    if (pack->uiModuleReference == 0 &&
        FS_ShiftedStrStr(qpath, "ztdzndrud}=;3iqq", -5) != NULL) {
                                  ...
                                  qboolean uniqueFile, qboolean quiet)
{
    ...
    if (handle == NULL) {
        for (searchpath_t *search = fs_searchpaths;
            ...
            fileInPack_t *packFile =
                pack != NULL ? pack->hashTable[hash] : NULL;
            if (packFile != NULL) {
                do {
    ...
    for (searchpath_t *search = fs_searchpaths;
        ...
        if (pack != NULL && packFile != NULL) {
            do {
                ...
                if (filesystem_compat_archive_open_entry(
                        pack, packFile, fileHandle,
                        uniqueFile, quiet) == qfalse) {
                ...
                if (fileLength < 0) {
                    Com_Printf(
                        "FS_FOpenFileRead: refusing negative length for %s\n",
                        qpath);
                    FS_FCloseFile(*handle);
                    *handle = 0;
                    return -1;
            ...
            const char *const extension =
                FS_GetExtensionSubString(qpath);
            ...
            if (fs_copyfiles->integer != 0 &&
                Q_stricmp(directory->path, fs_cdpath->string) == 0) {
                ...
                if (filesystem_compat_resolve_case_path(
                        directory->path, osPath,
                        resolvedSourcePath,
                        sizeof(resolvedSourcePath)) != qfalse) {
                    sourcePath = resolvedSourcePath;
                }
                FS_BuildOSPath_Internal(
                    fs_basepath->string, directory->gamedir,
                    qpath, copyPath, quiet);
    ...
    *handle = 0;
    if (impurePack != NULL) {
        ...
        Com_Error(
            ERR_DROP,
            "EXE_UNPURECLIENTDETECTED\x15\n%s",
            impurePack->pakFilename);
    }
    return -1;
}
