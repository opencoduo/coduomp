...
void FS_AddPakFilesForGameDirectory(const char *base, const char *game)
{
    ...
    if (filesystem_compat_resolve_case_path(base, osPath, resolvedDirectory, sizeof(resolvedDirectory)) != qfalse) {
        directoryPath = resolvedDirectory;
    }

    int32_t numPakFiles;
    char **const pakFiles = Sys_ListFiles(directoryPath, ".pk3", NULL, &numPakFiles, qfalse);
    if (numPakFiles > FS_MAX_PAKFILES_PER_DIRECTORY) {
        ...
        Com_Printf("WARNING: Exceeded max number of pak files in %s/%s "
                   "(%i/%i)\n",
                   base, game, numPakFiles, FS_MAX_PAKFILES_PER_DIRECTORY);
        numPakFiles = FS_MAX_PAKFILES_PER_DIRECTORY;
    }

    for (int32_t index = 0; index < numPakFiles; ++index) {
        sortedPakFiles[index] = pakFiles[index];
        if (Q_strncmp(sortedPakFiles[index], "localized_", FS_LOCALIZED_PREFIX_LENGTH) == 0) {
            memcpy(sortedPakFiles[index], "          ", FS_LOCALIZED_PREFIX_LENGTH);
        }
    }

    coduo_qsort(sortedPakFiles, (size_t)numPakFiles, sizeof(sortedPakFiles[0]), paksort);

    for (int32_t index = 0; index < numPakFiles; ++index) {
        qboolean localized;
        int32_t language;

        if (Q_strncmp(sortedPakFiles[index], "          ", FS_LOCALIZED_PREFIX_LENGTH) == 0) {
            memcpy(sortedPakFiles[index], "localized_", FS_LOCALIZED_PREFIX_LENGTH);
            localized = qtrue;

            const char *const languageName = PakFileLanguage(sortedPakFiles[index]);
            if (languageName[0] == '\0') {
            ...
            const int32_t languageCount = filesystem_compat_language_count();
            for (language = 0; language < languageCount; ++language) {
                const char *const supportedName = filesystem_compat_language_name(language);
                if (supportedName != NULL && Q_stricmp(languageName, supportedName) == 0) {
                    break;
                }
            }
            if (language == languageCount) {
                filesystem_compat_report_unsupported_pak_language(base, game, sortedPakFiles[index]);
                continue;
            }
        } else {
            localized = qfalse;
        ...
        if (filesystem_compat_resolve_case_path(base, osPath, resolvedPakPath, sizeof(resolvedPakPath)) != qfalse) {
            pakPath = resolvedPakPath;
        }
        pack_t *const pack = FS_LoadZipFile(pakPath, sortedPakFiles[index]);
        if (pack == NULL)
            continue;

        strcpy(pack->pakGamename, game);
        searchpath_t *const searchpath = Z_MallocInternal(sizeof(*searchpath));
...
void FS_AddGameDirectory(const char *base, const char *game, qboolean localized, int32_t language)
{
    ...
    if (localized != qfalse) {
        Com_sprintf(gameName, sizeof(gameName), "%s_%s", game, filesystem_compat_language_name(language));
    } else {
    ...
    for (searchpath_t *search = fs_searchpaths; search != NULL; search = search->next) {
        ...
        if (directory == NULL || base == NULL || Q_stricmp(directory->path, base) != 0 || Q_stricmp(directory->gamedir, gameName) != 0) {
            continue;
        }

        if (search->localized != localized) {
            const char *const existingFolderType = search->localized == qfalse ? "non-localized" : "localized";
        ...
        if (search->localized == qfalse || search->language == language) {
            return;
        }
    ...
    searchpath_t *const searchpath = Z_MallocInternal(sizeof(*searchpath));
    directory_t *const directory = Z_MallocInternal(sizeof(*directory));
...
void FS_AddLocalizedGameDirectory(const char *base, const char *game)
{
    ...
    for (int32_t language = filesystem_compat_language_count() - 1; language >= 0; --language) {
