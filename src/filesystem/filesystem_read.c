...
int32_t FS_Read(void *buffer, int32_t byteCount, int32_t handle)
{
    ...
    if (fileHandle->zipArchive != NULL) {
        ...
        const int32_t fileLength = filesystem_compat_archive_length(fileHandle);
        if (currentPosition < 0 || fileLength < currentPosition) {
            ...
            Com_Error(ERR_DROP,
                      "\x15"
                      "FS_Read: invalid archive state for %s",
                      entryName);
            return -1;
        }

        const uint32_t remaining = (uint32_t)(fileLength - currentPosition);
        const uint32_t expectedByteCount = (uint32_t)byteCount < remaining ? (uint32_t)byteCount : remaining;
        const int32_t bytesRead = filesystem_compat_archive_read(fileHandle, buffer, (uint32_t)byteCount);
        ...
        if (bytesRead < 0 || (uint32_t)bytesRead != expectedByteCount) {
            ...
            if (buffer != NULL && completed < expectedByteCount) {
                memset((uint8_t *)buffer + completed, 0, expectedByteCount - completed);
            }
            char entryName[FS_HANDLE_NAME_SIZE];
            memcpy(entryName, fileHandle->name, sizeof(entryName));
            entryName[sizeof(entryName) - 1u] = '\0';
            ...
            Com_Error(ERR_DROP,
                      "\x15"
                      "FS_Read: malformed archive entry %s (%i of %u bytes)",
    ...
    while (remaining != 0) {
        const uint32_t bytesRead = (uint32_t)fread(cursor, 1, (size_t)remaining, (FILE *)fileHandle->ioObject);
        if (bytesRead == 0) {
            if (sawZeroByteRead != qfalse)
                return byteCount - (int32_t)remaining;
            sawZeroByteRead = qtrue;
        } else if (bytesRead == UINT32_MAX) {
            if (handle >= FS_READ_UNIQUE_HANDLE_FIRST && handle < FS_HANDLE_COUNT) {
                return -1;
            }
...
int32_t FS_Read2(void *buffer, int32_t byteCount, int32_t handle)
{
    ...
    const int32_t bytesRead = filesystem_compat_streamed_read(buffer, byteCount, handle);
