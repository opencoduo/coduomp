...
int32_t FS_ReadFile(const char *qpath, void **buffer)
{
    ...
    const qboolean isConfig =
        strstr(qpath, ".cfg") != NULL ? qtrue : qfalse;
    if (isConfig != qfalse && com_journal != NULL &&
        com_journal->integer == 2) {
        int32_t length;
        Com_DPrintf("Loading %s from journal file.\n", qpath);
        if (FS_Read(&length, (int32_t)sizeof(length),
                    com_journalDataFile) != (int32_t)sizeof(length)) {
            if (buffer != NULL)
                *buffer = NULL;
            return -1;
        }
        ...
        if (length < 0) {
            Com_Printf(
                "FS_ReadFile: refusing negative journal length %i for %s\n",
                length, qpath);
            if (buffer != NULL)
                *buffer = NULL;
            return -1;
    ...
    if (handle == 0) {
        ...
        if (isConfig != qfalse && com_journal != NULL &&
            ...
            (void)FS_Write(&journalLength, (int32_t)sizeof(journalLength),
                           com_journalDataFile);
    ...
    if (buffer == NULL) {
        if (isConfig != qfalse && com_journal != NULL &&
            ...
            (void)FS_Write(&length, (int32_t)sizeof(length),
                           com_journalDataFile);
            FS_Flush(com_journalDataFile);
        }
        FS_FCloseFile(handle);
