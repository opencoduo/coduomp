...
static inline void bg_compat_controller_set_control_tag_angles(
    ...
    const vec3_t angles)
{
    (void)G_DObjSetControlTagAngles((gentity_t *)dobjOwner, partBits,
                                    tagName, angles);
}
...
{
    (void)G_DObjSetLocalTag((gentity_t *)dobjOwner, partBits, tagName,
                            offset, angles);
}
    ...
    int32_t entryCount, uint32_t animIndex)
{
    ...
    G_Error("\x15Player animation index out of range (%i): %i", entryCount,
            animIndex);
#else
...
{
#if defined(WINDOWS_BEHAVIOR)
    G_Error("\x15Player animation index %i out of 0 to %i range", animIndex,
            entryCount);
#else
...
{
#if defined(WINDOWS_BEHAVIOR)
    G_Error("\x15" "BG_GetAnimationForIndex: index out of bounds");
#else
    Com_Error(ERR_DROP,
...
{
#if defined(WINDOWS_BEHAVIOR)
    G_Error("\x15" "BG_AnimPlayerConditions: Vehicle type unknown: %i",
            vehicleType);
#else
    Com_Error(ERR_DROP,
