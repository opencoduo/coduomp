...
intptr_t SV_GameSystemCalls(intptr_t *arguments)
{
    ...
    case SYSCALL_CVAR_REGISTER:
        Cvar_Register(SV_PTR(vmCvar_t, 1), SV_STRING(2),
                      SV_STRING(3), (uint32_t)SV_ARG(4));
        return 0;
    ...
    case SYSCALL_CVAR_VARIABLE_STRING_BUFFER:
        Cvar_VariableStringBuffer(SV_STRING(1), SV_PTR(char, 2),
                                  SV_INT(3));
        return 0;
    case SYSCALL_ARGC:
        return Cmd_Argc();
    ...
    case SYSCALL_HUNK_ALLOC_LOW_ALIGN:
        return (intptr_t)SV_Hunk_AllocLowAlignInternal(SV_SIZE(1),
                                                       SV_SIZE(2));
    ...
    case SYSCALL_LOCATE_GAME_DATA:
        SV_LocateGameData(SV_PTR(sharedEntity_t, 1), SV_INT(2), SV_INT(3),
                          SV_PTR(playerState_t, 4), SV_INT(5));
        return 0;
    ...
    case SYSCALL_IS_LOCAL_CLIENT: {
        ...
        return NET_IsLocalAddress(
            svs.clients[clientNum].netchan.remoteAddress);
    }
    ...
    case SYSCALL_SET_BRUSH_MODEL:
        ...
        return 0;
    case SYSCALL_TRACE:
        SV_Trace(SV_PTR(trace_t, 1), SV_CONST_PTR(float, 2),
                 SV_CONST_PTR(float, 3), SV_CONST_PTR(float, 4),
                 SV_CONST_PTR(float, 5), SV_INT(6), SV_INT(7),
                 qfalse, qfalse, NULL, qfalse);
        return 0;
    case SYSCALL_TRACE_CAPSULE:
        SV_Trace(SV_PTR(trace_t, 1), SV_CONST_PTR(float, 2),
                 SV_CONST_PTR(float, 3), SV_CONST_PTR(float, 4),
                 SV_CONST_PTR(float, 5), SV_INT(6), SV_INT(7),
                 qtrue, qfalse, NULL, qfalse);
        return 0;
    case SYSCALL_SIGHT_TRACE:
        SV_SightTrace(SV_PTR(int32_t, 1), SV_CONST_PTR(float, 2),
                      SV_CONST_PTR(float, 3), SV_CONST_PTR(float, 4),
                      SV_CONST_PTR(float, 5), SV_INT(6), SV_INT(7),
                      SV_INT(8), qfalse);
        return 0;
    ...
    case SYSCALL_SIGHT_TRACE_TO_ENTITY:
        return SV_SightTraceToEntity(
            SV_CONST_PTR(float, 1), SV_CONST_PTR(float, 2),
            SV_CONST_PTR(float, 3), SV_CONST_PTR(float, 4),
            SV_INT(5), SV_INT(6), qtrue);
    case SYSCALL_CM_BOX_TRACE:
        CM_BoxTrace(SV_PTR(trace_t, 1), SV_CONST_PTR(float, 2),
                    SV_CONST_PTR(float, 3), SV_CONST_PTR(float, 4),
                    SV_CONST_PTR(float, 5), SV_INT(6), SV_INT(7), qfalse);
        return 0;
    case SYSCALL_CM_CAPSULE_TRACE:
        CM_BoxTrace(SV_PTR(trace_t, 1), SV_CONST_PTR(float, 2),
                    SV_CONST_PTR(float, 3), SV_CONST_PTR(float, 4),
                    SV_CONST_PTR(float, 5), SV_INT(6), SV_INT(7), qtrue);
        return 0;
    case SYSCALL_CM_BOX_SIGHT_TRACE:
        return CM_BoxSightTrace(
            0, SV_CONST_PTR(float, 1), SV_CONST_PTR(float, 2),
            SV_CONST_PTR(float, 3), SV_CONST_PTR(float, 4),
    ...
    case SYSCALL_LOCATIONAL_TRACE:
        SV_Trace(SV_PTR(trace_t, 1), SV_CONST_PTR(float, 2), NULL, NULL,
                 SV_CONST_PTR(float, 3), SV_INT(4), SV_INT(5),
                 qfalse, qtrue, SV_CONST_PTR(uint8_t, 6), qtrue);
        return 0;
    case SYSCALL_POINT_CONTENTS:
        return SV_PointContents(SV_CONST_PTR(float, 1), SV_INT(2),
                                SV_INT(3));
    case SYSCALL_IN_PVS:
    ...
    case SYSCALL_ENTITIES_IN_BOX:
        return CM_AreaEntities(
            SV_CONST_PTR(float, 1), SV_CONST_PTR(float, 2),
            SV_PTR(int32_t, 3), SV_INT(4), SV_INT(5));
    case SYSCALL_ENTITY_CONTACT:
        return SV_EntityContact(SV_CONST_PTR(float, 1),
                                SV_CONST_PTR(float, 2),
                                SV_PTR(sharedEntity_t, 3), qfalse);
    ...
    case SYSCALL_FS_GET_FILE_LIST:
        return FS_GetFileList(SV_STRING(1), SV_STRING(2),
                              SV_PTR(char, 3), SV_INT(4));
    ...
    case SYSCALL_GET_ARCHIVED_CLIENT_INFO: {
        ...
        return SV_GetArchivedClientInfo(
            clientNum, SV_PTR(int32_t, 2), SV_PTR(playerState_t, 3),
            SV_PTR(clientState_t, 4));
    }
    ...
    case SYSCALL_DOBJ_CREATE:
        Com_ServerDObjCreate(
            SV_PTR(DObjModel, 1), (uint16_t)SV_ARG(2),
    ...
    case SYSCALL_XANIM_CLEAR_TREE_GOAL_WEIGHTS:
        XAnimClearTreeGoalWeights(
            ...
            server_compat_game_syscall_float_argument(SV_ARG(3)));
        return 0;
    ...
    case SYSCALL_XANIM_SET_COMPLETE_GOAL_WEIGHT_KNOB:
        XAnimSetCompleteGoalWeightKnob(
            ...
            server_compat_game_syscall_float_argument(SV_ARG(5)),
            (uint16_t)SV_ARG(6), XANIM_ROOT_NODE_INDEX,
            (qboolean)SV_ARG(7));
        return 0;
    case SYSCALL_XANIM_SET_COMPLETE_GOAL_WEIGHT_KNOB_ALL:
        return XAnimSetCompleteGoalWeightKnobAll(
            ...
            (uint16_t)SV_ARG(7), XANIM_ROOT_NODE_INDEX,
            (qboolean)SV_ARG(8));
    ...
    case SYSCALL_XANIM_GET_LENGTH:
        ...
        return x87f_store_i32_trunc(x87f_mul(
            x87f_load_f32(sv_xanimMilliseconds),
            x87f_load_f64(XAnimGetLength(
                SV_PTR(XAnim, 1), SV_INT(2)))));
#else
        return x87f_store_i32_trunc(x87f_mul(
            x87f_load_f32(sv_xanimMilliseconds),
            XAnimGetLength(SV_PTR(XAnim, 1), SV_INT(2))));
#endif
#elif defined(CODUO_X87_TRUNCATE_I32)
        return CODUO_X87_SCALE_F32_TRUNCATE_I32(
            XAnimGetLength(SV_PTR(XAnim, 1), SV_INT(2)),
            sv_xanimMilliseconds);
#else
        return (int32_t)((long double)sv_xanimMilliseconds *
                         (long double)XAnimGetLength(
                             SV_PTR(XAnim, 1), SV_INT(2)));
#endif
    case SYSCALL_XANIM_GET_LENGTH_SECONDS: {
        ...
        return FloatAsInt(x87f_store_f32(
            x87f_load_f64(XAnimGetLength(tree, SV_INT(2)))));
#else
        return FloatAsInt(x87f_store_f32(
            XAnimGetLength(tree, SV_INT(2))));
#endif
#else
    ...
    case SYSCALL_XANIM_GET_REL_DELTA: {
        XAnim *tree = Scr_GetAnims((uint32_t)SV_ARG(1));
        XAnimGetRelDelta(tree, SV_INT(2), SV_PTR(float, 3),
                         SV_PTR(float, 4),
                         server_compat_game_syscall_float_argument(SV_ARG(5)),
                         server_compat_game_syscall_float_argument(SV_ARG(6)));
        return 0;
    ...
    case SYSCALL_XANIM_GET_ABS_DELTA: {
        XAnim *tree = Scr_GetAnims((uint32_t)SV_ARG(1));
        XAnimGetAbsDelta(tree, SV_INT(2), SV_PTR(float, 3),
                         SV_PTR(float, 4),
                         server_compat_game_syscall_float_argument(SV_ARG(5)));
        return 0;
    }
    ...
    case SYSCALL_XANIM_NOTETRACK_EXISTS: {
        XAnim *tree = Scr_GetAnims((uint32_t)SV_ARG(1));
        return XAnimNotetrackExists(tree, SV_INT(2),
                                    (uint16_t)SV_ARG(3));
    }
    ...
    case SYSCALL_DOBJ_CREATE_SKEL_FOR_BONES:
        return SV_DObjCreateSkelForBones(SV_INT(1),
                                         SV_CONST_PTR(uint32_t, 2));
    case SYSCALL_DOBJ_UPDATE_SERVER_TIME:
        return SV_DObjUpdateServerTime(
            SV_INT(1), server_compat_game_syscall_float_argument(SV_ARG(2)),
            (qboolean)SV_ARG(3));
    ...
    case SYSCALL_DOBJ_GET_HIERARCHY_BITS:
        SV_DObjGetHierarchyBits(SV_INT(1), SV_INT(2),
                                SV_PTR(uint32_t, 3));
        return 0;
    ...
    case SYSCALL_XANIM_CLONE_ANIM_TREE:
        XAnimCloneAnimTree(SV_PTR(XAnimTree, 1),
                           SV_PTR(XAnimTree, 2));
        return 0;
    ...
    case SYSCALL_DOBJ_SET_ROT_TRANS_INDEX:
        return SV_DObjSetRotTransIndex(SV_INT(1),
                                       SV_CONST_PTR(uint8_t, 2), SV_INT(3));
    case SYSCALL_DOBJ_SET_CONTROL_ROT_TRANS_INDEX:
        return SV_DObjSetControlRotTransIndex(
            SV_INT(1), SV_CONST_PTR(uint8_t, 2), SV_INT(3));
    ...
    case SYSCALL_GET_WEAPON_INFO_MEMORY:
        return (intptr_t)server_compat_get_weapon_info_memory(
            SV_INT(1), SV_PTR(int32_t, 2),
            SERVER_GAME_SYSCALL_WEAPON_MEMORY_OWNER);
    case SYSCALL_FREE_WEAPON_INFO_MEMORY:
        server_compat_free_weapon_info_memory(
            SERVER_GAME_SYSCALL_WEAPON_MEMORY_OWNER,
            (qboolean)SV_ARG(1));
        return 0;
    ...
    case SYSCALL_SIN:
        return FloatAsInt((float)sin(
            (double)server_compat_game_syscall_float_argument(SV_ARG(1))));
    case SYSCALL_COS:
        return FloatAsInt((float)cos(
            (double)server_compat_game_syscall_float_argument(SV_ARG(1))));
    case SYSCALL_ATAN2:
        return FloatAsInt((float)atan2(
            (double)server_compat_game_syscall_float_argument(SV_ARG(1)),
            (double)server_compat_game_syscall_float_argument(SV_ARG(2))));
    case SYSCALL_SQRT:
        return FloatAsInt((float)sqrt(
            (double)server_compat_game_syscall_float_argument(SV_ARG(1))));
    ...
    default:
        Com_Error(ERR_DROP,
                  "\x15" "Bad game system trap: %i", syscall);
        return -1;
    }
}
