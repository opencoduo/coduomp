                       ...
                       int32_t sizeofGameClient)
{
    ...
    if (gentities == NULL || clients == NULL || numGentities < 0 || numGentities > MAX_GENTITIES ||
        sizeofGentity < (int32_t)sizeof(sharedEntity_t) || sizeofGameClient < (int32_t)sizeof(playerState_t)) {
        Com_Error(ERR_DROP, "\x15"
                            "SV_LocateGameData: invalid game data");
        return;
    }
...
{
    return (int32_t)(((const uint8_t *)gentity - (const uint8_t *)sv_gentities) / sv_gentitySize);
}

sharedEntity_t *SV_GentityNum(int32_t entityNum)
{
    return (sharedEntity_t *)((uint8_t *)sv_gentities + (ptrdiff_t)entityNum * sv_gentitySize);
}

playerState_t *SV_GameClientNum(int32_t clientNum)
{
    return (playerState_t *)((uint8_t *)sv_gameClients + (ptrdiff_t)clientNum * sv_gameClientSize);
}

svEntity_t *SV_SvEntityForGentity(const sharedEntity_t *gentity)
{
    if (gentity == NULL || gentity->entityState.number < 0 || gentity->entityState.number >= MAX_GENTITIES) {
        Com_Error(ERR_DROP, "\x15"
