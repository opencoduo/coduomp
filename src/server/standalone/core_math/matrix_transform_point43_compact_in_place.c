...
#define CODUO_TP43_DOT(p0, p1, p2, m, j) \
    x87f_store_f32(x87f_add(x87f_add(x87f_add(x87f_mul(x87f_load_f32(p0), x87f_load_f32((m)->axis[0][j])), \
                                              x87f_mul(x87f_load_f32(p1), x87f_load_f32((m)->axis[1][j]))), \
...
void MatrixTransformPoint43CompactInPlace(vec3_t point, const matrix43_t *matrix)
{
    ...
    const float transformed0 =
        (((point0 * matrix->axis[0][0]) + (point1 * matrix->axis[1][0])) + (point2 * matrix->axis[2][0])) + matrix->origin[0];
    const float transformed1 =
        (((point0 * matrix->axis[0][1]) + (point1 * matrix->axis[1][1])) + (point2 * matrix->axis[2][1])) + matrix->origin[1];

    point[2] = (((point0 * matrix->axis[0][2]) + (point1 * matrix->axis[1][2])) + (point2 * matrix->axis[2][2])) + matrix->origin[2];
#endif
