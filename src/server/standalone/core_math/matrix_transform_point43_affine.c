...
{
#if EMULATE_X87
    /* x87-faithful transcription (stock coduo_lnxded 0x0806936a): each output is
     * the 80-bit affine dot ((in0*a0 + in1*a1) + in2*a2) + origin, rounded to
     * float at the store. */
    for (int j = 0; j < 3; ++j) {
        out[j] = x87f_store_f32(x87f_add(x87f_add(x87f_add(x87f_mul(x87f_load_f32(in[0]), x87f_load_f32(matrix->axis[0][j])),
                                                           x87f_mul(x87f_load_f32(in[1]), x87f_load_f32(matrix->axis[1][j]))),
                                                  x87f_mul(x87f_load_f32(in[2]), x87f_load_f32(matrix->axis[2][j]))),
