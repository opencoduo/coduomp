...
#define SOUND_ALIAS_SOURCE_SUBTITLE_STRING_FILE "../source_data/string_resources/subtitle.st"
...
void Com_LocalizeSoundAliasSubtitleText(void)
{
    ...
    FS_BuildOSPath(fs_homepath->string, SOUND_ALIAS_SOURCE_SUBTITLE_STRING_FILE, SOUND_ALIAS_EMPTY_STRING, sourceStringEdPath);
    ...
    FS_BuildOSPath(fs_basepath->string, fs_currentGameDir, SOUND_ALIAS_SUBTITLE_STRING_FILE, localStringEdPath);
    ...
    soundAliasFiles = FS_ListFiles(SOUND_ALIAS_CSV_DIRECTORY, SOUND_ALIAS_CSV_EXTENSION, &soundAliasFileCount);
    ...
    for (fileIndex = 0; fileIndex < soundAliasFileCount; fileIndex++) {
        Hunk_SetMarkTemp();
        Com_sprintf(csvPath, sizeof(csvPath), SOUND_ALIAS_CSV_QPATH_FORMAT, soundAliasFiles[fileIndex]);
