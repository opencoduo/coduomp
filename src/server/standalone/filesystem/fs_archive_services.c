...
int32_t filesystem_compat_archive_length(
    const fileHandleData_t *fileHandle)
{
    const coduo_fs_zip_io_file_t *const zipFile =
        (const coduo_fs_zip_io_file_t *)fileHandle->ioObject;
    /* NOT_FROM_ORIGINAL_SOURCE: repeat the mounted-entry size policy at the
     * live reader boundary before publishing its length. */
    if (zipFile->cur_file_info.uncompressed_size >
                                       ...
                                       void *buffer, uint32_t byteCount)
{
    return Unzip_ReadCurrentFile(
        (struct coduo_unz_s *)fileHandle->ioObject, buffer, byteCount);
}

int32_t filesystem_compat_archive_rewind(fileHandleData_t *fileHandle)
{
    struct coduo_unz_s *const archive =
        (struct coduo_unz_s *)fileHandle->ioObject;
    const int32_t positionStatus =
        Unzip_GoToFilePosition(archive, fileHandle->zipRewindOffset);
    if (positionStatus != 0)
    ...
    qboolean quiet)
{
    ...
    int32_t status = Unzip_GoToFilePosition(
        (struct coduo_unz_s *)archive, packFile->zipPosition);
    if (status == 0)
    ...
    char *filename, uint32_t filenameSize)
{
    ...
    const int32_t status = Unzip_GetCurrentFileInfo(
        (struct coduo_unz_s *)archive, &targetInfo,
        filename, filenameSize, NULL, 0, NULL, 0);
    if (status == 0) {
...
int32_t filesystem_compat_archive_get_current_position(void *archive)
{
    ...
    if (Unzip_GetCurrentFilePosition(
            (struct coduo_unz_s *)archive, &position) != 0) {
        return -1;
    }
