...
char *Sys_ConsoleInput(void)
{
     ...
     */
    if (dedicated == NULL || dedicated->value == 0.0f || sys_stdinActive == SYS_STDIN_INACTIVE || _kbhit() == 0) {
        return NULL;
    }
    ...
    if (input == sys_ttyEraseChar || input == SYS_TTY_ASCII_DELETE || input == SYS_TTY_ASCII_BACKSPACE) {
    ...
    if (input <= SYS_TTY_CONTROL_MAX || sys_ttyCurrentLine.cursor >= CON_INPUT_BUFFER_SIZE - 1) {
        return NULL;
    }
    ...
    if (ttycon != NULL && ttycon->value != 0.0f) {
        readCount = read(SYS_STDIN_FILE_DESCRIPTOR, &input, SYS_CONSOLE_INPUT_SINGLE_BYTE);
        ...
        if (readCount == SYS_CONSOLE_INPUT_SINGLE_BYTE) {
            ...
            if (input != SYS_TTY_ASCII_NUL && (signed char)input <= SYS_TTY_CONTROL_MAX) {
                if (input == SYS_TTY_ASCII_NEWLINE) {
                    Sys_TTYStoreHistoryLine(&sys_ttyCurrentLine);
                    strcpy(sys_ttyInputReturnBuffer, sys_ttyCurrentLine.buffer);
                    Sys_TTYResetLine(&sys_ttyCurrentLine);
                    input = SYS_TTY_ASCII_NEWLINE;
                    write(SYS_STDOUT_FILE_DESCRIPTOR, &input, SYS_CONSOLE_INPUT_SINGLE_BYTE);
                    return sys_ttyInputReturnBuffer;
                }
                ...
                if (input == SYS_TTY_ASCII_TAB) {
                    ...
                    Sys_TTYCompleteLine(&sys_ttyCurrentLine);
                    sys_ttyCurrentLine.cursor = strlen(sys_ttyCurrentLine.buffer);
                    if (sys_ttyCurrentLine.cursor > 0 && sys_ttyCurrentLine.buffer[0] == SYS_TTY_COMMAND_PREFIX) {
                        /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
                        memmove(sys_ttyCurrentLine.buffer, sys_ttyCurrentLine.buffer + 1, (size_t)sys_ttyCurrentLine.cursor);
                        sys_ttyCurrentLine.cursor--;
                    }
                    Sys_TTYShowInputLine();
                    return NULL;
                ...
                readCount = read(SYS_STDIN_FILE_DESCRIPTOR, &input, SYS_CONSOLE_INPUT_SINGLE_BYTE);
                if (readCount == SYS_CONSOLE_INPUT_SINGLE_BYTE &&
                    (input == SYS_TTY_ESCAPE_INTRO_CSI || input == SYS_TTY_ESCAPE_INTRO_SS3)) {
                    readCount = read(SYS_STDIN_FILE_DESCRIPTOR, &input, SYS_CONSOLE_INPUT_SINGLE_BYTE);
                    if (readCount == SYS_CONSOLE_INPUT_SINGLE_BYTE) {
            ...
            /* NOT_FROM_ORIGINAL_SOURCE: preserve this recovered boundary's validated input, state, and compatibility invariants. */
            if (sys_ttyCurrentLine.cursor < 0 || sys_ttyCurrentLine.cursor >= CON_INPUT_BUFFER_SIZE - 1) {
                return NULL;
            }
...

    {
        ...
        timeout.tv_sec = 0;
        timeout.tv_usec = 0;
        selectResult = select(SYS_SELECT_STDIN_NFDS, &stdinSet, NULL, NULL, &timeout);
        if (selectResult != SYS_SELECT_ERROR && FD_ISSET(SYS_STDIN_FILE_DESCRIPTOR, &stdinSet)) {
            readCount = read(SYS_STDIN_FILE_DESCRIPTOR, sys_ttyInputReturnBuffer, SYS_TTY_INPUT_RETURN_BUFFER_SIZE);
