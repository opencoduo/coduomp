...
void Com_PrintMessage(int32_t channel, const char *message)
{
    ...
    if (com_redirectBuffer != NULL) {
        if (channel != CODUO_COM_PRINT_CHANNEL_DEVELOPER) {
            ...
            if (redirectCapacityLimit < redirectLength + redirectMessageLength) {
    ...
    if (channel != CODUO_COM_PRINT_CHANNEL_DEVELOPER) {
        if (dedicated != NULL && dedicated->integer == CODUO_COM_DEDICATED_DISABLED) {
            CL_ConsolePrint(channel, message, CODUO_COM_CONSOLE_STUB_ARG2, CODUO_COM_CONSOLE_STUB_ARG3);
        }
        Sys_Print(message);
    }
    ...
    if (com_consoleLogFile == CODUO_COM_LOG_FILE_CLOSED_HANDLE && FS_Initialized() != qfalse && com_printMessageOpeningLog == qfalse) {
    ...
    if (com_consoleLogFile != CODUO_COM_LOG_FILE_CLOSED_HANDLE && FS_Initialized() != qfalse) {
        ...
        FS_Write(message, (int32_t)messageLength, com_consoleLogFile);
    }
}
