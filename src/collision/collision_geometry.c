...
{
#if EMULATE_X87
    const x87f deltaX = x87f_sub(x87f_load_f32(point[0]),
                                 x87f_load_f32(linePoint[0]));
    const x87f deltaY = x87f_sub(x87f_load_f32(point[1]),
                                 x87f_load_f32(linePoint[1]));
    const x87f deltaZ = x87f_sub(x87f_load_f32(point[2]),
                                 x87f_load_f32(linePoint[2]));

    for (int32_t axis = 0; axis < CM_VECTOR_AXIS_COUNT; ++axis) {
        const x87f dot = x87f_add(
            x87f_add(x87f_mul(deltaZ, x87f_load_f32(lineDirection[2])),
                     x87f_mul(deltaY, x87f_load_f32(lineDirection[1]))),
            x87f_mul(deltaX, x87f_load_f32(lineDirection[0])));
        projected[axis] = x87f_store_f32(x87f_add(
            x87f_mul(dot, x87f_load_f32(lineDirection[axis])),
            x87f_load_f32(linePoint[axis])));
    }
#else
    const long double deltaX =
        (long double)point[0] - (long double)linePoint[0];
    const long double deltaY =
        (long double)point[1] - (long double)linePoint[1];
    const long double deltaZ =
        (long double)point[2] - (long double)linePoint[2];

    for (int32_t axis = 0; axis < CM_VECTOR_AXIS_COUNT; ++axis) {
        const long double dot =
            (deltaZ * (long double)lineDirection[2] +
             deltaY * (long double)lineDirection[1]) +
            deltaX * (long double)lineDirection[0];
        projected[axis] = (float)(
            dot * (long double)lineDirection[axis] +
            (long double)linePoint[axis]);
    }
#endif
}
#else
                             ...
                             const vec3_t lineDirection, vec3_t projected)
{
    ...
    for (int32_t axis = 0; axis < CM_VECTOR_AXIS_COUNT; ++axis) {
        delta[axis] = x87f_store_f32(x87f_sub(
            x87f_load_f32(point[axis]), x87f_load_f32(linePoint[axis])));
    }
    for (int32_t axis = 0; axis < CM_VECTOR_AXIS_COUNT; ++axis) {
        const x87f dot = x87f_add(
            x87f_add(
                x87f_mul(x87f_load_f32(delta[0]),
                         x87f_load_f32(lineDirection[0])),
                x87f_mul(x87f_load_f32(delta[1]),
                         x87f_load_f32(lineDirection[1]))),
            x87f_mul(x87f_load_f32(delta[2]),
                     x87f_load_f32(lineDirection[2])));
        projected[axis] = x87f_store_f32(x87f_add(
            x87f_mul(dot, x87f_load_f32(lineDirection[axis])),
            x87f_load_f32(linePoint[axis])));
    }
    ...
    for (int32_t axis = 0; axis < CM_VECTOR_AXIS_COUNT; ++axis) {
        const long double dot =
            ((long double)delta[0] * (long double)lineDirection[0] +
             (long double)delta[1] * (long double)lineDirection[1]) +
...
#if EMULATE_X87
    const x87f deltaX = x87f_sub(x87f_load_f32(point[0]),
                                 x87f_load_f32(endpoint[0]));
    const x87f deltaY = x87f_sub(x87f_load_f32(point[1]),
                                 x87f_load_f32(endpoint[1]));
    const x87f deltaZ = x87f_sub(x87f_load_f32(point[2]),
                                 x87f_load_f32(endpoint[2]));
    const x87f squared = x87f_add(
        x87f_add(x87f_mul(deltaZ, deltaZ), x87f_mul(deltaY, deltaY)),
        x87f_mul(deltaX, deltaX));
    /* Windows PC=53 values are carried exactly by the host double return. */
    return (long double)x87f_store_f64(squared);
#else
    const long double deltaX =
        (long double)point[0] - (long double)endpoint[0];
    const long double deltaY =
        (long double)point[1] - (long double)endpoint[1];
    const long double deltaZ =
        (long double)point[2] - (long double)endpoint[2];
                                             ...
                                             const vec3_t direction)
{
    ...
    for (int32_t lane = 0; lane < CM_VECTOR_AXIS_COUNT; ++lane) {
        delta[lane] = x87f_store_f32(x87f_sub(
            x87f_load_f32(point[lane]), x87f_load_f32(endpoint[lane])));
    }
...
long double CM_DistanceSquared(const vec3_t start, const vec3_t end)
{
    ...
    const x87f deltaZ = x87f_sub(x87f_load_f32(end[2]),
...
#else
    const long double deltaX =
        (long double)end[0] - (long double)start[0];
    const long double deltaY =
        (long double)end[1] - (long double)start[1];
    const long double deltaZ =
...
long double CM_DistanceSquared(const vec3_t start, const vec3_t end)
{
    ...
    return ((long double)delta[0] * (long double)delta[0] +
            (long double)delta[1] * (long double)delta[1]) +
           (long double)delta[2] * (long double)delta[2];
#endif
}
...
long double CM_FastSqrt(float value)
{
    ...
    bits = CM_FAST_INV_SQRT_MAGIC -
           ((bits >> 1) | (bits & CM_FLOAT_SIGN_BIT));
    ...
    inverseRoot = x87f_store_f32(x87f_mul(
        inverse,
        x87f_sub(x87f_load_f32(1.5f),
                 x87f_mul(x87f_mul(half, inverse), inverse))));
    inverse = x87f_load_f32(inverseRoot);
    const x87f refined = x87f_mul(
        inverse,
        x87f_sub(x87f_load_f32(1.5f),
                 x87f_mul(x87f_mul(half, inverse), inverse)));
    return (long double)x87f_store_f64(
        x87f_mul(x87f_load_f32(value), refined));
#else
    const long double half = (long double)value * 0.5L;
    inverseRoot = (float)(
        (long double)inverseRoot *
        (1.5L - half * (long double)inverseRoot *
                    (long double)inverseRoot));
...
long double CM_FastSqrt(float value)
{
    ...
    float inverseRoot;
#if EMULATE_X87
    const float half = x87f_store_f32(
        x87f_mul(x87f_load_f32(value), x87f_load_f32(0.5f)));
#else
    ...
    for (int32_t iteration = 0; iteration < 2; ++iteration) {
        ...
        inverseRoot = x87f_store_f32(x87f_mul(
            ...
            x87f_sub(
                x87f_load_f32(1.5f),
                x87f_mul(x87f_mul(x87f_load_f32(half), inverse), inverse))));
    }
    return (long double)x87f_store_f64(
        x87f_mul(x87f_load_f32(value), x87f_load_f32(inverseRoot)));
#else
    inverseRoot = (float)(
        (long double)inverseRoot *
        ((long double)1.5f -
         (long double)half * (long double)inverseRoot *
             (long double)inverseRoot));
    inverseRoot = (float)(
        (long double)inverseRoot *
        ((long double)1.5f -
