    ...
    const vec3_t start, const vec3_t end)
{
    ...
    if (work->bestTrace.fraction <= startFraction ||
        (work->contentsMask &
    ...
    const float startDistance =
        start[axis] - sector->dist;
    const float endDistance =
        end[axis] - sector->dist;
    const float radius =
        work->expandedHalfSize[axis];

    if (startDistance >= radius &&
        endDistance >= radius) {
        CM_ClipMoveToEntities_r(
            work, sector->children[0],
            startFraction, endFraction,
            start, end);
    } else if (startDistance <= -radius &&
               endDistance <= -radius) {
        CM_ClipMoveToEntities_r(
            work, sector->children[1],
            startFraction, endFraction,
            start, end);
    } else {
        int32_t side;
        ...
        const float fractionSpan = (float)(
            (long double)endFraction -
            startFraction);
        const float leaveMiddleFraction = (float)(
            (long double)startFraction +
            (long double)fractionSpan *
                leaveFractionRaw);

        CM_ClipMoveToEntities_r(
            work, sector->children[side],
            startFraction, leaveMiddleFraction,
            start, middle);
        ...
        middle[2] = (float)(
            ...
            ((long double)end[2] -
             start[2]) *
                enterFraction);
        const float enterMiddleFraction = (float)(
            (long double)startFraction +
            (long double)fractionSpan *
                enterFraction);

        CM_ClipMoveToEntities_r(
            work, sector->children[1 - side],
            enterMiddleFraction, endFraction,
            middle, end);
    }

    for (svEntity_t *serverEntity =
             sector->entityLinkHead;
         serverEntity != NULL;
         serverEntity =
             serverEntity->nextInWorldSector) {
...
{
    CM_ClipMoveToEntities_r(
        work, &cm_worldSectorRoot,
        0.0f, work->bestTrace.fraction,
        work->start, work->end);
}
...
int32_t CM_ClipSightTraceToEntities_r(
    ...
    const vec3_t start, const vec3_t end)
{
    ...
    if (startDistance >= radius &&
        endDistance >= radius) {
        hit = CM_ClipSightTraceToEntities_r(
            work, sector->children[0],
            startFraction, endFraction,
            start, end);
    } else if (startDistance <= -radius &&
...
{
    return CM_ClipSightTraceToEntities_r(
        work, &cm_worldSectorRoot,
        0.0f, 1.0f, work->start, work->end);
}
    ...
    const vec3_t start, const vec3_t end)
{
    ...
    if (startDistanceRaw >= 0.0L &&
        endDistance >= 0.0f) {
        CM_PointTraceToEntities_r(
            work, sector->children[0],
            startFraction, endFraction,
            start, end);
    } else if (startDistanceRaw <= 0.0L &&
               endDistance <= 0.0f) {
        CM_PointTraceToEntities_r(
            work, sector->children[1],
            startFraction, endFraction,
        ...
        const long double splitFractionRaw =
            startDistanceRaw /
 ...
 * query. */
int32_t CM_PointSightTraceToEntities_r(
    ...
    const vec3_t start, const vec3_t end)
{
    ...
    if (startDistanceRaw >= 0.0L &&
        endDistance >= 0.0f) {
        hit = CM_PointSightTraceToEntities_r(
            work, sector->children[0],
            startFraction, endFraction,
            start, end);
    } else if (startDistanceRaw <= 0.0L &&
    ...
    for (svEntity_t *serverEntity =
         ...
         serverEntity =
             serverEntity->nextInWorldSector) {
        hit = SV_PointSightTraceToEntity(
            work, serverEntity);
        if (hit != 0)
            return hit;
    }
...
void CM_ClipMoveToEntities_r(cmClipMoveWork_t *traceWork,
                  worldSector_t *sector,
                  float startFraction,
                  float endFraction,
                  const vec3_t start,
                  const vec3_t end)
{
    ...
    if (!(startFraction >= traceWork->bestTrace.fraction) &&
        ...
        if (!(startDistance >= radius) || !(endDistance >= radius)) {
            if (!(-radius >= startDistance) ||
...

                {
                    ...
                    CM_ClipMoveToEntities_r(traceWork,
                                 sector->children[side],
                                 startFraction,
                                 middleFraction,
                                 start,
                                 middle);
        ...
        for (svEntity_t *svEntity =
                 sector->entityLinkHead;
             svEntity != NULL;
             svEntity = svEntity->nextInWorldSector) {
...
{
    CM_ClipMoveToEntities_r(traceWork, &cm_worldSectorRoot, 0.0f,
                 traceWork->bestTrace.fraction, traceWork->start,
                 traceWork->end);
}

int32_t
CM_ClipSightTraceToEntities_r(cmClipSightTraceWork_t *sightTraceWork,
             worldSector_t *sector,
             float startFraction,
             float endFraction,
             const vec3_t start,
             const vec3_t end)
{
    ...
    if (startDistance >= offset && endDistance >= offset) {
        hit = CM_ClipSightTraceToEntities_r(sightTraceWork,
                           sector->children[0],
                           startFraction,
                           endFraction,
                           start,
...
int32_t
CM_ClipSightTraceToEntities(cmClipSightTraceWork_t *sightTraceWork)
{
    return CM_ClipSightTraceToEntities_r(sightTraceWork, &cm_worldSectorRoot, 0.0f, 1.0f,
                        sightTraceWork->start, sightTraceWork->end);
}

void CM_PointTraceToEntities_r(cmPointTraceWork_t *sightTraceWork,
                  worldSector_t *sector,
                  float startFraction,
                  float endFraction,
                  const vec3_t start,
                  const vec3_t end)
{
     ...
     */
    if (!(startFraction >= sightTraceWork->bestTrace.fraction) &&
        (sightTraceWork->contentsMask & sector->entityContentsMask) != 0) {
        const int32_t axis = sector->axis;
        const float startDistance = start[axis] - sector->dist;
        const float endDistance = end[axis] - sector->dist;
        ...
        if (!(startDistance >= 0.0f) || !(endDistance >= 0.0f)) {
            if (!(0.0f >= startDistance) ||
                ...
                CM_PointTraceToEntities_r(sightTraceWork,
                             sector->children[side],
                             startFraction,
                             middleFraction,
                             start,
...
{
    CM_PointTraceToEntities_r(sightTraceWork, &cm_worldSectorRoot, 0.0f,
                 sightTraceWork->bestTrace.fraction, sightTraceWork->start,
                 sightTraceWork->end);
}

int32_t
CM_PointSightTraceToEntities_r(cmPointSightTraceWork_t *sightTraceWork,
             worldSector_t *sector,
             float startFraction,
             float endFraction,
             const vec3_t start,
             const vec3_t end)
{
    ...
    if (!(startDistance >= 0.0f) || !(endDistance >= 0.0f)) {
        if (!(0.0f >= startDistance) || !(0.0f >= endDistance)) {
            ...
            hit = CM_PointSightTraceToEntities_r(sightTraceWork,
                               sector->children[side],
                               startFraction,
                               middleFraction,
                               start,
...
int32_t
CM_PointSightTraceToEntities(cmPointSightTraceWork_t *sightTraceWork)
{
    return CM_PointSightTraceToEntities_r(sightTraceWork, &cm_worldSectorRoot, 0.0f, 1.0f,
                        sightTraceWork->start, sightTraceWork->end);
}
