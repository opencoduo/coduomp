...
{
    const long double cylinderHalfHeightRaw =
        (long double)traceWork->sphere.halfheight -
        traceWork->sphere.radius;
    float verticalSpan;
    if (terrainCollide->negativeZOnly != 0) {
        verticalSpan = (float)(
            -2.0L * cylinderHalfHeightRaw);
        traceWork->start[2] = (float)(
            (long double)traceWork->start[2] +
            cylinderHalfHeightRaw);
        traceWork->end[2] = (float)(
    ...
    const float expandedRadius =
        traceWork->sphere.radius + 0.125f;
    const float negativeExpandedRadius =
        -expandedRadius;
    const float fractionEpsilon =
    ...
    for (int32_t triangleIndex = 0;
        ...
        if (!(endDistanceRaw < expandedRadius ||
              isunordered(endDistanceRaw,
                          expandedRadius)))
            continue;
        ...
        if (!(distanceDelta > 0.0f ||
              isunordered(distanceDelta, 0.0f)))
            continue;

        if (!(startDistance >
                  negativeExpandedRadius ||
              isunordered(startDistance,
                          negativeExpandedRadius))) {
            const long double oppositeCapDistance =
                (long double)verticalSpan *
                    triangle->plane.components.normal[2] +
                startDistance;
            if (!(oppositeCapDistance >
                      negativeExpandedRadius ||
                  isunordered(
                      oppositeCapDistance,
                      negativeExpandedRadius))) {
            ...
            const float inverseNormalZ =
                1.0f /
                triangle->plane.components.normal[2];
            const float sAtStart = (float)(
                ((long double)triangle->svec[1] *
                     traceWork->start[1] +
                 (long double)triangle->svec[0] *
                     traceWork->start[0]) +
                (long double)triangle->svec[2] *
                    traceWork->start[2] -
                triangle->svec[3]);
            const float tAtStart = (float)(
                ((long double)triangle->tvec[1] *
                     traceWork->start[1] +
                 (long double)triangle->tvec[0] *
                     traceWork->start[0]) +
                (long double)triangle->tvec[2] *
                    traceWork->start[2] -
                triangle->tvec[3]);

            const long double firstZOffset =
                ((long double)negativeExpandedRadius -
                 startDistance) *
                inverseNormalZ;
            const long double firstSRaw =
                firstZOffset *
                    triangle->svec[2] +
                sAtStart;
            const float firstS =
                (float)firstSRaw;
            const long double firstT =
                firstZOffset *
                    triangle->tvec[2] +
                tAtStart;
            if (firstSRaw >= 0.0f &&
                firstT >= 0.0f &&
                (long double)firstS + firstT <=
                    1.0f) {
            ...
            const long double secondZOffset =
                !isunordered(oppositeCapDistance,
                             expandedRadius) &&
                        oppositeCapDistance <
                            expandedRadius
                    ? (long double)verticalSpan
                    : ((long double)expandedRadius -
                       startDistance) *
                          inverseNormalZ;
            const long double secondSRaw =
                secondZOffset *
                    triangle->svec[2] +
                sAtStart;
            const float secondS =
                (float)secondSRaw;
            const long double secondT =
                secondZOffset *
                    triangle->tvec[2] +
                tAtStart;
        ...
        if (!isunordered(startDistance,
                         expandedRadius) &&
            startDistance < expandedRadius) {
            faceFraction = 0.0L;
        ...
        const float faceS = (float)(
            ((long double)facePoint[1] *
                 triangle->svec[1] +
             (long double)facePoint[0] *
                 triangle->svec[0]) +
            (long double)facePoint[2] *
                triangle->svec[2] -
            triangle->svec[3]);
        const long double faceT =
            ((long double)facePoint[2] *
                 triangle->tvec[2] +
             (long double)facePoint[1] *
                 triangle->tvec[1]) +
        ...
        for (int32_t corner = 0;
             corner < CM_TRIANGLE_VERTEX_COUNT;
             ++corner) {
            ...
            if ((outsideMask &
                 (1 << corner)) != 0) {
                collisionSoupEdge_t *const edge =
                    triangle->oppositeEdges[corner];
                if (edge == NULL ||
                    edge->checkcount ==
                        cm_checkcount) {
                    continue;
                ...
                const float radialStart0 =
                    (float)(
                        ((long double)relativeStart[0] *
                             edge->radialAxes[0][0] +
                         (long double)relativeStart[2] *
                             edge->radialAxes[0][2]) +
                        (long double)relativeStart[1] *
                            edge->radialAxes[0][1]);
                const float radialStart1 =
                    (float)(
                        ((long double)relativeStart[0] *
                             edge->radialAxes[1][0] +
                         (long double)relativeStart[2] *
                ...
                const long double radialConstantRaw =
                        (long double)radialStart1 *
                            radialStart1 +
                        (long double)radialStart0 *
                            radialStart0 -
                        radiusSquared;
                const float radialConstant =
                    (float)radialConstantRaw;

                if (radialConstantRaw <= 0.0f) {
                    if (edgeStart < 0.0f ||
                        edgeStart > edge->length) {
                        continue;
                    }
                    traceWork->trace.normal[0] =
                        triangle->plane.components.normal[0];
                ...
                const float radialDelta1 =
                    (float)(
                        ...
                        (long double)traceWork->delta[2] *
                            edge->radialAxes[1][2]);
                const long double radialLinearRaw =
                        (long double)radialStart1 *
                            radialDelta1 +
                        (long double)radialStart0 *
                            radialDelta0;
                const float radialLinear =
                    (float)radialLinearRaw;
                if (!(radialLinearRaw < 0.0f ||
                      isunordered(radialLinearRaw,
                                  0.0f)))
                    continue;

                const long double radialQuadratic =
                    (long double)radialDelta1 *
                        radialDelta1 +
                    (long double)radialDelta0 *
                        radialDelta0;
                const long double discriminant =
                    (long double)radialLinear *
                        radialLinear -
                    radialQuadratic *
                        radialConstant;
                if (!(discriminant > 0.0L ||
                      isunordered(discriminant, 0.0L)))
                    continue;
                ...
                const long double
                    ...
                    radialQuadratic;
                featureFraction =
                    (float)featureFractionRaw;
                if (!(featureFractionRaw <
                      traceWork->trace.fraction)) {
                    continue;
                }

                const long double edgeAtHit =
                    (((long double)traceWork->delta[2] *
                          edge->unitDirection[2] +
                      (long double)traceWork->delta[0] *
                          edge->unitDirection[0]) +
                    ...
                    edgeStart;
                if (edgeAtHit < 0.0L ||
                    edgeAtHit > edge->length) {
                    continue;
                }

                const long double inverseRadius =
                    1.0L / expandedRadius;
                const long double normalAxis0 =
                    ((long double)radialDelta0 *
                         featureFraction +
                     radialStart0) *
                    inverseRadius;
                const float normalAxis1 = (float)(
                    ((long double)radialDelta1 *
                         featureFraction +
                     radialStart1) *
                    inverseRadius);
                for (int32_t axis = 0;
                     axis < 3; ++axis) {
                    const float axis0Term = (float)(
                        normalAxis0 *
                        edge->radialAxes[0][axis]);
                    featureNormal[axis] = (float)(
                        (long double)axis0Term +
                        (long double)normalAxis1 *
                            edge->radialAxes[1][axis]);
                }
            } else {
                collisionSoupVertex_t *const vertex =
                ...
                const long double vertexConstantRaw =
                        ...
                        (long double)relativeStart[0] *
                            relativeStart[0] -
                        radiusSquared;
                const float vertexConstant =
                    (float)vertexConstantRaw;
                ...
                const long double vertexLinearRaw =
                        ...
                        (long double)relativeStart[2] *
                            traceWork->delta[2];
                const float vertexLinear =
                    (float)vertexLinearRaw;
                if (!(vertexLinearRaw < 0.0f ||
                      isunordered(vertexLinearRaw,
                                  0.0f)))
                    continue;

                const long double discriminant =
                    (long double)vertexLinear *
                        vertexLinear -
                    (long double)
                        traceWork
                            ->deltaLengthSquared *
 ...
 */
static inline float coduo_collision_x87_plane_dist(float x, float y, float z,
...
#else
    const float radius =
        traceWork->sphere.radius + CM_TERRAIN_POINT_ENTER_EPSILON;
#endif
...
#if EMULATE_X87
        const float endDistance = coduo_collision_x87_plane_dist(
            ...
            if (!(0.0f >= surfaceDelta)) {
                if (startDistance <= negativeRadius) {
                    ...
                    const float capDistance =
                        startDistance + (capDelta * surfacePlane->components.normal[2]);
#endif
                    ...
                    if (negativeRadius < capDistance) {
                        ...
                        const float edge0StartDistance =
                            coduo_collision_x87_plane_dist(
                            traceWork->start[0], traceWork->start[1],
                            traceWork->start[2], facet->svec[0],
                            facet->svec[1],
                            facet->svec[2],
                            facet->svec[3]);
                        const float edge1StartDistance =
                            coduo_collision_x87_plane_dist(
                            traceWork->start[0], traceWork->start[1],
                            traceWork->start[2], facet->tvec[0],
                            facet->tvec[1],
                            facet->tvec[2],
                            facet->tvec[3]);
...
#if EMULATE_X87
                    const float edge0Distance = coduo_collision_x87_plane_dist(
                        hitPoint[0], hitPoint[1], hitPoint[2],
                        facet->svec[0],
                        facet->svec[1],
                        facet->svec[2],
                        facet->svec[3]);
                    const float edge1Distance = coduo_collision_x87_plane_dist(
                        hitPoint[0], hitPoint[1], hitPoint[2],
                        facet->tvec[0],
                        facet->tvec[1],
                        facet->tvec[2],
                    ...
                    const float edge0Distance =
                        ((hitPoint[0] * facet->svec[0]) +
                         (hitPoint[1] * facet->svec[1])) +
                        (hitPoint[2] * facet->svec[2]) -
                        facet->svec[3];
                    const float edge1Distance =
                        ((hitPoint[0] * facet->tvec[0]) +
                         (hitPoint[1] * facet->tvec[1])) +
                        (hitPoint[2] * facet->tvec[2]) -
                        facet->tvec[3];
                    ...
                    if (outsideFlags == 0U) {
                        ...
                        traceWork->trace.fraction = x87f_store_f32(x87f_sub(
                            x87f_load_f32(hitFraction),
                            x87f_load_f32(CM_TERRAIN_SPHERE_HIT_EPSILON)));
#else
                        traceWork->trace.fraction =
                            hitFraction - CM_TERRAIN_SPHERE_HIT_EPSILON;
#endif
                    } else {
                        for (int32_t edgeIndex = 0;
                             edgeIndex < CM_TERRAIN_EDGE_COUNT;
                             ++edgeIndex) {
                            if (((outsideFlags >> edgeIndex) & 1U) ==
                                0U) {
                                collisionSoupVertex_t *vertexSphere =
                                    facet->vertices[edgeIndex];
                                ...
                                if (vertexSphere != NULL &&
                                    ...
                                    const float deltaX = x87f_store_f32(x87f_sub(
                                        x87f_load_f32(traceWork->start[0]),
                                        x87f_load_f32(vertexSphere->position[0])));
                                    const float deltaY = x87f_store_f32(x87f_sub(
                                        x87f_load_f32(traceWork->start[1]),
                                        x87f_load_f32(vertexSphere->position[1])));
                                    const float deltaZ = x87f_store_f32(x87f_sub(
                                        x87f_load_f32(traceWork->start[2]),
                                        x87f_load_f32(vertexSphere->position[2])));
                                    ...
                                    const float deltaX =
                                        traceWork->start[0] -
                                        vertexSphere->position[0];
                                    const float deltaY =
                                        traceWork->start[1] -
                                        vertexSphere->position[1];
                                    const float deltaZ =
                                        traceWork->start[2] -
                                        vertexSphere->position[2];
                                    const float radiusDelta =
                                        ((deltaX * deltaX) +
                                         (deltaY * deltaY)) +
                                        (deltaZ * deltaZ) -
                                ...
                                collisionSoupEdge_t *edgeCylinder =
                                    facet->oppositeEdges[edgeIndex];
                                ...
                                if (edgeCylinder != NULL &&
                                    ...
                                    float axial = x87f_store_f32(x87f_add(
                                        x87f_add(
                                            x87f_mul(
                                                x87f_load_f32(deltaX),
                                                x87f_load_f32(
                                                    edgeCylinder->unitDirection[0])),
                                            x87f_mul(
                                                x87f_load_f32(deltaY),
                                                x87f_load_f32(
                                                    edgeCylinder->unitDirection[1]))),
                                        x87f_mul(
                                            x87f_load_f32(deltaZ),
                                            x87f_load_f32(
                                                edgeCylinder->unitDirection[2]))));
                                    ...
                                    float axial =
                                        ((deltaX * edgeCylinder->unitDirection[0]) +
                                         (deltaY * edgeCylinder->unitDirection[1])) +
