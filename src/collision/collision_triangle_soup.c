...
_Static_assert(_Alignof(collisionSoupBuildEdge_t) == 2,
               "i386 soup-build edge alignment changed");
_Static_assert(offsetof(collisionSoupBuildEdge_t, vertexIndices[0]) == 0x00,
               "i386 soup-build edge first vertex index moved");
_Static_assert(offsetof(collisionSoupBuildEdge_t, vertexIndices[1]) == 0x02,
               "i386 soup-build edge second vertex index moved");
_Static_assert(sizeof(collisionSoupBuildEdge_t) == 0x04,
               "i386 soup-build edge size changed");
#endif
...
static const float CM_TRIANGLE_SOUP_COPLANAR_EDGE_SCALE =
    6.4000000854491645e-09f;
static const float CM_TRIANGLE_SOUP_POSITIVE_Z_EPSILON =
    0.0010000000474974513f;
static const float CM_TRIANGLE_SOUP_NEGATIVE_Z_EPSILON =
    -0.0010000000474974513f;
...
static collisionSoupBuildEdge_t
    collisionSoupBuildEdges[CM_TRIANGLE_SOUP_EDGE_LIMIT]; /* 0x008e49b8 */
static int16_t
    collisionSoupBuildOppositeVertices[CM_TRIANGLE_SOUP_EDGE_LIMIT]; /* 0x009249b8 */
static int16_t
    collisionSoupBuildVertexRemap[CM_TRIANGLE_SOUP_VERTEX_LIMIT]; /* 0x009449b8 */
...
static int32_t
CM_FindTriangleSoupEdge(int32_t edgeCount,
                        const collisionSoupBuildEdge_t *edges,
                        int16_t first, int16_t second)
{
    for (int32_t edgeIndex = 0; edgeIndex < edgeCount; ++edgeIndex) {
        if ((edges[edgeIndex].vertexIndices[0] == first &&
             edges[edgeIndex].vertexIndices[1] == second) ||
            (edges[edgeIndex].vertexIndices[0] == second &&
             edges[edgeIndex].vertexIndices[1] == first)) {
            return edgeIndex;
        }
    }
...
static qboolean coduo_compat_terrain_indices_are_valid(
                          ...
                          vec3_t bounds[2])
{
    ...
    if (coduo_compat_terrain_indices_are_valid(
            indexCount, indices, vertexCount) == qfalse) {
        Com_Error(ERR_DROP, "\x15" "CM_GenerateTerrainCollide: bad vertex index, corrupt bsp?");
    }

    int32_t edgeCount = 0;
    for (int32_t triangleStart = 0; triangleStart < indexCount;
         triangleStart += CM_TRIANGLE_VERTEX_COUNT) {
        for (int32_t corner = 0; corner < CM_TRIANGLE_VERTEX_COUNT; ++corner) {
            ...
            const int16_t second =
                indices[triangleStart +
                        ((corner + 1) % CM_TRIANGLE_VERTEX_COUNT)];
            const int16_t opposite =
                indices[triangleStart +
                        ((corner + 2) % CM_TRIANGLE_VERTEX_COUNT)];
            ...
            const int32_t edgeIndex =
                CM_FindTriangleSoupEdge(edgeCount, collisionSoupBuildEdges,
                                        first, second);
            if (edgeIndex < 0) {
            ...
            const int16_t previousOpposite =
                collisionSoupBuildOppositeVertices[edgeIndex];
            vec3_t firstDelta;
            vec3_t secondDelta;
            vec3_t previousDelta;
            for (int32_t component = 0;
                 component < CM_TRIANGLE_SOUP_VECTOR_COMPONENT_COUNT;
                 ++component) {
                firstDelta[component] =
                    vertices[first][component] -
                    vertices[opposite][component];
                secondDelta[component] =
                    vertices[second][component] -
                    vertices[opposite][component];
                previousDelta[component] =
                    vertices[previousOpposite][component] -
                    vertices[opposite][component];
            }

            vec3_t cross;
            CrossProduct(secondDelta, firstDelta, cross);
            const float concavity = (float)(
                (long double)cross[2] * previousDelta[2] +
                (long double)cross[1] * previousDelta[1] +
                (long double)cross[0] * previousDelta[0]);
            if (!(concavity > 0.0f))
                continue;

            const float firstDistanceSquared =
                VectorDistanceSquared(vertices[first],
                                      vertices[previousOpposite]);
            const float secondDistanceSquared =
                VectorDistanceSquared(vertices[second],
                                      vertices[previousOpposite]);
            ...
            const long double crossLengthSquared =
                (long double)cross[2] * cross[2] +
                (long double)cross[1] * cross[1] +
                (long double)cross[0] * cross[0];
            const long double coplanarThreshold =
                (long double)maximumDistanceSquared *
                CM_TRIANGLE_SOUP_COPLANAR_EDGE_SCALE;

            if (crossLengthSquared < coplanarThreshold) {
                edgeCount--;
                collisionSoupBuildEdges[edgeIndex] =
                    collisionSoupBuildEdges[edgeCount];
                collisionSoupBuildOppositeVertices[edgeIndex] =
                    collisionSoupBuildOppositeVertices[edgeCount];
            }
        }
    }

    memset(collisionSoupBuildVertexRemap, -1,
           sizeof(collisionSoupBuildVertexRemap));
    int32_t usedVertexCount = 0;
    for (int32_t edgeIndex = 0; edgeIndex < edgeCount; ++edgeIndex) {
        const int16_t first =
            collisionSoupBuildEdges[edgeIndex].vertexIndices[0];
        const int16_t second =
            collisionSoupBuildEdges[edgeIndex].vertexIndices[1];

        if (collisionSoupBuildVertexRemap[first] < 0)
            collisionSoupBuildVertexRemap[first] =
                (int16_t)usedVertexCount++;
        if (collisionSoupBuildVertexRemap[second] < 0)
            collisionSoupBuildVertexRemap[second] =
                (int16_t)usedVertexCount++;
    }

    const int32_t triangleCount =
        indexCount / CM_TRIANGLE_VERTEX_COUNT;
    collisionTriangleSoup_t *soup = Hunk_AllocAlignInternal(
        sizeof(*soup) +
            (size_t)triangleCount * sizeof(soup->triangles[0]),
        CM_TRIANGLE_SOUP_HUNK_ALIGNMENT);
    collisionSoupVertex_t *collisionVertices = Hunk_AllocAlignInternal(
        (size_t)usedVertexCount * sizeof(collisionVertices[0]),
        CM_TRIANGLE_SOUP_HUNK_ALIGNMENT);
    collisionSoupEdge_t *collisionEdges = Hunk_AllocAlignInternal(
        (size_t)edgeCount * sizeof(collisionEdges[0]),
        CM_TRIANGLE_SOUP_HUNK_ALIGNMENT);
    ...
    for (int32_t triangleIndex = 0;
         triangleIndex < (int32_t)soup->triangleCount; ++triangleIndex) {
        const int32_t triangleStart =
            triangleIndex * CM_TRIANGLE_VERTEX_COUNT;
        ...
        collisionSoupTriangle_t *triangle =
            &soup->triangles[triangleIndex];

        (void)PlaneFromPoints(triangle->plane.equation, vertices[first],
                              vertices[second], vertices[third]);
        if (triangle->plane.components.normal[2] <
            CM_TRIANGLE_SOUP_NEGATIVE_Z_EPSILON) {
            hasNegativeZPlane = qtrue;
        } else if (triangle->plane.components.normal[2] >
                   CM_TRIANGLE_SOUP_POSITIVE_Z_EPSILON) {
        ...
        for (int32_t component = 0;
             ...
             ++component) {
            edge01[component] =
                vertices[second][component] - vertices[first][component];
            edge02[component] =
                vertices[third][component] - vertices[first][component];
        }
        const float edge01Length = VectorNormalize(edge01);
        const float edge02Length = VectorNormalize(edge02);
        const float dot = (float)(
            (long double)edge01[2] * edge02[2] +
            (long double)edge01[1] * edge02[1] +
            (long double)edge01[0] * edge02[0]);
        const float inverseDot = (float)(
            1.0L / (1.0L - (long double)dot * dot));

        for (int32_t component = 0;
             component < CM_TRIANGLE_SOUP_VECTOR_COMPONENT_COUNT;
             ++component) {
            edge01[component] =
                (float)((long double)edge01[component] * inverseDot);
            edge02[component] =
                (float)((long double)edge02[component] * inverseDot);
        }

        const float negativeDot = -dot;
        const float inverseEdge01Length =
            (float)(1.0L / (long double)edge01Length);
        const float inverseEdge02Length =
            (float)(1.0L / (long double)edge02Length);
        ...
        for (int32_t component = 0;
             component < CM_TRIANGLE_SOUP_VECTOR_COMPONENT_COUNT;
             ++component) {
            triangle->svec[component] = (float)(
                ((long double)negativeDot * edge02[component] +
                 edge01[component]) *
                inverseEdge01Length);
            triangle->tvec[component] = (float)(
                ((long double)negativeDot * edge01[component] +
                 edge02[component]) *
                inverseEdge02Length);
        }
        triangle->svec[3] = (float)(
            (long double)triangle->svec[2] * vertices[first][2] +
            (long double)triangle->svec[1] * vertices[first][1] +
            (long double)triangle->svec[0] * vertices[first][0]);
        triangle->tvec[3] = (float)(
            (long double)triangle->tvec[2] * vertices[first][2] +
            (long double)triangle->tvec[1] * vertices[first][1] +
        ...
        triangle->oppositeEdges[0] =
            edgeIndex < 0 ? NULL : &collisionEdges[edgeIndex];
        edgeIndex =
            CM_FindTriangleSoupEdge(edgeCount, collisionSoupBuildEdges,
                                    third, first);
    ...
    soup->negativeZOnly =
        hasNegativeZPlane != qfalse && hasPositiveZPlane == qfalse;

    ClearBounds(bounds[0], bounds[1]);
    for (uint32_t vertexIndex = 0; vertexIndex < vertexCount;
         ++vertexIndex) {
        AddPointToBounds(vertices[vertexIndex], bounds[0], bounds[1]);

        const int16_t remap =
            collisionSoupBuildVertexRemap[vertexIndex];
        if (remap >= 0) {
            collisionVertices[remap].checkcount = 0;
            collisionVertices[remap].position[0] =
                vertices[vertexIndex][0];
            collisionVertices[remap].position[1] =
                vertices[vertexIndex][1];
            collisionVertices[remap].position[2] =
                          ...
                          vec3_t bounds[2])
{
    ...
    for (int32_t triangleIndex = 0; triangleIndex < indexCount;
         triangleIndex += CM_TRIANGLE_VERTEX_COUNT) {
        for (int32_t corner = 0; corner < CM_TRIANGLE_VERTEX_COUNT;
             ++corner) {
            ...
            int32_t edgeIndex =
                CM_FindTriangleSoupEdge((int32_t)edgeCount, collisionSoupBuildEdges, first, second);
            if (edgeIndex < 0) {
    ...
    uint32_t soupBytes =
        (uint32_t)sizeof(collisionTriangleSoup_t) +
        (uint32_t)triangleCount * (uint32_t)sizeof(collisionSoupTriangle_t);
    collisionTriangleSoup_t *soup =
        Hunk_AllocInternal(soupBytes);
    collisionSoupVertex_t *collisionVertices =
        Hunk_AllocInternal(
            (size_t)usedVertexCount * sizeof(collisionVertices[0]));
    collisionSoupEdge_t *collisionEdges =
        Hunk_AllocInternal((size_t)edgeCount * sizeof(collisionEdges[0]));
    ...
    for (int32_t triangleIndex = 0; triangleIndex < publishedTriangleCount;
         ++triangleIndex) {
        int16_t first =
            indices[triangleIndex * CM_TRIANGLE_VERTEX_COUNT];
        int16_t second =
            indices[triangleIndex * CM_TRIANGLE_VERTEX_COUNT + 1];
        int16_t third =
            indices[triangleIndex * CM_TRIANGLE_VERTEX_COUNT + 2];
        collisionSoupTriangle_t *triangle =
            &soup->triangles[triangleIndex];

        PlaneFromPoints(triangle->plane.equation, vertices[first],
                        vertices[second], vertices[third]);
        if (triangle->plane.components.normal[2] <
            CM_TRIANGLE_SOUP_NEGATIVE_Z_EPSILON) {
            hasNegativeZPlane = qtrue;
        } else if (CM_TRIANGLE_SOUP_POSITIVE_Z_EPSILON <
        ...
        int32_t edgeIndex =
            CM_FindTriangleSoupEdge((int32_t)edgeCount, collisionSoupBuildEdges, second, third);
        triangle->oppositeEdges[0] =
            edgeIndex < 0
                ? NULL
