...
int32_t CM_PointContents(const vec3_t point, int32_t modelHandle)
{
    ...
    for (int32_t leafBrushIndex = 0;
        ...
        for (int32_t axis = 0; axis < 3; ++axis) {
            ...
            if (x87f_lt_signaling(x87f_load_f32(point[axis]),
                                  x87f_load_f32(brush->mins[axis])) ||
                x87f_lt_signaling(x87f_load_f32(brush->maxs[axis]),
                                  x87f_load_f32(point[axis]))) {
#elif EMULATE_X87
            if (x87f_lt(x87f_load_f32(point[axis]),
                        x87f_load_f32(brush->mins[axis])) ||
                x87f_lt(x87f_load_f32(brush->maxs[axis]),
                        x87f_load_f32(point[axis]))) {
#else
...
#else
            distance =
                ((long double)point[0] * (long double)plane->normal[0] +
                 (long double)point[1] * (long double)plane->normal[1]) +
                                    ...
                                    const vec3_t origin, const vec3_t angles)
{
    ...
    if (modelHandle != CM_TEMP_BOX_MODEL_HANDLE &&
        (angles[0] != 0.0f || angles[1] != 0.0f || angles[2] != 0.0f)) {
        vec3_t forward;
        vec3_t right;
        vec3_t up;
        const vec3_t translated = {
