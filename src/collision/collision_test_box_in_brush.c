...
void CM_TestBoxInBrush(traceWork_t *traceWork, const collisionBrush_t *brush)
{
    for (int32_t axis = 0; axis < 3; ++axis) {
        if (traceWork->bounds[0][axis] > brush->maxs[axis]) {
            return;
        }
    }
    for (int32_t axis = 0; axis < 3; ++axis) {
        if (brush->mins[axis] > traceWork->bounds[1][axis]) {
            return;
        }
    }
    ...
    if (traceWork->sphere.use != qfalse) {
        while (sideCount != 0) {
            ...
            const long double planeDistRaw = (long double)plane->dist + traceWork->sphere.radius;
            const long double sphereOffsetRaw = ((long double)plane->normal[1] * traceWork->sphere.offset[1] +
            ...
            if (sphereOffsetRaw > 0.0L) {
                for (int32_t axis = 0; axis < 3; ++axis) {
                    testPoint[axis] = (long double)traceWork->start[axis] - traceWork->sphere.offset[axis];
                }
            } else {
                for (int32_t axis = 0; axis < 3; ++axis) {
                    testPoint[axis] = (long double)traceWork->start[axis] + traceWork->sphere.offset[axis];
            ...
            const long double distanceRaw =
                (testPoint[0] * plane->normal[0] + testPoint[2] * plane->normal[2]) + testPoint[1] * plane->normal[1] - planeDistRaw;
            if (distanceRaw > 0.0L)
                return;
        ...
        while (sideCount != 0) {
            ...
            const long double distanceRaw =
                ((long double)traceWork->start[1] * plane->normal[1] + (long double)traceWork->start[0] * plane->normal[0]) +
                (long double)traceWork->start[2] * plane->normal[2] - planeDistRaw;
            if (distanceRaw > 0.0L)
                return;
    ...
    if (traceWork->sphere.use != 0) {
        for (; sideCount != 0; --sideCount, ++side) {
            ...
            const float distance =
                (((testPoint[0] * plane->normal[0]) + (testPoint[1] * plane->normal[1])) + (testPoint[2] * plane->normal[2])) - planeDist;
            if (distance > 0.0f) {
                return;
            }
        }
        ...
        for (; sideCount != 0; --sideCount, ++side) {
            ...
            const float distance = (((traceWork->start[0] * plane->normal[0]) + (traceWork->start[1] * plane->normal[1])) +
