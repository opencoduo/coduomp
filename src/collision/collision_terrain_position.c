    ...
    const collisionTriangleSoup_t *terrainCollide)
{
    ...
    for (int32_t triangleIndex = 0;
        ...
        const float planeDistance =
            (float)planeDistanceRaw;

        if (!(planeDistanceRaw < radius ||
              isunordered(planeDistanceRaw, radius)))
            continue;

        if (!(planeDistance > negativeRadius ||
              isunordered(planeDistance,
                          negativeRadius))) {
        ...
        for (int32_t axis = 0; axis < 2;
             ++axis) {
            projectedPoint[axis] = (float)(
                (long double)traceWork->start[axis] -
                (long double)planeDistance *
                    triangle->plane.components.normal[axis]);
        }
        const long double projectedPointZRaw =
            ...
            (long double)planeDistance *
                triangle->plane.components.normal[2];
        projectedPoint[2] =
            (float)projectedPointZRaw;

        const long double sCoordinate =
            (projectedPointZRaw *
                 triangle->svec[2] +
             (long double)projectedPoint[1] *
                 triangle->svec[1]) +
            (long double)projectedPoint[0] *
                triangle->svec[0] -
            triangle->svec[3];
        const long double tCoordinateRaw =
            ((long double)projectedPoint[2] *
                 triangle->tvec[2] +
             (long double)projectedPoint[1] *
                 triangle->tvec[1]) +
            (long double)projectedPoint[0] *
                triangle->tvec[0] -
            triangle->tvec[3];
        const float tCoordinate =
            (float)tCoordinateRaw;
        ...
        for (int32_t corner = 0;
            ...
            if ((outsideMask &
                ...
                const long double edgeDistanceRaw =
                    ((long double)edgeDelta[2] *
                         edge->unitDirection[2] +
                     (long double)edgeDelta[1] *
                         edge->unitDirection[1]) +
                    (long double)edgeDelta[0] *
                        edge->unitDirection[0];
                const float edgeDistance =
                    (float)edgeDistanceRaw;
                if (edgeDistanceRaw < 0.0f ||
                    edgeDistance > edge->length) {
                    continue;
                }

                const long double closestX =
                    (long double)edgeDelta[0] -
                    (long double)edgeDistance *
                        edge->unitDirection[0];
                const long double closestY =
                    (long double)edgeDelta[1] -
                    (long double)edgeDistance *
                        edge->unitDirection[1];
                const long double closestZ =
                    (long double)edgeDelta[2] -
                    (long double)edgeDistance *
                        edge->unitDirection[2];
                if (closestX * closestX +
                        closestY * closestY +
                        closestZ * closestZ <
                    radiusSquared) {
                    return qtrue;
...
qboolean
CM_PositionTestSphereWithTerrainCollide(traceWork_t *traceWork,
                             const collisionTriangleSoup_t *terrainCollide)
{
        ...
        if (!(surfaceDistance >= radius)) {
            if (surfaceDistance <= negativeRadius) {
                const float capDistance =
                    surfaceDistance + (capDelta * surfacePlane->components.normal[2]);
                ...
                if (!(negativeRadius >= capDistance)) {
                    ...
                    const float baseEdge0Distance =
                        ((traceWork->start[0] * (*edgePlane)[0]) +
                         (traceWork->start[1] * (*edgePlane)[1])) +
                        (traceWork->start[2] * (*edgePlane)[2]) -
                        (*edgePlane)[3];
                ...
                const float projectedX =
                    traceWork->start[0] -
                    (surfaceDistance * surfacePlane->components.normal[0]);
                const float projectedY =
                    traceWork->start[1] -
                    (surfaceDistance * surfacePlane->components.normal[1]);
                const float projectedZ =
                    traceWork->start[2] -
                    (surfaceDistance * surfacePlane->components.normal[2]);

                const vec4_t *edgePlane =
                    &facet->svec;
                const float edge0Distance =
                    ((projectedX * (*edgePlane)[0]) +
                     (projectedY * (*edgePlane)[1])) +
                    (projectedZ * (*edgePlane)[2]) - (*edgePlane)[3];
                ...
                for (int32_t edgeIndex = 0; edgeIndex < CM_TERRAIN_EDGE_COUNT;
                     ++edgeIndex) {
                    if (((outsideFlags >> edgeIndex) & 1U) != 0) {
                        const collisionSoupEdge_t *edgeCylinder =
                        ...
                        if (edgeCylinder != NULL) {
                            ...
                            if (axisDistance >= 0.0f &&
                                ...
                                deltaX -=
                                    axisDistance * edgeCylinder->unitDirection[0];
                                deltaY -=
                                    axisDistance * edgeCylinder->unitDirection[1];
                                deltaZ -=
                                    axisDistance * edgeCylinder->unitDirection[2];
                        ...
                        const collisionSoupVertex_t *vertexSphere =
                            facet->vertices[edgeIndex];

                        if (vertexSphere != NULL) {
                            const float deltaX =
                                traceWork->start[0] - vertexSphere->position[0];
                            const float deltaY =
                                traceWork->start[1] - vertexSphere->position[1];
                            const float deltaZ =
                                traceWork->start[2] - vertexSphere->position[2];
