...
{
    if (CM_TraceWorkIntersectsBounds(traceWork, terrainPatch->bounds[0],
                                     terrainPatch->bounds[1]) == qfalse) {
        return;
    }
    ...
    if (terrainPatch->curveCollide != NULL) {
        ...
        CM_TraceThroughTerrainCollide(traceWork,
                                      terrainPatch->terrainCollide);
    }
    ...
    const qboolean closer = x87f_lt_signaling(
        ...
        x87f_load_f32(savedFraction)) ? qtrue : qfalse;
#else
    ...
    const qboolean closer =
        traceWork->trace.fraction < savedFraction ? qtrue : qfalse;
#endif
    ...
    const collisionTerrainPatch_t *terrainPatch)
{
    ...
    if (terrainPatch->curveCollide != NULL) {
        return CM_SightTraceThroughPatchCollide(
