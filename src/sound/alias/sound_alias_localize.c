    ...
    int32_t fileHandle)
{
    ...
    (void)FS_Write(referencePrefix,
                   (int32_t)strlen(referencePrefix),
                   fileHandle);
    (void)FS_Write(reference,
                   (int32_t)strlen(reference),
                   fileHandle);
    (void)FS_Write(englishPrefix,
                   (int32_t)strlen(englishPrefix),
                   fileHandle);
    (void)FS_Write(englishText,
                   (int32_t)strlen(englishText),
                   fileHandle);
    (void)FS_Write(entrySuffix,
                   (int32_t)strlen(entrySuffix),
                   fileHandle);
}
...
{
    static const char subtitleFile[] =
    ...
    enum {
        ...
        END_MARKER_WRITE_TRIM = sizeof(endMarkerToken) - 1 +
            END_MARKER_LINE_ENDING_LENGTH
    };
    const char *const reference = subtitle + 9;
    qboolean found = qfalse;

    const int32_t temporaryHandle =
        FS_FOpenFileWrite(temporaryFile);
    ...
    for (;;) {
        ...
        if (strncmp(token, endMarkerToken,
                    sizeof(endMarkerToken)) == 0) {
            if (writeStart < parseCursor) {
                ...
                const int32_t writeLength =
                    (int32_t)markerSpan - END_MARKER_WRITE_TRIM;
                (void)FS_Write(
                    writeStart, writeLength, temporaryHandle);
            }
            break;
        }

        if (strncmp(token, referenceToken,
                    sizeof(referenceToken)) == 0) {
            ...
            if (strcmp(token, reference) == 0) {
                ...
                Com_WriteSoundAliasSubtitleEntry(
                    reference, englishText, temporaryHandle);
                found = qtrue;
                ...
                do {
                    ...
                    strncmp(token, referenceToken,
                            sizeof(referenceToken)) != 0 &&
                    strncmp(token, endMarkerToken,
                            sizeof(endMarkerToken)) != 0);
    ...
    Com_EndParseSession();
    FS_FreeFile(subtitleBuffer);
    (void)FS_Write(endMarkerSuffix,
                   (int32_t)strlen(endMarkerSuffix),
                   temporaryHandle);
    FS_FCloseFile(temporaryHandle);
    ...
    FS_BuildOSPath(fs_basepath->string, fs_currentGameDir,
                   temporaryFile, temporaryOSPath);
    FS_BuildOSPath(fs_basepath->string, fs_currentGameDir,
                   subtitleFile, subtitleOSPath);
...
{
    static const char temporaryCsv[] =
    ...
    char cellValues[SND_ALIAS_FIELD_COUNT]
                   [LOCALIZE_FIELD_CAPACITY];
    ...
    const int32_t outputHandle =
        FS_FOpenFileWrite(temporaryCsv);
    if (outputHandle == 0) {
        Com_Printf(
            "WARNING: Could not open output file %s for writing\n",
            temporaryCsv);
    ...
    while (parseCursor != NULL) {
        ...
        if (token[0] == '\0' || token[0] == '#') {
            ...
            (void)FS_Write(
                rowStart, (int32_t)(parseCursor - rowStart),
                outputHandle);
            continue;
        }
        ...
        if (columnCount == 0) {
            ...
            do {
                ...
                for (int32_t field = SND_ALIAS_FIELD_NAME;
                     field < SND_ALIAS_FIELD_COUNT; ++field) {
                    if (Q_stricmpn(
                            soundAliasFieldNames[field], token,
                            LOCALIZE_COMPARE_LIMIT) == 0) {
                ...
                ++columnCount;
                if (columnCount == LOCALIZE_MAX_COLUMNS ||
                    parseCursor == NULL ||
                    parseCursor[0] == '\n') {
                    break;
                }
        ...
        for (int32_t column = 0;
            ...
            const sndAliasField_t field =
                (sndAliasField_t)columnMap[column];
            strcpy(cellValues[field], token);
            if (token[0] != '\0') {
                Com_LoadSoundAliasField(
                    "", token, field, seenFields, &parseNode);
            }
            if (column != columnCount - 1)
                token = Com_ParseOnLine(&parseCursor);
        }

        if (!seenFields[SND_ALIAS_FIELD_NAME] ||
            !seenFields[SND_ALIAS_FIELD_FILE]) {
            Com_Error(
                1,
        ...
        if (seenFields[SND_ALIAS_FIELD_SUBTITLE]) {
            const char *const subtitle =
                cellValues[SND_ALIAS_FIELD_SUBTITLE];
            const int32_t subtitleLength =
                (int32_t)strlen(subtitle);
            ...
            if (characterIndex < subtitleLength ||
                strncmp(subtitle, subtitlePrefix,
                        sizeof(subtitlePrefix) - 1) != 0 ||
        ...
        for (int32_t column = 0;
            ...
            if (field == SND_ALIAS_FIELD_UNKNOWN ||
                !seenFields[field]) {
                if (column != columnCount - 1)
            ...
            if (field == SND_ALIAS_FIELD_SUBTITLE) {
                const char *outputText =
                    Com_SoundAliasSubtitleReferenceForText(
                        cellValues[SND_ALIAS_FIELD_SUBTITLE]);
                ...
                if (outputText == NULL) {
                    if (seenFields[SND_ALIAS_FIELD_SEQUENCE]) {
                        Com_sprintf(
                            subtitleReference,
                            sizeof(subtitleReference),
                            "%s%s_%s", subtitlePrefix,
                            cellValues[SND_ALIAS_FIELD_NAME],
                            cellValues[SND_ALIAS_FIELD_SEQUENCE]);
                    } else {
                        Com_sprintf(
                            subtitleReference,
                            sizeof(subtitleReference),
                    ...
                    Com_sprintf(
                        subtitleReference,
                        sizeof(subtitleReference),
                        "%s%s", subtitlePrefix, outputText);
                    outputText = Q_strupr(subtitleReference);
                }

                (void)FS_Write(
                    outputText, (int32_t)strlen(outputText),
                    outputHandle);
                continue;
            }

            const char *const cellText = cellValues[field];
            const qboolean needsQuotes =
                strchr(cellText, ',') != NULL ||
                strchr(cellText, ' ') != NULL ||
                strchr(cellText, '\n') != NULL ||
    ...
    const char *sourcePath, const char *destinationPath)
{
    ...
    if (fread(buffer, 1, (size_t)byteCount, source) !=
        (size_t)byteCount) {
        Com_Error(
            0,
    ...
    if (fwrite(buffer, 1, (size_t)byteCount, destination) !=
        (size_t)byteCount) {
        Com_Error(
            0,
